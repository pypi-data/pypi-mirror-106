        ...
        while 1:
            ...
            while 1:
                if len(self.Cmd) > 0:
                    ...
                    # print('Trang thai van tay truoc khi vao IF',self.VanTayDangDayDuLieu)
                    # try:
                    ...
                    if (dta[1] == 'Fused' and dta[2] != "OK\n") or dta[1] == "Fopen\n" or dta[1] == 'FDK\n':
                        ...
                        if self.ThreadValue.Name != 'finger_print':
                            self.ThreadValue.Thread_Object.signal = False
                    ...
                    if dta[1] == 'Cused' and dta[2] != "OK\n":
                        if self.ThreadValue.ThreadName == None:
                            self.ThreadValue.ThreadName ='te'
                            self.ThreadValue.Thread_Object = t2

                        if self.ThreadValue.ThreadName != 'Cused':
                            self.ThreadValue.Thread_Object.signal = False
                            ...
                            self.ThreadValue.ThreadName = 'Cused'
                            self.ThreadValue.Thread_Object = t21
                            self.ThreadValue.Thread_Object.mes = dta
                            self.ThreadValue.Thread_Object.TypeRead = dta[1]
                            self.ThreadValue.Thread_Object.start()
                        break
                    if dta[1] == 'Copen\n':
                        if self.ThreadValue.ThreadName == None:
                            self.ThreadValue.ThreadName ='rg'
                            self.ThreadValue.Thread_Object = t2

                        if self.ThreadValue.ThreadName != 'Copen':
                            self.ThreadValue.Thread_Object.signal = False
                            self.ThreadValue.ThreadName = 'Copen'
                            t22 = MyTask_Tag.MyTask_Tag(
                                 lstInput=self.lstinput, lstLock=self.lstLock,
                                host=self.host, Port=self.Port,
                                input1=self._input1, input2=self._input2, output1=self._output1, output2=self._output2,
                                Pn532=self.pn532, main=self
                            )
                            self.ThreadValue.Thread_Object = t22
                            self.ThreadValue.Thread_Object.mes = dta
                            self.ThreadValue.Thread_Object.TypeRead = dta[1]
                            self.ThreadValue.Thread_Object.start()
                            break
