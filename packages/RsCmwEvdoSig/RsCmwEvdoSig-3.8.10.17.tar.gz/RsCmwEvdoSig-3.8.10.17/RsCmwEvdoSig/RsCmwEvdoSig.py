...
class RsCmwEvdoSig:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.10.0017'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwEvdoSig._driver_options, options, direct_session)
		self._core.driver_version = '3.8.10.0017'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwEvdoSig':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwEvdoSig session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwEvdoSig with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.10.0017'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwEvdoSig version failed. Current version: '3.8.10.0017', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 20 Sub-classes, 2 commands."""
	...
	@property
	def pdata(self):
		"""pdata commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_pdata'):
			from .Implementations.Pdata import Pdata
			self._pdata = Pdata(self._core, self._base)
		return self._pdata

	@property
	def per(self):
		"""per commands group. 1 Sub-classes, 3 commands."""
		if not hasattr(self, '_per'):
			from .Implementations.Per import Per
			self._per = Per(self._core, self._base)
		return self._per

	@property
	def throughput(self):
		"""throughput commands group. 1 Sub-classes, 3 commands."""
	...
	def clone(self) -> 'RsCmwEvdoSig':
		"""Creates a deep copy of the RsCmwEvdoSig object. Also copies:
		...
		cloned = RsCmwEvdoSig.from_existing_session(self.get_session_handle(), self._options)
