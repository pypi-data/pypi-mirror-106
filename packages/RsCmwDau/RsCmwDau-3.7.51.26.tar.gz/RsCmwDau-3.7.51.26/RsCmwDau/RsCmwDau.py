...
class RsCmwDau:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.51.0026'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwDau._driver_options, options, direct_session)
		self._core.driver_version = '3.7.51.0026'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwDau':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwDau session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwDau with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.51.0026'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwDau version failed. Current version: '3.7.51.0026', minimum required version: '{min_version}'")
	...
	def _add_all_global_repcaps(self) -> None:
		...
		self._core.io.add_global_repcap('<MeasInstance>', RepeatedCapability("ROOT", 'repcap_measInstance_get', 'repcap_measInstance_set', repcap.MeasInstance.Inst1))

	def repcap_measInstance_get(self) -> repcap.MeasInstance:
		"""Returns Global Repeated capability MeasInstance \n
		Selects the instrument"""
		return self._core.io.get_global_repcap_value('<MeasInstance>')

	def repcap_measInstance_set(self, value: repcap.MeasInstance) -> None:
		"""Sets Global Repeated capability MeasInstance \n
		Selects the instrument
		Default value after init: MeasInstance.Inst1"""
		self._core.io.set_global_repcap_value('<MeasInstance>', value)
	...
	@property
	def rdau(self):
		"""rdau commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_rdau'):
			from .Implementations.Rdau import Rdau
			self._rdau = Rdau(self._core, self._base)
		return self._rdau

	def clone(self) -> 'RsCmwDau':
		"""Creates a deep copy of the RsCmwDau object. Also copies:
		...
		cloned = RsCmwDau.from_existing_session(self.get_session_handle(), self._options)
		self._base.synchronize_repcaps(cloned)
		cloned.repcap_measInstance_set(self.repcap_measInstance_get())
		return cloned

	def restore_all_repcaps_to_default(self) -> None:
		"""Sets all the Group and Global repcaps to their initial values"""
		self._base.restore_repcaps()
		self.repcap_measInstance_set(repcap.MeasInstance.Inst1)
