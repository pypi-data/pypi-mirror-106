...
def execute(command, cwd='/'):
    ...
    """
    sub = subprocess.Popen(command, cwd=cwd, stdin=subprocess.PIPE, stdout=sys.stdout,
...
def get_last_commit_sha_by_branch(git_host, git_token, git_project_id, git_branch):
    url = '%s/api/v4/projects/%s/repository/commits/%s?private_token=%s' % (
        git_host, git_project_id, quote(git_branch, 'utf-8'), git_token)
...
def get_gitlab_last_pipeline_id_by_branch(git_host, git_token, git_project_id, git_branch):
    url = '%s/api/v4/projects/%s/pipelines?status=success&ref=%s&private_token=%s&per_page=1&order_by=id&sort=desc' % (
        git_host, git_project_id, quote(git_branch, 'utf-8'), git_token)
    logging.info(url)
    res = json.loads(request.urlopen(url).read())
    if len(res) == 1:
        ...
        raise Exception('pipeline id get failure %s' % url)


def get_gitlab_file_cache(git_host, git_token, git_project_id, git_branch, unzip_path, unzip_rename, namespace):
    commit_sha = get_last_commit_sha_by_branch(git_host=git_host, git_token=git_token, git_project_id=git_project_id,
                                               git_branch=git_branch)
    # 初始化项目
    file_name = '%s.zip' % (commit_sha)
    zip_save_path = os.path.join(unzip_path, file_name)
    unzip_rename_path = os.path.join(unzip_path, unzip_rename)
    url = "%s/api/v4/projects/%s/repository/archive.zip?sha=%s&private_token=%s" % (git_host,
                                                                                    git_project_id, commit_sha,
                                                                                    git_token)
    if not os.path.exists(zip_save_path):
        FileCache.set_namespace(namespace=namespace).get(key=file_name,
                                                         local_file=zip_save_path,
                                                         default=url)

    if os.path.exists(zip_save_path):
        if not os.path.exists(unzip_rename_path):
            logging.debug('unzip from %s to  %s' % (zip_save_path, unzip_path))
            unzip(zip_save_path, unzip_path)
            unzip_dir_name = glob.glob('%s/*-%s-*' % (unzip_path, commit_sha))[0]
            os.rename(unzip_dir_name, unzip_rename_path)
    else:
        logging.info(file_name)
        logging.info(url)
        raise Exception('file not exist :%s' % zip_save_path)
