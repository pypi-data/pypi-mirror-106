...
class WS_Server:
    on: Callable[[WS_Session, WS_Response], Any]
    new_connection: Callable[[WS_Session], Any]
    break_connection: Callable[[WS_Session], Any]
    all_conn: list = []
    __on = True

    def __init__(self, ip, port, on: Callable[[WS_Session, WS_Response], Any] = None,
    ...
    def handle_client_connection(self, ws_session: WS_Session):
        ...
        while True:
            try:
                ...
                ws_response: WS_Response = pickle.loads(bytes_response)
                if self.on is not None:
                    self.on(ws_session, ws_response)
...
class WS_Client:
    ...
    on: Callable[[WS_Response], Any]
    break_connection: Callable[[], Any]
    __on = True

    def __init__(self, ip, port, on: Callable[[WS_Response], Any] = None,
    ...
    def send(self, method: str, data: dict = None):
        try:
            ws_request = WS_Response(method, data)
            self.server_sock.send(pickle.dumps(ws_request))
        except OSError:
            return
