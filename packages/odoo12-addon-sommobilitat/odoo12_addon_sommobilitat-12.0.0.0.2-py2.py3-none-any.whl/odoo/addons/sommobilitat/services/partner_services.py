...
class PartnerService(Component):
  ...
  def update_general_info(self, _id=None, **params):
    ...
    partner.sudo().write({
      ...
      'birthdate_date': params['birthdate_date'],
      'vat': params['vat'],
    })
    return Response("Actualitzat correctament", status=200)

  def get_num_member(self, _id=None, **params):
    num_partners = self.env['res.partner'].sudo().search_count([('cooperator_register_number', '>', '0')])
  ...
  def create(self, **params):
    ...
    """
    last_cooperator_register_number_id = self.env['res.partner'].sudo().search([
      ("cooperative_member", "=", True)
    ], order="cooperator_register_number desc", limit=1).cooperator_register_number

    params['cooperator_register_number'] = last_cooperator_register_number_id + 1
  ...
  def get_res_partner_by_member_nr(self, member_nr):
    return self.env['res.partner'].sudo().search([('cooperator_register_number', '=', member_nr)])
  ...
  def _validator_create(self):
    res = {
      ...
      'birthdate_date': {'type': 'string', 'required': True, 'empty': False},
