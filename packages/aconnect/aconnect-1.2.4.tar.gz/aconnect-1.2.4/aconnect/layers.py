	...
	"""	
		...
		def forward(self,W,bias,X):
			...
			def grad(dy):
				...
				return dW,dB,dX
			return Z, grad """
	return FC_AConnect(output_size,Wstd=Wstd,Bstd=Bstd,isBin=isBin,pool=pool,Slice=Slice,d_type=d_type,
...
def Conv_AConnect(filters,kernel_size,strides=1,padding="VALID",Wstd=0,Bstd=0,pool=None,isBin='no',Op=1
	...
	def Z_reshape(Z,F,X,padding,strides): #Used to reshape the output of the layer
		...
		#return out              			
	return Conv_AConnect(filters,kernel_size,strides=strides,padding=padding,Wstd=Wstd,Bstd=Bstd,pool=pool,isBin=isBin,Op=Op
