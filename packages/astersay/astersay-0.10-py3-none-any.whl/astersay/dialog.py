...
class Dialog:
    ...
    def _script_capture(self, script, variables):
        ...
        # Переменная для определения молчания.
        _speech = speech
        if stop:
        ...
        while not stop and second < maxtime:
            ...
            speech, stop = func(stopwords, mode=mode)
            if not stop:
                stop = silence_second >= silence and len(speech) >= minlength
            if _speech != speech:
                # log_debug('old speech %r', _speech)
                # log_debug('new speech %r', speech)
                silence_second = 0
                _speech = speech
            else:
                log_debug(_('Тишина %r секунд.'), silence_second)
                pass
            if self.is_closed:
