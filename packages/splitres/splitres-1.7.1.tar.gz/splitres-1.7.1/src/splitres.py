...
def splitres_for_file(file_path, result_dir):
    ...
    _, results_dict = counting_values(data)
...
def write_to_file(path, results: Dict[str, float]):
    ...
    if not isinstance(results, dict):
        return -1, 'Incorrect type results'

    if not isinstance(path, str):
        return -1, 'Incorrect type path'
...
def check_exist_dir(dir_name):
    ...
    if not isinstance(dir_name, str):
        return -1, 'Your enter incorrect value'
...
def parse_file_kernel():
    """
        This function parse messtat files and cut from them
        necessary information(info about kernels).

        Args:
        Returns:
            data_dict (list): list of values which parsed from messtat file.
    """
    data_dict = {}

    for address, dirs, files in os.walk(str(pathlib.Path.cwd())+'/web_spl/media/'):
        for file in files:
            if not file.endswith('messtat'):
                continue
            file = open(address + '/' + file)
            file_data = file.read().splitlines()
            file.close()

            data_file_with_zero = [ [ float(d) for d in file_data[i - 1].split('\t')[0:-1]]
                     for i in range(2, len(file_data) + 1) if i  % 3 == 0 ]

            data_file_without_zero = [ [ dd for dd in d if dd != 0.00] for d in data_file_with_zero]
            data_file_kernel_len = [len(d) for d in data_file_without_zero]

            data_dict[file.name.split('//')[1]] = {
                'data'              : data_file_with_zero,
                'data_without_zero' : data_file_without_zero,

                'min_kernel_uses' : min(data_file_kernel_len),
                'avg_kernel_uses' : statistics.mean(data_file_kernel_len),
                'max_kernel_uses' : max(data_file_kernel_len),

                'min_kernel_proc' : min([min(d) for d in data_file_without_zero if d != [] ]),
                'avg_kernel_proc' : statistics.mean([statistics.mean(d) for d in data_file_without_zero if d != []]),
                'max_kernel_proc' : max([max(d) for d in data_file_without_zero  if d != []])
            }
