...
from ..modules.modules import TransformerEncoder, Embeddings
...
class DistilBertModel(torch.nn.Module):
    def __init__(
            self, vocab_size: int, hidden_size: int, padding_idx: int, max_position_embeddings: int, ffn_size: int,
            n_heads: int, n_layers: int
    ):
        super().__init__()

        self.embeddings = Embeddings(
            vocab_size=vocab_size, dim=hidden_size, padding_idx=padding_idx,
            max_position_embeddings=max_position_embeddings)
        self.encoder = TransformerEncoder(
            n_heads=n_heads, n_layers=n_layers, embedding_size=hidden_size, ffn_size=ffn_size)
...
class DistilBertForMaskedLM(Module):
    def __init__(
        ...
        self.transformer = DistilBertModel(
            ...
            max_position_embeddings=max_position_embeddings, n_heads=n_heads, n_layers=n_layers, ffn_size=ffn_size
        )
        self.vocab_transform = Linear(hidden_size, hidden_size)
        self.vocab_layer_norm = LayerNorm(hidden_size)
        self.vocab_projector = Linear(hidden_size, vocab_size)

    @classmethod
    def from_pretrained(cls, model_path: str) -> 'DistilBertForMaskedLM':
        model = cls(
            vocab_size=28996, hidden_size=768, padding_idx=0, max_position_embeddings=512, ffn_size=3072, n_heads=12,
            n_layers=6
        )
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        return self.vocab_projector(prediction_logits)
