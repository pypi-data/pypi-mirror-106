...
from ..modules.modules import TransformerEncoder, MaskEmbeddings
...
class RobertaModel(torch.nn.Module):
    def __init__(
            self, vocab_size: int, hidden_size: int, padding_idx: int,
        ...
        self.encoder = TransformerEncoder(
            n_heads=n_heads, embedding_size=hidden_size, ffn_size=ffn_size, n_layers=n_layers)
...
class RobertaForQuestionAnswering(torch.nn.Module):
    def __init__(
        ...
        self.qa_outputs = Linear(hidden_size, num_labels)

    @classmethod
    def from_pretrained(cls, model_path: str) -> 'RobertaForQuestionAnswering':
        model = cls(
            vocab_size=50265, hidden_size=768, padding_idx=1, max_position_embeddings=514,
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        sequence_output = self.transformer(input_ids, attention_mask=attention_mask)
        logits = self.qa_outputs(sequence_output).to('cpu')
