...
def waitSend(chat_id, media_group_size):
	wait = next_send_time.get(chat_id, 0) + media_group_size ** 2 + media_group_size * 10 - time.time()
	next_send_time[chat_id] = time.time() + media_group_size ** 2 + media_group_size * 10
	if wait > 0:
		time.sleep(wait)
