...
class TourBase:
    ...
    * `tip_text` or (empty string): tour page tooltip text fstring message text template.
    ...
    def auto_switch_next_page(self):
        ...
        main_app.vpo(f"TourOverlay.auto_switch_next_page from page #{self.page_idx} delay={delay}")
...
class TourDropdownFromButton(TourBase):
    ...
    def setup_prev_page(self):
        ...
        if self.page_idx == 1:
            if flow_action(self.page_ids[self.page_idx - 1]) == 'open':
    ...
    def update_page_ids(self):
        ...
        if self.page_idx == 1 and page_ids[-1] == TourDropdownFromButton.determine_page_ids:
            main_app = self.main_app
            popups = main_app.popups_opened()
            if not popups:
                main_app.po("TourDropDownFromButton.update_page_ids: dropdown not opened")
                return

            children = main_app.widget_tourable_children_page_ids(popups[-1])
            if not children:
                main_app.po(f"TourDropDownFromButton.update_page_ids missing tour-able child in {popups[-1]}")
                return
...
class OnboardingTour(TourBase):
    ...
    def update_page_ids(self):
        ...
        super().update_page_ids()
        if self.last_page_id == 'switch_page':
