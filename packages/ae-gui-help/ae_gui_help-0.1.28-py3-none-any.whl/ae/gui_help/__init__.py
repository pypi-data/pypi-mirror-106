...
class TourBase:
    ...
    * `tip_text` or (empty string): tour page tooltip text fstring message text template.
    ...
    def update_page_ids(self):
        ...
        self.main_app.vpo(f"TourBase.update_page_ids ids={self.page_ids}")
...
class TourDropdownFromButton(TourBase):
    ...
    def update_page_ids(self):
        ...
        if self.page_idx == 1 and len(page_ids) == 2 and page_ids[1] == TourDropdownFromButton.determine_page_ids:
            ...
            if not popups:
                self.main_app.po("TourDropDownFromButton.update_page_ids: dropdown not opened")
                return

            children = self.main_app.widget_tourable_children_page_ids(popups[-1])
            if not children:
                self.main_app.po(f"TourDropDownFromButton.update_page_ids missing tour-able child in {popups[-1]}")
                return
...
class OnboardingTour(TourBase):
    ...
    def setup_page(self):
        ...
        super().setup_page()
        if self.page_ids[self.page_idx] == 'layout_font_size':
            main_app = self.main_app
    ...
    def update_page_ids(self):
        ...
        super().update_page_ids()
        try:
            remove_idx = self.page_ids.index('switch_page')
        except ValueError:
            return              # already removed this tour page

        delta_idx = self.page_idx - remove_idx
        if delta_idx == 1 or (delta_idx == -1 and self.came_back):
            self.page_ids.remove('switch_page')
            if delta_idx == 1:
                self.page_idx -= 1
                self.load_page_data()   # reload data of the page, situated one idx after the removed page
