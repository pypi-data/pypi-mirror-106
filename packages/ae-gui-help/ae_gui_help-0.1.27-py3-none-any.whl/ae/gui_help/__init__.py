...
def layout_ps_hints(wid_x: float, wid_y, wid_width, wid_height, win_width, win_height) -> PsHintsType:
...
class HelpAppBase(MainAppBase, ABC):
    ...
    def save_app_states(self) -> str:
        """ overwrite MainAppBase method to auto-cancel active tour on app pause/close. """
        if self.tour_layout:
...
class TourBase:
    ...
    attr:`~TourBase.page_ids` and :attr:`~TourBase.page_idx` attributes are correctly set. An UI-framework-dependent
    ...
    """
    auto_switch_pages: bool = False
    """ if True switch automatically tour pages within the seconds specified by :attr:`.page_data`['next_page_delay'].
    """

    page_data: Dict[str, Any] = dict(help_vars=dict())
    ...
    * `app_flow_delay`: time in seconds to wait until app flow change is fully completed (def=1.2, >0.9 for auto-width).
    ...
    * `next_page_delay`: time in seconds to read the current tour page before next auto_switch_next_page() (def=9.6).
    * `tip_text` or (empty string): tour page tooltip text fstring message text template.
    """

    page_ids: List[str] = list()
    """ tour page ids, either initialized via this class attribute or dynamically. """

    pages_explained_matcher: Dict[str, Union[Callable[[Any], bool], str]] = dict()
    ...
    def __init__(self, main_app: HelpAppBase):
        ...
        self.auto_switch_next_page_request = None
        self._saved_app_states: Dict[str, Any] = dict()

        self.main_app = main_app
        assert main_app.tour_layout
        self.layout = main_app.tour_layout

        self.page_ids = self.page_ids.copy()        # copy page ids from class to instance attribute
        self.page_idx = 0
        self.came_back = False

        self.backup_app_states()

    def auto_switch_cancel(self):
        """ cancel auto switch callback if requested. """
        if self.auto_switch_next_page_request:
            self.auto_switch_next_page_request.cancel()
            self.auto_switch_next_page_request = None
        self.auto_switch_pages = False

    def auto_switch_next_page(self):
        """ initiate automatic switch to next tour page. """
        main_app = self.main_app
        delay = self.page_data.get('next_page_delay', 9.6)
        main_app.vpo(f"TourOverlay.auto_switch_next_page from page idx={self.page_idx} delay={delay}")
        if self.page_idx + 1 < len(self.page_ids):
            self.auto_switch_next_page_request = main_app.call_method_delayed(delay, self.next_page)
    ...
    def load_page_data(self):
        """ load page before switching to it. """
        ...
        page_data = deepcopy(self.__class__.page_data)        # reset to tour class default: dict(help_vars=dict(), ...)
    ...
    def next_page(self):
        ...
        this_page_id = ids[self.page_idx]
        self.page_idx += 1
        next_page_id = ids[self.page_idx]
        self.main_app.vpo(f"TourBase.next_page(): tour page{self.page_idx} id={this_page_id}->{next_page_id}")
    ...
    def prev_page(self):
        ...
        self.came_back = True
        self.auto_switch_cancel()
        self.teardown_page()

        self.setup_prev_page()

        assert self.page_idx > 0, f"TourBase.prev_page wrong/missing page {self.__class__.__name__}:{self.page_idx - 1}"
        this_page_id = self.page_ids[self.page_idx]
        self.page_idx -= 1
        prev_page_id = self.page_ids[self.page_idx]
        self.main_app.vpo(f"TourBase.prev_page(): tour page{self.page_idx} id={this_page_id}->{prev_page_id}")

        self.setup_page()

    def remove_page(self, page_id: str):
        """ remove tour page in teardown page process.

        :param page_id:         id of the tour page to remove from page ids.
        """
        if page_id in self.page_ids:
            self.page_ids.remove(page_id)
            if not self.came_back:              # if user is leaving the removed page - moving to the next one
                self.page_idx -= 1              # .. then correct page index

    def reset_app_flow(self):
        """ delayed reset of flow path/id for to be processed after dispatch of popups dismiss. """
    ...
    def setup_layout(self):
        ...
        if self.auto_switch_pages:
            self.auto_switch_next_page()

    def setup_next_page(self):
        """ prepare app flow to display next page. """
        self.main_app.vpo("TourBase.setup_next_page")

    def setup_page(self):
        """ setup tour overlay to load data for to display new tour page. """
        self.main_app.vpo(f"TourBase.setup_page page_data={self.page_data}")

        self.load_page_data()

        app_flow_delay = self.page_data.get('app_flow_delay', 1.2)  # > 0.9 to complete auto width animation
        self.main_app.call_method_delayed(app_flow_delay, self.setup_layout)

    def setup_page_ids(self):
        """ setup page ids before the tour overlay/layout setup. """
        self.main_app.vpo(f"TourBase.setup_page_ids ids={self.page_ids}")

    def setup_prev_page(self):
        """ setup app flow to display the previous tour page. """
        self.main_app.vpo("TourBase.setup_prev_page")
    ...
    def teardown_page(self):
        """ restore tour page state before switch to prev/next page or tour finishing. """
        self.main_app.vpo("TourBase.teardown_page")


class TourDropdownFromButton(TourBase):
    """ generic tour base class for to auto-explain a dropdown menu, starting with the button opening the dropdown. """
    determine_page_ids = '_v_'

    def setup_next_page(self):
        """ overwrite to prevent opening of sub-menus. """
        super().setup_next_page()
        if self.page_idx == 0:
            this_page_id = self.page_ids[self.page_idx]
            if flow_action(this_page_id) == 'open':
                self.main_app.change_flow(this_page_id, **update_tap_kwargs(self.layout.explained_widget))

    def setup_prev_page(self):
        """ overwrite to prevent close of dropdown if dropdown child would open another dropdown. """
        super().setup_prev_page()
        if self.page_idx == 1:
            prev_page_id = self.page_ids[self.page_idx - 1]
            if flow_action(prev_page_id) == 'open':
                self.main_app.popups_opened()[-1].close()

    def setup_page_ids(self):
        """ prepare all tour pages - first page is explaining opening dropdown button. """
        super().setup_page_ids()
        page_ids = self.page_ids
        if self.page_idx == 1 and len(page_ids) == 2 and page_ids[1] == TourDropdownFromButton.determine_page_ids:
            popups = self.main_app.popups_opened()
            if not popups:
                self.main_app.po("TourDropDownFromButton.setup_page_ids: dropdown not opened")
                return

            children = self.main_app.widget_tourable_children_page_ids(popups[-1])
            if not children:
                self.main_app.po(f"TourDropDownFromButton.setup_page_ids missing tour-able child in {popups[-1]}")
                return
...
class OnboardingTour(TourBase):
    ...
    page_ids = ['', 'switch_page', 'help_activate', 'tooltip', 'responsible_layout', 'layout_font_size', 'stop_tour']

    pages_explained_matcher = dict(
        help_activate=lambda widget: widget.__class__.__name__ == 'HelpToggler',
        tooltip=lambda widget: getattr(widget, 'tap_flow_id', None) == _OPEN_USER_PREFERENCES_FLOW_ID,
    ...
    def teardown_page(self):
        ...
        if page_id == 'switch_page':
            self.remove_page('switch_page')
...
class UserPreferencesTour(TourDropdownFromButton):
    ...
    auto_switch_pages = True
    page_data = dict(help_vars=dict(), next_page_delay=3.6)
    page_ids = [_OPEN_USER_PREFERENCES_FLOW_ID, TourDropdownFromButton.determine_page_ids]
