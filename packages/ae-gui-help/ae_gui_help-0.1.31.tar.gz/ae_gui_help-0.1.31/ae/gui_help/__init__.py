...
class TourBase:
    ...
    * `tip_text` or (empty string): tour page tooltip text fstring message text template.
    ...
    def __init__(self, main_app: HelpAppBase):
        ...
        self._auto_switch_next_page_request = None
    ...
    def auto_switch_cancel(self):
        ...
        if self._auto_switch_next_page_request:
            self._auto_switch_next_page_request.cancel()
            self._auto_switch_next_page_request = None
    ...
    def auto_switch_next_page(self):
        ...
        if self.page_idx + 1 < len(self.page_ids):
            self._auto_switch_next_page_request = main_app.call_method_delayed(delay, self.next_page)
...
class TourDropdownFromButton(TourBase):
    ...
    def setup_page(self):
        ...
        if self.last_page_idx == 0 and self.page_idx == 1:
            ...
            if flow_action(self.page_ids[self.page_idx]) == 'open':
