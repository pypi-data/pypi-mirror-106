...
import getpass
import datetime
from .tools.geom_dtf import feature_id, geom_prop
...
# B.1) Identify layers and each base and top
        try:
            rl_flag, bases, tops, wct = \
                feature_id(alt_arr[i], sig = prod_arr[i], err = prod_err_arr[i],
                           alpha = alphas[i], snr_factor = snr_factor, 
                           wct_peak_margin = wct_peak_margin, log = logger)
        except Exception as e:
...
# B.2) Use base and top to the profile to extract aditional geometrical properties
        ...
        # debug_layers(alt = alt_arr[i], sig = prod_arr[i], std = prod_err_arr[i], 
        #               wct = wct, geom = geom, max_alt = 10., 
        #               snr_factor = snr_factor, prod_ulim = 5., 
        #               date = meas_id, dir_out = './', dpi_val = 300)
            ...
            try:
                ...
                geom_dts = \
                    export_nc(geom, metadata = metadata[i], alpha = alphas[i], 
                              wave = wave[i], snr_factor = snr_factor, 
