...
class zxtouch:
    ...
    def touch(self, type, finger_index, x, y):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_PERFORM_TOUCH,
                '1{}{:02d}{:05d}{:05d}'.format(type, finger_index, int(x * 10), int(y * 10)),
            )
        )
    ...
    def touch_with_list(self, touch_list: list):
        ...
        for touch_event in touch_list:
            event_data += '{}{:02d}{:05d}{:05d}'.format(
                touch_event['type'], touch_event['finger_index'], touch_event['x'] * 10, touch_event['y'] * 10
            )
    ...
    def image_match(self, template_path, acceptable_value=0.8, max_try_times=4, scaleRation=0.8):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_TEMPLATE_MATCH, template_path, max_try_times, acceptable_value, scaleRation
            )
        )
    ...
    def show_toast(self, toast_type, content, duration, position=0, fontSize=0):
        ...
        """
        self.s.send(
            datahandler.format_socket_data(tasktypes.TASK_SHOW_TOAST, toast_type, content, duration, position, fontSize)
        )
        return datahandler.decode_socket_data(self.s.recv(1024))
    ...
    def search_color(self, region, red_min, red_max, green_min, green_max, blue_min, blue_max, pixel_to_skip=0):
        ...
        self.s.send(
            datahandler.format_socket_data(
                ...
                colorsearchtasktypes.SEARCH_RGB_SINGLE_POINT,
                region[0],
                region[1],
                region[2],
                region[3],
    ...
    def set_clipboard_text(self, text):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_KEYBOARDIMPL, kbdtasktypes.KEYBOARD_SAVE_TEXT_TO_CLIPBOARD, text
            )
        )
        return datahandler.decode_socket_data(self.s.recv(1024))
    ...
    def insert_text(self, text):
        ...
        for i, ch in enumerate(text):
            if ch == "\b":
                self.s.send(
                    datahandler.format_socket_data(
                        tasktypes.TASK_KEYBOARDIMPL, kbdtasktypes.KEYBOARD_DELETE_CHARACTERS, 1
                    )
                )
    ...
    def get_screen_size(self):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_GET_DEVICE_INFO, deviceinfotasktypes.DEVICE_INFO_TASK_GET_SCREEN_SIZE
            )
        )
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
    ...
    def get_screen_orientation(self):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_GET_DEVICE_INFO, deviceinfotasktypes.DEVICE_INFO_TASK_GET_SCREEN_ORIENTATION
            )
        )
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
    ...
    def get_screen_scale(self):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_GET_DEVICE_INFO, deviceinfotasktypes.DEVICE_INFO_TASK_GET_SCREEN_SCALE
            )
        )
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
    ...
    def get_device_info(self):
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_GET_DEVICE_INFO, deviceinfotasktypes.DEVICE_INFO_TASK_GET_DEVICE_INFO
            )
        )
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
    ...
    def get_battery_info(self):
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_GET_DEVICE_INFO, deviceinfotasktypes.DEVICE_INFO_TASK_GET_BATTERY_INFO
            )
        )
        result = datahandler.decode_socket_data(self.s.recv(1024))
        if not result[0]:
        ...
        return True, {
            ...
            "battery_level": str(int(float(result[1][1]))),
            "battery_state_string": battery_state_list[battery_state_return],
    ...
    def ocr(
        ...
        self.s.send(
            datahandler.format_socket_data(
                tasktypes.TASK_TEXT_RECOGNIZER,
                1,
                rect_data,
                custom_words_data,
                minimum_height,
        ...
        for raw_info in raw_info_list:
            ...
            temp_dict = {
                "text": single_string_info[0],
                "x": single_string_info[1],
                "y": single_string_info[2],
                "width": single_string_info[3],
                "height": single_string_info[4],
            }
    ...
    def get_supported_ocr_languages(self, recognition_level):
        ...
        """
        self.s.send(datahandler.format_socket_data(tasktypes.TASK_TEXT_RECOGNIZER, 2, recognition_level))
