...
class RsCmwGprfGen:
	"""178 total commands, 4 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.50.0053'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwGprfGen._driver_options, options, direct_session)
		self._core.driver_version = '3.7.50.0053'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwGprfGen':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwGprfGen session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwGprfGen with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.50.0053'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwGprfGen version failed. Current version: '3.7.50.0053', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 1 Sub-classes, 1 commands."""
	...
	def clone(self) -> 'RsCmwGprfGen':
		"""Creates a deep copy of the RsCmwGprfGen object. Also copies:
		...
		cloned = RsCmwGprfGen.from_existing_session(self.get_session_handle(), self._options)
