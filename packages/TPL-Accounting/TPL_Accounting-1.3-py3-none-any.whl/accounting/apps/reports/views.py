...
class SettingsListView(LoginRequiredMixin, generic.TemplateView):
...
class BusinessSettingsUpdateView(
    LoginRequiredMixin,
    GenericSettingsMixin,
    generic.UpdateView
):
...
class FinancialSettingsUpdateView(
    LoginRequiredMixin,
    GenericSettingsMixin,
    generic.UpdateView
):
...
class PayRunSettingsUpdateView(
    LoginRequiredMixin,
    GenericSettingsMixin,
    generic.UpdateView
):
...
class ProfitAndLossReportView(LoginRequiredMixin, generic.TemplateView):
...
class InvoiceDetailsView(
    LoginRequiredMixin, TimePeriodFormMixin, generic.FormView
):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        orga = organization_manager.get_selected_organization(self.request)
        report = InvoiceDetailsReport(
            orga, start=self.period.start, end=self.period.end
        )
        report.generate()
