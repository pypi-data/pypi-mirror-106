...
class Organization(models.Model):
    display_name = models.CharField(
        max_length=150, help_text="Name that you communicate"
    )
    legal_name = models.CharField(
        max_length=150,
        help_text="Official name to appear on your reports,\
        sales invoices and bills"
    )

    owner = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        related_name="owned_organizations", null=True
    )
    ...
    def overdue_total(self):
        ...
        try:
            overdue_total = due_turnonver - total_paid
...
class TaxRate(models.Model):
    ...
    *inspired by Xero*
    ...
    organization = models.ForeignKey(
        'books.Organization', on_delete=models.SET_NULL,
        null=True,
...
class AbstractSale(CheckingModelMixin, models.Model):
    ...
    def check_date_dued(self, check):
        ...
        if self.is_fully_paid():
            ...
            check.mark_pass(message=f"Has been paid on the {formatted_date}")
            return check
...
class Estimate(AbstractSale):
    organization = models.ForeignKey(
        'books.Organization', on_delete=models.CASCADE,
        related_name="estimates",
        verbose_name="From Organization"
    )
    client = models.ForeignKey(
        'people.Client', on_delete=models.SET_NULL,
        null=True,
        verbose_name="To Client"
    )
    payments = GenericRelation(
        'books.Payment', on_delete=models.SET_NULL,
        null=True
    )
...
class EstimateLine(AbstractSaleLine):
    invoice = models.ForeignKey(
        'books.Estimate', on_delete=models.CASCADE,
        related_name="lines"
    )
    tax_rate = models.ForeignKey(
        'books.TaxRate', on_delete=models.PROTECT
    )
...
class Invoice(AbstractSale):
    ...
    client = models.ForeignKey(
        'people.Client', on_delete=models.SET_NULL,
        verbose_name="To Client", null=True
    )
    receipt_photo = models.FileField(
        _('Receipt photo'), upload_to="receipt_photo/%Y/%m/%d/",
        blank=True, null=True
    )
    payments = GenericRelation(
        'books.Payment', on_delete=models.SET_NULL,
...
class InvoiceLine(AbstractSaleLine):
    invoice = models.ForeignKey(
        'books.Invoice', on_delete=models.CASCADE,
        related_name="lines"
    )
    tax_rate = models.ForeignKey(
        'books.TaxRate', on_delete=models.PROTECT
...
class Bill(AbstractSale):
    ...
    client = models.ForeignKey(
        'people.Client', on_delete=models.SET_NULL,
        verbose_name="From Client", null=True
    )
    payments = GenericRelation(
        'books.Payment', on_delete=models.SET_NULL,
        null=True
...
class BillLine(AbstractSaleLine):
    bill = models.ForeignKey(
        'books.Bill', on_delete=models.CASCADE,
        related_name="lines"
    )
    tax_rate = models.ForeignKey(
        'books.TaxRate', on_delete=models.PROTECT
...
class ExpenseClaim(AbstractSale):
    ...
    employee = models.ForeignKey(
        'people.Employee', on_delete=models.SET_NULL,
        verbose_name="Paid by employee", null=True
    )
    payments = GenericRelation(
        'books.Payment', on_delete=models.SET_NULL, null=True
    )
...
class ExpenseClaimLine(AbstractSaleLine):
    expense_claim = models.ForeignKey(
        'books.ExpenseClaim', on_delete=models.CASCADE,
        related_name="lines"
    )
    tax_rate = models.ForeignKey(
        'books.TaxRate', on_delete=models.SET_NULL, null=True
    )
