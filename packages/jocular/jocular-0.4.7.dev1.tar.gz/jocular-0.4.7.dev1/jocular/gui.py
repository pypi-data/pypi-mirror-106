...
from jocular.widgets import (JLabel, JToggleButton, JLever, JIconButton,
	JButton, JMulti, JFilterButton, JMDIconButton)
...
class GUI(FloatLayout):
	...
	def redraw(self, *args):
		...
		logger.debug('resized to {:}'.format(Window.size))
	...
	def draw_element(self, wname, spec):
		...
		# to do: see if we can combine with JButton
			...
			w = JIconButton(angle=angle, origin=orig, radius=rad, icon=text) #, tooltip_text='tooltip')
	...
	def enable(self, names=None):
		...
		logger.debug('enabling {:}'.format(names))
	...
	def disable(self, names):
		...
		logger.debug('disabling {:}'.format(names))
