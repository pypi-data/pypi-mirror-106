...
class SimulatorCamera(GenericCamera):
	...
	def on_new_object(self):
		# here we should choose a random capture directory
		self.last_capture = None
...
class ASCOMCamera(GenericCamera):
	...
	def connect(self):
		...
		try:
			...
			self.status = 'No driver chosen ({:})'.format(e)
			return
	...
	def _connect(self, driver):
		...
		self.camera.connected = True   # this is camera-specific prop
...
class SXCamera(GenericCamera):
	...
	def connect(self):
		...
		self.status = 'NEW SX Lodestar: 752 x 580 pixels'
...
class WatchedCamera(GenericCamera):
	...
	def connect(self):
		...
		# do we have a watched directory?
	...
	def get_possible_fits(self):
		# look not only in watched but in the place ASILive dumps stuff
		wdir = self.watched_dir
		fits = [os.path.join(wdir, d) for d in os.listdir(wdir)]
	...
	def watch(self, dt):
		...
		for path in self.get_possible_fits():
			...
			# any non-directories that are not fits are also moved
	...
	def process_alien_sub(self, s, path):
		...
		if self.colour_space == 'Mono' or s.is_master:
	...
	def save_mono(self, sub, im, nm, filt=None, sub_type=None):
		...
		exposure = {
			'auto': sub.exposure,
			'from user': details.get('exposure', None),
		}[self.exposure]
		...
		if filt is None:
			filt = {
				'auto': sub.filter,
				'from user': details.get('filter', None),
				'assume L': 'L'
			}[self.filt]
		...
		if sub_type is None:
			sub_type = {
				'auto': sub.sub_type,
				'from user': details.get('sub_type', None),
				'assume light': 'light'
			}[self.sub_type]

		temperature = {
			'auto': sub.temperature,
			'from user': details.get('temperature', None),
		}[self.temperature]


		if sub.is_master:
			Component.get('Calibrator').new_master(
				data=im,
				exposure=exposure, 
				filt=filt, 
