...
class Stacker(Component, Settings):
    ...
    def add_sub(self, sub):
        ...
        # Component.get('CaptureScript').set_external_details(
        #     sub_type=sub.sub_type, exposure=sub.exposure, filter=sub.filter)


        self.app.gui.set('exposure_button', 
            '?' if sub.exposure is None else exp_to_str(sub.exposure))
        self.app.gui.set('filter_button', 
            '?' if sub.filter is None else ''.join(sub.filter))
        self.app.gui.set('script_button', 
            '?' if sub.sub_type is None else sub.sub_type)
