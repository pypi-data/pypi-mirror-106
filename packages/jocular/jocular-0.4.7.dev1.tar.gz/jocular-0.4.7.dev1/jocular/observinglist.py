...
class ObservingList(Component, Settings):
    ...
    configurables = [
        ('latitude', {
            'float': (-90, 90, .1),
            'fmt': '{:.1f} degrees',
            'help': 'In degrees [-90, 90], positive in N hemisphere, negative S of the equator'
            }),
        ('longitude', {'float': (-180, 180, .1),
            'fmt': '{:.1f} degrees',
            'help': 'In degrees [-180, 180], positive E of Greenwich, negative W'
            })
    ]
    ...
    # called once we are sure we have an object
    def lookup_details(self, nm):
        return self.objects.get(nm, {})

    def lookup_OTs(self, nm):
        if not hasattr(self, 'objects'):
            self.load()
        nm = nm.lower() + '/'
        return [n.split('/')[1] for n in self.objects.keys() if n.lower().startswith(nm)]
