...
class GrpcHandler(AbsMilvus):
    ...
    @error_handler(None)
    def get_query_segment_infos(self, collection_name, timeout=None, **kwargs):
    ...
    def wait_for_loading_collection(self, collection_name, timeout=None):
        ...
        """
        print("load collection start:" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + "\n")

        unloaded_segments = {info.segmentID: info.num_rows for info in
                                self.get_persistent_segment_infos(collection_name, timeout)}
        print("unloaded segment numbers: {}".format(len(unloaded_segments)) + ", collection:" + collection_name)

        while len(unloaded_segments) > 0:
            print("\n***********  waiting for load collection ***********" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ", unloaded num:{}".format(len(unloaded_segments)) +"\n")
            time.sleep(0.5)
    ...
    def wait_for_loading_partitions(self, collection_name, partition_names, timeout=None):
        ...
        print("unloaded segment numbers xxxxxx: {}".format(len(unloaded_segments))+ ", collection:" + collection_name)
    ...
    def wait_for_flushed(self, collection_name, timeout=None, **kwargs):
        ...
        print("flush start:" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + "\n")
        first_segment_ids = get_first_segment_ids()
        while True:
            time.sleep(0.5)
            print("\n***********  waiting for flushed ***********: " + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) +"\n")
