...
class load:
    def __init__(self, 
        ...
        **kwargs):
        ...
        def replaceCustomMissing(df, auditing_config):
            if (auditing_config != None):
                if (len(auditing_config["audit"]["define_custom_missing_value"]) > 0):
                    customMissing = auditing_config["audit"]["define_custom_missing_value"]
                    
                    df = df.replace(customMissing, np.nan)
        ...
        def checkPlatform():
            ...
            if platform == "linux" or platform == "linux2" or platform == "darwin":
                ...
                userPlatform = "Window"
            print("User Platform: {}".format(userPlatform))
            return userPlatform
        ...
        self.df         = replaceCustomMissing(self.df, self.auditing_config)
