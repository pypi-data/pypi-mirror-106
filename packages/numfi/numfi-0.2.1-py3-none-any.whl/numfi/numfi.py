...
class numfi(np.ndarray):
    ...
    def __new__(cls, input_array=[], s=None, w=None, f=None, **kwargs):
        ...
        obj.f = f 
        if w<=0:
            raise ValueError("word bits must be positive")
        if f<0:
            raise ValueError("fraction bits must be non-nagetive")
        if obj.i<0:
    ...
    #region property 
    # TODO: s/w/f property getter setter s_/w_/f_, move if w<=0 to setter checker
    ...
    def __fixed_arithmetic__(self, func, y):
        fiy = y if isinstance(y, numfi) else numfi(y, like=self)
        s = 1 if self.s or fiy.s else 0
        name = func.__name__[-5:-2]
        if name == 'add' or name == 'sub': 
            i = max(self.i, fiy.i)
            f = max(self.f, fiy.f)            
            result = numfi(func(fiy), s, i+f+s+1, f, like=self)
        elif name == 'mul':
            result = numfi(func(fiy), s, self.w+fiy.w, self.f+fiy.f, like=self) 
        elif name == 'div':
            if isinstance(y,numfi):
                result = numfi(func(y), s, max(self.w,y.w), self.f-y.f, like=self) 
            else:
                result = numfi(func(y), like=self) 
    ...
    __iadd__        = lambda self,y: self.__fixed_arithmetic__(super().__iadd__, y)
