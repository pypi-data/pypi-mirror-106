...
from ._code_maker import CodeMaker
...
class StructureMeta(DataMeta):
    ...
    def __new__(mcs, name, bases, namespace, byteorder: str = "little"):
        ...
        if new_members:
            ...
            BitFieldMember.organize(members_dict, byteorder)

            code_injector = CodeInjector(namespace)

            index = 0
            for member_name, member in members_dict.items():
                index = member.add_name_index(
                    member_name, index, code_injector, annotations
                )
