...
class BitFieldMember(Member):
    ...
    def _set_typ(self, typ: Any):
        try:
            is_int_subclass = issubclass(typ, int)
        ...
        if isinstance(typ, (FlagX, EnumX)) or is_int_subclass:
            ...
            raise TypeError(f"bit field {self.name!r} type must be int-like")
    ...
    @classmethod
    def organize(cls, members: Dict[str, Member], byteorder: str):
        ...
        for i, group in enumerate(collection):
            nbytes = cls.position_and_size(f"fields_{i}", group)
    ...
    @staticmethod
    def position_and_size(group_name: str, members: Dict[str, "BitFieldMember"]) -> int:
        """Fill in any unassigned positions, determine overall size."""
        # fill in missing positions (and assign group name)
        lsb = 0
        for member in members.values():
            member._group_name = group_name  # pylint: disable=protected-access

            if member._lsb is None:  # pylint: disable=protected-access
                member.lsb = lsb

            lsb = member.lsb + member.size
    ...
    def add_name_index(
        ...
        index = super().add_name_index(name, index, code_injector, annotations)
        ...
        lines = [
            f"{aux.group_name} |= ({aux.group_name} & {hi_mask}) | "
            f"((m_{self.name} & 0x{lo_mask:x}) << {self.lsb})"
        ]
        store.pack += lines
...
def bitfield_member(
    ...
    *,
    ...
    typ: Optional[Callable[[int], Any]] = None,
    signed: bool = False,
    default: Any = None,
    ignore: bool = False,
    readonly: bool = False,
