...
from .items import calcsize as _calcsize, items, Format
from .transform import Transform


def calcsize(fmt: Format) -> int:
    """Calculate format size (raise SizeError if variably sized)."""
    if isinstance(fmt, (Data, DataMeta, Transform)):
        return fmt.nbytes
...
def pack(value: Any, fmt: Format = None) -> bytes:
...
def pack_and_dump(value: Any, fmt: Format = None) -> Tuple[bytes, Dump]:
...
def unpack(fmt: Format, buffer: bytes) -> Any:
...
def unpack_and_dump(fmt: Format, buffer: bytes):
