...
class BitField(property):
    ...
    def __init__(
        ...
        self._name = None  # filled in later by BitFieldsMeta
        self._lsb = lsb
        ...
        if typ is None:
            # filled in later using type annotation byte BitFieldsMeta
            self._typ = None
        else:
            self._set_typ(typ)
    ...
    def argrepr(self) -> str:
        ...
        elif isinstance(self.typ, EnumMeta):
            repr_format = f"{self.name}={{self.{self.name}!s}}"
        else:
    ...
    def typ(self) -> Any:
        ...
        """
        if self._typ is None:  # pragma: no cover, for mypy purposes
            raise RuntimeError("internal error")
    ...
    def finish_initialization(self, name, annotations):
        ...
        self._type_hint = annotations.get(name, None)

        if self._typ is None:
            self._set_typ(self._type_hint or int)

    def __repr__(self):
        return f"BitField(name={self._name!r})"

    def getter(self, fget):
        # FUTURE - make a copy of self
