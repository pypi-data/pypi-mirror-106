...
SimpleFormat = Union[Transform, Type[Data]]
ComplexFormat = Union[Callable[[Type[Any]], SimpleFormat], Transform, Type[Data]]
...
class Member(property):
    ...
    def argrepr(self) -> str:
        ...
        if isinstance(self._argrepr, str):
            ...
            # EnumX, FlagX => use "s" to get cleaner enum representation
            # NoneX => likely a BitFieldMember, use "s" to handle enums
            style = "s" if isinstance(self.fmt, (IntX, EnumX, FlagX, NoneX)) else "r"
            repr_format = f"{self.name}={{self.{self.name}!{style}}}"
    ...
    def index(self) -> int:
        ...
        if self._index is None:  # pragma: no cover (for mypy)
            # FUTURE likely cause was getter/setter not immediately following member
    ...
    def add_name_index(
        ...
        if self.fmt_is_a_factory:
            ...
            cls_name = self.fmt.__name__
            try:
                is_same = code_injector.evaluate(cls_name) is self.fmt
            except NameError:
                is_same = False

            store.cls_name = cls_name if is_same else f"cls.{name}.fmt"
