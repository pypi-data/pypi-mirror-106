...
class ForStrictEnum(BitFields, nbytes=1):
    ...
    f1: int = bitfield(lsb=0, size=2)
    f2: MyEnum = bitfield(lsb=2, size=4)
    f3: int = bitfield(lsb=6, size=2)
...
class AutoAll(BitFields, nbytes=1):
...
class TestAutoPosition:
    ...
    """Test position automatically assigned when not provided."""

    def test_start_at_zero(self):
        """Test position starts at zero."""
        assert AutoAll.f1.lsb == 0
        assert AutoAll.f2.lsb == 2
        assert AutoAll.f3.lsb == 6
        assert repr(AutoAll.from_int(0x85)) == "AutoAll(f1=1, f2=1, f3=2)"

    def test_mix(self):
        """Test position automatically assigned when not provided."""
        assert AutoMix.f1.lsb == 0
        assert AutoMix.f2.lsb == 2
        assert AutoMix.f3.lsb == 7
        assert repr(AutoMix.from_int(0x85)) == "AutoMix(f1=1, f2=1, f3=1)"


class TestMeta:

    """Test creation of data store class."""
