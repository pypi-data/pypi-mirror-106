...
class TqApi(object):
    ...
    def get_quote_list(self, symbols: List[str]) -> List[Quote]:
        ...
        """
        if self._loop.is_running():
            self.create_task(self._get_quote_async(symbols), _caller_api=True)  # 协程中需要等待合约信息，然后发送订阅请求
    ...
    async def _get_quote_async(self, symbol: Union[str, List[str]]):
        # 协程中, 在收到合约信息之后再发送订阅行情请求，来保证，不会发送订阅请求去订阅不存在的合约
        symbol_list = symbol if isinstance(symbol, list) else [symbol]
        await self._ensure_symbol_async(symbol_list)
        self._auth._has_md_grants(symbol_list)
        if any([symbol not in self._requests["quotes"] for symbol in symbol_list]):
            self._requests["quotes"] = self._requests["quotes"].union(set(symbol_list))
            self._send_pack({
                "aid": "subscribe_quote",
                "ins_list": ",".join(self._requests["quotes"]),
            })
