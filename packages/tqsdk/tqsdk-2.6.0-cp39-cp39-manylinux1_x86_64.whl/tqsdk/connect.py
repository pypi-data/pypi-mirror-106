        ...
        while True:
            ...
            # 而这里的 except 又需要处理所有子函数及子函数的子函数等等可能抛出的例外, 因此这里只能遇到问题之后再补, 并且无法避免 false positive 和 false negative
                ...
                if client.reader._start_read_message:
                    print(f"{url}，当前时间 {time.time()}, 最后一次开始下载数据时间 {client.reader._start_read_message}，"
                          f"已读数据大小 {client.reader._read_size}", file=sys.stderr)
                ...
                if pack.get("aid") == "subscribe_quote":
                    ...
                    if self._subscribed_queue.full():
                        ...
                        if time.time() - first_time < 1:
                            warnings.warn(f"1s 内订阅请求次数超过 {self._subscribed_per_seconds} 次", stacklevel=3)
