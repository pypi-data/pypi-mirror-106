...
def _get_price(direction, price_mode, quote):
    """根据最新行情和下单方式计算出最优的下单价格"""
    # 主动买的价格序列(优先判断卖价，如果没有则用买价)
    price_list = [quote.ask_price1, quote.bid_price1]
    if direction == "SELL":
        price_list.reverse()
    ...
    if limit_price != limit_price:
        limit_price = quote.pre_close
    return limit_price


def _check_price(price, quote):
    if price in ("ACTIVE", "PASSIVE"):
        return lambda direction: _get_price(direction, price, quote)
