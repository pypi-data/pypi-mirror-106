...
class TargetPosTask(object, metaclass=TargetPosTaskSingleton):
    ...
    def __init__(self, api: TqApi, symbol: str, price: Union[str, Callable[[str], Union[float, int]]] = "ACTIVE",
        ...
        **注意**
        ...
        self._price = _check_price(price, self._quote)
    ...
    def cancel(self):
        ...
        Example1::
            ...
            api = TqApi(auth=TqAuth("信易账户", "账户密码"))
            target_pos = TargetPosTask(api, "SHFE.rb1810", price="PASSIVE")
            target_pos.set_target_volume(5)
            api.wait_update()
            # ... 用户代码 ...
            ...
            while True:
                # 调用wait_update()，会对已经发出但还是未成交的委托单撤单
                api.wait_update()
                if target_pos.is_finished():
                    break

            # 创建新的 TargetPosTask 实例
            target_pos = TargetPosTask(api, "SHFE.rb1810", price="ACTIVE")
