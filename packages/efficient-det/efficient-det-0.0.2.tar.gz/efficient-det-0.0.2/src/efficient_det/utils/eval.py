...
from layers.filter_predictions import get_detections
from utils.overlap import compute_intersection_over_union
from utils.anchors import generate_anchors_for_batch
...
def _get_detections(generator, model, score_threshold=0.05, max_detections=100, save_path=None):
    ...
    for i in range(generator.size()):
        ...
        boxes, scores, labels = get_detections(regression, classification, anchors_batch, generator.image_shape)
        boxes = boxes.numpy()
        scores = scores.numpy()
        labels = labels.numpy()
...
def _get_annotations(generator):
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Parsing annotations: '):
        ...
        for label in range(generator.num_classes):
            all_annotations[i][label] = annotations['boxes'][annotations['labels'] == label, :].copy()
    ...
    for label in range(generator.num_classes):
        ...
        for i in range(generator.size()):
            ...
            for d in detections:
                ...
                overlaps            = compute_intersection_over_union(np.expand_dims(d, axis=0), annotations)
