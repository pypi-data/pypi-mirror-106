...
def _parse_input_workspace_name(ctx, param, value):
    ...
    if not existing_workspaces:
        click.echo((
            'No AzureML Workspaces exists in this '
            'resource group: %s'
        ) % ctx.params['resource_group'])
        raise click.Abort()
...
def init_project(ctx, path, name, subscription_id, resource_group,
    ...
    #     pipelines = click.confirm(
    #         'Would you like Azure DevOps Pipelines added to your project?',
    #         default=True
    #     )
    #
    # if pipelines:
    ...
    if gitignore is None:
        gitignore = click.confirm(
            'Would you like a .gitignore added to your project?',
            default=True,
        )
