...
@tag2_seq2seq
    ...
    the PyTorch structures you need for seq2seq.  To that end, starting with
    Version 2.0.9, DLStudio incorporates the following two additional
    classes:
...
THE MAIN INNER CLASSES OF THE MODULE:
    ...
    (10) class Seq2SeqWithLearnableEmbeddings
         ...
         learning (seq2seq) is about predicting a target sequence from a
         source sequence.  This class uses English-to-Spanish translation to
         illustrate the PyTorch structures and the programming idioms you
         need to become familiar with if you wish to write your own seq2seq
         code.  Note that any attempt at seq2seq for machine translation must
         answer the following question at the outset: How to represent the
         words of a language for neural-network based processing? In general,
         you have two options: (1) Have your overall network learn on its own
         what are known as vector embeddings for the words; or (2) Use
         pre-trained embeddings as provided by word2vec or Fasttext.  The
         class Seq2SeqWithLearnableEmbeddings learns on its own the best
         embedding vectors to use for the words in the two languages.  After
         you have resolved the issue of word representation, your next
         challenge is how to implement the attention mechanism that you're
         going to need for aligning the similar grammatical units in the two
         languages. The Seq2SeqWithLearnableEmbeddings class (and also the
         next class) uses the attention model proposed by Bahdanau, Cho, and
         Bengio. The name of this attention class is Attention_BCB.  In a
         separate attention class named Attention_SR, I have also included
         the attention mechanism used by Sean Robertson in his very popular
         NLP tutorial at the main PyTorch website. Finally, if you wish to
         become familiar with DLStudio's class
    ...
    (11) class Seq2SeqWithPretrainedEmbeddings
         ...
         use for the words in the two languages. In
         Seq2SeqWithPretrainedEmbeddings, we use the Google's word2vec
         embeddings for the words of the source language (English).
         Importing pre-trained word embeddings for the source language means
         that the encoder part of the seq2seq network no longer needs an
         nn.Embeddings layer to learn those embeddings on its own.   The rest
         of the network, including the part that takes care of the attention
         mechanism, is the same as in the previous class. The following
         script in the Examples directory is based on this class:
