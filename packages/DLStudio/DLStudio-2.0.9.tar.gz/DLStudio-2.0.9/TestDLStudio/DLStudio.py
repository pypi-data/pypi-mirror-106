...
__date__      = '2021-May-17'   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.0.9.html'
...
@tag2_seq2seq
    ...
    Sequence-to-sequence (Seq2Seq) learning is about predicting an outcome
    ...
    probably one of the most popular applications of Seq2Seq learning.
    DLStudio uses English-to-Spanish translation to illustrate the
    programming idioms and structures you need for seq2seq learning.  To
    that end, starting with Version 2.0.9, DLStudio now incorporates the
    following two additional classes:
    ...
    The specific example of seq2seq addressed in my implementation code is
    translation from English to Spanish. (I chose this example because
    learning and keeping up with Spanish is one of my hobbies.)  In the
    Seq2SeqWithLearnableEmbeddings class, the learning framework learns the
    best embedding vectors to use for the two languages involved. On the 
    other hand, in the Seq2SeqWithPretrainedEmbeddings class, I use the
    word2vec embeddings provided by Google for the source language.  As
    to why I use the pre-training embeddings for just the source language
    ...
    undetstanding of the attention mechanism proposed by Bahdanau, Cho, and
    ...
    the Seq2Seq section of DLStudio.

    The following two scripts in the Examples directory are your main entry
    points for experimenting with the seq2seq code in DLStudio:

        1.  seq2seq_with_learnable_embeddings.py

        2.  seq2seq_with_pretrained_embeddings.py
    
    The first script assumes that you want the overall network to learn on
    its own the best embeddings to use for representing the words in the
    source language.  And the second script assumes that you want the
    Google word2vec embeddings to be used for the source language words.
...
@tag2_adversarial
    ...
    the weights in the networks, one can backprop the distance and update
...
THE MAIN INNER CLASSES OF THE MODULE:
    ...
    (10) class Seq2SeqWithLearnableEmbeddings

         As previously mentioned on this doc page, sequence-to-sequence
         (Seq2Seq) learning is about predicting a target sequence from a
         source sequence.  This class uses English-to-Spanish translation to
         illustrate the PyTorch structures and idioms that you need to become
         familiar with if you wish to write code for seq2seq learning.  Note
         that any attempt at seq2seq for machine translation must answer the
         following question at the outset: How to represent the words of a
         language for neural-network processing? In general, you have two
         options: (1) Have your overall network learn on its own what are
         known as vector embeddings for the words; or (2) Use pre-trained
         embeddings as provided by word2vec or Fasttext.  The class
         Seq2SeqWithLearnableEmbeddings learns on its own the best embedding
         vectors to use for the words in the two languages.  Additionally, an
         important part of programming for seq2seq is writing code for the
         attention mechanism whose goal is to align the similar grammatical
         units in the two languages. The Seq2SeqWithLearnableEmbeddings class
         (and also the next class) uses the attention model proposed by
         Bahdanau, Cho, and Bengio. The name of this attention class is
         Attention_BCB.  In a separate attention class named Attention_SR, I
         have also included the attention mechanism used by Sean Robertson in
         his very popular NLP tutorial at the main PyTorch website.  If you
         wish to become familiar with DLStudio's class
         Seq2SeqWithLearnableEmbeddings, your starting point should be
         following script in the Examples directory:

                seq2seq_with_learnable_embeddings.py

    (11) class Seq2SeqWithPretrainedEmbeddings

         The previous class shows how to carry out seq2seq learning when you
         allow the overall network to learn on its own the best embeddings to
         use for the words. In Seq2SeqWithPretrainedEmbeddings, we use the
         Google's word2vec embeddings for the words of the source language
         (English).  Importing pre-trained word embeddings for the source
         language means that the encoder part of the seq2seq network no
         longer needs an nn.Embeddings layer to learn the embeddings on its
         own.  The rest of the network, including the part that takes care of
         the attention mechanism, is the same as in the previous class. The
         following script in the Examples directory is based on this class:
...
Examples DIRECTORY:
    ...
    (16) seq2seq_with_learnable_embeddings.py
    ...
    (17) seq2seq_with_pretrained_embeddings.py
...
@tag2_seq2seq_data
    FOR Seq2Seq LEARNING:
        ...
        You can download the following English-Spanish translation corpus 

            11.   en_es_8_98988.tar.gz

        for experimenting with seq2seq learning part of DLStudio.  This data
        archive is a lighly curated version of the main dataset posted at
        ...
        contractions like "it's", "I'm", "don't", "didn't", "you'll", etc.,
        into their "it is", "i am", "do not", "did not", "you will", etc. The
        original form of the dataset contains 417 such unique contractions.
        Another alteration I made to the original data archive is to surround
        each sentence in both English and Spanish by the "SOS" and "EOS"
        tokens, with the former standing for "Start of Sentence" and the
        latter for "End of Sentence".
...
#                cell = torch.tanh(self.linear_for_cell(hidden_clone))
    ...
    class Seq2SeqWithLearnableEmbeddings(nn.Module):             
        ...
        def __init__(self, dl_studio, dataroot, data_archive, max_length, embedding_size, num_trials):
            super(DLStudio.Seq2SeqWithLearnableEmbeddings, self).__init__()
            self.dl_studio = dl_studio
            self.dataroot  = dataroot
        ...
        class EncoderRNN(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithLearnableEmbeddings  ->  EncoderRNN
            """
        ...
        class DecoderRNN(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithLearnableEmbeddings  ->  DecoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
...
#                                                               dls,embedding_size, hidden_size, max_length)     ## (D)
        ...
        class Attention_BCB(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithLearnableEmbeddings  ->  Attention_BCB
            """
        ...
        class Attention_SR(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithLearnableEmbeddings  ->  Attention_SR
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
    ...
    class Seq2SeqWithPretrainedEmbeddings(nn.Module):             
        ...
        def __init__(self, dl_studio, dataroot, data_archive, path_to_saved_embeddings_en, 
            ...
            super(DLStudio.Seq2SeqWithPretrainedEmbeddings, self).__init__()
            self.dl_studio = dl_studio
            self.dataroot  = dataroot
            self.data_archive = data_archive
        ...
        def sentence_to_tensor(self, sentence, lang):
            ...
            part of DLStudio.  You are currently in the Seq2SeqWithPretrainedEmbeddings part.
        ...
        class EncoderRNN(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithPretrainedEmbeddings ->  EncoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
        ...
        class DecoderRNN(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithPretrainedEmbeddings ->  DecoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
                super(DLStudio.Seq2SeqWithPretrainedEmbeddings.DecoderRNN, self).__init__()
                self.hidden_size = hidden_size
                self.target_vocab_size = s2s.vocab_es_size
                self.max_length = max_length
                self.embed = nn.Embedding(self.target_vocab_size, embedding_size)
                self.attn_calc = DLStudio.Seq2SeqWithPretrainedEmbeddings.Attention_BCB(dls, 
                                                               embedding_size, hidden_size, max_length)  
#                self.attn_calc = DLStudio.Seq2SeqWithPretrainedEmbeddings.Attention_SR(dls,
#                                                               embedding_size, hidden_size, max_length)
        ...
        class Attention_BCB(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithPretrainedEmbeddings ->  Attention_BCB
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
                super(DLStudio.Seq2SeqWithPretrainedEmbeddings.Attention_BCB, self).__init__()
                self.dl_studio = dl_studio
                self.max_length = max_length
                self.WC1 = nn.Linear( 2 * hidden_size, hidden_size )
                self.WC2 = nn.Linear( 2*hidden_size + embedding_size, embedding_size )
        ...
        class Attention_SR(nn.Module):
            ...
            ClassPath:  DLStudio -> Seq2SeqWithPretrainedEmbeddings ->  Attention_SR
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
                super(DLStudio.Seq2SeqWithPretrainedEmbeddings.Attention_SR, self).__init__()
                self.W = nn.Linear(embedding_size + hidden_size, max_length)
                self.attn_combine = nn.Linear(3*hidden_size, hidden_size)      
