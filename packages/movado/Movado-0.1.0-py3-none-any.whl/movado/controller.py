...
class Controller(ABC):
    ...
    def get_ies(self, response_time: float, error: float) -> float:
        self.__time_dataset.append(response_time)
        self.__error_dataset.append(error)
        return response_time / (1 - error)

    def __min_max_standardization(self, to_standardize: float, dataset: List[float]):
        minimum = np.min(dataset)
        maximum = np.max(dataset)
        max_min = maximum - minimum
        return (to_standardize - minimum) / max_min if max_min != 0 else 1
