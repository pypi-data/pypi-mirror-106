...
def test_water_line_flow_1():
    ...
    melting_queue = Queue(
        "MeltingQueue", [melting1, melting2], break_funcs={"in": lambda old, new: 1}
    )
    ...
    container = Container(
        "Container3", item="b", max_pressures=[200, None], limits=[150, None]
    )
    # slow packing...
    processor = Processor(
        "Processor3", items=["b", "b2"], max_pressures=[50, None], processing_time=0
    )
...
def test_water_line_flow_2_multiple_boilings():
    ...
    packing_queue1 = Queue(
        "PackingQueue1", [packing1_1, packing2_1, packing1_2, packing2_2]
    )
