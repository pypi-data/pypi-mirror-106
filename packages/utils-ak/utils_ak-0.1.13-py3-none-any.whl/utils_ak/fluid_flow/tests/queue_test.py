...
def test_flow_queue_2_out():
    parent1 = Container(
        "Parent1", value=100, max_pressures=[None, 10], limits=[None, 100]
    )
    parent2 = Container(
        "Parent2", value=100, max_pressures=[None, 20], limits=[None, 100]
    )
...
def test_flow_queue_3():
    ...
    child1 = Processor(
        "Child1", max_pressures=[20, None], processing_time=5, limits=[40, None]
    )
    child2 = Processor(
        "Child2", max_pressures=[10, None], processing_time=5, limits=[50, None]
    )
...
def test_flow_queue_4_different_items():
    parent1 = Container(
        "Parent1", item="a", value=100, max_pressures=[None, 10], limits=[None, 100]
    )
    parent2 = Container(
        "Parent2", item="b", value=100, max_pressures=[None, 20], limits=[None, 100]
    )
    queue = Queue("Parent", [parent1, parent2])
