...
def render_line_centric_advice(
    messages, project_root, settings, settings_modpath, code_style, code_reader=read_code_from_message
):
    ...
    for key, messages in group.items():
        path = messages[0]['path'].replace(project_root, '')
    ...
    for message in messages:
        ...
        message_id_centric_messages.append({
            **message,
            ...
            'path': message['path'].replace(project_root, ''),
...
def get_message_diff(message, cache, seen, code_reader, parent, code_style, settings_modpath, settings):
...
def render_report_messages(*args, **kwargs):
    return (
        ...
        render_line_centric_advice(*args, **kwargs) +
