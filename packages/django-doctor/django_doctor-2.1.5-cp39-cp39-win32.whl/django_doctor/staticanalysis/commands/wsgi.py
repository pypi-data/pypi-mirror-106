...
def home_handler(environ, respond, app, log_output):
...
def static_handler(environ, respond, app, log_output):
...
def submit_handler(environ, respond, app, log_output):
    body_size = int(environ.get('CONTENT_LENGTH', 0))
    request_body = environ['wsgi.input'].read(body_size)
    whitelist = json.loads(request_body)
    helpers.trasform_files(project_root=app.project_root, whitelist=whitelist, output=log_output)
...
class Application(object):
    def __init__(self, messages, project_root, log_output):
    ...
    def __call__(self, environ, respond):
        ...
        return handler(environ=environ, respond=respond, app=self, log_output=self.log_output)
...
def create(address, port, messages, project_root, log_output):
    app = Application(messages=messages, project_root=project_root, log_output=log_output)
