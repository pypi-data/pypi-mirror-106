...
class Board(Board):
    ...
    def from_json(self, json_str):
        ...
        for key, raw_hand in hands.items():
            if key.isnumeric():
                key = int(key)
            self.hands[key] = Hand()
            self.hands[key].from_json(raw_hand)
    ...
    def get_contract(self):
        ...
        contract = Contract()
        if self._auction:
            if self._three_final_passes(self._auction.calls):
                dealer_index = SEATS.index(self.dealer)
                contract_index = -4
                if self._auction.calls[contract_index].is_redouble:
                    contract_index -= 1
                if self._auction.calls[contract_index].is_double:
                    contract_index -= 1
                denomination = self._auction.calls[contract_index].denomination
                for index, call in enumerate(self._auction.calls):
                    if call.denomination == denomination:
                        break
                declarer_index = (dealer_index + index) % 4
                declarer = SEATS[declarer_index]
                contract = Contract(self._auction.calls[-4].name, declarer)
        return contract
