...
class GSDBS:
    ...
    def executeStatement(self, statement=None):
        ...
        result = loop.run_until_complete(self.asyncExecuteStatement(statement))
        return result

    # ---------
    async def asyncSchemaQuery(self, dtablename):
        statement = "{ __type(name: \"" + "gun" + "\") {name fields { name type { name kind } } } }"
        api_call_headers = {'Authorization': 'Bearer ' + self.accessToken['access_token']}
        async with Client(
                transport=AIOHTTPTransport(url="https://glass-sphere-ai.de/gsgraphql",
                                           headers=api_call_headers),
            ...
            schema = None
            try:
                schema = await session.execute(query)
            except:
                print("Unexpected error:", sys.exc_info())
            return schema
    ...
    def schemaQuery(self, dtablename):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(self.asyncSchemaQuery(dtablename))
        return result
