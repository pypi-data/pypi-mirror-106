...
for i in cmdlist:
    if i.istitle():
        c += 1
d = len(cmdlist) - c
__version__ = str(c) + '.' + str(d) + '.' + '58' #easy_functions版本号
__author__ = 'Jerry\n我的QQ号：2711893794' #作者
...
def literally(text, wait=0.1): #逐字显示
    if not wait.isdigit():
        raise FunctionSyntaxError('你输入的值%s不符合运行要求，请输入数字'%wait)
    import sys, time
    sys.stdout.write("\n " + " " * 60 + "\r")
    sys.stdout.flush()
    for c in text:
...
def wait(wait=1): #等待功能
    ...
    try:
        ...
        raise FunctionSyntaxError('你输入的值%s不符合运行要求，请输入数字'%wait) from e
    return
def cls(): #清除屏幕功能
    import os
    os.system('cls')
