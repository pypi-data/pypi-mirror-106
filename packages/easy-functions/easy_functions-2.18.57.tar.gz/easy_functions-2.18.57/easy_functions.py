...
"""
#更新状态:活跃
cmdlist = ['bigfont','literally','wait','cls','title','find_file','start','call','Vbs','color','mode','shield','choice','Code','mkdir','copy','help','helpcmd','version','log'] #命令列表，以大写字母开头的为类(class)
__all__ = cmdlist
c = 0
d = 0
for i in cmdlist:
    if i.istitle():
        c += 1
d = len(cmdlist) - c
__version__ = str(c) + '.' + str(d) + '.' + '57' #easy_functions版本号
__author__ = 'Jerry\n我的QQ号：2711893794' #作者
...
def literally(text, wait=0.1): #逐字显示
    if not wait.isdigit():
        raise FunctionSyntaxError('你输入的值',wait,'不符合运行要求，请输入数字')
    import sys, time
    sys.stdout.write("\n " + " " * 60 + "\r")
    sys.stdout.flush()
    for c in text:
...
def wait(wait=1): #等待功能
    ...
    try:
        ...
        raise FunctionSyntaxError('你输入的值',wait,'不符合运行要求，请输入数字') from e
    return
def cls(): #清除屏幕功能
    import os
    os.system('cls')
...
def color( color ): #更改颜色
    ...
    try:
        ...
        try:
            ...
            raise FunctionSyntaxError('这不是一个完整的颜色编码') from e
        else:
            if color[0].isupper() or color[1].isupper():
                raise FunctionSyntaxError('颜色编码必须小写')
                ...
                raise FunctionSyntaxError('无效颜色编码') from e
    else:
        raise FunctionSyntaxError('不支持2个以上的颜色编码') from e
    return
def mode(cols, lines): #更改cmd外框大小
    ...
    try:
        ...
        try:
            ...
            raise FunctionSyntaxError('请不要输入除数字外的字母，符号等') from e
            return
        else:
            c = cols
    else:
        raise FunctionSyntaxError('该功能需要输入字符串，而你输入了数字') from e
        return
    try:
        lines + 1
    except TypeError as e:
...
def shield(words, WordsBlackList): #词语屏蔽
    ...
    try:
        ...
        raise FunctionSyntaxError('功能shield的词语黑名单需要传入列表') from e
    else:
        WordsBlackList.pop()
...
def choice(choose='YN', text='Y/N', default='False', timeout='10', *, hide=False): #按键选择功能
    ...
    for i in range(len(text)):
        if text[i] == ' ':
            raise FunctionSyntaxError('显示的文字中不能含有空格')
    if hide is True: 
    ...
    if default != 'False':
        if default in choose:
            ...
            raise FunctionSyntaxError('按键默认值不在设置的按键中')
...
def log(): #更新日志
    ...
    print('')
    print('[2021-5-6] [1条更新]')
    print('1.因为python官网的重名，本模块由pymod更名为SimpleFunctions')
    print('')
    print('[2021-5-7] [1条更新]')
...
def helpcmd( cmd ): #文件命令功能解释
    ...
    if cmd == 'call':
        print('<',cmd,'>','文件调用功能')
        print('具体用法详见start命令')
        print('惟一的区别是start命令在执行时会打开另一个窗口，而call命令不会')
        print('注意：此命令只能用于文件')
        return
    if cmd == 'Vbs':
        print('<',cmd,'>','弹窗创建功能')
