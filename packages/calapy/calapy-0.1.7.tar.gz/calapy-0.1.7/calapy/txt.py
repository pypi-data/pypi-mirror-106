...
def files_to_txt_file(files, filenames):

    n_files = len(files)
    for f in range(n_files):
        lines_to_txt_file(filenames[f], files[f])
...
def csv_file_to_array(filename, rows=None, columns=None, dtype=None):
...
def conditions_of_csv_files_to_arrays(conditions_of_directories, rows, columns, dtype=None):
    ...
    for a in range(n_arrays):
        ...
        indexes_out = np.empty(n_axes_a, dtype='O')
