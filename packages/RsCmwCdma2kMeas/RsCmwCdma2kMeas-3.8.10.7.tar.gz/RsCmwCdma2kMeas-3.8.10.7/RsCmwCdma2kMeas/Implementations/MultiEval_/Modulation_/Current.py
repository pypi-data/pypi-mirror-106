...
class Current:
	...
	def read(self) -> ReadStruct:
		...
		Snippet: value: ReadStruct = driver.multiEval.modulation.current.read() \n
		...
		return self._core.io.query_struct(f'READ:CDMA:MEASurement<Instance>:MEValuation:MODulation:CURRent?', self.__class__.ReadStruct())
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.modulation.current.fetch() \n
		Return the current, average, minimum, maximum and standard deviation modulation single value results.
		The values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. The ranges indicated below apply to all results except standard deviation results.
		The minimum for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two.
		...
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:MODulation:CURRent?', self.__class__.FetchStruct())
	...
	def calculate(self) -> CalculateStruct:
		...
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:MODulation:CURRent?', self.__class__.CalculateStruct())
