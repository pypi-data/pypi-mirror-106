...
class Maximum:
	...
	class ResultData(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_float('Ms_Power_1_M_23'),
		...
		def __init__(self):
			...
			self.Ms_Power_1_M_23: float = None
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.modulation.maximum.read() \n
		...
		return self._core.io.query_struct(f'READ:CDMA:MEASurement<Instance>:MEValuation:MODulation:MAXimum?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:CDMA:MEASurement<Instance>:MEValuation:MODulation:MAXimum \n
		Snippet: value: ResultData = driver.multiEval.modulation.maximum.fetch() \n
		Return the current, average, minimum, maximum and standard deviation modulation single value results.
		The values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. The ranges indicated below apply to all results except standard deviation results.
		The minimum for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two.
		...
		the parameter position within the result array. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:MODulation:MAXimum?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		...
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:MODulation:MAXimum?', self.__class__.CalculateStruct())
