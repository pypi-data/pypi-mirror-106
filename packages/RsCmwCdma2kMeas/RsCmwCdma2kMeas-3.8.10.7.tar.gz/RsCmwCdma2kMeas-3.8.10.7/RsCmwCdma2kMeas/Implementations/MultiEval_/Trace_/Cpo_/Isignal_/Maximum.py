...
class Maximum:
	...
	def read(self) -> ResultData:
		...
		return self._core.io.query_struct(f'READ:CDMA:MEASurement<Instance>:MEValuation:TRACe:CPO:ISIGnal:MAXimum?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CPO:ISIGnal:MAXimum \n
		Snippet: value: ResultData = driver.multiEval.trace.cpo.isignal.maximum.fetch() \n
		Returns the phase offset for the indicated channels in the in-phase signal path (I-signal) . The results of the current,
		average, maximum and minimum traces can be retrieved. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CPO:ISIGnal:MAXimum?', self.__class__.ResultData())
	...
	class CalculateStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_enum('Dcch', enums.ResultStatus2),
		...
		def __init__(self):
			...
			self.Dcch: enums.ResultStatus2 = None
			self.Sch_1: enums.ResultStatus2 = None

	def calculate(self) -> CalculateStruct:
		"""SCPI: CALCulate:CDMA:MEASurement<Instance>:MEValuation:TRACe:CPO:ISIGnal:MAXimum \n
		Snippet: value: CalculateStruct = driver.multiEval.trace.cpo.isignal.maximum.calculate() \n
		Returns the phase offset for the indicated channels in the in-phase signal path (I-signal) . The results of the current,
		average, maximum and minimum traces can be retrieved. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:TRACe:CPO:ISIGnal:MAXimum?', self.__class__.CalculateStruct())
