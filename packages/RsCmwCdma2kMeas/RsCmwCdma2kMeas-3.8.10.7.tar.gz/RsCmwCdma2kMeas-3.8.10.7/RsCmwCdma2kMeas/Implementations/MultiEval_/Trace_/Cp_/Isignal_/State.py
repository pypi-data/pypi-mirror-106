...
class State:
	...
	def fetch(self) -> List[enums.SigChStateA]:
		...
		Snippet: value: List[enums.SigChStateA] = driver.multiEval.trace.cp.isignal.state.fetch() \n
		...
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: cp_isig_ch_state: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CP:ISIGnal:STATe?', suppressed)
