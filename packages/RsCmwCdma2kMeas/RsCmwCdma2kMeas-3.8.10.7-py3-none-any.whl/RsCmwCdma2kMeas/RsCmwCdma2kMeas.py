...
class RsCmwCdma2kMeas:
	"""777 total commands, 6 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.10.0007'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwCdma2kMeas._driver_options, options, direct_session)
		self._core.driver_version = '3.8.10.0007'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwCdma2kMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwCdma2kMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwCdma2kMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.10.0007'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwCdma2kMeas version failed. Current version: '3.8.10.0007', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 3 Sub-classes, 2 commands."""
	...
	@property
	def oltr(self):
		"""oltr commands group. 2 Sub-classes, 3 commands."""
		if not hasattr(self, '_oltr'):
			from .Implementations.Oltr import Oltr
			self._oltr = Oltr(self._core, self._base)
		return self._oltr

	@property
	def rpInterval(self):
		"""rpInterval commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_rpInterval'):
			from .Implementations.RpInterval import RpInterval
			self._rpInterval = RpInterval(self._core, self._base)
		return self._rpInterval

	def clone(self) -> 'RsCmwCdma2kMeas':
		"""Creates a deep copy of the RsCmwCdma2kMeas object. Also copies:
		...
		cloned = RsCmwCdma2kMeas.from_existing_session(self.get_session_handle(), self._options)
