...
class Average:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.trace.cto.isignal.average.read() \n
		...
		return self._core.io.query_struct(f'READ:CDMA:MEASurement<Instance>:MEValuation:TRACe:CTO:ISIGnal:AVERage?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CTO:ISIGnal:AVERage \n
		Snippet: value: ResultData = driver.multiEval.trace.cto.isignal.average.fetch() \n
		Returns channel time offset for the indicated channels in the in-phase signal path (I-signal) . The results of the
		current, average and maximum traces can be retrieved. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CTO:ISIGnal:AVERage?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.trace.cto.isignal.average.calculate() \n
		Returns channel time offset for the indicated channels in the in-phase signal path (I-signal) . The results of the
		current, average and maximum traces can be retrieved. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:TRACe:CTO:ISIGnal:AVERage?', self.__class__.CalculateStruct())
