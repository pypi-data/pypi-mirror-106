...
class Maximum:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.trace.cp.isignal.maximum.read() \n
		...
		return self._core.io.query_struct(f'READ:CDMA:MEASurement<Instance>:MEValuation:TRACe:CP:ISIGnal:MAXimum?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CP:ISIGnal:MAXimum \n
		Snippet: value: ResultData = driver.multiEval.trace.cp.isignal.maximum.fetch() \n
		Returns the values of the channel power I-Signal traces. The results of the current, average, maximum and minimum traces
		can be retrieved. The values described below are returned by FETCh and READ commands. CALCulate commands return limit
		check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CP:ISIGnal:MAXimum?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.trace.cp.isignal.maximum.calculate() \n
		Returns the values of the channel power I-Signal traces. The results of the current, average, maximum and minimum traces
		can be retrieved. The values described below are returned by FETCh and READ commands. CALCulate commands return limit
		check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:TRACe:CP:ISIGnal:MAXimum?', self.__class__.CalculateStruct())
