...
def plot_line2_coaxial(df01,ticker1,colname1,label1, \
    ...
    if power > 0:
        try:
            ...
            f = np.poly1d(parameter)
            plt.plot(df1.index, f(df1.id),"r--", label=com.codetranslate(ticker1)+"(趋势线)",linewidth=1)
        except: pass
    ...
    if power > 0:
        try:
            ...
            f = np.poly1d(parameter)
            plt.plot(df2.index, f(df2.id),"r--", label=com.codetranslate(ticker2)+"(趋势线)",linewidth=1)
        except: pass
...
def plot_line2_twinx(df01,ticker1,colname1,label1, \
    ...
    if power > 0:
        ...
        parameter = np.polyfit(df1.id, df1[colname1], power)
        f = np.poly1d(parameter)
        ax.plot(df1.index, f(df1.id),"r--", label=com.codetranslate(ticker1)+"(趋势线)",linewidth=1)
    ...
    if power > 0:
        ...
        parameter = np.polyfit(df2.id, df2[colname2], power)
        f = np.poly1d(parameter)
        ax2.plot(df2.index, f(df2.id),"c--", label=com.codetranslate(ticker2)+"(趋势线)",linewidth=1)        
...
def plot_2lines(df01,colname1,label1, \
    ...
    #证券2：先绘制折线图
    plt.plot(df2.index,df2[colname2],label=label2,linestyle='-.',linewidth=1)
    """
    #是否绘制水平虚线
    if not (hline == 0):
...
#==============================================================================
def df_smooth(df,method='pchip',sampling='H',order=3):
    """
    ...
    """
    try:
        dfh=df.resample(sampling)
    except:
        print('  #Warning(df_smooth): resampling failed for frequency',sampling)
        return df
