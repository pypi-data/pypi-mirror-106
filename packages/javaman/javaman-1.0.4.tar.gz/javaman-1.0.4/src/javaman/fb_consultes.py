...
where p.ID not in (1268, 2127, 4607) and pl.CONTROL_ESTOCS = 'S';
"""
...
select
    ...
    cd.REPARTIDOR_ID,
    CASE WHEN c_cli.COMANDA_ID IS NULL THEN cd_ter.NOM_LLARG ELSE c_cli.TERCER END NOM_CLIENT,
...
join clients cd_cli on cd_ter.ID = cd_cli.TERCER_ID    
...
sync_comanda_linies = """
select cl_atr.ID ATRIBUT_ID, sum(cl.QUANTITAT) QUANTITAT, min(cl.NUMERO_LINIA) nl, cl.DESCRIPCIO, 
...
group by ATRIBUT_ID, cl.DESCRIPCIO  
...
where CE.MAG_PENDENT_SERVIR = 'S' AND C.DATA_ANULACIO IS NULL;
"""
