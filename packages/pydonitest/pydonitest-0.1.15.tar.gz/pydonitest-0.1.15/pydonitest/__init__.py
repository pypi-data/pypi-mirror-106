...
def append_filename_suffix(fpath, suffix):
    """
...
def test_value(value, dtype, return_coerced_value=False, stop=False):
    """
...
def rename_dict_keys(dct, key_dict):
    """
...
class Verbose(object):
    ...
    """
    def __init__(self, verbose=True, debug=False, timestamp=False):
    ...
    def section_header(self, string, time_in_sec=None, round_digits=2):
        """
    ...
    def pbar_write(self, msg, refer_debug=False):
        """
...
def echo(msg,
    ...
    if letter in ['D', 'I', 'W', 'E']:
        ...
        custom_level_map = dict(S=' SUCCESS', T='', F=' FATAL')
        custom_level_str = custom_level_map[letter]
        if timestamp:
            prefix = f'[{ts}]{custom_level_str}]'
        else:
            prefix = f'{custom_level_map[letter].strip()}'
