...
class TestPydonitest(unittest.TestCase):
    ...
    def test_version(self):
        ...
        with open(os.path.join(main_package_dir, 'pydonitest', '__init__.py')) as f:
    ...
    def test_extract_colorpalette(self):
        result = pydonitest.extract_colorpalette('Blues')
        self.assertEqual(result[0:3], ['#f7fbff', '#f6faff', '#f5fafe'])

    def test_rename_dict_keys(self):
        dct = {'a': 1, 'b': 2, 'c': 3}
        result = pydonitest.rename_dict_keys(dct=dct, key_dict={'c': 'd'})
    ...
    def test_append_filename_suffix(self):
        ...
        self.assertEqual(pydonitest.append_filename_suffix(fpath=testfile, suffix='-1'), expectation)

    def test_textfile_len(self):
        testfile = 'test_data/txt/nonempty_dir_subdirs/textfile_15.txt'
        self.assertEqual(pydonitest.textfile_len(fpath=testfile), 10)

    def test_dirsize(self):
        testdir = 'test_data/txt/nonempty_dir_subdirs/subdir_1'
        self.assertEqual(pydonitest.dirsize(dpath=testdir), 25187)
    ...
    def test_collapse_df_columns(self):
        ...
        result = list(pydonitest.collapse_df_columns(testdf).columns)
        expectation = ['col_group_a', 'col_group_b', 'col_group_c']
        self.assertEqual(result, expectation)

    def test_print_columns(self):
        pydonitest.print_columns(lst=['a', 'b', 'c', 'd'], ncol=1)
        pydonitest.print_columns(lst=['a', 'b', 'c', 'd'], ncol=2)
        pydonitest.print_columns(lst=['a', 'b', 'c', 'd'], ncol=3)
        pydonitest.print_columns(lst=['a', 'b', 'c', 'd'], ncol=3, delay=.01)

    def test_stabilize_postfix(self):
        result = pydonitest.stabilize_postfix(key='test', max_len=20, fillchar='•', side='right')
        self.assertEqual(result, '••••••••••••••••test')
        result = pydonitest.stabilize_postfix(key='test', max_len=10, fillchar='•', side='left')
        self.assertEqual(result, 'test••••••')
        result = pydonitest.stabilize_postfix(key='test', max_len=3, fillchar='•', side='left')
