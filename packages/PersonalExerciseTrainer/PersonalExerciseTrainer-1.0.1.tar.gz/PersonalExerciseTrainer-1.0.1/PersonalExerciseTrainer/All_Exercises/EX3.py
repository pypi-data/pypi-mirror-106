import cv2
from  All_Exercises.Backend import Mediapipe_backend as pm 
import time
import sys
...
while True:
    ...
    for i in lmlist:
        ...
        exercise_two(int(left_elbow_to_waist)   ,int(right_elbow_to_waist))
        exercise_three(int(left_wrist_to_waist) ,int(right_wrist_to_waist))

        flag = exercise_four(int(left_wrist_to_waist)  ,int(right_wrist_to_waist))
        if flag == True:
            if dir == 0:
                time.sleep(0.05)
                count = count + 1
    ...
    cv2.imshow('Your Exercise - Three' , poses)
    cv2.waitKey(1)
