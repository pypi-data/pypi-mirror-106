...
def _try_rslex_upload(destination, source_path, base_dir, is_volume=False):
    logger = _LoggerFactory.get_logger('dprep.rslex_upload')
    tracer = trace.get_tracer(__name__)

    try:
        from azureml.dataprep.rslex import Copier, PyLocationInfo, StreamInfo
        ...
        with tracer.start_as_current_span('_try_rslex_upload', trace.get_current_span()):
            ...
            copier = Copier(destination_info, base_dir)
        ...
        logger.error('Uploading with rslex failed with error {}. Falling back to clex', e)
        return False
...
class EngineAPI:
    ...
    def upload_directory(self, message_args: typedefinitions.UploadDirectoryMessageArguments, cancellation_token: CancellationToken = None) -> None:
        if _try_rslex_upload(message_args.destination, message_args.folder_path, message_args.base_path, True):
            return None
    ...
    def upload_file(self, message_args: typedefinitions.UploadFileMessageArguments, cancellation_token: CancellationToken = None) -> None:
        if _try_rslex_upload(message_args.destination, message_args.local_path, message_args.base_path):
            return None
