...
from azureml.dataprep.rslex import Copier, StreamInfo, PyLocationInfo
...
class _DPrepFuse(FuseBase):
    ...
    def __init__(self,
        ...
        try:
            ...
            self._copier = Copier(PyLocationInfo('Local', self._cache.data_dir, {}), rslex_base)
    ...
    def _download_stream(self, stream_details: StreamDetails, target_path: str) -> str:
        ...
        try:
            ...
            downloaded_path = self._copier.copy_stream_info(stream_info, span.to_w3c_traceparent())
            ...
            if downloaded_path != target_path:
                ...
                print(f"Downloaded path: {downloaded_path} is different from target path: {target_path}")
                try:
...
def rslex_mount(dataflow: Optional[Dataflow],
    ...
    if rslex_fuse_flight == "1":
        try:
            ...
            log.warning('Failed to run rslex based mount due to exception of type {} with message {}'.format(type(e).__name__, e))
