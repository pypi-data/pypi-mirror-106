...
class CurrentDirectory:
	...
	def set(self, directory_name: str = None) -> None:
		...
		If <DirectoryName> contains not only a directory, but also a drive letter or server name, the command method RsCmwBase.
		MassMemory.msis is also executed automatically. \n
	...
	def get(self, directory_name: str = None) -> str:
		...
		MassMemory.msis is also executed automatically. \n
			:param directory_name: String parameter to specify the directory. Wildcards are not allowed.
			:return: directory_name: String parameter to specify the directory. Wildcards are not allowed."""
