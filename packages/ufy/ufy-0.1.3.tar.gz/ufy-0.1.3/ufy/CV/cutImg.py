...
from ufy.utils.tools import energy
...
def find_file(dir_source, file_format=['mrxs', 'tiff', 'npdi'], file_list=[]):
    ...
    for f in dirlist:
        f_abs = os.path.join(dir_source, f)
        if os.path.isdir(f):
            find_file(f, file_format, file_list)
        elif str.lower(os.path.basename(f).split('.')[-1]) in file_format:
            file_list.append([f_abs, f])


def cut_img_energy(img_file, todir, file_format=['mrxs', 'tiff', 'npdi'], energy_threshold=0.6, start_k=0, end_k=0,
...
def cut_img_dir(dir_source='', todir_pre='', file_format=['mrxs', 'tiff', 'npdi'], energy_threshold=0.6, start_k=0,
    ...
    img_files = []
    find_file(dir_source=dir_source, file_format=file_format, file_list=img_files)
    print('图片文件数：', len(img_files))

    count = 0
    for img_file, f_tail in img_files:
