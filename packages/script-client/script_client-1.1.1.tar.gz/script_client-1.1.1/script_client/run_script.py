...
def _send_resurl(data):
    ...
    try:
        rep = mr.run(url=url, method='post', headers=headers,json=json.dumps(data))
        ...
        while True:
            try:
                ...
                ret_data = {
                    ...
                    'result':1,  # 0失败，1通过
                    'result_body': {
                        'type': 0,  # 1=过程异常，2=断言异常,0=无异常
                        'stdout':data['http_body'],
                        'stderr':'',
                    }
                }
                try:
                    data['func_info'][0](data)
                except AssertionError as e:
                    ret_data['result']=0
                    ret_data['result_body']['type'] = 2
                    ret_data['result_body']['stderr'] = repr(e)
                except Exception as e:
                    ret_data['result'] = 0
