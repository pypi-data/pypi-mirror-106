    ...
    def post(self):
        try:
            ...
            if not all((data['app_code'],data['moniter_path'],data['http_body'])):
                ...
                return
            if run_script.q.full():
                logger.warning(f"{config.local_ip}:{config.local_port}的队列已满")
                self.write({'code': 0, 'msg': f"{config.local_ip}:{config.local_port}的队列已满"})
                return
            path = file_manage.find_file(app_code=data['app_code'], moniter_path=data['moniter_path'])
            if isinstance(path, str):
                cmd = ['python', path, str(data['http_body'])]
                data['cmd'] = cmd
                run_script.q.put(data)
                # logger.debug(f"q队列长度 = {run_script.q.qsize()}")
                self.write({'code': 1, 'msg': 'success'})
                return
            else:
                logger.info(f"没找到py文件")
                self.write({'code': 0, 'msg': '没找到py文件'})
        except Exception as e:
