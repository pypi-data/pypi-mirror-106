                    ...
                    if (name not in exclude_names) and (name not in parent_names) and (cond.find('?') >= 0):
                        ...
                        if len(v) > 1:
                            ...
                            if x:
                                ...
                                tmp["condition"] = [cond]
                                depends.append(tmp)
                                # print("add depend:%s with cond:%s for %s" % (name, cond, comp.name))
                ...
                if (name not in exclude_names) and (cond.find('?') >= 0):
                    ...
                    if len(v) > 1:
                        ...
                        if x:
                            ...
                            tmp["comp_name"] = name
                            tmp["count"] = parent_found["count"] + 1
                            if cond not in parent_found["condition"]:
                                tmp["condition"] = parent_found["condition"] + [cond]
                            else:
                                tmp["condition"] = parent_found["condition"]
                            depends.append(tmp)
                            # print(tmp["count"])
                            # print("add depend:%s with inherrited cond:%s and append cond: %s for %s" % (name, parent_found["condition"], cond, comp.name))
