...
class Qctrl:
    ...
    def get_result(
        ...
        response = refresh_query(action_id)

        result = self.gql_env.load_data(
            field_type, response["coreAction"]["coreAction"]
        )
        self.gql_env.wait_for_completion(field_type, result, refresh_query)
        return result
