...
from easyrec.utils import SparseFeat, DenseFeat
...
def create_criteo_dataset(file, embed_dim=8, read_part=True, sample_num=100000, test_size=0.2):
    ...
    feature_columns = [[DenseFeat(feat, 1) for feat in dense_features]] + \
                      [[SparseFeat(feat, len(data_df[feat].unique()), embed_dim=embed_dim)
                        for feat in sparse_features]]
    feature_names = set([feat.name for feat in feature_columns])
