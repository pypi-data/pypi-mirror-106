...
async def test_update_software_environment_conda(
    ...
    out2.seek(0)
    text = out2.read()
    assert "Environment created" in text
...
async def test_client_python_version_included_in_conda_deps_for_pip_only_env(
    ...
    # Is this ghetto? Oh yeah.
    # Is it better than creating a whole new
    # software environment just to confirm we're
    # calling the server with python=<version> in the
    # conda dependencies? ...maybe?
    class SpySession:
        ...
        async def ws_connect(self, *args, **kwargs):
            return self.Ws(output)

        async def ws_stream(self, *args, **kwargs):
            return None
    ...
    output = []
    spy = SpySession(output)
    cloud.session = spy
    cloud._websocket_stream = spy.ws_stream
    await cloud.create_software_environment(name="env-1", pip=["botocore"])

    # see what we sent to the server
    data = output[0]
    v = ".".join(map(str, sys.version_info[:2]))
    assert f"python={v}" in data["conda"]["dependencies"]
...
async def test_docker_build_reports_failure(
    ...
    out.seek(0)
    text = out.read()
    assert "232" in text  # 232 is the exit code that gets thrown by conda run
    assert "failed" in text.lower()
