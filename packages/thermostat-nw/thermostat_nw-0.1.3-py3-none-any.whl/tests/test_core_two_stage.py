...
def test_thermostat_furnace_or_boiler_two_stage_none_single_stage(
    thermostat_furnace_or_boiler_two_stage_none_single_stage,
):
    with pytest.raises(ValueError):
...
def test_thermostat_furnace_or_boiler_two_stage_none_single_stage(
    ...
    core_heating_day_sets = (
        thermostat_furnace_or_boiler_two_stage_none_single_stage.get_core_heating_days(
            method="year_mid_to_mid"
        )
    )
    assert len(core_heating_day_sets) == 2
...
def test_thermostat_core_heating_day_set_attributes(
    ...
    assert isinstance(
        core_heating_day_set_hpeb_2_hp_2_entire.start_date, datetime
    ) or isinstance(core_heating_day_set_hpeb_2_hp_2_entire.start_date, np.datetime64)
    assert isinstance(
        core_heating_day_set_hpeb_2_hp_2_entire.end_date, datetime
    ) or isinstance(core_heating_day_set_hpeb_2_hp_2_entire.end_date, np.datetime64)


def test_thermostat_core_cooling_day_set_attributes(
    core_cooling_day_set_hpeb_2_hp_2_entire,
):
    ...
    assert isinstance(
        core_cooling_day_set_hpeb_2_hp_2_entire.start_date, datetime
    ) or isinstance(core_cooling_day_set_hpeb_2_hp_2_entire.start_date, np.datetime64)
    assert isinstance(
        core_cooling_day_set_hpeb_2_hp_2_entire.end_date, datetime
    ) or isinstance(core_cooling_day_set_hpeb_2_hp_2_entire.end_date, np.datetime64)


def test_thermostat_hpeb_2_hp_2_total_heating_runtime(
    thermostat_hpeb_2_hp_2,
    core_heating_day_set_hpeb_2_hp_2_entire,
    metrics_hpeb_2_hp_2_data,
):

    total_runtime = thermostat_hpeb_2_hp_2.total_heating_runtime(
        core_heating_day_set_hpeb_2_hp_2_entire
    )
    assert_allclose(
        total_runtime,
        metrics_hpeb_2_hp_2_data[1]["total_core_heating_runtime"],
        rtol=1e-3,
    )


def test_thermostat_hpeb_2_hp_2_total_emergency_heating_runtime(
    thermostat_hpeb_2_hp_2,
    core_heating_day_set_hpeb_2_hp_2_entire,
    metrics_hpeb_2_hp_2_data,
):

    total_runtime = thermostat_hpeb_2_hp_2.total_emergency_heating_runtime(
        core_heating_day_set_hpeb_2_hp_2_entire
    )
    assert_allclose(
        total_runtime,
        metrics_hpeb_2_hp_2_data[1]["total_emergency_heating_core_day_runtime"],
        rtol=1e-3,
    )


def test_thermostat_hpeb_2_hp_2_total_auxiliary_heating_runtime(
    thermostat_hpeb_2_hp_2,
    core_heating_day_set_hpeb_2_hp_2_entire,
    metrics_hpeb_2_hp_2_data,
):

    total_runtime = thermostat_hpeb_2_hp_2.total_auxiliary_heating_runtime(
        core_heating_day_set_hpeb_2_hp_2_entire
    )
    assert_allclose(
        total_runtime,
        metrics_hpeb_2_hp_2_data[1]["total_auxiliary_heating_core_day_runtime"],
        rtol=1e-3,
    )


def test_thermostat_hpeb_2_hp_2_total_cooling_runtime(
    thermostat_hpeb_2_hp_2,
    core_cooling_day_set_hpeb_2_hp_2_entire,
    metrics_hpeb_2_hp_2_data,
):

    total_runtime = thermostat_hpeb_2_hp_2.total_cooling_runtime(
        core_cooling_day_set_hpeb_2_hp_2_entire
    )
    assert_allclose(
        total_runtime,
        metrics_hpeb_2_hp_2_data[0]["total_core_cooling_runtime"],
        rtol=1e-3,
    )


def test_thermostat_hpeb_2_hp_2_get_resistance_heat_utilization_bins_rhu1(
    thermostat_hpeb_2_hp_2,
    core_heating_day_set_hpeb_2_hp_2_entire,
    metrics_hpeb_2_hp_2_data,
):
    ...
    rhu = thermostat_hpeb_2_hp_2.get_resistance_heat_utilization_bins(
        rhu_runtime, temperature_bins, core_heating_day_set_hpeb_2_hp_2_entire
    )

    assert len(rhu) == 12

    for item in rhu.itertuples():
        bin_name = thermostat_hpeb_2_hp_2._format_rhu(
...
def test_thermostat_hpeb_2_hp_2_get_resistance_heat_utilization_bins_rhu2(
    thermostat_hpeb_2_hp_2,
    core_heating_day_set_hpeb_2_hp_2_entire,
    metrics_hpeb_2_hp_2_data,
):
...
def core_days(
    ...
    tests = [
        (thermostat_hpeb_2_hp_2, core_cooling_day_set_hpeb_2_hp_2_entire, 0, "cooling"),
        (thermostat_hpeb_2_hp_2, core_heating_day_set_hpeb_2_hp_2_entire, 1, "heating"),
    ]
...
def test_day_counts(core_days, metrics_hpeb_2_hp_2_data):
    ...
    assert (
        n_days_insufficient == metrics_hpeb_2_hp_2_data[i]["n_days_insufficient_data"]
    )
    assert (
        n_core_days
        == metrics_hpeb_2_hp_2_data[i]["n_core_{}_days".format(heating_or_cooling)]
    )
    assert (
