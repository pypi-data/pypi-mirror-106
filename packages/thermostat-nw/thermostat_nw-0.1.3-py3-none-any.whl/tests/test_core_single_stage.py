...
def test_rhu_formatting(thermostat_type_1):
    ...
    assert "rhu2_05F_to_10F_aux_duty_cycle" == thermostat_type_1._format_rhu(
...
def test_thermostat_type_1_baseline_regional_comfort_temperature_none(
    thermostat_type_1,
    core_heating_day_set_type_1_entire,
    core_cooling_day_set_type_1_entire,
):
...
def test_thermostat_type_1_get_core_heating_days_with_params(thermostat_type_1):
    core_heating_day_sets = thermostat_type_1.get_core_heating_days(
        min_minutes_heating=0, max_minutes_cooling=0, method="year_mid_to_mid"
    )
    assert len(core_heating_day_sets) == 5
...
def test_get_core_cooling_day_baseline_setpoint_not_implemented(
    thermostat_type_1, core_cooling_day_set_type_1_entire
):
    with pytest.raises(NotImplementedError):
        thermostat_type_1.get_core_cooling_day_baseline_setpoint(
            core_cooling_day_set_type_1_entire, method="bad"
        )
    with pytest.raises(NotImplementedError):
        thermostat_type_1.get_core_cooling_day_baseline_setpoint(
            core_cooling_day_set_type_1_entire, source="bad"
        )
    with pytest.raises(NotImplementedError):
        thermostat_type_1.get_core_cooling_day_baseline_setpoint(
            core_cooling_day_set_type_1_entire, source="cooling_setpoint"
        )


def test_get_core_heating_day_baseline_setpoint_not_implemented(
    thermostat_type_1, core_heating_day_set_type_1_entire
):
    with pytest.raises(NotImplementedError):
        thermostat_type_1.get_core_heating_day_baseline_setpoint(
            core_heating_day_set_type_1_entire, method="bad"
        )
    with pytest.raises(NotImplementedError):
        thermostat_type_1.get_core_heating_day_baseline_setpoint(
            core_heating_day_set_type_1_entire, source="bad"
        )
    with pytest.raises(NotImplementedError):
        thermostat_type_1.get_core_heating_day_baseline_setpoint(
            core_cooling_day_set_type_1_entire, source="heating_setpoint"
        )
...
def test_thermostat_core_heating_day_set_attributes(core_heating_day_set_type_1_entire):
    ...
    assert isinstance(
        core_heating_day_set_type_1_entire.start_date, datetime
    ) or isinstance(core_heating_day_set_type_1_entire.start_date, np.datetime64)
    assert isinstance(
        core_heating_day_set_type_1_entire.end_date, datetime
...
def test_thermostat_core_cooling_day_set_attributes(core_cooling_day_set_type_1_entire):
    ...
    assert isinstance(
        core_cooling_day_set_type_1_entire.start_date, datetime
    ) or isinstance(core_cooling_day_set_type_1_entire.start_date, np.datetime64)
    assert isinstance(
        core_cooling_day_set_type_1_entire.end_date, datetime
...
def test_thermostat_equipment_protect_resistance_heat(
    thermostat_type_1, thermostat_type_2
):
...
def test_thermostat_type_1_total_heating_runtime(
    thermostat_type_1, core_heating_day_set_type_1_entire, metrics_type_1_data
):

    total_runtime = thermostat_type_1.total_heating_runtime(
        core_heating_day_set_type_1_entire
    )
    assert_allclose(
        total_runtime, metrics_type_1_data[1]["total_core_heating_runtime"], rtol=1e-3
    )


def test_thermostat_type_1_total_emergency_heating_runtime(
    thermostat_type_1, core_heating_day_set_type_1_entire, metrics_type_1_data
):

    total_runtime = thermostat_type_1.total_emergency_heating_runtime(
        core_heating_day_set_type_1_entire
    )
    assert_allclose(
        total_runtime,
        metrics_type_1_data[1]["total_emergency_heating_core_day_runtime"],
        rtol=1e-3,
    )


def test_thermostat_type_1_total_auxiliary_heating_runtime(
    thermostat_type_1, core_heating_day_set_type_1_entire, metrics_type_1_data
):

    total_runtime = thermostat_type_1.total_auxiliary_heating_runtime(
        core_heating_day_set_type_1_entire
    )
    assert_allclose(
        total_runtime,
        metrics_type_1_data[1]["total_auxiliary_heating_core_day_runtime"],
        rtol=1e-3,
    )


def test_thermostat_type_1_total_cooling_runtime(
    thermostat_type_1, core_cooling_day_set_type_1_entire, metrics_type_1_data
):

    total_runtime = thermostat_type_1.total_cooling_runtime(
        core_cooling_day_set_type_1_entire
    )
    assert_allclose(
        total_runtime, metrics_type_1_data[0]["total_core_cooling_runtime"], rtol=1e-3
    )


def test_thermostat_type_1_get_resistance_heat_utilization_bins_rhu1(
    thermostat_type_1, core_heating_day_set_type_1_entire, metrics_type_1_data
):
    ...
    rhu = thermostat_type_1.get_resistance_heat_utilization_bins(
        rhu_runtime, temperature_bins, core_heating_day_set_type_1_entire
    )

    assert len(rhu) == 12

    for item in rhu.itertuples():
        bin_name = thermostat_type_1._format_rhu(
            "rhu1", item.Index.left, item.Index.right, duty_cycle=None
        )
        bin_value = item.rhu
        assert_allclose(bin_value, metrics_type_1_data[1][bin_name], rtol=1e-3)

    rhu = thermostat_type_1.get_resistance_heat_utilization_bins(
        None, temperature_bins, core_heating_day_set_type_1_entire
    )

    assert rhu is None


def test_thermostat_type_1_get_resistance_heat_utilization_bins_rhu2(
    thermostat_type_1, core_heating_day_set_type_1_entire, metrics_type_1_data
):
    ...
    for item in rhu.itertuples():
        bin_name = thermostat_type_1._format_rhu(
            "rhu2", item.Index.left, item.Index.right, duty_cycle=None
        )
        bin_value = item.rhu
        assert_allclose(bin_value, metrics_type_1_data[1][bin_name], rtol=1e-3)

    rhu = thermostat_type_1.get_resistance_heat_utilization_bins(
        None, temperature_bins, core_heating_day_set_type_1_entire, VAR_MIN_RHU_RUNTIME
    )
...
def test_day_counts(core_days, metrics_type_1_data):
    ...
    assert (
        n_core_days
        == metrics_type_1_data[i]["n_core_{}_days".format(heating_or_cooling)]
    )
    assert (
        n_days_in_inputfile_date_range
        == metrics_type_1_data[i]["n_days_in_inputfile_date_range"]
    )
