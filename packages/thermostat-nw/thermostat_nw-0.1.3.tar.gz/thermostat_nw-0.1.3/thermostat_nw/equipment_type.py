...
EQUIPMENT_MAPPING = [
    ...
    {
        "heat_type": "heat_pump_no_electric_backup",
        "heat_stage": "single_stage",
        "cool_type": "heat_pump",
        "cool_stage": None,
    },  # 2
    {
        "heat_type": "furnace_or_boiler",
        "heat_stage": "single_stage",
        "cool_type": "central",
        "cool_stage": "single_speed",
    },  # 3
    {
        "heat_type": "furnace_or_boiler",
        "heat_stage": "single_stage",
...
def validate_heat_type(heat_type):
    if heat_type is None or heat_type == "" or heat_type in HEAT_TYPE:
        return True
    logging.warn(
        "Heating type {heat_type} is not a recognized heating type.".format(
            heat_type=heat_type
        )
    )
    return False
...
def validate_cool_type(cool_type):
    if cool_type is None or cool_type == "" or cool_type in COOL_TYPE:
        return True
    logging.warn(
        "Cooling type {cool_type} is not a recognized cooling type.".format(
            cool_type=cool_type
        )
    )
    return False
...
def validate_heat_stage(heat_stage):
    if heat_stage is None or heat_stage == "" or heat_stage in HEAT_STAGE:
        return True
    logging.warn(
        "Heating stage {heat_stage} is not a recognized heating stage.".format(
...
def validate_cool_stage(cool_stage):
    if cool_stage is None or cool_stage == "" or cool_stage in COOL_STAGE:
        return True
    logging.warn(
        "Cooling stage {cool_stage} is not a recognized cooling stage.".format(
