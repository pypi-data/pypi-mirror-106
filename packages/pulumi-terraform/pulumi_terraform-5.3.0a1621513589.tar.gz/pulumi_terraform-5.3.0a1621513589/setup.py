...
class InstallPluginCommand(install):
    def run(self):
        install.run(self)
        try:
            check_call(['pulumi', 'plugin', 'install', 'resource', 'terraform', '5.3.0-alpha.1621513589+9be917ca'])
            ...
            if error.errno == errno.ENOENT:
                ...
                `pulumi plugin install resource terraform 5.3.0-alpha.1621513589+9be917ca`
                """)
