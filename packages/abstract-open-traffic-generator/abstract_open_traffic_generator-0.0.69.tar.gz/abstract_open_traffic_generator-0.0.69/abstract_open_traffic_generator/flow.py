...
class Burst(object):
    ...
    Args
    ----
    - bursts (int): The number of packet bursts transmitted per flow
    ...
    """
    def __init__(self, bursts=0, packets=None, gap=12, inter_burst_gap=0, inter_burst_gap_unit='bytes'):
        if isinstance(bursts, (float, int, type(None))) is True:
            self.bursts = bursts
        else:
            raise TypeError('bursts must be an instance of (float, int, type(None))')
        if isinstance(packets, (float, int, type(None))) is True:
            self.packets = packets
        else:
            raise TypeError('packets must be an instance of (float, int, type(None))')
