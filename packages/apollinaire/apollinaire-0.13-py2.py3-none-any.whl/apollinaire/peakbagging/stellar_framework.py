...
def stellar_framework (freq, psd, r, m, teff, nwalkers=64, dnu=None, back=None, numax=None, Hmax=None, Wenv=None, epsilon=None, 
                       ...
                       num=500, reboxing_strategy='median', save_only_after_sampling=False, apodisation=False, fit_angle=False, guess_angle=90, 
                       fit_splittings=False, guess_split=0, frozen_harvey_exponent=False, fit_amp=False, extended=False, author=None, 
                       pkb_fmt=None, projected_splittings=False, bins=100, existing_chains_back='read', existing_chains_pattern='read', 
                       existing_chains_pkb='read', strategy='order') :
  ...
  :param reboxing_strategy: reboxing strategy to take box values when using ``quickfit``. Can be ``median`` or ``mean``. Optional, default
    ``mean``.
  ...
  :param existing_chains_back: controls the behaviour of the function concerning a background parameters existing hdf5 file. If, ``read``, existing files will be read
  ...
  if strategy=='pair' :
    raise Exception ("stellar_framework is unable to use 'pair' fitting strategy. You need to manually build your a2z file and to use the 'peakbagging' function.")
  ...
  if back is None :
    fitted_back, param_back = perform_mle_background (freq, psd, n_harvey=n_harvey, r=r, m=m, teff=teff, guess=guess_back, 
                              ...
                              apodisation=apodisation, num=num, reboxing_strategy=reboxing_strategy)
    ...
    if back_mcmc :
      fitted_back, param_back, sigma_back  = explore_distribution_background (freq, psd,
                                       ...
                                       apodisation=apodisation, num=num, reboxing_strategy=reboxing_strategy, bins=bins, existing_chains=existing_chains_back)
      np.savetxt (filename_back+'_parameters.dat', np.c_[param_back, sigma_back], fmt='%-s', 
                  header=make_header_background_param (n_harvey=n_harvey, author=author))
    
    if use_numax_back :
      numax = param_back[n_harvey*3+3]
    Wenv = param_back[n_harvey*3+4] 

    np.savetxt (filename_back+'.dat', fitted_back, fmt='%-s', header=make_header_background_vector (author=author))
  ...
  if a2z_in is None :
    ...
    if pattern_mcmc :
      df_a2z, pattern, sigma_pattern = explore_distribution_pattern (dnu, numax, Hmax, Wenv, teff, freq, psd, back=fitted_back, wdw=None,
                           ...
                           thin=thin_pattern, save_only_after_sampling=save_only_after_sampling, bins=bins, existing_chains=existing_chains_pattern)
  ...
  save_pkb (a2z_file[:len(a2z_file)-3]+'pkb', pkb, fmt=pkb_fmt, author=author, extended=extended, spectro=spectro, 
