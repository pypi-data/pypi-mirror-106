...
class LookerExtractor(BaseExtractor):
    ...
    def _extract_charts(
        ...
        for e in filter(lambda e: e.type == "vis", dashboard_elements):
            charts.append(Chart(title=e.title))
            ...
            for f in e.result_maker.filterables:
                ...
                if explore is None:
                    # TODO(ch1290): Log this as warning until the issue is fixed
                    logger.warn(f"Chart {e.title} references invalid explore {f.view}")
                    continue
