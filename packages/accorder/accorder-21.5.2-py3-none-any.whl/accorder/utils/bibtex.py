...
def no_calibre():
    click.echo(
        "Please, check if you have Calibre installed and it is possible to run it as simple as `calibre` from your command line. Also, make sure it doesn't run while you run accorder against your Bibtex file. Once ready, please try this again."
    )
    exit(1)
    ...
    for bb in bib_data.entries.items():
        ...
        if "file" in b.fields:
            ...
            for l in b.fields["file"].split(sep):
                if (
                    l.startswith("files/")
                    and l.split(".")[-1].lower() in book_extensions
                ):
        ...
        authors = []
        authors_sort = []
        for role in ["author", "editor", "director"]:
            if role in b.persons:
                for author in b.persons[role]:
                    ...
                    author = f"{first_names} {prelast_names} {last_names}".replace(
                        "  ", " "
                    ).strip()
                    author_sort = (
                        f"{last_names}, {first_names} {prelast_names}".replace(
                            "  ", " "
                        ).strip()
                    )
            ...
            keywords = set(
                list(
                    itertools.chain.from_iterable(
                        [
                            [
                                sanitize(t).lower().strip().replace(",", "")
                                for t in k.split("/")
                            ]
        ...
        bibhash = base64.b64encode(
            hashlib.md5(
                BibliographyData({bb[0]: b}).to_string("bibtex").encode()
            ).digest()
        ).decode()[:6]
...
def set_metadata(entry, cid, calibre_directory, bibtex_path):
    ...
    if rp.returncode != 0:
        click.echo(
            f"[bibhash: {entry['bibhash']}: '{entry['title']}' by ['authors']]. FAILED to set metadata for {cid} in Calibre"
        )
        exit(1)
    else:
        for fline in entry["file"].values():
            ...
            if fp.returncode != 0:
                click.echo(
                    f"[bibhash: {entry['bibhash']}: '{entry['title']}' by ['authors']]. FAILED to add_format for {cid} in Calibre"
                )
                exit(1)
        else:
            return "SUCCESS"
...
def run_calibre(entries, bibtex_path, calibre_directory):
    ...
    for i, entry in enumerate(entries):
        ...
        cl = [
            ...
            "id",
            "-s",
            f'identifier:"bibhash:{bibkey}"',
            f"--with-library={calibre_directory}",
            "--for-machine",
        ]
        cl2 = [
            "calibredb",
            ...
            "id",
            "-s",
            f"identifiers:bibhash:{entry['bibhash']}",
            f"--with-library={calibre_directory}",
            "--for-machine",
        ]
            ...
            if bids:
                ...
                if bp.returncode != 0:
                    ...
                    if len(hids) > 1:
                        ...
                        click.echo(
                            f"{n}/{total} DUPLICATE: {entry['title']} by {entry['authors']}"
                        )
                        total_d += 1
                        continue
                    elif not hids:
        ...
        if not cid:
            ...
            if p.stderr.decode().startswith(
                "The following books were not added as they already exist in the database"
            ):
                click.echo(
                    f"{n}/{total} DUPLICATE: {entry['title']} by {entry['authors']} (via calibredb stderr)"
                )
                total_d += 1
                continue
            elif p.returncode != 0:
                ...
                if not cid.isdigit():
                    click.echo(
                        f"Reading ID of the empty book jus added failed: {p.stdout}"
                    )
                    exit(1)
