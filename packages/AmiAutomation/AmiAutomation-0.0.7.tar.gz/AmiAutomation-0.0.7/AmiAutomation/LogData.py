...
def RegModeDivisor(mode):
    div = ExtModeDivisor(mode)
    if div ==1:
        return IntModeDivisor(mode)
    else:
        return div

def getDivisors(divList, RegModeList):
    divColumnReturn = []
    for i,div in enumerate(divList):
        regulationMode = RegModeList[i]
        if div<0:
            if (div == -1): #Int. mode
                divColumnReturn.append(IntModeDivisor(regulationMode))
            elif (div == -2): #Ext. mode
                divColumnReturn.append(ExtModeDivisor(regulationMode))
            elif (div== -3): #Int. or Ext. mode
                divColumnReturn.append(RegModeDivisor(regulationMode))
        else:
            divColumnReturn.append(div)
    return divColumnReturn

def readString(bytesArray,bytePos):
...
def processVersionN(version, Properties, bytesArray, innerBufferSize):
    """
...
def binStreamToDF(file):
    ...
    for items in range(intItemsCount): 
        ...
        dataType, size = getDatatype(dataTypeEnum)
        if dataType == "s":#string
            value, bytePos = readString(bytesArray, bytePos)
    ...
    if 'Version' in Properties:
        if Properties['Version'] == 1:
            logData = processVersionN(1, Properties, bytesArray[bytePos:bytePos+innerLength], innerLength)
        elif Properties['Version'] == 2:
            logData = processVersionN(2, Properties, bytesArray[bytePos:bytePos+innerLength], innerLength)
        elif Properties['Version'] == 3:
            logData = processVersionN(3, Properties, bytesArray[bytePos:bytePos+innerLength], innerLength)
        else:
    ...
    try:
        divColumns = getDivisors(logData._mainDivisors,dFrame["RegStatus.RegulationMode"] )
    except:
