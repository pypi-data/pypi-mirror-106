...
class RsCMPX_UwbMeas:
	"""299 total commands, 4 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMP/CMX/CMW, SupportedIdnPatterns = CMP/CMX/CMW, SimulationIdnString = 'Rohde&Schwarz,CMP,100001,4.0.7.0005'"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_UwbMeas version failed. Current version: '4.0.7.0005', minimum required version: '{min_version}'")
	...
	def uwbMeas(self):
		...
		if not hasattr(self, '_uwbMeas'):
			from .Implementations.UwbMeas import UwbMeas
