...
from .common.functions import single_task, wechat_info_err
from .init import db, scheduler
                ...
                if not req_url:
                    ...
                    if req_url.find('http') == -1:
                        ...
                        try:
                            ...
                            req = requests.get(
                                req_url, params={'xiaoniu_cron_log_id': xiaoniu_cron_log_id},
                                timeout=2 * 60, headers={'user-agent': 'xiaoniu_cron'})
                            ...
                            jl = JobLog(cron_info_id=cron_id, content=ret, create_time=nows,
                                        take_time=time.time() - t, log_id=xiaoniu_cron_log_id)
                            db.session.add(jl)
                            db.session.commit()
                        except Exception as e:
                            jl = JobLog(cron_info_id=cron_id, content="发生严重错误:%s" %
                                        str(e), create_time=nows, take_time=time.time() - t)
                            db.session.add(jl)
                            db.session.commit()
                ...
                for item in crons:
                    ...
                    if counts > int(job_log_counts):
                        sql = "delete from job_log where cron_info_id = '%s' limit %s" % (
                            item.id, (counts - int(job_log_counts)))
