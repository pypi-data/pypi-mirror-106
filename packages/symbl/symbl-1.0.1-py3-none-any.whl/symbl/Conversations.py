...
class Conversation():
    ...
    def __init__(self, conversationId: str, jobId: str,  wait=True, credentials=None):
        ...
        self.job = Job(conversation_id=conversationId, job_id=jobId, wait=wait)
        self.wait = wait
    ...
    def getJobStatus(self):
        return self.job.__job_status.value

    def on_complete(self, func):
        self.job.__success_func = func
        return self

    def on_error(self, func):
        self.job.__error_func = func
        return self
    ...
    def monitorJob(self):
        self.job.monitor_job(self, interval=self.INTERVAL_TIME_IN_SECONDS, wait=self.wait, credentials=self.credentials)