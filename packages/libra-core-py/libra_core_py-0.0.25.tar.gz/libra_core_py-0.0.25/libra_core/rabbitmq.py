...
class RabbitmqClient(object):
    ...
    def consume_loop(self, callback, auto_ack=True):
        while True:
            self._consume_once(callback, auto_ack)

    def _consume_once(self, callback, auto_ack=True):
        while True:
            try:
                while not self._connect():
                    sleep(0.1)
                if not self._queue:
                    ...
                    self._queue = q.method.queue
                self._channel.basic_qos(prefetch_count=1)
                mframe, hframe, body = self._channel.basic_get(queue=self._queue, auto_ack=auto_ack)
                if body is not None:
                    self._check_times = 0
                    callback(body)
                    if not auto_ack:
                    ...
                    self._check_times += 1
                    if self._check_times == 10:
                        log_info("not get new queue message for {0}".format(self._queue))
                        self._check_times = 0
                    sleep(1)
            except Exception as e:
                log_error('consume {} msg error({})'.format(self._queue, e))
