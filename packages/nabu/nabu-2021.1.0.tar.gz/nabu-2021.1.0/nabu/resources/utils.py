...
def is_hdf5_extension(ext, errors="raise"):
    if ext not in files_formats:
        if errors == "raise":
            raise ValueError("Unknown data file format: %s" % ext)
    return files_formats[ext] == files_formats["hdf5"]
