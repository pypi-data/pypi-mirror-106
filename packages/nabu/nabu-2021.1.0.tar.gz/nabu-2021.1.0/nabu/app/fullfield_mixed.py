...
class FullRadiosPipeline(FullFieldPipeline):
    """
    A class for processig a radios stack.
    """

    def __init__(self, process_config, images_group, sub_region=None, logger=None, extra_options=None):
        """
        Initialize a FullRadiosPipeline object.
        ...
        Contrarily to `FullFieldPipeline`, the sub_region (start_x, end_x, start_z, end_z) cannot be changed.
        What can be changed is only the "images group".
        In short:

           - Use `FullFieldPipeline` to process `images[:, 0:100, :]` then `images[:, 100:200, :]`, etc
           - Use `FullRadiosPipeline` to process `images[0:1000, :, :]` then `images[1000:2000, :, :]`, etc
    ...
    def _set_params(self, process_config, images_group, sub_region, extra_options):
        ...
        self.dataset_info = self.process_config.dataset_infos
        self.dataset_infos = self.process_config.dataset_infos # other shorthand - deprecated
    ...
    def _init_reader_finalize(self):
        ...
        self._radios_cropped = self.radios # compat. with FullFieldPipeline
    ...
    def _init_flatfield(self, shape=None):
        ...
        FullFieldPipeline._init_flatfield(self, shape=shape)
    ...
    def _retrieve_phase_ctf(self):
        ...
        for i in range(self.radios.shape[0]):
            self.phase_retrieval.retrieve_phase(
                ...
                translation_vh=self.dataset_info.ctf_translations[i],
...
class SinoStackPipeline:
    ...
    SinoNormalizationClass = FullFieldPipeline.SinoNormalizationClass
    SinoProcessingClass = FullFieldPipeline.SinoProcessingClass
    SinoDeringerClass = FullFieldPipeline.SinoDeringerClass
    FBPClass = FullFieldPipeline.FBPClass
    HistogramClass = FullFieldPipeline.HistogramClass


    """
    A pipeline for processing tomography data from sinograms.
    This class is to be used to complement FullRadiosPipeline.
    """
    ...
    def _init_reader(self):
        if isinstance(self.projections, str):
            ...
            expected_radios_shape = self.projections.shape
        else:
    ...
    _allocate_array = FullFieldPipeline._allocate_array
    _allocate_sinobuilder_output = FullFieldPipeline._allocate_sinobuilder_output
    _allocate_recs = FullFieldPipeline._allocate_recs
    _init_sino_normalization = FullFieldPipeline._init_sino_normalization
    _init_sino_builder = FullFieldPipeline._init_sino_builder
    _init_sino_rings_correction = FullFieldPipeline._init_sino_rings_correction
    _init_histogram = FullFieldPipeline._init_histogram
    _init_writer = FullFieldPipeline._init_writer
    ...
    def _get_projections(self):
        if isinstance(self.projections, str):
            self.radios = load_images_stack_from_hdf5(
                self.projections,
                self._data_h5_path,
                sub_region=(None, None, self.stack[0], self.stack[1], None, None)
            )
    ...
    _normalize_sinos = FullFieldPipeline._normalize_sinos
    _build_sino = FullFieldPipeline._build_sino
    _destripe_sinos = FullFieldPipeline._destripe_sinos
    _init_reconstruction = FullFieldPipeline._init_reconstruction
    _reconstruct = FullFieldPipeline._reconstruct
    _compute_histogram = FullFieldPipeline._compute_histogram
    _write_data = FullFieldPipeline._write_data
    _write_histogram = FullFieldPipeline._write_histogram
    _get_process_name = FullFieldPipeline._get_process_name


    def _prepare_reconstruction(self):
        FullFieldPipeline._prepare_reconstruction(self)
        self._rec_roi = None # not implemented for OpenCL backend
