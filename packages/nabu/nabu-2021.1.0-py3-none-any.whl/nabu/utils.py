...
def get_num_threads(n=None):
    ...
    """
    n_slurm_cores = os.environ.get("SLURM_CPUS_PER_TASK", None)
    if n_slurm_cores is not None:
        n_cores = int(n_slurm_cores)
    else:
        n_cores = cpu_count()
    if n is None:
        return n_cores
...
def deprecated(msg, do_print=False):
    def decorator(func):
        def wrapper(*args, **kwargs):
            if not _warnings.get(func.__name__, False):
                deprecation_warning(msg)
                if do_print:
                    print("Deprecation warning:", msg)
                _warnings[func.__name__] = 1
            res = func(*args, **kwargs)
            return res
        return wrapper
    return decorator
