...
class ExpressionFilter(BaseFilter):
    ...
    def __init__(self, key: str, op: str, value: str) -> None:
        ...
        self.op = op
        self.value = value
        self.getattr = operator.attrgetter(key)
...
class PatternFilter(ExpressionFilter):
    """Pattern filter."""

    operator_re = r"=~|!~"

    def __init__(self, key: str, op: str, value: str) -> None:
        """Initialize pattern filter from a regex pattern."""
    ...
    def match(self, value: t.Union[str, t.Any]) -> bool:
        ...
        path = value if isinstance(value, str) else self.getattr(value)
...
class SizeFilter(ExpressionFilter):
    ...
    def match(self, value: t.Union[int, t.Any]) -> bool:
        ...
        size = value if isinstance(value, int) else self.getattr(value)
...
class TimeFilter(ExpressionFilter):
    ...
    def match(self, value: t.Union[int, t.Any]) -> bool:
        ...
        timestamp = value if isinstance(value, int) else self.getattr(value)
        time_obj = dt.datetime.fromtimestamp(timestamp).strftime("%Y%m%d%H%M%S")
        return OPERATORS[self.op](time_obj[: len(self.time)], self.time)
...
class NumberFilter(ExpressionFilter):
    ...
    def match(self, value: t.Union[int, float, t.Any]) -> bool:
        ...
        value = value if isinstance(value, (int, float)) else self.getattr(value)
