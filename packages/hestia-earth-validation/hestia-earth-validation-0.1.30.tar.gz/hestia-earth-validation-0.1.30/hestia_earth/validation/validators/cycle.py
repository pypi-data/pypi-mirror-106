...
from hestia_earth.validation.utils import _flatten, _find_linked_node, _value_average, _list_sum_terms
...
def validate_cycle(cycle: dict, nodes=[]):
    ...
    ] if len(cycle.get('emissions', [])) > 0 else []) + _flatten([
    ...
    ] if len(cycle.get('inputs', [])) > 0 else []) + _flatten([
    ...
    ] if len(cycle.get('practices', [])) > 0 else []) + _flatten([
