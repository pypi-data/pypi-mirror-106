...
def compare_history(tickers,items, \
    ...
    if mode == 'T1I1':
        ...
        ylabeltxt=''
        titletxt=codetranslate(ticker1)+": 业绩历史"
    ...
    if mode == 'T1I2':
        ...
        ylabeltxt=''
        titletxt=codetranslate(ticker1)+": 业绩历史对比"
        import datetime; today=datetime.date.today()
        footnote="数据来源: 雅虎财经, "+str(today)
    ...
    if mode == 'T2I1':
        ...
        label2=ectranslate(item2)
        ylabeltxt=''
        titletxt=codetranslate(ticker1)+" vs "+codetranslate(ticker2)+": 业绩对比"
        import datetime; today=datetime.date.today()
        footnote="数据来源: 雅虎财经, "+str(today)
...
def compare_tax(tickers,graph=True):
    """
    功能：比较公司最新的实际所得税率
    """
    #检查股票代码列表
    ...
    if graph:
        ...
        footnote=ectranslate(itemk)+" -->"+ \
            "\n数据来源: 雅虎财经, "+str(today)
        plot_barh(df,colname,titletxt,footnote)   
...
def calc_fin_rates(fsdf):
    ...
    fs['Operating Margin']=fs['OperatingIncome']/fs['TotalRevenue']
