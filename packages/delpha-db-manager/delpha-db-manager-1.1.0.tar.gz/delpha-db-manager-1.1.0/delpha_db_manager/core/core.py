...
class AWSManager:
    ...
    """
    def __init__(self, key, secret, flow_type, region='us-east-2'):
    ...
    def _load_settings(self, flow_type):
        ...
        self.settings = json.loads(self.s3_handler.get_object(Bucket=self.buckets[0], Key = "config/flow_settings.json")["Body"].read().decode('utf-8'))
        self.current_settings = self.settings[flow_type]
        self.current_settings["bucket_name"] = self.buckets[0]
        
    def set_flow_settings(self, flow_type):
        """
    ...
    def get_flow_data(self, bucket_name=None, prefix=None, flow_name=None):
        """
        ...
        last_flow = self._get_last_flow_id(flow_name)
        files = self.list_files(self.buckets[0], prefix+last_flow)[1:]
        res_df = pd.DataFrame([])
        for file in files:
            res = self.s3_handler.get_object(Bucket=self.buckets[0], Key = file)
            current_df = pd.read_csv(res['Body'])
            res_df = pd.concat([res_df, current_df], ignore_index=True)
        return res_df
    ...
    def start_flow(self, flow_name):
        ...
        flow_last_execution_record = self.flow_handler.describe_flow_execution_records(flowName='accountFlow')["flowExecutions"].pop(0)["executionId"]
        return flow_status, flow_last_execution_record
    
    def _get_last_flow_id(self, flow_name):
