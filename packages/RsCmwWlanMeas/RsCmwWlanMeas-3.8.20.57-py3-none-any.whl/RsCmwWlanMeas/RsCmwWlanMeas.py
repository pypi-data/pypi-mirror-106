...
class RsCmwWlanMeas:
	"""1158 total commands, 5 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.20.0057'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '3.8.20.0057'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.20.0057'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwWlanMeas version failed. Current version: '3.8.20.0057', minimum required version: '{min_version}'")
	...
	def tmode(self):
		...
		if not hasattr(self, '_tmode'):
			from .Implementations.Tmode import Tmode
