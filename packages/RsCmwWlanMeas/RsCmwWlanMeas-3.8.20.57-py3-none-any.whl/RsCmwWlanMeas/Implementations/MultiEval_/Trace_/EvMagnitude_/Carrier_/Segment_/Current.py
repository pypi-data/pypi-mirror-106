...
class Current:
	...
	def read(self, start: float = None, count: float = None, decimation: float = None, segment=repcap.Segment.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.evMagnitude.carrier.segment.current.read(start = 1.0, count = 1.0, decimation = 1.0, segment = repcap.Segment.Default) \n
	...
	def fetch(self, start: float = None, count: float = None, decimation: float = None, segment=repcap.Segment.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.evMagnitude.carrier.segment.current.fetch(start = 1.0, count = 1.0, decimation = 1.0, segment = repcap.Segment.Default) \n
		Return the values of the EVM vs. subcarrier traces for 80+80 MHz, SISO connections. The results of the current, average,
		minimum and maximum traces can be retrieved. \n
		Use RsCmwWlanMeas.reliability.last_value to read the updated reliability indicator. \n
			:param start: numeric For the optional query parameters start, count and decimation, see 'Trace Sub-Arrays'.
