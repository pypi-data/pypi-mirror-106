...
class StandardDev:
	...
	def fetch(self, user=repcap.User.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.modulation.evMagnitude.user.standardDev.fetch(user = repcap.User.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:MODulation:EVMagnitude:USER{user_cmd_val}:SDEViation?', self.__class__.FetchStruct())
