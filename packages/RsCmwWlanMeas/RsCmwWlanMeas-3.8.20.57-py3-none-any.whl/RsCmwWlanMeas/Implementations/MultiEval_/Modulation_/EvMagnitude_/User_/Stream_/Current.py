...
class Current:
	...
	def fetch(self, user=repcap.User.Default, stream=repcap.Stream.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:MODulation:EVMagnitude:USER<user>:STReam<str>:CURRent \n
		Snippet: value: FetchStruct = driver.multiEval.modulation.evMagnitude.user.stream.current.fetch(user = repcap.User.Default, stream = repcap.Stream.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:MODulation:EVMagnitude:USER{user_cmd_val}:STReam{stream_cmd_val}:CURRent?', self.__class__.FetchStruct())
