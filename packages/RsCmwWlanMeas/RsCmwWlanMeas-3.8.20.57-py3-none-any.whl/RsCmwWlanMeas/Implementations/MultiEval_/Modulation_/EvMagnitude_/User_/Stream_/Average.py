...
class Average:
	...
	def fetch(self, user=repcap.User.Default, stream=repcap.Stream.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:MODulation:EVMagnitude:USER<user>:STReam<str>:AVERage \n
		Snippet: value: FetchStruct = driver.multiEval.modulation.evMagnitude.user.stream.average.fetch(user = repcap.User.Default, stream = repcap.Stream.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:MODulation:EVMagnitude:USER{user_cmd_val}:STReam{stream_cmd_val}:AVERage?', self.__class__.FetchStruct())
