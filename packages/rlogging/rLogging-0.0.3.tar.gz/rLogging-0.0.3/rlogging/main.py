...
def __construct_logger_example():
    """ Пример инициализации логера """

    logger = get_logger('rlogging')

    lineFormater = formaters.LineFormater()

    # Вывод критических ошибок
    terminalPrinter = printers.TerminalPrinter()
    terminalPrinter.formater = lineFormater

    mainProcessHandler = handlers.MainProcessHandler()
    mainProcessHandler.printersPool = printers.PrintersPool([
        terminalPrinter
    ])

    # Вывод всех сообщений
    filePrinter = printers.FilePrinter()
    filePrinter.fileName = 'log.log'
    filePrinter.formater = lineFormater

    subProcessHandler = handlers.SubProcessHandler()
    subProcessHandler.printersPool = printers.PrintersPool([
        filePrinter
    ])

    logger.handlersPool = handlers.HandlersPool([
        mainProcessHandler,
        subProcessHandler
    ])
