...
class CostReporter(costexplorer.CostExplorer):
    ...
    def __init__(self, *, account_id: str=None, role: str=AWS_FINOPS_ROLE, granularity: str='MONTHLY', period: relativedelta=None, date: datetime.datetime=None) -> None:
        super().__init__(granularity=granularity, period=period, date=date)
        if account_id:
            self.client = self.assume_role_client(account_id=account_id, role=role)
    ...
    def by_regions(self) -> list:
        return self.get_cost_by(dimension="REGION")

    def by_services(self) -> list:
        return self.get_cost_by(dimension="SERVICE")
