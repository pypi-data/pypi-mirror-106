...
def register_localstack_plugins():
 ...
 if DbULh([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if 'No module named' not in DbULE(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return DbULf(os.environ.get('LOCALSTACK_API_KEY'))
