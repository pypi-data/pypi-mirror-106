...
class CloudPodManager(RQPCj):
 BACKEND='_none_'
 def __init__(self,pod_name=RQPCL,config=RQPCL):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise RQPCI('Not implemented')
 def pull(self):
  raise RQPCI('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert RQPCN(statuses)==RQPCN(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise RQPCI('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @RQPCX
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise RQPCI('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==RQPCN([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=RQPCi)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise RQPCI('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=RQPCi)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=RQPCW(self,'clone_dir',RQPCL)
...
class PodConfigManagerMeta(RQPCu):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=RQPCL
   for manager in cls.CHAIN:
    try:
     tmp=RQPCW(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif RQPCp(tmp,RQPCw)and RQPCp(result,RQPCw):
       result.extend(tmp)
    except RQPCI:
     pass
   if result is not RQPCL:
    return result
   raise RQPCI('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(RQPCj,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @RQPCX
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise RQPCI('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(RQPCj):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=RQPCi).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=RQPCN([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=RQPCv):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except RQPCI:
   if safe:
    return{}
   raise RQPCI('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(RQPCj):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise RQPCI('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise RQPCI('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise RQPCI('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
