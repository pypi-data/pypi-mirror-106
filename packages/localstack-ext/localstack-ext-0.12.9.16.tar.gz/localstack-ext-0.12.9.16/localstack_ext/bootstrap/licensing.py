...
def read_api_key():
 ...
 if key:
  return key
 raise yFrCx('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=yFrCB)
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=yFrCv(yFrCG(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in yFrCp(yFrCK(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs['cached_key']={'timestamp':yFrCG(timestamp),'key_hash':md5(api_key),'key':key_b64}
...
def load_cached_key(api_key):
 ...
 if cached_key.get('key_hash')!=md5(api_key):
  raise yFrCx('Cached key was created for a different API key')
 now=now_utc()
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise yFrCx('Cached key expired')
 timestamp=yFrCv(cached_key['timestamp'])
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get('finalized'):
  try:
   key=fetch_key()
   if not key:
    raise yFrCx('Unable to fetch and validate API key from environment')
   if key!='test':
    decrypt_files(key)
...
def get_auth_headers():
 ...
 if not login_info:
  raise yFrCx('Please log in first')
