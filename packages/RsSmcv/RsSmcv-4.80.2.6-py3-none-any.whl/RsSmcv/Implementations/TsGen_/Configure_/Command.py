...
class Command:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("command", core, parent)
	...
	def set_with_opc(self, index=repcap.Index.Default) -> None:
		index_cmd_val = self._base.get_repcap_cmd_value(index, repcap.Index)
		"""SCPI: TSGen:CONFigure:COMMand<CH> \n
		...
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmcv.utilities.opc_timeout_set() to set the timeout value. \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Command')"""
		self._core.io.write_with_opc(f'TSGen:CONFigure:COMMand{index_cmd_val}')

	def clone(self) -> 'Command':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Command(self._core, self._base.parent)
