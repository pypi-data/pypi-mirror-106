...
class Iqcoder:
	"""Iqcoder commands group definition. 11 total commands, 6 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("iqcoder", core, parent)

	@property
	def atsm(self):
		"""atsm commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_atsm'):
			from .Iqcoder_.Atsm import Atsm
	...
	def dtmb(self):
		...
		if not hasattr(self, '_dtmb'):
			from .Iqcoder_.Dtmb import Dtmb
	...
	def dvbs2(self):
		...
		if not hasattr(self, '_dvbs2'):
			from .Iqcoder_.Dvbs2 import Dvbs2
	...
	def dvbs(self):
		...
		if not hasattr(self, '_dvbs'):
			from .Iqcoder_.Dvbs import Dvbs
	...
	def dvbt(self):
		...
		if not hasattr(self, '_dvbt'):
			from .Iqcoder_.Dvbt import Dvbt
	...
	def isdbt(self):
		...
		if not hasattr(self, '_isdbt'):
			from .Iqcoder_.Isdbt import Isdbt
	...
	def clone(self) -> 'Iqcoder':
		...
		new_group = Iqcoder(self._core, self._base.parent)
