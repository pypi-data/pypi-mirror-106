...
class Drm:
	"""Drm commands group definition. 24 total commands, 3 Sub-groups, 12 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("drm", core, parent)

	@property
	def msc(self):
		"""msc commands group. 3 Sub-classes, 1 commands."""
		if not hasattr(self, '_msc'):
			from .Drm_.Msc import Msc
			self._msc = Msc(self._core, self._base)
		return self._msc
	...
	def sdc(self):
		...
		if not hasattr(self, '_sdc'):
			from .Drm_.Sdc import Sdc
			self._sdc = Sdc(self._core, self._base)
		return self._sdc
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Drm_.Setting import Setting
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmcv.Source.Bb.Drm.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:DRM:PRESet')
	...
	def clone(self) -> 'Drm':
		...
		new_group = Drm(self._core, self._base.parent)
