...
class Msc:
	"""Msc commands group definition. 4 total commands, 3 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("msc", core, parent)

	@property
	def level(self):
		"""level commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_level'):
			from .Msc_.Level import Level
	...
	def profile(self):
		...
		if not hasattr(self, '_profile'):
			from .Msc_.Profile import Profile
	...
	def clone(self) -> 'Msc':
		...
		new_group = Msc(self._core, self._base.parent)
