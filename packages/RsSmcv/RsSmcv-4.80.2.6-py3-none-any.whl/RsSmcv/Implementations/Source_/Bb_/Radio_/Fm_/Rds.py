...
class Rds:
	"""Rds commands group definition. 135 total commands, 7 Sub-groups, 11 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("rds", core, parent)

	@property
	def af(self):
		"""af commands group. 2 Sub-classes, 1 commands."""
		if not hasattr(self, '_af'):
			from .Rds_.Af import Af
	...
	def di(self):
		...
		if not hasattr(self, '_di'):
			from .Rds_.Di import Di
			self._di = Di(self._core, self._base)
		return self._di
	...
	def eon(self):
		...
		if not hasattr(self, '_eon'):
			from .Rds_.Eon import Eon
			self._eon = Eon(self._core, self._base)
		return self._eon
	...
	def opf(self):
		...
		if not hasattr(self, '_opf'):
			from .Rds_.Opf import Opf
			self._opf = Opf(self._core, self._base)
		return self._opf
	...
	def tmc(self):
		...
		if not hasattr(self, '_tmc'):
			from .Rds_.Tmc import Tmc
			self._tmc = Tmc(self._core, self._base)
		return self._tmc
	...
	def clone(self) -> 'Rds':
		...
		new_group = Rds(self._core, self._base.parent)
