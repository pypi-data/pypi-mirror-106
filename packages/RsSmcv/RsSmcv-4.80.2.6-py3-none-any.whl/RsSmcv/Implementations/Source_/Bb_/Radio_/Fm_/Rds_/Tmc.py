...
class Tmc:
	"""Tmc commands group definition. 8 total commands, 3 Sub-groups, 2 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tmc", core, parent)

	@property
	def apply(self):
		"""apply commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_apply'):
			from .Tmc_.Apply import Apply
	...
	def clone(self) -> 'Tmc':
		...
		new_group = Tmc(self._core, self._base.parent)
