...
class T2Dvb:
	"""T2Dvb commands group definition. 117 total commands, 15 Sub-groups, 18 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("t2Dvb", core, parent)

	@property
	def bandwidth(self):
		"""bandwidth commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_bandwidth'):
			from .T2Dvb_.Bandwidth import Bandwidth
	...
	def channel(self):
		...
		if not hasattr(self, '_channel'):
			from .T2Dvb_.Channel import Channel
	...
	def delay(self):
		...
		if not hasattr(self, '_delay'):
			from .T2Dvb_.Delay import Delay
	...
	def fef(self):
		...
		if not hasattr(self, '_fef'):
			from .T2Dvb_.Fef import Fef
			self._fef = Fef(self._core, self._base)
		return self._fef
	...
	def guard(self):
		...
		if not hasattr(self, '_guard'):
			from .T2Dvb_.Guard import Guard
	...
	def inputPy(self):
		...
		if not hasattr(self, '_inputPy'):
			from .T2Dvb_.InputPy import InputPy
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .T2Dvb_.Setting import Setting
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmcv.Source.Bb.T2Dvb.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:T2DVb:PRESet')
	...
	def clone(self) -> 'T2Dvb':
		...
		new_group = T2Dvb(self._core, self._base.parent)
