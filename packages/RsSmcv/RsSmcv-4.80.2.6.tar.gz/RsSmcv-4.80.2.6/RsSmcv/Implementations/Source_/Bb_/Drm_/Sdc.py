...
class Sdc:
	"""Sdc commands group definition. 4 total commands, 3 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("sdc", core, parent)

	@property
	def level(self):
		"""level commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_level'):
			from .Sdc_.Level import Level
	...
	def profile(self):
		...
		if not hasattr(self, '_profile'):
			from .Sdc_.Profile import Profile
	...
	def clone(self) -> 'Sdc':
		...
		new_group = Sdc(self._core, self._base.parent)
