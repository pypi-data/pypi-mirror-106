...
class Ping:
	"""Ping commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""
	...
	def set_with_opc(self, ipVersion=repcap.IpVersion.Default) -> None:
		ipVersion_cmd_val = self._base.get_repcap_cmd_value(ipVersion, repcap.IpVersion)
		"""SCPI: [SOURce<HW>]:BB:INPut:IP<CH>:IGMP:[SOURce]:PING \n
		...
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmcv.utilities.opc_timeout_set() to set the timeout value. \n
			:param ipVersion: optional repeated capability selector. Default value: Nr4 (settable in the interface 'Ip')"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:INPut:IP{ipVersion_cmd_val}:IGMP:SOURce:PING')
