...
class IsPy:
	"""IsPy commands group definition. 4 total commands, 3 Sub-groups, 1 group commands
	...
	def formatPy(self):
		...
		if not hasattr(self, '_formatPy'):
			from .IsPy_.FormatPy import FormatPy
	...
	def tsChannel(self):
		...
		if not hasattr(self, '_tsChannel'):
			from .IsPy_.TsChannel import TsChannel
			self._tsChannel = TsChannel(self._core, self._base)
	...
	def dataRate(self):
		...
		if not hasattr(self, '_dataRate'):
			from .IsPy_.DataRate import DataRate
	...
	def set(self, input_py: enums.CodingInputSignalInputA, inputStream=repcap.InputStream.Default) -> None:
		...
		inputStream_cmd_val = self._base.get_repcap_cmd_value(inputStream, repcap.InputStream)
