...
class InputPy:
	"""InputPy commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""
	...
	def formatPy(self):
		...
		if not hasattr(self, '_formatPy'):
			from .InputPy_.FormatPy import FormatPy
	...
	def stuffing(self):
		...
		if not hasattr(self, '_stuffing'):
			from .InputPy_.Stuffing import Stuffing
			self._stuffing = Stuffing(self._core, self._base)
	...
	def testSignal(self):
		...
		if not hasattr(self, '_testSignal'):
			from .InputPy_.TestSignal import TestSignal
	...
	def dataRate(self):
		...
		if not hasattr(self, '_dataRate'):
			from .InputPy_.DataRate import DataRate
			self._dataRate = DataRate(self._core, self._base)
