...
class Ip:
	"""Ip commands group definition. 7 total commands, 6 Sub-groups, 0 group commands
	Repeated Capability: IpVersion, default value after init: IpVersion.Nr4"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("ip", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_ipVersion_get', 'repcap_ipVersion_set', repcap.IpVersion.Nr4)
	...
	def alias(self):
		...
		if not hasattr(self, '_alias'):
			from .Ip_.Alias import Alias
			self._alias = Alias(self._core, self._base)
		return self._alias
	...
	def igmp(self):
		...
		if not hasattr(self, '_igmp'):
			from .Ip_.Igmp import Igmp
			self._igmp = Igmp(self._core, self._base)
		return self._igmp
	...
	def multicast(self):
		...
		if not hasattr(self, '_multicast'):
			from .Ip_.Multicast import Multicast
			self._multicast = Multicast(self._core, self._base)
	...
	def clone(self) -> 'Ip':
		...
		new_group = Ip(self._core, self._base.parent)
