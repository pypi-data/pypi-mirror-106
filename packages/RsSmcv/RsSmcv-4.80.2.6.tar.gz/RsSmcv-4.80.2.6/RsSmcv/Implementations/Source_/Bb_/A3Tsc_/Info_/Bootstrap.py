...
class Bootstrap:
	"""Bootstrap commands group definition. 12 total commands, 7 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("bootstrap", core, parent)

	@property
	def basic(self):
		"""basic commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_basic'):
			from .Bootstrap_.Basic import Basic
	...
	def bsr(self):
		...
		if not hasattr(self, '_bsr'):
			from .Bootstrap_.Bsr import Bsr
			self._bsr = Bsr(self._core, self._base)
		return self._bsr
	...
	def fft(self):
		...
		if not hasattr(self, '_fft'):
			from .Bootstrap_.Fft import Fft
	...
	def guard(self):
		...
		if not hasattr(self, '_guard'):
			from .Bootstrap_.Guard import Guard
	...
	def pilot(self):
		...
		if not hasattr(self, '_pilot'):
			from .Bootstrap_.Pilot import Pilot
	...
	def preamble(self):
		...
		if not hasattr(self, '_preamble'):
			from .Bootstrap_.Preamble import Preamble
	...
	def time(self):
		...
		if not hasattr(self, '_time'):
			from .Bootstrap_.Time import Time
	...
	def clone(self) -> 'Bootstrap':
		...
		new_group = Bootstrap(self._core, self._base.parent)
