...
class Dvbs2:
	"""Dvbs2 commands group definition. 41 total commands, 8 Sub-groups, 10 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dvbs2", core, parent)

	@property
	def inputPy(self):
		"""inputPy commands group. 1 Sub-classes, 2 commands."""
		if not hasattr(self, '_inputPy'):
			from .Dvbs2_.InputPy import InputPy
	...
	def s2X(self):
		...
		if not hasattr(self, '_s2X'):
			from .Dvbs2_.S2X import S2X
			self._s2X = S2X(self._core, self._base)
		return self._s2X
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Dvbs2_.Setting import Setting
	...
	def symbols(self):
		...
		if not hasattr(self, '_symbols'):
			from .Dvbs2_.Symbols import Symbols
	...
	def tsl(self):
		...
		if not hasattr(self, '_tsl'):
			from .Dvbs2_.Tsl import Tsl
			self._tsl = Tsl(self._core, self._base)
		return self._tsl
	...
	def special(self):
		...
		if not hasattr(self, '_special'):
			from .Dvbs2_.Special import Special
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmcv.Source.Bb.Dvbs2.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:DVBS2:PRESet')
	...
	def clone(self) -> 'Dvbs2':
		...
		new_group = Dvbs2(self._core, self._base.parent)
