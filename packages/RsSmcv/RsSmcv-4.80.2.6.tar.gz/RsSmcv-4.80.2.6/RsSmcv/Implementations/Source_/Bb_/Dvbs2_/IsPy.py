...
class IsPy:
	"""IsPy commands group definition. 5 total commands, 4 Sub-groups, 0 group commands
	...
	def packetLength(self):
		...
		if not hasattr(self, '_packetLength'):
			from .IsPy_.PacketLength import PacketLength
	...
	def stuffing(self):
		...
		if not hasattr(self, '_stuffing'):
			from .IsPy_.Stuffing import Stuffing
	...
	def testSignal(self):
		...
		if not hasattr(self, '_testSignal'):
			from .IsPy_.TestSignal import TestSignal
	...
	def useful(self):
		...
		if not hasattr(self, '_useful'):
			from .IsPy_.Useful import Useful
