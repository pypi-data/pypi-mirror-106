...
class Til:
	"""Til commands group definition. 8 total commands, 8 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("til", core, parent)

	@property
	def blocks(self):
		"""blocks commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_blocks'):
			from .Til_.Blocks import Blocks
	...
	def cil(self):
		...
		if not hasattr(self, '_cil'):
			from .Til_.Cil import Cil
			self._cil = Cil(self._core, self._base)
		return self._cil
	...
	def extended(self):
		...
		if not hasattr(self, '_extended'):
			from .Til_.Extended import Extended
	...
	def inter(self):
		...
		if not hasattr(self, '_inter'):
			from .Til_.Inter import Inter
			self._inter = Inter(self._core, self._base)
		return self._inter
	...
	def maxBlocks(self):
		...
		if not hasattr(self, '_maxBlocks'):
			from .Til_.MaxBlocks import MaxBlocks
	...
	def ntiBlocks(self):
		...
		if not hasattr(self, '_ntiBlocks'):
			from .Til_.NtiBlocks import NtiBlocks
			self._ntiBlocks = NtiBlocks(self._core, self._base)
	...
	def clone(self) -> 'Til':
		...
		new_group = Til(self._core, self._base.parent)
