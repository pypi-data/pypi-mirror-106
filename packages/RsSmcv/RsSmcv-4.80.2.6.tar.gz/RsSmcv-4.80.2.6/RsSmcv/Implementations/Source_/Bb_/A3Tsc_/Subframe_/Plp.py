...
class Plp:
	"""Plp commands group definition. 2 total commands, 2 Sub-groups, 0 group commands"""
	...
	def nidPlp(self):
		...
		if not hasattr(self, '_nidPlp'):
			from .Plp_.NidPlp import NidPlp
			self._nidPlp = NidPlp(self._core, self._base)
		return self._nidPlp
	...
	def nplp(self):
		...
		if not hasattr(self, '_nplp'):
			from .Plp_.Nplp import Nplp
			self._nplp = Nplp(self._core, self._base)
		return self._nplp
