...
class T2Mi:
	"""T2Mi commands group definition. 13 total commands, 3 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("t2Mi", core, parent)

	@property
	def max(self):
		"""max commands group. 0 Sub-classes, 4 commands."""
		if not hasattr(self, '_max'):
			from .T2Mi_.Max import Max
	...
	def resetLog(self):
		...
		if not hasattr(self, '_resetLog'):
			from .T2Mi_.ResetLog import ResetLog
			self._resetLog = ResetLog(self._core, self._base)
	...
	def clone(self) -> 'T2Mi':
		...
		new_group = T2Mi(self._core, self._base.parent)
