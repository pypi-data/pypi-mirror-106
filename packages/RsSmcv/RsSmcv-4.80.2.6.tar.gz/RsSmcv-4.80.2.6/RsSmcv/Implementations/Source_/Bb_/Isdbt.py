...
class Isdbt:
	"""Isdbt commands group definition. 71 total commands, 17 Sub-groups, 14 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("isdbt", core, parent)

	@property
	def channel(self):
		"""channel commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_channel'):
			from .Isdbt_.Channel import Channel
	...
	def constel(self):
		...
		if not hasattr(self, '_constel'):
			from .Isdbt_.Constel import Constel
	...
	def eew(self):
		...
		if not hasattr(self, '_eew'):
			from .Isdbt_.Eew import Eew
			self._eew = Eew(self._core, self._base)
		return self._eew
	...
	def iip(self):
		...
		if not hasattr(self, '_iip'):
			from .Isdbt_.Iip import Iip
			self._iip = Iip(self._core, self._base)
		return self._iip
	...
	def inputPy(self):
		...
		if not hasattr(self, '_inputPy'):
			from .Isdbt_.InputPy import InputPy
	...
	def payload(self):
		...
		if not hasattr(self, '_payload'):
			from .Isdbt_.Payload import Payload
	...
	def segments(self):
		...
		if not hasattr(self, '_segments'):
			from .Isdbt_.Segments import Segments
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Isdbt_.Setting import Setting
	...
	def source(self):
		...
		if not hasattr(self, '_source'):
			from .Isdbt_.Source import Source
	...
	def testSignal(self):
		...
		if not hasattr(self, '_testSignal'):
			from .Isdbt_.TestSignal import TestSignal
			self._testSignal = TestSignal(self._core, self._base)
	...
	def tsPackets(self):
		...
		if not hasattr(self, '_tsPackets'):
			from .Isdbt_.TsPackets import TsPackets
			self._tsPackets = TsPackets(self._core, self._base)
	...
	def useful(self):
		...
		if not hasattr(self, '_useful'):
			from .Isdbt_.Useful import Useful
	...
	def special(self):
		...
		if not hasattr(self, '_special'):
			from .Isdbt_.Special import Special
	...
	def get_packet_length(self) -> enums.InputSignalPacketLength:
		...
		Queries the packet length of the external transport stream in bytes. \n
			:return: packet_length: INValid| P208| P188| P204 P188|P204|P208 188/204/208 byte packets specified for serial input and parallel input. INValid Packet length does not match the specified length.
		"""
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmcv.Source.Bb.Isdbt.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:ISDBt:PRESet')
	...
	def clone(self) -> 'Isdbt':
		...
		new_group = Isdbt(self._core, self._base.parent)
