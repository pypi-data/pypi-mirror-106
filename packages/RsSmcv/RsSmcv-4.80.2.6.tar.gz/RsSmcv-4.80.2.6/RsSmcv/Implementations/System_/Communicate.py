...
class Communicate:
	"""Communicate commands group definition. 27 total commands, 9 Sub-groups, 0 group commands"""
	...
	def bb(self):
		...
		if not hasattr(self, '_bb'):
			from .Communicate_.Bb import Bb
	...
	def bcIp(self):
		...
		if not hasattr(self, '_bcIp'):
			from .Communicate_.BcIp import BcIp
			self._bcIp = BcIp(self._core, self._base)
		return self._bcIp
