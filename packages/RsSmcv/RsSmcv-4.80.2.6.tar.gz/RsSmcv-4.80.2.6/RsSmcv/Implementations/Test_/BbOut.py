...
class BbOut:
	"""BbOut commands group definition. 2 total commands, 1 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("bbOut", core, parent)

	@property
	def ttest(self):
		"""ttest commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_ttest'):
			from .BbOut_.Ttest import Ttest
			self._ttest = Ttest(self._core, self._base)
		return self._ttest
	...
	def clone(self) -> 'BbOut':
		...
		new_group = BbOut(self._core, self._base.parent)
