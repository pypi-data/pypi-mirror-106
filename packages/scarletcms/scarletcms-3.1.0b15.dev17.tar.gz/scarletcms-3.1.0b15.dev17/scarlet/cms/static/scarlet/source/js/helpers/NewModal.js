...
class NewModal {
	...
	open(qry, hideFields) {
		...
		$(frame).load(() => {
			this.onLoad(qry, frame, hideFields)
		})
	}
...
function buildDialog(url, name, options) {
	if (isModalOpen()) {
         return $('<div class="modal__frameWrap modal__inline"></div>')
            .html(`<iframe id="${name}" src="${url}" style="border: 0;" width="100%" height="100%"></iframe>`)
    } else {
         return $('<div class="modal__framesContainer"></div>')
            .html(`<div class="modal__frameWrap"><iframe id="${name}" style="border: 0; " src="${url}" width="100%" height="100%"></iframe></div>`)
            .dialog(options)
	}
}
 ...
 */
const clickOpenAndHideModal = function(e, name, cb, hideFields) {
	e.preventDefault()
	let url = $(e.currentTarget).attr('href')
	let modal = new NewModal(url, name, false, function(data) {
		cb(data)
	})
	modal.open(false, hideFields)
...
export { clickOpenAndHideModal, isModalOpen }
