...
class Oscilloscope(Moku):
    ...
    def set_defaults(self):
        """
        Reset the Oscilloscope to default state.
        """
    ...
    def set_frontend(self, channel, coupling, attenuation=1, strict=True):
        """
        Configures the input impedance, coupling, and attenuation for each
        channel.
        ...
        :type attenuation: `number`
        :param attenuation: Input attenuation

        """
        operation = "set_frontend"

        params = dict(strict=strict, channel=channel, coupling=validate_range(
            coupling, list(['AC', 'DC'])), attenuation=attenuation,)
