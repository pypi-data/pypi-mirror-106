...
class SpectrumAnalyzer(Moku):
    ...
    def set_defaults(self):
        """
        Set the Spectrum Analyzer to sane defaults.
        """
    ...
    def sa_output(
            ...
            strict=True):
        """
        Generate a waveform on the output channels.
    ...
    def set_rbw(self, mode, rbw_value=5000, strict=True):
        """
        Set desired Resolution Bandwidth. Actual resolution bandwidth will be
        rounded to the nearest allowable unit when settings are applied to the
        device.
    ...
    def set_span(self, frequency1, frequency2, strict=True):
        """
        Set the desired frequency span to be analyzed. Rounding and
        quantization in the instrument limits the range of spans for which a
        full set of 1024 data points can be calculated. This means that the
        resultant number of data points in SpectrumData frames will vary with
        the set span. Note however that the associated frequencies are given
        with the frame containing the data.
    ...
    def sa_measurement(
            ...
            strict=True):
        """
        Sets up commonly used configurations in the Spectrum Analyzer,
        including the frequency range, channel signal source, resolution
        bandwidth, and window function.
