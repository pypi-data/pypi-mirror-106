...
class FrequencyResponseAnalyzer(Moku):
    ...
    def set_defaults(self):
        """
        Reset the Frequency Response Analyzer to default state.
        """
    ...
    def set_frontend(self, channel, coupling, range, strict=True):
        """
        Configures the input impedance, coupling, and range for each input
        channel.
    ...
    def fra_measurement(
            ...
            strict=True):
        """
        Sets up commonly used configurations in the Frequency Response
        Analyzer, including the frequency range, averaging, and swept sine
        amplitude.
    ...
    def measurement_mode(self, input_only=False, strict=True):
        """
        Toggle measurement between Input and Input/Output.
    ...
    def set_input_range(self, channel, range, strict=True):
        """
        ...
        :type range: `string`, {'10Vpp', '50Vpp'}
        :param range: The peak to peak voltage (Vpp) range of the inputs.

        """
        operation = "set_input_range"
    ...
    def set_output(self, channel, amplitude, offset=0, strict=True):
        """
        Configures the output sweep amplitude and offset. Ensure the input
        signal passing through the device under test will not exceed the input
        range as configured by set_frontend.
