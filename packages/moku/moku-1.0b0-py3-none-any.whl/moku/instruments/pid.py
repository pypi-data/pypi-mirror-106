...
class PIDController(Moku):
    ...
    def set_hysteresis(self, hysteresis_mode, value=0, strict=True):
        """
        Configures the hysteresis around trigger point.
    ...
    def get_data(self):
        """
        Get 1 frame of the data from the instrument.
    ...
    def set_defaults(self):
        """
        Set the PID Controller to sane defaults.
        """
    ...
    def set_monitor(self, monitor_channel, source, strict=True):
        ...
        instrument signal.There are two monitoring channels available, each of
        these can be assigned to source signals from any of the internal PID
        instrument monitoring points. Signals larger than 12-bits must be
        either truncated or clipped to the allowed size.
    ...
    def set_by_gain_and_section(
            ...
            strict=True):
        """
        Configure the selected PID controller and its sections using gain
        coefficients.
