...
class TaskRegister:
    ...
    def register(self):
        try:
            ...
            prod_task_name = content_dict['metadata']['task_name']
            if os.getenv('ENVIRONMENT', 'dev') != 'prod':
                content_dict['metadata'][
                    'task_name'] = f'{prod_task_name}_{os.getenv("ENVIRONMENT", "dev")}'
            ...
            try:
                ...
                if 'RepositoryAlreadyExistsException' in str(e):
                    is_new = False
                    repository_uri = f'{account_id}.dkr.ecr.{region_name}.amazonaws.com/{content_dict["metadata"]["task_name"]}'
            ...
            if os.getenv('DOCKER_HOST'):
                client = docker.APIClient(base_url=os.getenv('DOCKER_HOST'))
            else:
            ...
            with open(self.__get_primerobot_file_path__(), 'w') as primerobotfile:
                ...
                content_copy['metadata'] = content_dict['metadata'].copy()

                content_copy['metadata']['task_name'] = prod_task_name
                yaml.dump(content_copy, primerobotfile)
