...
var IntegrationExternalUserMappings = /** @class */ (function (_super) {
    ...
    function IntegrationExternalUserMappings() {
        ...
        <Body>
          <IntegrationExternalMappingForm organization={organization} integration={integration} onSubmitSuccess={function () {
                        ...
                        closeModal();
                    }} mapping={mapping} sentryNames={_this.sentryNames} type="user" onCancel={closeModal} baseEndpoint={"/organizations/" + organization.slug + "/external-users/"}/>
        </Body>
      </Fragment>);
            });
        };
    ...
    Object.defineProperty(IntegrationExternalUserMappings.prototype, "sentryNames", {
        get: function () {
            var members = this.state.members;
            return members.map(function (_a) {
                var id = _a.id, email = _a.email, name = _a.name, rest = __rest(_a, ["id", "email", "name"]);
                var label = email !== name ? name + " - " + email : "" + email;
                return __assign(__assign({}, rest), { id: id, name: label, email: email });
            });
        },
        enumerable: false,
        configurable: true
