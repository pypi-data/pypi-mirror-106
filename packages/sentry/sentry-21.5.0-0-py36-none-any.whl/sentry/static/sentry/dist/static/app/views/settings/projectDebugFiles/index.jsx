...
var ProjectDebugSymbols = /** @class */ (function (_super) {
    ...
    ProjectDebugSymbols.prototype.getAppStoreConnectId = function () {
        var project = this.props.project;
        var appStoreConnectData = (project.symbolSources
            ? JSON.parse(project.symbolSources)
            : []).find(function (symbolSource) { return symbolSource.type === 'appStoreConnect'; });
        return appStoreConnectData === null || appStoreConnectData === void 0 ? void 0 : appStoreConnectData.id;
    };
    ...
    ProjectDebugSymbols.prototype.getFieldsSymbolSources = function () {
        if (this.getAppStoreConnectId()) {
            return fields.symbolSources;
        }
        var fieldsSymbolSources = fields.symbolSources;
        if (!!fieldsSymbolSources.addDropdown.items.find(function (item) { return item.value === 'appStoreConnect'; })) {
            return fields.symbolSources;
        }
    ...
    ProjectDebugSymbols.prototype.renderAppStoreConnectAlert = function (data) {
        if (!data || Object.keys(data).every(function (key) { return data[key]; })) {
            return null;
        }
        return (<Alert type="warning" icon={<IconWarning size="xs"/>}>
        {t('We were unable to validate your credentials with App Store Connect. Please update the following data:')}
        <List symbol="bullet">
          {!data.appstoreCredentialsValid && (<ListItem>{t('App Store Connect credentials')}</ListItem>)}
    ...
    ProjectDebugSymbols.prototype.renderBody = function () {
        var _this = this;
        var _a, _b;
        var _c = this.props, organization = _c.organization, project = _c.project, params = _c.params;
        var _d = this.state, loading = _d.loading, showDetails = _d.showDetails, builtinSymbolSources = _d.builtinSymbolSources, debugFiles = _d.debugFiles, debugFilesPageLinks = _d.debugFilesPageLinks;
        var orgId = params.orgId, projectId = params.projectId;
        var features = organization.features, access = organization.access;
        var hasAppStoreConnectFeatureFlag = !!((_a = organization.features) === null || _a === void 0 ? void 0 : _a.includes('app-store-connect'));
        var fieldsSymbolSources = hasAppStoreConnectFeatureFlag
        ...
        {features.includes('symbol-sources') && (<Fragment>
            <PermissionAlert />
            {hasAppStoreConnectFeatureFlag && (<AppStoreConnectContext.Consumer>
                {function (data) { return _this.renderAppStoreConnectAlert(data); }}
              </AppStoreConnectContext.Consumer>)}
            <Form saveOnBlur allowUndo initialData={project} apiMethod="PUT" apiEndpoint={"/projects/" + orgId + "/" + projectId + "/"} onSubmitSuccess={ProjectActions.updateSuccess} key={__spreadArray(__spreadArray([], __read(((_b = project.builtinSymbolSources) !== null && _b !== void 0 ? _b : []))), __read((builtinSymbolSources !== null && builtinSymbolSources !== void 0 ? builtinSymbolSources : []))).join() || project.id}>
              <JsonForm features={new Set(features)} title={t('External Sources')} disabled={!access.includes('project:write')} fields={[fieldsSymbolSources, fields.builtinSymbolSources]} additionalFieldProps={fieldProps}/>
            </Form>
          </Fragment>)}
