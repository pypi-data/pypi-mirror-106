...
currentOnly) {
    ...
    switch (yAxis) {
        case YAxis.FAILED_TRANSACTIONS:
            ...
            return EventView.fromSavedQuery(__assign(__assign({}, baseQuery), { query: stringifyQueryObject(new QueryResults(__spreadArray(['event.type:transaction', releaseFilter], __read(statusFilters)).filter(Boolean))) }));
        ...
        default:
            return EventView.fromSavedQuery(__assign(__assign({}, baseQuery), { fields: ['issue', 'title', 'count()', 'count_unique(user)', 'project'], query: stringifyQueryObject(new QueryResults(["release:" + version, '!event.type:transaction'])), orderby: '-count' }));
    }
}
