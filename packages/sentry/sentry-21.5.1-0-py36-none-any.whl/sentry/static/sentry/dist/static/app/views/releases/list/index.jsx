...
var ReleasesList = /** @class */ (function (_super) {
    ...
    ReleasesList.prototype.getEndpoints = function () {
        ...
        var query = __assign(__assign({}, pick(location.query, ['project', 'environment', 'cursor', 'query', 'sort'])), { summaryStatsPeriod: statsPeriod, per_page: 20, flatten: activeSort === SortOption.DATE ? 0 : 1, status: activeStatus === StatusOption.ARCHIVED
                ? ReleaseStatus.Archived
                : ReleaseStatus.Active });
        var endpoints = [
            ['releases', "/organizations/" + organization.slug + "/releases/", { query: query }],
        ];
        return endpoints;
    };
    ...
    ReleasesList.prototype.renderInnerBody = function (activeDisplay) {
        ...
        var _b = this.state, releases = _b.releases, reloading = _b.reloading, releasesPageLinks = _b.releasesPageLinks;
        ...
        {function (_a) {
                ...
                var selectedProjectId = selection.projects &&
                    selection.projects.length === 1 &&
                    selection.projects[0];
                var selectedProject = organization.projects.find(function (p) { return p.id === "" + selectedProjectId; });
                return (<Fragment>
              {selectedProject && (<Feature features={['organizations:release-adoption-chart']}>
                  <Projects orgId={organization.slug} slugs={[selectedProject.slug]}>
                    {function (_a) {
                            ...
                            var project = projects && projects.length === 1 ? projects[0] : null;
                            if (fetchError ||
                                !project ||
                            ...
                            var showPlaceholders = !initiallyLoaded || isHealthLoading;
                            return (<ReleaseAdoptionChart organization={organization} selection={selection} releases={releases} project={project} getHealthData={getHealthData} activeDisplay={activeDisplay} showPlaceholders={showPlaceholders}/>);
                        }}
                  </Projects>
                </Feature>)}
