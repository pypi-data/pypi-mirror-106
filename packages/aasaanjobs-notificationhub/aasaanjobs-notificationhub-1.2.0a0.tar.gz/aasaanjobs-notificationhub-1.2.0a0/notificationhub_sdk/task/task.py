...
from ..common import MessageType, Platform, WaterfallMode, WhatsappContentType
...
class Task:
    ...
    def __init__(self, name: str,
        ...
        Parameters:
            ...
            chatbot_type (ChatbotType, optional): Type of the chatbot (defaults to ChatbotType.NONE)
            whatsapp_content_type (WhatsappContentType, optional): Type of the whatsapp message (defaults to C)
            message_id(str): message id is used as a session id in case the application wants to store its own session
            extra_params(str): additional information to be sent to the external vendors
        ...
        if not whatsapp and chatbot and chatbot.chatbot_whatsapp is not None:
            raise AssertionError('ChatbotWhatsapp is used without Whatsapp structure')
        self.__set_id()
    ...
    def __set_chatbot(self, value: Chatbot):
        if not value:
            return
        self._task.chatbot.CopyFrom(value.proto)
