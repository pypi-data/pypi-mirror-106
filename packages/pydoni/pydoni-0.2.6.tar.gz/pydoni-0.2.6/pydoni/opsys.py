...
def du_by_filetype(dpath,
    ...
    :param dpath {str} path to directory to check
    :param recursive {bool} list files recursively
    :param verbose {bool} print output dictionary to console
    :param human_readable {bool} display filesize in output in human-readable format
    :param progress {bool} display progress bar while scanning directory
    :param total {bool} add final line to output with the total directory filesize
    ...
    os.chdir(dpath)
    logger.info("Changed dir to '%s'" % dpath)
...
def delete_empty_dirs(root, recursive=False, true_remove=False, count_hidden_files=True):
    ...
    for root, dnames, fnames in os.walk(root, topdown=False):
