...
class Postgres(object):
    ...
    def build_update(self, schema, table, pkey_name, pkey_value, columns, values, validate=True, newlines=False):
        ...
        for col, val in zip(columns, values):
            ...
            elif pydoni.test_value(val, 'bool') or pydoni.test_value(val, 'int') or pydoni.test_value(val, 'float'):
                pass
    ...
    def infoschema(self, columns=[], infoschema_table='columns'):
        ...
        self.logger.info(f"Retrieved information_schema.{infoschema_table}")
    ...
    def colnames(self, schema_name, table_name):
        """
        Get column names of table as a list.

        :param schema_name: schema_name name
        ...
        df_cols = df_cols.loc[(df_cols['table_schema'] == schema_name) & (df_cols['table_name'] == table_name)]
        cols = df_cols['column_name'].tolist()

        self.logger.info(f'Columns retrieved from {schema_name}.{table_name}: {cols}')
        return cols

    def coldtypes(self, schema_name, table_name):
        """
        ...
        columns = [k for k, v in dtype.items()]
        self.logger.info(f'Dtypes retrieved from {schema_name}.{table_name} for columns: {columns}')
    ...
    def read_table(self, schema_name, table_name):
        ...
        self.logger.info(f"Read dataframe {schema_name}.{table_name}, shape: {df.shape}")
        return df
    ...
    def drop_schema(self, table_schema, cascade=True):
        """
    ...
    def drop_and_recreate_schema(self, table_schema):
        """
