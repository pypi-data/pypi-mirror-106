...
class EXIF(object):
    ...
    def __init__(self, fpath):
        ...
        self.bin = pydoni.sh.find_binary('exiftool', abort=True)
    ...
    def extract(self, method='doni', clean_keys=False, clean_values=False):
        """
        Extract EXIF metadata from file or files.

        :param method: method for metadata extraction, one of 'doni' or 'pyexiftool'
        :type method: str
        :param clean_keys: apply EXIF.clean_keys() to EXIF output
        :type clean_keys: bool
        :param clean_values: apply EXIF.clean_values() to EXIF output
        :type clean_values: bool
        ...
        self.logger.var('self.clean', clean_keys)
        self.logger.var('self.clean', clean_values)
        ...
        def detect_dtype(val):
            ...
            for dtype in ['bool', 'float', 'int', 'datetime', 'date', 'str']:
                if dtype == 'str':
                    ...
                    if pydoni.test_value(val, dtype):
                        return dtype
        ...
        for file, d in exifd.items():
            ...
            for k, v in d.items():
                ...
                if dtype in ['bool', 'date', 'datetime', 'int', 'float']:
                    coerced_value = pydoni.test_value(v, dtype, return_coerced_value=True)
                    if v != coerced_value:
    ...
    def clean_keys(self, exifd):
        ...
        with open(join(dirname(__file__), 'data', 'exif_column_map.json'), 'r') as f:
        ...
        for filename, dct in exifd.items():
            ...
            for exif_key in newd[filename].keys():
                if exif_key not in column_map.keys() and exif_key not in column_map.values():
                    self.logger.warn("Key not found in `column_map` used for renaming keys: '%s'" % str(exif_key))
...
class FFmpeg(object):
    ...
    def __init__(self):
        ...
        self.bin = pydoni.sh.find_binary('ffmpeg', abort=True)
    ...
    def join(self, audiofiles, outfile):
        ...
        tmpfile = os.path.join(
            ...
            '.tmp.pydoni.audio.FFmpeg.join.%s.txt' % pydoni.systime(compact=True))
