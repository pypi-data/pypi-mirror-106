# Generated from QueryExprParser.g4 by ANTLR 4.9.2
from antlr4 import *
if __name__ is not None and "." in __name__:
    from .QueryExprParser import QueryExprParser
else:
    from QueryExprParser import QueryExprParser

# This class defines a complete listener for a parse tree produced by QueryExprParser.
class QueryExprParserListener(ParseTreeListener):

    # Enter a parse tree produced by QueryExprParser#exprs.
    def enterExprs(self, ctx:QueryExprParser.ExprsContext):
        pass

    # Exit a parse tree produced by QueryExprParser#exprs.
    def exitExprs(self, ctx:QueryExprParser.ExprsContext):
        pass


    # Enter a parse tree produced by QueryExprParser#expr.
    def enterExpr(self, ctx:QueryExprParser.ExprContext):
        pass

    # Exit a parse tree produced by QueryExprParser#expr.
    def exitExpr(self, ctx:QueryExprParser.ExprContext):
        pass



del QueryExprParser