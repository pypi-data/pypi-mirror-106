...
class Scraper(object):
    ...
    def getImageList(self):
        self.pbar = tqdm(desc='Finding new posts of %s with lookback of %s' % (self.username, self.lookback), unit=' posts')
        with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        while count>0:
            for url in z:
                if '%s.jpg' % str(url["upload_date"])[:-3] in os.listdir() or '%s.mp4' % str(url["upload_date"])[:-3] in os.listdir():
                    continue
                if round(time.time()) - int(str(url["upload_date"])[:-3]) > self.lookback:
                    continue
...
def main():
    ...
    if args.lookback:
        scraper = Scraper(args.username)
        scraper.set_lookback(args.lookback * 86400) #seconds in a day, so args takes number of days
        scraper.getImages()
