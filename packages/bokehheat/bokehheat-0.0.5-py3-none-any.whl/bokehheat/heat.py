...
# the stackedbar
def stackedbar(df_matrix, ds_stack_color, b_sum_to1=True):
    """
    input:
        df_matrix: matrix dataframe.
          ...
          b_sum_to1 set to False is processed like the columns are the bars.
        ...
        b_sum_to1: boolean, if fuction should check, if df_matrix is  
    ...
    if (b_sum_to1 and not b_sum1_column and not b_sum1_row):
        sys.exit("Error at bokehheat.stackedbar: {}".format(
    ...
    elif (b_sum_to1 and not b_sum1_column and b_sum1_row) \
        or (not b_sum_to1 and not b_color_column and b_color_row):
    ...
    elif (b_sum_to1 and b_sum1_column and not b_sum1_row) or (b_sum_to1 and b_sum1_column and b_sum1_row) \
        or  (not b_sum_to1 and b_color_column and not b_color_row) or (not b_sum_to1 and b_color_column and b_color_row):
    ...
    # error case:
        ...
        sys.exit("Error at bokehheat.stackedbar: {}".format(
            ds_error["matrix_else"].format(b_sum_to1, b_sum1_column, b_sum1_row, b_color_column, b_color_row)
        ))
...
# the cluster stackedbar diagram
def clusterbar(
        df_matrix,
        ds_stack_color,
        b_sum_to1 = True,
    ...
    b_bar_column = False
    # !left & right: right stochatstic matrix
    if (b_sum_to1 and not b_sum1_column and b_sum1_row) \
        or (not b_sum_to1 and not b_color_column and b_color_row):
        b_bar_row = True
        ls_stack_label = sorted(df_matrix.columns.values)
        s_axis = df_matrix.columns.name 
    # (left & !right) | (left & right): left or doubly stochastic matrix
    if (b_sum_to1 and b_sum1_column and not b_sum1_row) or  (b_sum_to1 and b_sum1_column and b_sum1_row) \
        or  (not b_sum_to1 and b_color_column and not b_color_row) or (not b_sum_to1 and b_color_column and b_color_row):
        b_bar_column = True
...
if __name__ == '__main__':
    ...
    # process stackedbar
    #p = stackedbar(df_matrix=df_matrix, ds_stack_color=ds_stack_color, b_sum_to1=True)
    #show(p)
