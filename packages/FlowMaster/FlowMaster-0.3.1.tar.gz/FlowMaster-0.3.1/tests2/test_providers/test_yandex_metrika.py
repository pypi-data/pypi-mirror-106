...
from tests.fixtures.yandex_metrika import (
    ymm_goals_to_file_config,
    ymm_counters_to_file_config,
    ymm_clients_to_file_config,
    ymstats_to_file_config,
...
def test_management_clients():
    ymm_clients_to_file_config.export.credentials = credentials[
        "yandex-metrika-management"
    ]
    config = ETLFlowConfig(**dict(ymm_clients_to_file_config))
    etl_flow = ETLOperator(config)
    list(etl_flow(dt.datetime(2021, 2, 1), dt.datetime(2021, 2, 1)))


def test_management_goals():
    ymm_goals_to_file_config.export.credentials = credentials[
        "yandex-metrika-management"
    ]
    config = ETLFlowConfig(**dict(ymm_goals_to_file_config))
    etl_flow = ETLOperator(config)
    list(etl_flow(dt.datetime(2021, 2, 1), dt.datetime(2021, 2, 1)))


def test_management_counters():
    ymm_counters_to_file_config.export.credentials = credentials[
        "yandex-metrika-management"
    ]
    config = ETLFlowConfig(**dict(ymm_counters_to_file_config))
    etl_flow = ETLOperator(config)
    list(etl_flow(dt.datetime(2021, 2, 1), dt.datetime(2021, 2, 1)))


def test_stats():
    ymstats_to_file_config.export.credentials = credentials["yandex-metrika-stats"][
        "credentials"
    ]
...
def test_logs():
    yml_visits_to_file_config.export.credentials = credentials["yandex-metrika-logs"]
    yml_visits_to_file_config.export.params.columns = ["ym:s:bounce"]
