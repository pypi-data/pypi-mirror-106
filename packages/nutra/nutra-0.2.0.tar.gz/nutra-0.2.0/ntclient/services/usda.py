...
def sort_foods_by_nutrient_id(nutrient_id):
    """Default sort, by amount / 100 g"""
    # TODO: sub shrt_desc for long if available, and support FOOD_NAME_TRUNC
    results = truncate_results(sort_foods(nutrient_id))
    return print_results(results, nutrient_id)


def sort_foods_by_kcal_nutrient_id(nutrient_id):
    """Caloric-weighted sort, by amount / 200 kcal"""
    results = truncate_results(sort_foods_by_kcal(nutrient_id))
    return print_results(results, nutrient_id)
