...
from ..persistence import VERBOSITY
...
def foods_analyze(food_ids, grams=None):
    ...
    # TODO: from ..utils.nutprogbar import nutprogbar
    # Get analysis
    raw_analyses = analyze_foods(food_ids)
    analyses = {}
...
def day_analyze(day_csv_paths, rda_csv_path=None):
    ...
    for rda in rdas:
        ...
        for nutrient in nutrients:
            if nutrient[0] == nutrient_id:
                ...
                if VERBOSITY > 1:
                    print(
                        f"INJECT RDA: {f'{_rda} {nutrient[2]}'.ljust(12)} -->  {nutrient[4]}"
                    )
    ...
    for log in logs:
        ...
        for entry in log:
            if entry["id"]:
                # TODO: was this broken with pycharm/pylint refactoring?
                #  Is it log_id or nutrient_id?
