...
class Trainer(TrainerBase):
    def __init__(
        ...
        self._scheduler_warm_up = scheduler_warm_up
        ...
        self._update_log_parameters(clearml_additional_params_to_log or {})
    ...
    def _validation_run(self, master_bar_logger: master_bar, global_steps: int = 0) -> Optional[Dict[str, float]]:
        ...
        with torch.no_grad():
            ...
            num_validation_batches = math.ceil(
                self._validation_iterator.get_num_batches(self._validation_data) / num_gpus
            )

            batches_this_epoch = 0
            val_loss = 0
            for batch_group in progress_bar(
                val_generator, total=num_validation_batches, parent=master_bar_logger, leave=False
            ):
                _, loss = self._batch_outputs_and_loss(self._model, batch_group)
                batches_this_epoch += 1
    ...
    def _update_log_parameters(self, clearml_additional_params_to_log: Dict[str, Any]) -> None:

        model_log_params = {"model_class": self._model.__class__.__name__, **self._model.get_serializable_params()}

        optimization_log_params = {
            "optimizer": self._optimizer.__class__.__name__,
            "scheduler_warm_up": self._scheduler_warm_up,
            "get_lr_scheduler_name": None if self._get_lr_scheduler is None else self._get_lr_scheduler.__name__,
            "scheduler": None if self._lr_scheduler is None else self._lr_scheduler.__class__.__name__,
            **{
                f"optimizer_initial_param_{name}": value
                for name, value in getattr(self._optimizer, "defaults", {}).items()
            },
            "grad_norm": self._grad_norm,
        }

        data_log_params = {
            "train_iterator_class": self._iterator.__class__.__name__,
            "train_batch_size": self._iterator._batch_size,
            "validation_iterator_class": self._validation_iterator.__class__.__name__,
            "num_train_instances": getattr(self._train_data, "__len__", None),
            "num_validation_instances": getattr(self._validation_data, "__len__", None),
            "shuffle": self._shuffle,
        }

        has_early_stopping = isinstance(self._callbacks.early_stopping, EarlyStopping)
        if has_early_stopping:
            patience = self._callbacks.early_stopping._patience  # type: ignore
            early_stopping_metric = self._callbacks.early_stopping.metric_name  # type: ignore
        else:
            patience = None
            early_stopping_metric = None
        early_stopping_log_params = {
            "early_stopping": has_early_stopping,
            "patience": patience,
            "early_stopping_metric": early_stopping_metric,
        }  # think that +/- can be omitted for logging

        train_steps_log_params = {
            "num_epochs": self._num_epochs,
            "accumulation_steps": self._accumulation_steps,
            "gradual_unfreezing_steps": self._gradual_unfreezing_steps,
            "checkpoint_steps": self._checkpoint_steps,
            "run_validation_each_global_steps": self._run_validation_each_global_steps,  # TODO: Does it really useful?
        }

        fp16_log_params = {"fp16": self._fp16, "fp16_opt_level": self._fp16_opt_level}

        self._callbacks.clearml_update_parameters(
            {
                **model_log_params,
                **optimization_log_params,
                **data_log_params,
                ...
                **train_steps_log_params,
                **fp16_log_params,
                **clearml_additional_params_to_log,
            }
        )
