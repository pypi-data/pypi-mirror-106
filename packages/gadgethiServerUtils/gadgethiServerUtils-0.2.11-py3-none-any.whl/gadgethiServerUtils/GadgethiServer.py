...
class GadgetHiHTTPHandler(SimpleHTTPRequestHandler):
	...
	def authentication_helper(self, headers, ip, **configs):
		...
		auth = GadgethiAuthenticationStandardDecryption(headers['Gadgethi-Key'],headers['Hmac256-Result'],headers['time'])
		print("headers = ",headers)
		print ("key = ",headers["Gadgethi-Key"])
		c = auth.authentication_decryption()
		if c['indicator']:
