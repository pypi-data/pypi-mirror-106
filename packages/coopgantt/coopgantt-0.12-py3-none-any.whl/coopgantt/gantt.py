...
class Gantt:
    ...
    def gantt_from_listdict(cls, name: str, tasks_listdict: List[Dict], resources_dict: Dict[str, Resource]=None):
        ...
        for row in tasks:
            #TODO: Error out if not unique id
        ...
        for id, dependency in dependency_map.items():
             dep_dict = {activities[dependency_id]: completion_threshold
                           for dependency_id, completion_threshold in dependency.items()
                           if dependency_id in activities.keys()}
             activities[id].dependencies = dep_dict
    ...
    def _clean_input_data(data: List[Dict]) -> List[Dict]:
        ...
        for row in data:
            ...
            row_dependency_list = eval(row['dependencies']) if row['dependencies'] != '' else {}
            row_dependency_list = {int(id): completion_thresh for id, completion_thresh in row_dependency_list.items()}
    ...
    def date_plus_working_days(self, start_date: date, days: int):
        epoch = start_date
        ii = 0
        while ii < days:
            if not self.date_skipped(epoch):
              ii += 1
    ...
    def has_been_scheduled(self):
        return not any(self.activities_not_scheduled)
    ...
    def as_dataframe(self) -> pd.DataFrame:
        ...
        df['dependencies'] = ['&&'.join([str(dependency.id) for dependency in task.dependencies]) for task in self.tasks]
...
class ScheduledActivity(ABC):
    ...
    @dependencies.setter
    def dependencies(self, dependants: Dict):
    ...
    def schedule(self, start_date, max_resources_by_type_for_project: Dict[str, int]):
        ...
        # Dependencies
        for activity, completion_req in self.dependencies.items():
            ...
            dep_completion_thresh_days = round(activity.duration * completion_req, 0)
            dep_completion_thresh_date = self.project.chart.date_plus_working_days(activity.start_date, dep_completion_thresh_days)
            schedule_start_date = max(dep_completion_thresh_date, schedule_start_date)
    ...
    def end_date_estimated(self):
        ...
        work_to_do = (1 - self.percent_done/100.0) * self.duration - 1
