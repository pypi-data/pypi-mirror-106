...
class Debugger(object):
    ...
    def should_break(self, frame, event, arg):
        ...
        # Step
        if (
            self.mode == 'continue' and
            frame == self.return_frame or
            self.return_frame not in inspect.getouterframes(frame)
        ):
            frames = inspect.getouterframes(frame)
            if self.return_frame not in frames:
                self.return_frame = frames[-1]
            return True
