...
class ParallelFnExecutor:
    ...
    def log_fn(self, worker_name):
        def _log_fn(message, error=False):
            self.log(f'[{worker_name}]{message}', error)
    ...
    def __execute_job(self, worker_name, params):
        ...
        # execute fn
        data = self.__fn_cls().apply(worker_name, self.log_fn(worker_name), **params)
    ...
    def __merge_result(self, clean_up: bool) -> List:
        result = []
    ...
    def execute(self, fn, static_params: Optional[Dict] = None, params: Optional[Dict] = None, clean_up=True) -> List:
        ...
        return self.__merge_result(clean_up)
