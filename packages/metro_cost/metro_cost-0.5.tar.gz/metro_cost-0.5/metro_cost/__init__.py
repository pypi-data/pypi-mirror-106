...
class Indicator:
    ...
    def discrete_value_tran(self,one_variable, value):
        discrete_var = ['适用时间', '所属线路', '规划期', '施工工艺', '连续墙宽度', '车站地质', '混凝土强度']
        if one_variable in discrete_var:
            discrete = one_variable
        ...
        discrete_data = list(self.data[discrete])
        discrete_value = list(np.unique(discrete_data))
        discrete_dict = dict(zip(discrete_var_value, discrete_value))
        tran_value = discrete_dict[value]
        return tran_value
        ...
        for x in feature_list:
            if x in discrete_var:
                ...
                if discrete == '适用时间':
                    ...
                    discrete_var_value = ['C20', 'C25', 'C15', 'C30', 'C35']
                print(discrete+'只能输入以下取值:','\n',[y for y in discrete_var_value])

        for i in range(len(feature_list)):
            x = input()
            if feature_list[i] in discrete_var:
                x = self.discrete_value_tran(feature_list[i], x)
            value_list.append(x)
        predict_one = pd.DataFrame(value_list)
        predict_one = pd.DataFrame(predict_one.values.T, index=predict_one.columns, columns=predict_one.index)
        predict_one.columns = feature_list
