...
class PyocsJenkins:
    ...
    @staticmethod
    def _get_customer_name(cur_dir: str, modle_id: str):
        """
        #name:get_customer_name
        #function:获得当前MODLE_ID的客户名
        #return:CUSTOMER_XX
        """
        str_modle_id = modle_id
        if 'CS' in str_modle_id or 'CP' in str_modle_id:
            customer_dir = Path(str(cur_dir) + '/customers/customer/')
            str_grep_cmd = "grep -rIsl " + modle_id + " " + str(customer_dir)
            logging.info(str_grep_cmd)
            file_name = os.popen(str_grep_cmd).read()
            logging.info(file_name)
            if 'customer_' not in file_name:
                raise RuntimeError("未找到此MODLE_ID，请确认输入")
        else:
    ...
    def get_param_from_interaction(self, model_id, build_cmd_param_dict):
        ...
        customer_name = self._get_customer_name(cur_dir=code_root_dir, modle_id=model_id)
