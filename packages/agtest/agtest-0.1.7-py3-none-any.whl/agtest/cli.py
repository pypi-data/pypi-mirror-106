...
def get_package_id(base_url, mac_id, mac_token, host, releaseid, content_definition_id):
    ...
    # package_ids = [] # TODO is there ever a reason there would be more than one item in the returned entitie
...
def validate(file, rename):
    ...
    if diff_def["added_files"]:
        ...
        report.write('ADDED:\n')
        report.write('------\n')
        for location, file_hash in diff_def['added_files'].items():
    ...
    if diff_def['changed_files']:
        ...
        report.write('--------\n')
        for location, file_hash in diff_def['changed_files'].items():
            if os.path.isfile(location):
                matches_definition = compare_hashes(location, file_hash)
                if matches_definition:
...
def api(ctx, env, username, password):
    ...
    # testuser = os.environ.get('AGTEST_USER', '-1')
    # testpw = os.environ.get('AGTEST_PW', '-1')
    # print(f'Testuser: {testuser}')
    # print(f'Testpw: {testpw}')
    ctx.obj['MAC_ID'] = m_id
