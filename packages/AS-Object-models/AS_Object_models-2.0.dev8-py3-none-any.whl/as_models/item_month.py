...
class ItemMonthSummary(SalesInvBase):
    ...
    def _publishRefreshListsName(cls, refreshList):
        ...
        if qsVal is None:
            ...
            pubList = {"item_id":refreshList['item_id'],
                       ...
                       'grow_months':strGrowMonths,
                       'start_month':refreshList.get('start_month',None)}
            publish_message(UPDATE_MONTH_INVENTORY,'Start Processing Name Refresh',pubList)
        else:
    ...
    def PopProcessPublish(cls,publishInfo,doPublish=True):
        ...
        startMonth = publishInfo.get('start_month',None)
        growMonth = gms.pop()
        ...
        ## TODO:  Add check... if inventory did not change... stop publishing the refresh
        ims.add_summary(im)
        if doPublish:
...
class ItemMonth(SalesInvBase):
    ...
    def GetItemMonthDict(cls,invLoc, growPeriod, periodType, itemType, itemId):
        ...
        if supplyDoc.exists:
            try:
                #supply = supplyDoc.getData(f"supply.`{itemId}`") ## If there is not supply.. this will fail
                supply = []
                supplyDict = supplyDoc.getData('supply')
                if supplyDict:
                    supply = supplyDict.get(itemId,[])
