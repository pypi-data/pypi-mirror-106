...
class TitofistoTestCase(TestCase):
    ...
    def test_token_deterministic(self):
        ...
        token_1, _ = self.storage.get_token(self.test_file_1_name, ts)
        token_2, _ = self.storage.get_token(self.test_file_1_name, ts)
    ...
    def test_token_file_dependent(self):
        ...
        token_1, _ = self.storage.get_token(self.test_file_1_name, ts)
        token_2, _ = self.storage.get_token(self.test_file_2_name, ts)

        self.assertNotEqual(token_1, token_2)

    def test_token_ts_dependent(self):
        """The generated token is different for different files"""
        ts_1 = datetime.now().strftime("%s")
        ts_2 = ts_1 + "5"

        token_1, _ = self.storage.get_token(self.test_file_1_name, ts_1)
        token_2, _ = self.storage.get_token(self.test_file_2_name, ts_2)

        self.assertNotEqual(token_1, token_2)

    def test_ts_matches_passed(self):
        """The returned timestamp with the token matches the passed timestamp"""
        now = datetime.now()
        _, ts = self.storage.get_token(self.test_file_1_name, now.strftime("%s"))

        self.assertEqual(ts, now.strftime("%s"))

    def test_get_valid_token(self):
        """A file can be retrieved with a valid token"""
        token, ts = self.storage.get_token(self.test_file_1_name)

        url = (
            f"{settings.MEDIA_URL}{self.test_file_1_name}?"
            f"{self.param_prefix}token={token}&"
            f"{self.param_prefix}ts={ts}"
        )
    ...
    def test_get_invalid_token(self):
        ...
        url = (
            f"{settings.MEDIA_URL}{self.test_file_1_name}?"
            f"{self.param_prefix}token={token}FAIL&"
            f"{self.param_prefix}ts={ts}"
        )
        res = self.client.get(url)
    ...
    def test_get_close_to_timeout(self):
        ...
        now = datetime.now()
        token, ts = self.storage.get_token(self.test_file_1_name, now.strftime("%s"))
