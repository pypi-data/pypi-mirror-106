...
class scAEspy(object):
	...
	def __init__(self,
		...
		if self.__constrained and self.__rec_loss in ["MSE","BCE", "MAPE"]:
			print("* Warning, a constrained version of MSE and BCE is not supported. constrained sets to False")
		...
		if self.__rec_loss not in ["Poisson", "ZIP", "NB", "ZINB", "MSE", "BCE", "MAPE"]:
			print("* %s is not supported. The provided loss functions are: Poisson, ZIP, NB, ZINB, MSE, BCE, and MAPE"%self.__rec_loss)
			exit(-1)
	...
	def __BCE(self, x, logits, axis=-1):
		return tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=x, logits=logits), axis)

	def __MAPE(self, x, logits, axis=-1):
		return 100*tf.reduce_mean(tf.abs(tf.divide(tf.subtract(logits,x),(x + 1e-10))), axis)

	# Definition of the MSE loss function
	def __MSE(self, x, logits, axis=-1):
		return tf.reduce_mean(tf.square(tf.subtract(logits,x)), axis)
	...
	def __labeled_loss(self, x, z, zm, zv, zm_prior, zv_prior, px_logit, ps_logit=None, pp_logit=None):
		...
		elif self.__rec_loss=="BCE":
			loss  = self.__BCE(x, px_logit)		

		elif self.__rec_loss=="MAPE":
			loss  = self.__MAPE(x, px_logit)
	...
	def __build(self):
		...
		for i in range(self.__num_gaussians):
			with tf.name_scope('graphs/hot_at{:d}'.format(i)):
				...
				if self.__constrained and self.__rec_loss not in ["MSE","BCE", "MAPE"]:
	...
	def sampling(self, num_samples=1):
		...
		for i in range(num_samples):
			...
			elif self.__rec_loss in ["MSE", "BCE", "MAPE"]:
				sampled[i] = r
	...
	def reconstructedRepresentation(self, data):
		...
		for decode in range(decoded.shape[0]):
			for i in range(self.__num_gaussians):
				if self.__rec_loss in ["MSE", "BCE", "MAPE"]:
