...
class InstallerGUI(Frame):
	def __init__(self, root, to_dl):
		...
		if to_dl != "":
			self.filename = checkname.return_filename(to_dl)
...
class Installer(object):
	...
	def unzip(self):
		try:
			with zipfile.ZipFile(self.filename, 'r') as zip_ref:
			    zip_ref.extractall(checkname.return_filename_without_format(self.filename))
			    zip_ref.close()
	...
	def rmvFiles(self):
		...
		self.master.waitingText['text'] = "Installed " + self.ilename + " with 0 errors"
