...
import pythoncom
from installerapp.uitilities import checkname
...
class InstallerGUI(Frame):
	def __init__(self, root, to_dl):
		...
		if to_dl != "":
			self.installer = Installer(to_dl, self)
		else:
		...
		print(checkname.return_filename(to_dl))
...
class Installer(object):
	...
	def unzip(self):
		try:
			with zipfile.ZipFile("installed.zip", 'r') as zip_ref:
			    zip_ref.extractall("Installed")
			    zip_ref.close()
	...
	def rmvFiles(self):
		os.remove("installed.zip")

		for i in range(26):
			self.master.progressBar['value'] = 75 + i
			time.sleep(0.01)

		self.master.waitingText['text'] = "Installed with 0 errors"
