...
def deconstruct(text: str, filename: str, path: str):
    ...
    if os.path.isdir(path):
        if filename.rpartition('.')[1] and filename.rpartition('.')[2] == 'pickle':
            with open(os.path.join(path, filename), 'wb') as dc:
                pickle.dump(dics, dc)
        elif filename.rpartition('.')[1] and filename.rpartition('.')[2] == 'json':
...
def construct(file: str) -> str:
    ...
    if os.path.isfile(file) and file.rpartition('.')[2] == 'pickle':
        ...
        raise Exception('Either file not exist or file is not .pickle!!!')
    if rd:
        try:
            strng = ''
            for i in list(rd)[::-1]:
...
def textsortchrs(txts: str, alph: list = None):
    ...
    if isinstance(alph, list):
        ...
        if len(maxi) > 2:
            print(f'Char that used most in text: [{", ".join(sorted(repr(i) for i in maxi[:-1]))}], {maxi[-1]} occurences.')
        else:
