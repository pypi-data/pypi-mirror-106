...
from lfl_admin.user_ext.models.persons import Persons, PersonsManager
    ...
    return JsonResponse(
        DSResponse(
            ...
            data=Persons.objects.
                ...
                get_range_rows1(
                request=request,
                function=PersonsManager.getRecord
            ),
...
def Persons_Info(request):
    return JsonResponse(DSResponse(request=request, data=Persons.objects.get_queryset().get_info(request=request), status=RPCResponseConstant.statusSuccess).response)
