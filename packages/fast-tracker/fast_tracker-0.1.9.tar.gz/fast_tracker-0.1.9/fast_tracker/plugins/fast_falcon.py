    ...
    def _fast_falcon_api(this: API, env, start_response):
        try:
            ...
            with context.new_entry_span(op="/", carrier=carrier) as span:
                ...
                http_path = get_url_path(str(req.url).split("?")[0])
                span.op = http_path
                ...
                span.tag(Tag(key=tags.HttpPath, val=http_path))
                if req.params:
            ...
            functions.log("探针falcon插件发生错误，错误信息：%r", str(e))
