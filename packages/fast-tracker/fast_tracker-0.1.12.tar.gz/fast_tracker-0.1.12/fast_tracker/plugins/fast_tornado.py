...
def _gen_fast_get_response_func(old_execute):
    ...
    if awaitable:
        ...
        async def _fast_get_response(self, *args, **kwargs):
            ...
            with context.new_entry_span(op=request.path, carrier=carrier) as span:
                ...
                try:
                    ...
                    pass
                temp_args = request.arguments
                temp_arg = []
                for key, arg in temp_args.items():
                    val = []
                    for i in arg:
                        val.append(i.decode("utf-8"))
                    temp_arg.append(str(key) + "=" + ",".join(val))
                temp_arg = "&".join(temp_arg)
                http_url = (
                    "{}://{}{}?{}".format(request.protocol, request.host, request.path, temp_arg)
                    if temp_arg
                    else "{}://{}{}".format(request.protocol, request.host, request.path)
                )
        ...
        def _fast_get_response(self, *args, **kwargs):
            ...
            carrier = Carrier()
            trace_id_name = config.get_trace_id_name()
            trace_id_name = trace_id_name.replace("_", "-") if "_" in trace_id_name else trace_id_name
            for item in carrier:
                if item.key.capitalize() in request.headers:
                    item.val = request.headers[item.key.capitalize()]
                if item.key.upper() in request.headers:
