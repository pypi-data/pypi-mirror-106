...
from fast_tracker import config, plugins
...
__queue = Queue(maxsize=DEFAULT_QUEUE_SIZE)
__finished = Event()
...
def start():
    ...
    # 打印配置
    functions.log(
        "探针enable配置为：%r, debug配置为：%r， logging_level配置为：%r， debug_level配置为：%r, config_path配置为：%r",
        config.enable,
        config.debug,
