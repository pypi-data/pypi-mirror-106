...
def cmd_2_image (app, cmd2img):
    ...
    if text:
        ...
        if cmd_args[0] != "gnuplot" and cmd_args[0] != "python":
            # Gnuplot/python might read files inline, so convert in current
            # directory and copy the iamge to the destation.
            # Other convert in the destanation directory.
    ...
    if not path.isfile(out["outfullfn"]):
        ...
        if (os.getcwd() == currpath and options.get("image", None)):
            ...
            try:
                ...
                print("mv %s %s" %(options.get("image", outfname),
                        path.join(app.builder.outdir,
                            app.builder.imagedir,
                            outfname)))
        ...
        if options.get("convert", None):
            print("convert %s %s %s"
                    %(out["outfullfn"], options["convert"], out["outfullfn"]))
