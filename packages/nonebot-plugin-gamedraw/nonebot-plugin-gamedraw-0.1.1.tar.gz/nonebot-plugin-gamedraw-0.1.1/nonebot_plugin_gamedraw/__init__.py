...
from .prts_handle import update_prts_info, prts_draw
...
prts_update = on_command('更新明日方舟信息', aliases={'更新方舟信息'}, permission=SUPERUSER, priority=1, block=True)

genshin = on_regex('.*?原神[1-9|一][0-9]{0,2}[抽|井]', priority=5, block=True)
reset_genshin = on_command('重置原神抽卡次数', priority=5, block=True)
genshin_update = on_command('更新原神信息', permission=SUPERUSER, priority=1, block=True)

pretty = on_regex('.*?马娘卡?[1-9|一][0-9]{0,2}[抽|井]', priority=5, block=True)
pretty_update = on_command('更新赛马娘信息', aliases={'更新马娘信息'}, permission=SUPERUSER, priority=1, block=True)
...
@reset_genshin.handle()
async def _(bot: Bot, event: MessageEvent, state: T_State):
    reset_count(event.user_id)
    await reset_genshin.send('重置了原神抽卡次数', at_sender=True)
