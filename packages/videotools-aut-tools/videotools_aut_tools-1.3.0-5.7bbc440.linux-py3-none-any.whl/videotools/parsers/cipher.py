...
from videotools import APP_KEY_FILE, DEFAULT_ENCODING
from videotools.model import ColoredException

logger = logging.getLogger('cipher_cmd')

# -- cypher
_FERNET = None


class NotCipherKeyFoundException(ColoredException):
    """
...
def get_cypher_key(keyfile=APP_KEY_FILE):
    ...
    if not os.path.exists(keyfile):
        raise NotCipherKeyFoundException('Key file not found')
...
def decrypt(**kwargs):
    ...
    """
    logger.debug('decrypting supplied text...')
...
def init_parser(parent_parser):
    cipher_parser = parent_parser.add_parser('cipher', help='Used to encrypt, decrypt or generate a key file')
