...
class DesignSpace:
    ...
    def transform(self, data : pd.DataFrame) -> (Tensor, Tensor):
        ...
        xc = Tensor(xc.astype(float), ms.float32) if xc.shape[1] > 0 else mnp.zeros((xc.shape[0], 0))
        xe = Tensor(xe.astype(int), ms.int32) if xe.shape[1] > 0 else mnp.zeros((xe.shape[0], 0)).astype(ms.int32)
        return xc, xe
    ...
    def inverse_transform(self, x : Tensor, xe : Tensor) -> pd.DataFrame:
        ...
        for i, name in enumerate(self.numeric_names):
            df_num[name] = self.paras[name].inverse_transform(x.asnumpy()[:, i])
        for i, name in enumerate(self.enum_names):
            df_cat[name] = self.paras[name].inverse_transform(xe.asnumpy()[:, i])
