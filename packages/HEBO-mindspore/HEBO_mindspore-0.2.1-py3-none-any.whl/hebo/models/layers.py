...
class EmbTransform(nn.Cell):
    ...
    def construct(self, xe):
        return self.cat([self.emb[i](xe.astype(ms.int32)[:, i]).view(xe.shape[0], -1) for i in range(len(self.emb))])
...
class OneHotTransform(nn.Cell):
    ...
    def one_hot(self, x, num_uniq : int):
        return self.one_hot_op(x, num_uniq, Tensor(1.0, ms.float32), Tensor(0., ms.float32))
    ...
    def construct(self, xe):
        return self.cat([self.one_hot(xe.astype(ms.int32)[:, i], self.num_uniqs[i]) for i in range(xe.shape[1])])
