...
class HEBO(AbstractOptimizer):
    ...
    def __init__(self, space, model_name = 'gpy', rand_sample = None, acq_cls = MACE, es = 'nsga2', verbose = False):
        """
    ...
    def suggest(self, n_suggestions=1, fix_input = None):
        if self.X.shape[0] < self.rand_sample:
            ...
            try:
                if self.y.min() <= 0:
                    y = Tensor(power_transform(self.y / self.y.std(), method = 'yeo-johnson'))
                else:
                    y = Tensor(power_transform(self.y / self.y.std(), method = 'box-cox'))
            ...
            opt = EvolutionOpt(self.space, acq, pop = 100, iters = 100, verbose = self.verbose, es=self.es)
            ...
            if n_suggestions == 1:
                select_id = np.random.choice([select_id[0], best_pred_id, best_unce_id], 1, p = [6/8, 1/8, 1/8]).tolist()
