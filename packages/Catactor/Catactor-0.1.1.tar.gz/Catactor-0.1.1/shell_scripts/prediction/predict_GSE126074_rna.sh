...
Catactor --verbose --scmobj output/scobj/${GSE}_rna_${dist}_${clabel}__all_scanpy_obj_with_feat.pyn \
         ...
         --mdir $mdir --markers $marker_file --cluster cluster,cluster_leiden,cluster_louvain,Ident,celltype \
...
Catactor --top-markers 100 --verbose --scmobj output/scobj/${GSE}_rna_${dist}_${clabel}__all_scanpy_obj_with_feat.pyn \
         --train ${GSE}_rna_train --cluster-ann /data/rkawaguc/data/191003_BICCN_sf_marker_more/cluster_annotation/${GSE}_icluster_celltype_annotation.csv \
         --mdir $mdir --markers $marker_file --cluster cluster,cluster_leiden,cluster_louvain,Ident,celltype \
         --output ${GSE}_rna prediction 
elif [ "$1" == "test" ]; then 
Catactor --top-markers 100 --verbose --scmobj output/scobj/${GSE}_rna_${dist}_${clabel}__all_scanpy_obj_with_feat.pyn \
         --test ${GSE}_rna_test --train ${GSE_train}_${dist}_train --cluster-ann /data/rkawaguc/data/191003_BICCN_sf_marker_more/cluster_annotation/${GSE}_icluster_celltype_annotation.csv \
         --mdir $mdir --markers $marker_file --cluster cluster,cluster_leiden,cluster_louvain,Ident,celltype \
         --output ${GSE}_rna prediction 
else
echo
