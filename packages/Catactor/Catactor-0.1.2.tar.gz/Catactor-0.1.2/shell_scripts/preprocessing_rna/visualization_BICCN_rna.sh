...
Catactor  --rna --pca 50 --tsne-params nn=30,perplexity=50,learning_rate=1000  --gene-name Name --clabel $clabel --cindex global_index --rindex local_index --rlabel '' --verbose \
	--dir /data/rkawaguc/data/190813_meta_scATAC/processed/${GSE} --adir /data/rkawaguc/data/190813_meta_scATAC/processed/${GSE} \
  --row ${GSE}_cell_ng_snSS.csv --skip 2 \
	--column ${GSE}_bin_ng_snSS.csv \
	--dir /data/rkawaguc/data/190813_meta_scATAC/processed/${GSE} --mdir $mdir --markers $marker_file --cluster cluster,cluster_leiden,cluster_louvain,cluster_label.1,subclass_label,celltype \
	--output ${GSE}_rna_${dist} visualization ${GSE}_sparse_mat_snSS.mtx
