...
def cluster_fwe_correct(p, p_threshold):
    ...
    permutation_voxels = np.zeros([100])
    for k in range(100):
        pi = np.copy(p01)
        np.random.shuffle(np.reshape(pi, [px * py * pz]))
    ...
    voxels_threshold = permutation_voxels[int(100*(1-p_threshold))]
    ...
    for i in range(nclusters):
        clusterp[i] = (100 - np.max(np.array(np.where(np.sort(np.append(permutation_voxels, voxelsinluster[i])) == voxelsinluster[i])))) / 100
