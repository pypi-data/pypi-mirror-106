...
class Pilot extends React.Component<IProps, IState> {
    ...
    private previousRateUpdate: any
    private async receiveRateUpdate() {
		...
		ServerConnection.makeRequest(
			...
			if (this.polling) {
				...
				setTimeout(() => this.receiveRateUpdate(), 2000);
			}
            Global.handleResponse(response);
			return response.json();
		}).then((body: any) => {
			if (body) {
				if (JSON.stringify(body) !== JSON.stringify(this.previousRateUpdate)) {
                    this.safeSetState({ rate: body.total });
                    this.previousRateUpdate = body
                }
			}
		});
	}
		...
		return (
			<>
				...
				{this.state.page == Page.Deploy ? (
					...
					<MachinesPage style={{flexGrow: 1}} rate={this.state.rate} cost={this.state.cost} balance={this.state.balance}/>
				) : (
					<></>
				)}
			</>
