...
export class App {
	...
	constructor(path: string, notebook: any = {}, config: OptumiConfig = new OptumiConfig(), uuid: string = "",
		initializing: Update[] = [], preparing: Update[] = [], running: Update[] = [], timestamp = new Date(), runNum: number = 0) {		
		...
		if (this._running.started) {
			if (!this._preparing.completed) {
				this._preparing.addUpdate(new Update("Unable to retrieve preparing updates", ""));
				this._preparing.addUpdate(new Update("stop", ""));
			}
	...
	public static reconstruct(appMap: any): App {
		...
		var app: App = new App(appMap.name, JSON.parse(appMap.notebook), new OptumiConfig(JSON.parse(appMap.nbConfig)), appMap.uuid, initializing, preparing, running, new Date(appMap.timestamp), appMap.runNum);
		// Add modules
		for (let module of appMap.modules) {
            ...
            for (let i = 0; i < module.files.length; i++) {
                if (module.files[i] != '') {
                    files.push({
						last_modified: module.filesmod[i],
			...
			if (mod.modStatus == Status.RUNNING) {
                ...
                if (!app._initializing.completed || !app._preparing.completed || !app._running.completed) {
