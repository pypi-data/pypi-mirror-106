...
export class PopupAppComponent extends React.Component<IProps, IState> {
    ...
    private openSession = () => {
        window.open('http://' + window.location.hostname + ':' + this.props.app.sessionPort + '?token=' + this.props.app.sessionToken, '_blank');
    }

    private openAsNewNotebook = () => {
        const path = this.props.app.path.replace('.ipynb', '-' + (this.props.app.annotationOrRunNum.replace(/\W/g, '')) + '.ipynb');
                            ...
                            <DialogContent style={{
                                ...
                                {this.state.selectedPanel == Page.SUMMARY ? (
                                    ...
                                    <div style={{overflow: 'hidden', width: '100%', height: '100%'}}>
                                        <ScrollableDiv
                                            key='output'
                                            source={Object.assign([], app.modules[0].output)}
                                            autoScroll={true}
                                        />
                                    </div>
								)}
