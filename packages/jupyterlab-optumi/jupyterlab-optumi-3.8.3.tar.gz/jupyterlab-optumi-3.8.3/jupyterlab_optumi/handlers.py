...
class LaunchNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            uuid = data['uuid']
            timestamp = data['timestamp']
            IOLoop.current().run_in_executor(None, launch_notebook, requirementsFile, dataFiles, uuid, timestamp)
...
class PullModuleStatusUpdateHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            response = await IOLoop.current().run_in_executor(None, pull_module_status_updates, workloadUUIDs, moduleUUIDs, lastUpdateLines, lastOutputLines, lastPatches)
            self.set_status(401 if response.geturl().endswith('/login') else response.getcode())
            self.write(response.read())
            if response.getcode() >= 300: jupyter_log.info(optumi_start(self) + str(response))
        except (ConnectionError, URLError) as e:
...
class DeleteFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            response = await IOLoop.current().run_in_executor(None, delete_files, filePaths)
            self.set_status(401 if response.geturl().endswith('/login') else response.getcode())
            self.write(response.read())
            if response.getcode() >= 300: jupyter_log.info(optumi_start(self) + str(response))
        except Exception as e:
...
class UploadFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            IOLoop.current().run_in_executor(None, upload_files, key, fileNames, compress)
            self.write(json.dumps({'message': 'success'}))
        except Exception as e:
            self.set_status(500)
...
class DownloadFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            response = await IOLoop.current().run_in_executor(None, download_files, workloadUUID, moduleUUID, key, files, sizes, overwrite)
...
def launch_notebook(requirementsFile, dataFiles, uuid, timestamp):
...
def upload_cleanup(key, pairs):
    for response, fileName in pairs:
        response.read()
        try:
            lock.acquire()
            uploadProgress[key]['progress'] = -1
...
def upload_files(key, fileNames, compress):
    ...
    try:
        ...
        ## Check last chunk
        for exists, file in zip(json.load(check_if_files_exist(fileNames))['exists'], fileNames):
            if not exists:
                filesToUpload.append((file, os.path.getsize(file)))
...
def pull_module_status_updates(workloadUUIDs, moduleUUIDs, lastUpdateLines, lastOutputLines, lastPatches):
...
def delete_files(filePaths):
    ...
    try:
        ...
        for filePath in filePaths:
            form.add_field('filePaths', str(filePath))
        data = bytes(form)
...
def add_content(content, decoded):
    ...
    try:
        lock.acquire()
        downloadProgress[key]['progress'] = downloadProgress[key]['progress'] + len(encoded)
    except:
        pass
    finally:
        lock.release()

def download_files_helper(key, form, fileNames, overwrite):
    ...
    while True:
        ...
        if ',' in decoded:
            ...
            for chunk in split[:-1]:
                ...
                save_file(fileNames.pop(0), content, overwrite)
                content = io.BytesIO()
...
def download_files(workloadUUID, moduleUUID, key, files, sizes, overwrite):
    try:
        total = sum(sizes)
        try:
            lock.acquire()
        ...
        filesToDownload = zip(files, sizes)
        # Sort by file size
        filesToDownload = sorted(filesToDownload, key=lambda x:x[1], reverse=True)
        ...
        for file, size in filesToDownload:
            if size > MAX_SIZE:
                ...
                download_files_helper(key, form, fileNames, overwrite)
