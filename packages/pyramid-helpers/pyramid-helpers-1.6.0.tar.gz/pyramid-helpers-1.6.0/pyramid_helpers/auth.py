    ...
    def __init__(self, policies, hashalg='sha512', realm='Pyramid Helpers', secret=None):
        ...
        for policy in policies:
            if policy not in self.POLICIES:
                raise Exception('[AUTH] Invalid policy: {0}'.format(policy))
...
def parse_settings(settings, include_defaults=True):
    ...
    if func is None:
        raise Exception('[AUTH] Invalid value for parameter get_principals in section [auth]: {0}'.format(settings['auth']['get_principals']))
