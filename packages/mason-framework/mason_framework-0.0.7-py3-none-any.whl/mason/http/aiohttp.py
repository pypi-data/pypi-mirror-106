...
async def get_library(request: web.Request) -> web.Response:
    ...
    del request  # Unused.
    return web.json_response(mason.io.dump_library())
...
async def list_storage(request: web.Request) -> web.Response:
    ...
    return web.json_response([item.asdict() for item in items])
...
async def get_blueprint(request: web.Request) -> web.Response:
    ...
    return web.json_response({'blueprint': data.pop(''), 'extras': data})
