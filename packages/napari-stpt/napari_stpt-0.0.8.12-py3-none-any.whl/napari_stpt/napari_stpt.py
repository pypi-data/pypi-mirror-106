...
class NapariSTPT:
    ...
    def Load(self, text):
        ...
        with napari.gui_qt():
            if self.cb_C1.isChecked():
                ...
                if self.crop:
                    size_y = int(math.floor(volume_1_temp.shape[1]/self.crop_size_ratio_x))
                    size_z = int(math.floor(volume_1_temp.shape[2]/self.crop_size_ratio_y))
                    start_y = int(math.floor(volume_1_temp.shape[1]/self.crop_start_ratio_x))
                    start_z = int(math.floor(volume_1_temp.shape[2]/self.crop_start_ratio_y))
                else:
                ...
                volume_1 = np.zeros((self.optical_slices*volume_1_temp.shape[0],size_y,size_z), dtype=np.float32)
                volume_1[0::self.optical_slices,:,:] = volume_1_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]
                ...
                for optical_slice in range(1, self.optical_slices):
                    ...
                    volume_1[optical_slice::self.optical_slices,:,:] = volume_1_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]
            ...
            if self.cb_C2.isChecked():
                ...
                volume_2_temp = (ds.sel(channel=2, type='mosaic', z=0).to_array().data * bscale + bzero).astype(dtype=np.float32)

                if self.crop:
                    size_y = int(math.floor(volume_2_temp.shape[1]/self.crop_size_ratio_x))
                    size_z = int(math.floor(volume_2_temp.shape[2]/self.crop_size_ratio_y))
                    start_y = int(math.floor(volume_2_temp.shape[1]/self.crop_start_ratio_x))
                    start_z = int(math.floor(volume_2_temp.shape[2]/self.crop_start_ratio_y))
                else:
                ...
                volume_2 = np.zeros((self.optical_slices*volume_2_temp.shape[0],size_y,size_z), dtype=np.float32)
                volume_2[0::self.optical_slices,:,:] = volume_2_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]

                for optical_slice in range(1, self.optical_slices):
                    print(f"loading optical slice {optical_slice}")
                    volume_2_temp = (ds.sel(channel=2, type='mosaic', z=optical_slice).to_array().data * bscale + bzero).astype(dtype=np.float32)
                    volume_2[optical_slice::self.optical_slices,:,:] = volume_2_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]
            ...
            if self.cb_C3.isChecked():
                ...
                if self.crop:
                    size_y = int(math.floor(volume_3_temp.shape[1]/self.crop_size_ratio_x))
                    size_z = int(math.floor(volume_3_temp.shape[2]/self.crop_size_ratio_y))
                    start_y = int(math.floor(volume_3_temp.shape[1]/self.crop_start_ratio_x))
                    start_z = int(math.floor(volume_3_temp.shape[2]/self.crop_start_ratio_y))
                else:
                ...
                volume_3 = np.zeros((self.optical_slices*volume_3_temp.shape[0],size_y,size_z), dtype=np.float32)
                volume_3[0::self.optical_slices,:,:] = volume_3_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]

                for optical_slice in range(1, self.optical_slices):
                    print(f"loading optical slice {optical_slice}")
                    volume_3_temp = (ds.sel(channel=3, type='mosaic', z=optical_slice).to_array().data * bscale + bzero).astype(dtype=np.float32)
                    volume_3[optical_slice::self.optical_slices,:,:] = volume_3_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]
            ...
            if self.cb_C4.isChecked():
                print("loading C4")
                volume_4_temp = (ds.sel(channel=4, type='mosaic', z=0).to_array().data * bscale + bzero).astype(dtype=np.float32)

                if self.crop:
                    size_y = int(math.floor(volume_4_temp.shape[1]/self.crop_size_ratio_x))
                    size_z = int(math.floor(volume_4_temp.shape[2]/self.crop_size_ratio_y))
                    start_y = int(math.floor(volume_4_temp.shape[1]/self.crop_start_ratio_x))
                    start_z = int(math.floor(volume_4_temp.shape[2]/self.crop_start_ratio_y))
                else:
                ...
                volume_4 = np.zeros((self.optical_slices*volume_4_temp.shape[0],size_y,size_z), dtype=np.float32)
                volume_4[0::self.optical_slices,:,:] = volume_4_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]

                for optical_slice in range(1, self.optical_slices):
                    print(f"loading optical slice {optical_slice}")
                    volume_4_temp = (ds.sel(channel=4, type='mosaic', z=optical_slice).to_array().data * bscale + bzero).astype(dtype=np.float32)
                    volume_4[optical_slice::self.optical_slices,:,:] = volume_4_temp[:,start_y:start_y+size_y,start_z:start_z+size_z]
    ...
    def LoadInRegion(self, text):
        ...
        l = len(self.viewer.layers.selected[0].data[0])
        print(l)
        minX = 100000000
        maxX = 0
        minY = 100000000
    ...
    def CropToRegion(self):
        ...
        if self.aligned_1 is not None and self.cb_C1.isChecked() and any(i.name == 'C1' for i in self.viewer.layers):
            self.aligned_1 = self.aligned_1[:, int(minX):int(maxX), int(minY):int(maxY)]
            self.viewer.layers.remove('C1')
            self.viewer.add_image([self.aligned_1], name='C1', scale=(
        ...
        if self.aligned_2 is not None and self.cb_C2.isChecked() and any(i.name == 'C2' for i in self.viewer.layers):
            self.aligned_2 = self.aligned_2[:, int(minX):int(maxX), int(minY):int(maxY)]
            self.viewer.layers.remove('C2')
            self.viewer.add_image([self.aligned_2], name='C2', scale=(
        ...
        if self.aligned_3 is not None and self.cb_C3.isChecked() and any(i.name == 'C3' for i in self.viewer.layers):
            self.aligned_3 = self.aligned_3[:, int(minX):int(maxX), int(minY):int(maxY)]
            self.viewer.layers.remove('C3')
            self.viewer.add_image([self.aligned_3], name='C3', scale=(
        ...
        if self.aligned_4 is not None and self.cb_C4.isChecked() and any(i.name == 'C4' for i in self.viewer.layers):
            self.aligned_4 = self.aligned_4[:, int(minX):int(maxX), int(minY):int(maxY)]
            self.viewer.layers.remove('C4')
            self.viewer.add_image([self.aligned_4], name='C4', scale=(
    ...
    def run_stardist(self):
        ...
        #return
        # normalize image
        im12 = self.ds1[slice_name].sel( channel=2, type='mosaic', z=0).data * bscale + bzero
        im12 = np.array(im12[int(minX):int(maxX), int(minY):int(maxY)])
        im13 = self.ds1[slice_name].sel( channel=3, type='mosaic', z=0).data * bscale + bzero
        ...
        #with napari.gui_qt():
        self.viewer.add_image(img, name='img', blending='additive', translate=(int(minX),int(minY)))

        self.viewer.add_image(im12, name='im12', blending='additive', translate=(int(minX),int(minY)))
        self.viewer.add_image(im13, name='im13', blending='additive', translate=(int(minX),int(minY)))
        self.viewer.add_labels(labels, name='labels', blending='additive', translate=(int(minX),int(minY)))
        self.viewer.add_points(details['points'], translate=(int(minX),int(minY)))

    def SaveMovie(self):
        
        widget = QtWidgets.QWidget()
        fname = QtWidgets.QFileDialog.getSaveFileName(widget, 'Save file as', 'c:\\',"Image files (*.gif *.mp4)")
    ...
    def SaveVolume(self):
        widget = QtWidgets.QWidget()
        fname = QtWidgets.QFileDialog.getSaveFileName(widget, 'Save file as', 'c:\\',"Image files (*.tiff *.mha)")

        if isinstance(fname,tuple):
            print(fname[0])

            output_resolution = float(self.pixel_size.text())
            spacing = (output_resolution, output_resolution, 15/self.optical_slices)

            #volume_2 = 5000 * (volume_2 / np.max(volume_2))
            #volume_2 = np.clip(volume_2, 0, 255)
            #volume_2 = np.round(volume_2)
            #volume_2 = volume_2.astype(np.ubyte)

            #volume_3 = 5000 * (volume_3 / np.max(volume_3))
            #volume_3 = np.clip(volume_3, 0, 255)
            #volume_3 = np.round(volume_3)
            #volume_3 = volume_3.astype(np.ubyte)

            #volume_4 = np.zeros_like(volume_2)
            #volume = np.stack((volume_2, volume_3), axis=3)
            #volume = np.concatenate((volume, np.expand_dims(volume_4, axis=3)), axis=3)

            #rgbImage_itk = sitk.GetImageFromArray(volume, isVector=True)
            #rgbImage_itk.SetSpacing(spacing)
            #sitk.WriteImage(rgbImage_itk, "S:/Tristan/RGB.tif")

            
            #aligned_4 = np.zeros_like(self.aligned_2)
            #volume = np.stack((self.aligned_2, self.aligned_3), axis=3)
            #volume = np.concatenate((volume, np.expand_dims(aligned_4, axis=3)), axis=3)


            #if self.aligned_1 != None:
            #    volume = np.zeros_like(self.aligned_1)
            #elif self.aligned_2 != None:
            #    volume = np.zeros_like(self.aligned_2)
            #elif self.aligned_2 != None:
           #     volume = np.zeros_like(self.aligned_2)
            #elif self.aligned_3 != None:
            #    volume = np.zeros_like(self.aligned_3)
    ...
    def main(self):
        ...
        with napari.gui_qt():
            ...
            hbox = QtWidgets.QHBoxLayout()
            hbox.addWidget(QtWidgets.QLabel("Maximum number of optical slices:"))
            ...
            #hbox = QtWidgets.QHBoxLayout()
            bLoad3D = QtWidgets.QPushButton('Reload image 3D in region')
            bLoad3D.setCheckable(True)
            ...
            bStardist = QtWidgets.QPushButton('Run Stardist')
