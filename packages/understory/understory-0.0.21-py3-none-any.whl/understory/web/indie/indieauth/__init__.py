...
def add_owner(name):
    ...
    tx.pub.create("feed", uid="default", name="All Posts", slug="all")
...
def wrap_server(handler, app):
    ...
    tx.db.define(auths="""auth_id TEXT, initiated DATETIME NOT NULL DEFAULT
                          ...
                          redirect_uri TEXT, response JSON, token TEXT""")
    try:
        tx.user.is_owner = tx.user.session["uid"] == tx.host.owner["uid"][0]
    except (KeyError, IndexError):
        tx.user.is_owner = False
    passthrough = ("auth", "auth/sign-in", "auth/sign-ins/token",
                   "auth/visitors/sign-in")
    if tx.request.uri.path.startswith(("auth", "pub", "sub")) and \
       tx.request.uri.path not in passthrough and \
       not tx.user.is_owner and \
       not tx.request.headers.get("Authorization"):  # TODO validate token
    ...
    def post(self):
        ...
        if scope and self.is_token_request(scope):
            response.update(access_token=web.nbrandom(12),
                            token_type="Bearer")
            response["me"] = (f"{tx.request.uri.scheme}://"
                              f"{tx.request.uri.netloc}")
