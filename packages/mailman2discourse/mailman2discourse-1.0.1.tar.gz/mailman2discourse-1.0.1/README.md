...
* echo $(cat v2.7.0/ip) forumv2.7.0.example.com | sudo tee -a /etc/hosts
* firefox http://forumv2.7.0.example.com # user: api, password: BefShnygs33SwowCifViwag
* tox
* PYTEST_ADDOPTS="--color=no" ../mailman2discourse-virtualenv/bin/tox -e py3 -- --discourse v2.7.0 -k test_category_set tests/test_discourse.py
* When discourses fails with "Internal Server Error", details about the error can be found in http://forumv2.7.0.example.com/logs/
* When the sidekiq queue grows too much http://forumv2.7.0.example.com/sidekiq shows which jobs are waiting
...
release (for instance v2.7.0, the following files must be updated)
...
* tests/build-discourse figure out which commit hash of discourse_docker matches
  a given version and add it using v2.7.0 as a model
