...
def main(_args: List[str] = None) -> Any:
    ...
    if ("-g" in sys.argv) or ("--gen_batch" in sys.argv):
        input_file: str = (
            INPUT_TXT_DEFAULT if (args.gen_batch is None) else args.gen_batch
        )
        logger.info('Generate batch file mode called with arg "{}"'.format(input_file))

        csv_files_in_curr_dir = [
            f for f in os.listdir(os.curdir) if utils.isValidCSV(f)
        ]
        console.print(f'Generating files from "{Path(os.curdir).resolve()}"')
        i = 1
