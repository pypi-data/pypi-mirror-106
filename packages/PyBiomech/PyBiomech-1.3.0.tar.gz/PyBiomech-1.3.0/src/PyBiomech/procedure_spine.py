...
def performSpineAnalysis2(
    ...
    markersNew['L4'] = 0.5 * (markers['RL4'] + markers['LL4'])
    ...
    angleT12ToSACRToAntAxisSag = np.zeros((Nf,))
    angleT12ToL4ToLatAxisFro = np.zeros((Nf,))
    ...
    for i in iRange:
        ...
        tmp = pointsDataSag[idxs['idx_T12'],:] - pointsDataSag[idxs['idx_SACR'],:]
        angleT12ToSACRToAntAxisSag[i] = np.rad2deg(np.arctan2(tmp[1], tmp[0]))
        
        tmp = pointsDataSag[idxs['idx_T12'],:] - pointsDataSag[idxs['idx_L4'],:]
        angleT12ToL4ToLatAxisFro[i] = np.rad2deg(np.arctan2(tmp[1], tmp[0]))
    ...
    res['newMarkers'] = markersNew
    res['spineAngles']['T12ToSACRToAntAxisSag'] = angleT12ToSACRToAntAxisSag
    res['spineAngles']['T12ToL4ToLatAxisFro'] = angleT12ToL4ToLatAxisFro
