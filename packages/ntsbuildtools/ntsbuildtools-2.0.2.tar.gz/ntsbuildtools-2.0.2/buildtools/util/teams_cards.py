...
class PullRequestCard(TeamsCardTemplate):
    ...
    def build(self, args):
        ...
        if util.hasattr_nonempty_str(args, 'build_status'):  # build_status is optional
        ...
        self.add_button(f"See proposed changes!", pull_request_diff_url)
...
class TeamsCardFactory:
    ...
    def _add_message(args, card, message):
        ...
        trimmed = False
        if args.max_size:
            fragments = util.split_markdown(message, args.max_size//2)
            if len(fragments) >= 2:
                trimmed = True
                message = ''
                i = -1
