...
class PostTeamsCard(BuildToolsTemplate):
    ...
    def config_parser(self, parser):
        BUILD_STATUSES = ['STARTED', 'SUCCESS', 'UNSTABLE', 'FAILURE', 'NOT_BUILD', 'ABORTED']
        ...
        parent_parser.add('--message', help='Custom messgage to be printed at the bottom of the card.')
        parent_parser.add('--file', help='Custom message to be printed at the bottom of the card, loaded from the '
        ...
        parent_parser.add('--max-size', '--tail', dest='max_size', type=int, default=21000,  # 25kb is the max, so this leaves around => 4kb for all our meta-data...
            help='''Only print the last `max-comment-size` bytes of the provided message/file (i.e. this has a `tail` 
                 'behavior by default, to help ensure that the message fits within the Teams message).''')
        ...
        sp.add('status', choices=BUILD_STATUSES,
        ...
        sp.add('--playbook-limit', env_var='PLAYBOOK_LIMIT',
            help='A string indicating the PLAYBOOK_LIMIT for this build.')
        sp.add('--build-status', choices=BUILD_STATUSES,
