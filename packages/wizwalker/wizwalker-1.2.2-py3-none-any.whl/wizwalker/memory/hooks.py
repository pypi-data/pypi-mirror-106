...
class User32GetClassInfoBaseHook(AutoBotBaseHook):
    ...
    async def alloc(self, size: int) -> int:
        if self._autobot_addr is None:
            addrs = await self.pattern_scan(
                self.AUTOBOT_PATTERN, module="user32.dll", return_multiple=True
            )
            # GetClassInfoExA and GetClassInfoExW are too similar (only data differences) so just select one of them
            addr = addrs[0]
