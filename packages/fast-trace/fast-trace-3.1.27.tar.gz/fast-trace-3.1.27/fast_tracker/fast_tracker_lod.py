...
class FastTracker:
    ...
    def __return_false_msg(self, msg: str = "", *args):
        functions.log(msg, args)
        # 发生异常则关闭span
        self.span.stop()
        return False
    ...
    def set_component(self, name: str = "CustomEvent"):
        ...
        if type(name) is not str:
            return self.__return_false_msg("传入参数类型不匹配，只允许传入str类型参数")
        if not name:
            return self.__return_false_msg("name为空")
        if "custom_event" in config.disable_plugins:
            return self.__return_false_msg("已在配置Event-Components-CustomEvent中关闭此项")
    ...
    def add_tag(self, key: str = "", val: str = ""):
        ...
        if not key or not val:
            return self.__return_false_msg("key或val为空")
        if "custom_event" in config.disable_plugins:
            return self.__return_false_msg("已在配置Event-Components-CustomEvent中关闭此项")
        ...
        try:
            ...
            return self.__return_false_msg("调用add_tag时发生错误，错误信息:%r", str(e))
    ...
    def add_log(self, message: str = ""):
        ...
        if type(message) is not str:
            return self.__return_false_msg("传入参数类型不匹配，只允许传入str类型参数")
        if not message:
            return self.__return_false_msg("message为空")
        if "custom_event" in config.disable_plugins:
            return self.__return_false_msg("已在配置Event-Components-CustomEvent中关闭此项")
        ...
        try:
            ...
            return self.__return_false_msg("调用add_log时发生错误，错误信息:%r", str(e))
    ...
    def error_occurred(self, error: Exception):
        if not self.span:
            ...
            return False
        if not error:
            self.__return_false_msg("参数为空")
        if type(error) is not Exception:
            self.__return_false_msg("传入参数类型不匹配，只允许传入Exception类型参数")
        if "custom_event" in config.disable_plugins:
            self.__return_false_msg("已在配置Event-Components-CustomEvent中关闭此项")

        try:
            self.span.log(error)
        except Exception as e:
            return self.__return_false_msg("调用error_occurred时发生错误，错误信息:%r", str(e))
