...
def set_event(cus_event: dict):
    ...
    if event.get("Components"):
        ...
        if component.get("SqlClient") in ("false", "False", "0", False, 0, -1):
            cus_disable_plugins.extend(["fast_pymysql"])
        if component.get("Datastore") in ("false", "False", "0", False, 0, -1):
            cus_disable_plugins.extend(["fast_pymongo", "fast_elasticsearch", "fast_redis"])
