...
class RsCmwCdma2kSig:
	"""331 total commands, 8 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.10.0024'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwCdma2kSig._driver_options, options, direct_session)
		self._core.driver_version = '3.8.10.0024'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwCdma2kSig':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwCdma2kSig session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwCdma2kSig with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.10.0024'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwCdma2kSig version failed. Current version: '3.8.10.0024', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 20 Sub-classes, 3 commands."""
	...
	@property
	def sense(self):
		"""sense commands group. 5 Sub-classes, 1 commands."""
	...
	@property
	def soption(self):
		"""soption commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_soption'):
			from .Implementations.Soption import Soption
	...
	@property
	def rxQuality(self):
		"""rxQuality commands group. 5 Sub-classes, 0 commands."""
	...
	def clone(self) -> 'RsCmwCdma2kSig':
		"""Creates a deep copy of the RsCmwCdma2kSig object. Also copies:
		...
		cloned = RsCmwCdma2kSig.from_existing_session(self.get_session_handle(), self._options)
