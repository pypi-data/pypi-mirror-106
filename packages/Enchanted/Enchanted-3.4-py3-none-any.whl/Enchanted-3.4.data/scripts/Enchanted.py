...
\u001b[35;1m┗━(Go to https://enchantedsniper.ga/ for more info)
    ...
    async def gather_mojang_info() -> None:
        ...
        if "yes" in yesorno43:
            ...
            if "yes" in yesorno55:
                payload2 = input(f"{inputxd} url link : ")
                payload23 = input(f"{inputxd} account info : ")
                os.system(f"rentry.py new -p random -u {payload2} {payload23}")
                url2 = input(f"{inputxd} account info url : ")
            if "no" in yesorno55:
                url2 = input(f"{inputxd} account info url : ")
                url2 = os.system(f"rentry.py raw -u {url2}")
                print(url2)
        if "no" in yesorno43:
            pass
