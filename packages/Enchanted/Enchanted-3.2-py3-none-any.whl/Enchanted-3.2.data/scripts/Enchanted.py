...
if platform.system() == "Linux":
    print("sorry this script is only for windows!")
...
\u001b[35;1m┗━(Go to https://enchantedsniper.ga/ for more info)
        ...
        while True:
            ...
            payload = json.dumps({
                ...
                "embeds": [
                    ...
                    "title": "New Snipe Try",
                    "color": 16723694,
                    "description": f"@{discordusernames} is trying to snipe : **`{username}`**",
                    "timestamp": "",
                    "author": {
                        "name": "Enchanted Sniper"
                    },
                    "image": {},
                    "thumbnail": {},
                    "footer": {
                        "text": "Send by Enchanted Sniper Bot",
                        "icon_url": ""
                    },
                    "fields": [
                        {
                        "name": "NameMC",
                        "value": f"[click me for link!](https://en.namemc.com/search?q={username})",
                        "inline": True
                        },
                        {
                        "name": "MC Api",
                        "value": f"[click me for link!](https://mc-heads.net/minecraft/profile/{username})",
                        "inline": True
                        },
                        {
                        "name": f"How can i snipe `{username}`?",
                        "value": "use Enchanted Sniper today! https://enchantedsniper.ga/"
                        }
                    ]
                    }
                ]
            ...
            'Cookie': '__dcfduid=711b754b0be143ad882de1e3a02a0ed8'
            }
        ...
        async with s.post(
            ...
            if await r.status()==200:
                ...
                "embeds": [
                    ...
                    "title": "New Snipe!",
                    "color": 13385408,
                    "description": f"{discordusernames} Sniped : **`{username}`**!",
                    "timestamp": "",
                    "author": {
                        "name": "Enchanted Sniper",
                        "icon_url": ""
    ...
    async def gather_mojang_info() -> None:
        with open("pass.txt", "r") as xDf:
            print(f"{info} your latest account info")
            print(f"{info}{Fore.GREEN}" + xDf.read())
        email = input(f"{inputxd} Account Email : ")
        password = getpass(f"{inputxd} Password : ")
        with open("pass.txt", "w") as Xdf2:
            Xdf2.write(f" Email = {email} {Fore.WHITE}--{Fore.GREEN} pass = {password}")
