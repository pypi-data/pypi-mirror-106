...
class SQSMixin(object):
    ...
    def setup_sqs(self, crawler=None):
        ...
        if not self.sqs_queue_url and not self.sqs_queue_url:
        ...
        self.logger.info("Reading start URLs from sqs queue %(sqs_queue_url) %(sqs_queue_name)", __dict__)
...
class SQSSpider(SQSMixin, Spider):
    ...
    CONCURRENT_REQUESTS : int
        Default number of messages to fetch from sqs on each attempt.
