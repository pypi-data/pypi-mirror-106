"""
 * $Id: 1.0+16 utils.py 52dfe2315c83 2021-05-18 od $
 *
...
class Client(object):
    ...
    def __init__(self, data: Dict = None, metainfo: Dict = None,
                 ...
                 descr: str = '', http_status: int = 0, conf: Dict = None):
    ...
    def __get(url: str, section: str = PARAMETER, conf: Dict = None) -> "Client":
        ...
        return Client(data, metainfo=mi, http_status=http_status, conf=conf)

    @staticmethod
    def __get0(url: str, conf: Dict):
        """HTTP GET to generate a Client"""
        conf = conf or {}
        to = conf.get('get_timeout', 4.0)
        retry = conf.get('retry', 3)
        allow_redirects = conf.get('allow_redirects', True)
        http_status = 200
        response = None
        while retry > 0:
            try:
                response = requests.get(url, allow_redirects=allow_redirects, timeout=to)
    ...
    @staticmethod
    def __post0(url: str, files: Dict, headers: Dict, conf: Dict):
        """HTTP POST to generate a Client"""
        conf = conf or {}
        to = conf.get('post_timeout', 10.0)
        retry = conf.get('retry', 3)
        allow_redirects = conf.get('allow_redirects', True)
        http_status = 200
        response_json = None
    ...
    def get_catalog(url: str, conf: Dict = None) -> List['Client']:
        ...
        response_json = response.json()
        return [Client(metainfo=service, conf=conf) for service in response_json]
    ...
    def execute(self, url: str = None, sync: bool = True, files: List[str] = None, headers: Dict = None) -> 'Client':
        ...
        response_json, http_status = Client.__post0(url, multipart, headers, self.conf)
        ...
        return Client(data, parent=self, metainfo=mi, http_status=http_status, conf=self.conf)

    def execute_async(self, url: str = None, files: List[str] = None,
                      first_poll: int = None, next_poll: int = None,
                      callback=None, headers: Dict = None) -> 'Client':
        ...
        # -> Submitted
        resp = self.execute(url, files=files, sync=False, headers=headers)
        callback(resp, "")
    ...
    def query_execute(self, suid: str = None) -> 'Client':
        ...
        # print(query_url)
        c = Client.__get(query_url, section=self.RESULT, conf=self.conf)
    ...
    def download_data_files(self, files: List[str] = None, dir: str = ".") -> None:
        ...
        for name in files:
            ...
            val = self.get_data_value(name)
            print("  --> " + name)
            response = Client.__get0(val, self.conf)
            open(os.path.join(dir, name), "wb").write(response.content)
