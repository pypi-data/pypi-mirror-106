"""
 * $Id: 1.1+3 utils.py e48fa1383c14 2021-05-19 od $
 *
...
class Client(object):
    ...
    # CONFIG defaults
    GET_TIMEOUT = (2,10) # (connect, read)
    POST_TIMEOUT = (2,30) # (connect, read)
    RETRY = 3
    ...
    def __get0(url: str, conf: Dict):
        """HTTP GET to generate a Client"""
        conf = conf or {}
        timeout = conf.get('get_timeout', Client.GET_TIMEOUT)
        retry = conf.get('retry', Client.RETRY)
        allow_redirects = conf.get('allow_redirects', Client.ALLOW_REDIRECTS)
        http_status = 200
        response = None
        while retry > 0:
            try:
    ...
    def __post0(url: str, files: Dict, headers: Dict, conf: Dict):
        ...
        conf = conf or {}
        timeout = conf.get('post_timeout', Client.POST_TIMEOUT)
        retry = conf.get('retry', Client.RETRY)
        allow_redirects = conf.get('allow_redirects', Client.ALLOW_REDIRECTS)
        http_status = 200
        response_json = None
    ...
    def execute(self, url: str = None, sync: bool = True,
        ...
        response_json, http_status = Client.__post0(url, multipart, headers, conf)
