...
def emcee_run (ndim,nwalkers,starts,sigmas,nburnt,niters,lnprob,args):

    pos = emcee.utils.sample_ball(starts,sigmas,nwalkers)

    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=args)

    npos = sampler.run_mcmc(pos, nburnt)
    sampler.reset()
    sampler.run_mcmc(npos, niters)

    samples = sampler.chain[:, :, :].reshape((-1,ndim))

    meanaccfract = np.mean(sampler.acceptance_fraction)

    return {"sampler": sampler, "meanacceptancefraction": meanaccfract, "samples": samples}
