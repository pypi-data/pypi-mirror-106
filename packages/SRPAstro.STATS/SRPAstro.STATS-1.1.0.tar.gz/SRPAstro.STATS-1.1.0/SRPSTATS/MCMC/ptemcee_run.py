...
def ptemcee_run (ndim,nwalkers,ntemps,starts,sigmas,niters,lnlike,lnprob,llargs,lpargs):
    ...
    samples = sampler.chain[:, :, int(4*atime):, :].reshape((-1,ndim))
    
    logevidence = sampler.log_evidence_estimate(fburnin=4*atime/niters)
