...
class LoadReplayDialog(SimpleDialog):
    ...
    def ok(self, replay_id):
        logger.info(f"Loading replay from id: {replay_id}")
...
class NewReplayDialog(Dialog):
    ...
    def ok(self):
        ...
        logger.info(f"Creating new replay with level id {level_id} and character {character}")
        self.app.file = None
...
class App(tk.Tk):
    ...
    def watch(self):
        logger.info("Watching replay")
        if self.save_file():
            dustforce.watch_replay(self.file)

    def load_state_and_watch(self):
        logger.info("Loading state and watching replay")
        if self.save_file():
    ...
    def set_dustforce_directory(self):
        ...
        if new_path:
            logger.info(f"Dustforce directory set to: {new_path}")
