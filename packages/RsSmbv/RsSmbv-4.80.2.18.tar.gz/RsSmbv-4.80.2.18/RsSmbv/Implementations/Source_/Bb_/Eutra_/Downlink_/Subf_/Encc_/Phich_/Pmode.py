...
class Pmode:
	...
	def set(self, power_mode: enums.EutraPhichPwrMode, subframeNull=repcap.SubframeNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST0>]:ENCC:PHICh:PMODe \n
		Snippet: driver.source.bb.eutra.downlink.subf.encc.phich.pmode.set(power_mode = enums.EutraPhichPwrMode.CONSt, subframeNull = repcap.SubframeNull.Default) \n
	...
	def get(self, subframeNull=repcap.SubframeNull.Default) -> enums.EutraPhichPwrMode:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST0>]:ENCC:PHICh:PMODe \n
		Snippet: value: enums.EutraPhichPwrMode = driver.source.bb.eutra.downlink.subf.encc.phich.pmode.get(subframeNull = repcap.SubframeNull.Default) \n
		Determines whether the PHICHs in a PHICH group are sent with the same power or enables the adjustment of each PPHICH
		individually. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
