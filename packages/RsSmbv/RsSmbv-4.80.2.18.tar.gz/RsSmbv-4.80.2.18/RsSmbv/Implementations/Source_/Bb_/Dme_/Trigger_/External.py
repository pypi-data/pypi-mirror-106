...
class External:
	"""External commands group definition. 3 total commands, 3 Sub-groups, 0 group commands
	Repeated Capability: External, default value after init: External.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("external", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_external_get', 'repcap_external_set', repcap.External.Nr1)

	def repcap_external_set(self, enum_value: repcap.External) -> None:
	...
	def delay(self):
		...
		if not hasattr(self, '_delay'):
			from .External_.Delay import Delay
	...
	def inhibit(self):
		...
		if not hasattr(self, '_inhibit'):
			from .External_.Inhibit import Inhibit
			self._inhibit = Inhibit(self._core, self._base)
