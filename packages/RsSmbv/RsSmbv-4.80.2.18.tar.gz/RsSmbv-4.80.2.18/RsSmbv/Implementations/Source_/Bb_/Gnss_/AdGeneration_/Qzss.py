...
class Qzss:
	"""Qzss commands group definition. 16 total commands, 5 Sub-groups, 0 group commands"""
	...
	def coordinates(self):
		...
		if not hasattr(self, '_coordinates'):
			from .Qzss_.Coordinates import Coordinates
	...
	def location(self):
		...
		if not hasattr(self, '_location'):
			from .Qzss_.Location import Location
	...
	def synchronize(self):
		...
		if not hasattr(self, '_synchronize'):
			from .Qzss_.Synchronize import Synchronize
	...
	def toaData(self):
		...
		if not hasattr(self, '_toaData'):
			from .Qzss_.ToaData import ToaData
