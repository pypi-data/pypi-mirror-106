...
class Delay:
	...
	def set(self, delay: int, output=repcap.Output.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:DM:TRIGger:OUTPut<CH>:DELay \n
		Snippet: driver.source.bb.dm.trigger.output.delay.set(delay = 1, output = repcap.Output.Default) \n
	...
	def get(self, output=repcap.Output.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:DM:TRIGger:OUTPut<CH>:DELay \n
		Snippet: value: int = driver.source.bb.dm.trigger.output.delay.get(output = repcap.Output.Default) \n
		Defines the delay between the signal on the marker outputs and the start of the signal, expressed in terms of symbols. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
