...
class Cblock:
	...
	def __init__(self, core: Core, parent):
		...
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_commandBlock_get', 'repcap_commandBlock_set', repcap.CommandBlock.Nr1)

	def repcap_commandBlock_set(self, enum_value: repcap.CommandBlock) -> None:
	...
	def aupdate(self):
		...
		if not hasattr(self, '_aupdate'):
			from .Cblock_.Aupdate import Aupdate
			self._aupdate = Aupdate(self._core, self._base)
	...
	def bupdate(self):
		...
		if not hasattr(self, '_bupdate'):
			from .Cblock_.Bupdate import Bupdate
			self._bupdate = Bupdate(self._core, self._base)
	...
	def copy_with_opc(self, commandBlock=repcap.CommandBlock.Default) -> None:
		commandBlock_cmd_val = self._base.get_repcap_cmd_value(commandBlock, repcap.CommandBlock)
		"""SCPI: [SOURce<HW>]:BB:NFC:CBLock<CH>:COPY \n
		...
		Same as copy, but waits for the operation to complete before continuing further. Use the RsSmbv.utilities.opc_timeout_set() to set the timeout value. \n
			:param commandBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cblock')"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:NFC:CBLock{commandBlock_cmd_val}:COPY')
	...
	def delete_with_opc(self, commandBlock=repcap.CommandBlock.Default) -> None:
		commandBlock_cmd_val = self._base.get_repcap_cmd_value(commandBlock, repcap.CommandBlock)
		"""SCPI: [SOURce<HW>]:BB:NFC:CBLock<CH>:DELete \n
		...
		Same as delete, but waits for the operation to complete before continuing further. Use the RsSmbv.utilities.opc_timeout_set() to set the timeout value. \n
			:param commandBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cblock')"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:NFC:CBLock{commandBlock_cmd_val}:DELete')
