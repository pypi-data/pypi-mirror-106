...
class RsSmbv:
	"""8799 total commands, 26 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMW/SMBV/SMCV, SupportedIdnPatterns = SMW/SMBV/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMBV100B,100001,4.80.2.0018'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsSmbv._driver_options, options, direct_session)
		self._core.driver_version = '4.80.2.0018'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.80.2.0018'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmbv version failed. Current version: '4.80.2.0018', minimum required version: '{min_version}'")
