...
class Subf:
	"""Subf commands group definition. 131 total commands, 5 Sub-groups, 0 group commands
	...
	def alCount(self):
		...
		if not hasattr(self, '_alCount'):
			from .Subf_.AlCount import AlCount
			self._alCount = AlCount(self._core, self._base)
	...
	def encc(self):
		...
		if not hasattr(self, '_encc'):
			from .Subf_.Encc import Encc
			self._encc = Encc(self._core, self._base)
		return self._encc
