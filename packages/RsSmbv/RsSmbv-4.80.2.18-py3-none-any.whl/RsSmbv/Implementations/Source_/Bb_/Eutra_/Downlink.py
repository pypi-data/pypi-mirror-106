...
class Downlink:
	"""Downlink commands group definition. 616 total commands, 25 Sub-groups, 14 group commands"""
	...
	def emtc(self):
		...
		if not hasattr(self, '_emtc'):
			from .Downlink_.Emtc import Emtc
	...
	def mbsfn(self):
		...
		if not hasattr(self, '_mbsfn'):
			from .Downlink_.Mbsfn import Mbsfn
	...
	def niot(self):
		...
		if not hasattr(self, '_niot'):
			from .Downlink_.Niot import Niot
	...
	def pdcch(self):
		...
		if not hasattr(self, '_pdcch'):
			from .Downlink_.Pdcch import Pdcch
