...
class Inhibit:
	...
	def set(self, inhibit: int, external=repcap.External.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:DME:TRIGger:[EXTernal<CH>]:INHibit \n
		Snippet: driver.source.bb.dme.trigger.external.inhibit.set(inhibit = 1, external = repcap.External.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DME:TRIGger:EXTernal{external_cmd_val}:INHibit {param}')

	def get(self, external=repcap.External.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:DME:TRIGger:[EXTernal<CH>]:INHibit \n
		Snippet: value: int = driver.source.bb.dme.trigger.external.inhibit.get(external = repcap.External.Default) \n
		Specifies the duration by which a restart is inhibited. \n
			:param external: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
			:return: inhibit: integer Range: 0 to 67108863"""
		external_cmd_val = self._base.get_repcap_cmd_value(external, repcap.External)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DME:TRIGger:EXTernal{external_cmd_val}:INHibit?')
