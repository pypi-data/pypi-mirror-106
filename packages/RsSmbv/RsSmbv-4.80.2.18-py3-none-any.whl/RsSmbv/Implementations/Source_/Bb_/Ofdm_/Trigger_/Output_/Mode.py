...
class Mode:
	...
	def set(self, mark_mode: enums.C5GmarkMode, output=repcap.Output.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:OFDM:TRIGger:OUTPut<CH>:MODE \n
		Snippet: driver.source.bb.ofdm.trigger.output.mode.set(mark_mode = enums.C5GmarkMode.RESTart, output = repcap.Output.Default) \n
	...
	def get(self, output=repcap.Output.Default) -> enums.C5GmarkMode:
		"""SCPI: [SOURce<HW>]:BB:OFDM:TRIGger:OUTPut<CH>:MODE \n
		Snippet: value: enums.C5GmarkMode = driver.source.bb.ofdm.trigger.output.mode.get(output = repcap.Output.Default) \n
		Defines the signal for the selected marker output. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
