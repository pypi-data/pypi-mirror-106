...
class SfOffset:
	...
	def set(self, sys_frm_num_off: int, cellNull=repcap.CellNull.Nr0) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CH0>:SYINfo:SFOFfset \n
		Snippet: driver.source.bb.nr5G.node.cell.syInfo.sfOffset.set(sys_frm_num_off = 1, cellNull = repcap.CellNull.Nr0) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:NODE:CELL{cellNull_cmd_val}:SYINfo:SFOFfset {param}')

	def get(self, cellNull=repcap.CellNull.Nr0) -> int:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CH0>:SYINfo:SFOFfset \n
		Snippet: value: int = driver.source.bb.nr5G.node.cell.syInfo.sfOffset.get(cellNull = repcap.CellNull.Nr0) \n
		Sets an offset value for the system frame number. The first generated frame starts with the given system frame number
		offset. \n
			:param cellNull: optional repeated capability selector. Default value: Nr0
			:return: sys_frm_num_off: integer Range: 0 to 1023"""
		cellNull_cmd_val = self._base.get_repcap_cmd_value(cellNull, repcap.CellNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NR5G:NODE:CELL{cellNull_cmd_val}:SYINfo:SFOFfset?')
