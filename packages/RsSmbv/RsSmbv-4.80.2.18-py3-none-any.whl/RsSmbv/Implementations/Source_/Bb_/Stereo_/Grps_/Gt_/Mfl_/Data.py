...
class Data:
	"""Data commands group definition. 2 total commands, 2 Sub-groups, 0 group commands
	Repeated Capability: DataNull, default value after init: DataNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("data", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_dataNull_get', 'repcap_dataNull_set', repcap.DataNull.Nr0)

	def repcap_dataNull_set(self, enum_value: repcap.DataNull) -> None:
	...
	def mf(self):
		...
		if not hasattr(self, '_mf'):
			from .Data_.Mf import Mf
			self._mf = Mf(self._core, self._base)
		return self._mf
	...
	def tf(self):
		...
		if not hasattr(self, '_tf'):
			from .Data_.Tf import Tf
			self._tf = Tf(self._core, self._base)
		return self._tf
