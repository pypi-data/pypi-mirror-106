...
class Slot:
	"""Slot commands group definition. 47 total commands, 5 Sub-groups, 0 group commands
	Repeated Capability: SlotNull, default value after init: SlotNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("slot", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_slotNull_get', 'repcap_slotNull_set', repcap.SlotNull.Nr0)

	def repcap_slotNull_set(self, enum_value: repcap.SlotNull) -> None:
	...
	def channel(self):
		...
		if not hasattr(self, '_channel'):
			from .Slot_.Channel import Channel
	...
	def dconflict(self):
		...
		if not hasattr(self, '_dconflict'):
			from .Slot_.Dconflict import Dconflict
	...
	def prac(self):
		...
		if not hasattr(self, '_prac'):
			from .Slot_.Prac import Prac
			self._prac = Prac(self._core, self._base)
		return self._prac
