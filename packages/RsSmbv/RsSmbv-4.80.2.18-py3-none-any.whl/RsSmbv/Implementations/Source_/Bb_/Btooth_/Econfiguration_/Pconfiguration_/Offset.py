...
class Offset:
	"""Offset commands group definition. 2 total commands, 2 Sub-groups, 0 group commands
	Repeated Capability: OffsetNull, default value after init: OffsetNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("offset", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_offsetNull_get', 'repcap_offsetNull_set', repcap.OffsetNull.Nr0)

	def repcap_offsetNull_set(self, enum_value: repcap.OffsetNull) -> None:
	...
	def value(self):
		...
		if not hasattr(self, '_value'):
			from .Offset_.Value import Value
