...
class Fsbit:
	"""Fsbit commands group definition. 1 total commands, 1 Sub-groups, 0 group commands
	Repeated Capability: IndexNull, default value after init: IndexNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("fsbit", core, parent)
	...
	def state(self):
		...
		if not hasattr(self, '_state'):
			from .Fsbit_.State import State
	...
	def clone(self) -> 'Fsbit':
		...
		new_group = Fsbit(self._core, self._base.parent)
