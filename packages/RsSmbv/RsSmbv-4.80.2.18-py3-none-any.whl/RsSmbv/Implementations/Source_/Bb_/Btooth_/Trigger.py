...
class Trigger:
	"""Trigger commands group definition. 20 total commands, 4 Sub-groups, 5 group commands"""
	...
	def get_source(self) -> enums.TriggerSourceA:
		...
		Snippet: value: enums.TriggerSourceA = driver.source.bb.btooth.trigger.get_source() \n
	...
	def set_source(self, source: enums.TriggerSourceA) -> None:
		"""SCPI: [SOURce<HW>]:BB:BTOoth:TRIGger:SOURce \n
		Snippet: driver.source.bb.btooth.trigger.set_source(source = enums.TriggerSourceA.BBSY) \n
			INTRO_CMD_HELP: Selects the trigger signal source and determines the way the triggering is executed. Provided are: \n
			- Internal triggering by a command (INTernal)
			- External trigger signal via one of the User x connectors
			Table Header:  \n
