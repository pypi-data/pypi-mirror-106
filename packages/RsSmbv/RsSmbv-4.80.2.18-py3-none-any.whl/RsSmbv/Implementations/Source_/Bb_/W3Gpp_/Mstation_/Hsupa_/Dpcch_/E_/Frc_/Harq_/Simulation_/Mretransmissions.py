...
class Mretransmissions:
	"""Mretransmissions commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("mretransmissions", core, parent)
	...
	def set(self, mretransmission: int, mobileStation=repcap.MobileStation.Default) -> None:
		...
		No command help available \n
			:param mretransmission: No help available
	...
	def get(self, mobileStation=repcap.MobileStation.Default) -> int:
		...
		No command help available \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:return: mretransmission: No help available"""
		mobileStation_cmd_val = self._base.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
