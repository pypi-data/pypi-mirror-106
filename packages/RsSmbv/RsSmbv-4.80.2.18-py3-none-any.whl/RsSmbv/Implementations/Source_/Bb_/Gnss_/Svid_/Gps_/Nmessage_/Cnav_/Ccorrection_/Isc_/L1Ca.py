...
class L1Ca:
	"""L1Ca commands group definition. 2 total commands, 1 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("l1Ca", core, parent)

	@property
	def unscaled(self):
		"""unscaled commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_unscaled'):
			from .L1Ca_.Unscaled import Unscaled
	...
	def clone(self) -> 'L1Ca':
		...
		new_group = L1Ca(self._core, self._base.parent)
