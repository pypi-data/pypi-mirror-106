...
class Source:
	...
	def set(self, source: enums.AmFmSource, generatorIx=repcap.GeneratorIx.Default) -> None:
		...
		Snippet: driver.source.pm.source.set(source = enums.AmFmSource.EXT1, generatorIx = repcap.GeneratorIx.Default) \n
		...
		generatorIx_cmd_val = self._base.get_repcap_cmd_value(generatorIx, repcap.GeneratorIx)
	...
	def get(self, generatorIx=repcap.GeneratorIx.Default) -> enums.AmFmSource:
		"""SCPI: [SOURce<HW>]:PM<CH>:SOURce \n
		Snippet: value: enums.AmFmSource = driver.source.pm.source.get(generatorIx = repcap.GeneratorIx.Default) \n
		Selects the modulation source for phase modulation signal. \n
			:param generatorIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pm')
