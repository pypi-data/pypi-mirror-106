...
class Tagger:
    ...
    :type DM: float
    :param DG: :math:`\Delta\Gamma` frequency (set by default)
    :type DG: float
    ...
    def __init__(self, name, eta_data, dec_data, B_ID, mode, tau_ps=None, tauerr_ps=None, weight=None, selection=None, resolution_model=None, analytic_gradient=False, DM=None, DG=None):
        ...
        if self.mode == "Bd":
            self.tau        = pd.Series(tau_ps)[self.stats.tagged_sel]  #: decay time in picoseconds
            self.tauerr     = pd.Series(tauerr_ps)[self.stats.tagged_sel] if tauerr_ps is not None else None  #: decay time uncertainty in picoseconds
            self.DeltaM     = DM or 0.5065  #: B oscillation frequency :math:`\Delta m`
            self.DeltaGamma = DG or 0  #: Decay width difference of B mass eigenstates :math:`\Delta\Gamma`
            self.Aprod      = 0  #: Production asymmetry (WIP)
        elif self.mode == "Bs":
            self.tau        = pd.Series(tau_ps)[self.stats.tagged_sel]
            self.tauerr     = pd.Series(tauerr_ps)[self.stats.tagged_sel] if tauerr_ps is not None else None
        ...
        if self.mode in ("Bd", "Bs"):
            ...
            raise_error(len(self.tau) == self.stats.Nts, "Tagging data must have matching dimensions")
            if self.tauerr is not None:
                raise_error(len(self.tauerr) == self.stats.Nts, "Tagging data must have matching dimensions")
    ...
    def __init_oscillation_dilution(self):
        """ Computes flavour impurity for each measurement
            Needed for production flavour estimate correction
        """
        if self.mode == "Bu":
            self.osc_dilution = np.zeros(self.stats.Nts)
            Amix = None
        else:
            Amix = mixing_asymmetry(self.tau[self.stats.tagged_sel],
                                    DM     = self.DeltaM,
                                    DG     = self.DeltaGamma,
                                    tauerr = self.tauerr,
                                    a      = 0,
                                    res    = self.resolution_model)
    ...
    def __nll_oscil(self, params):
        ...
        correct_terms  = (1.0 - self.osc_dilution[self.stats.correct_tags]) * (1.0 - omega_given[self.stats.correct_tags])  # No mixing (tag == prod flav == decay flav)
        correct_terms +=        self.osc_dilution[self.stats.correct_tags]  * omega_oscil[self.stats.correct_tags]          # mixing    (tag == prod flav != decay flav)

        wrong_terms    = (1.0 - self.osc_dilution[self.stats.wrong_tags]) * omega_given[self.stats.wrong_tags]          # No mixing (tag != prod flav == decay flav)
        wrong_terms   +=        self.osc_dilution[self.stats.wrong_tags]  * (1.0 - omega_oscil[self.stats.wrong_tags])  # mixing    (tag != prod flav != decay flav)
