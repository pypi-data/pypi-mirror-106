...
def sin_cos_transformation(df:pd.DataFrame, col: str, max_val: int) -> pd.DataFrame:
    """
    ...
    df[col + '_sin'] = np.sin(2 * np.pi * df[col]/max_val)  # sine transformation
    df[col + '_cos'] = np.cos(2 * np.pi * df[col]/max_val)  # cosine transformation
