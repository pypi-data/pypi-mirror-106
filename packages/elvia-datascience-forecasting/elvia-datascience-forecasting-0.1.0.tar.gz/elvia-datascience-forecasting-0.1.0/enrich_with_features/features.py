...
def enrich(df: pd.DataFrame, df_weekly, holiday_path: str='', token_weather: str ='', time_zone:bool = False) -> pd.DataFrame:
    """
    ...
    for hour in [72, 96, 168]:
        ...
        df = df.merge(df_shift, left_on=['date_tz', 'trafo'], right_on=['date_tz', 'trafo'], how='left' )

    # Adding days of a week to dataframe
    df_day = dayofweek(df)    
    df = df.merge(df_day, left_on=['date_tz', 'trafo'], right_on=['date_tz', 'trafo'], how='left' )
   
    # Adding hours of a day to dataframe
    df_hour = hourofday(df)   
    df = df.merge(df_hour, left_on=['date_tz', 'trafo'], right_on=['date_tz', 'trafo'], how='left' )
    
    # Adding hour of week to dataframe (0 - 167)
    df_weekhour = hourofweek(df)
    df = df.merge(df_weekhour, left_on=['date_tz', 'trafo'], right_on=['date_tz', 'trafo'], how='left' )

    # Adding month of a year to dataframe
    df_month = monthyear(df)
    df = df.merge(df_month, left_on=['date_tz', 'trafo'], right_on=['date_tz', 'trafo'], how='left' )

    df.reset_index(inplace=True) # we reset index before merging with average_weekly_data, otherwise we will lose 'date_tz' index 
    # Adding month of a year to dataframe
    df_average_weekly = average_weekly_data(df_weekly)
    df = df.merge(df_average_weekly, left_on=['hourofweek', 'trafo'], right_on=['houroftheweek', 'trafo'], how='left' ) 
    ...
    df = df.merge(df_school_holiday, left_on=['date_tz', 'trafo'], right_on=['date_tz', 'trafo'], how='left' )
    df['school_holiday'] = df['school_holiday'].astype('bool') # casting type from object to boolean
    
    # Adding national holiday to dataframe
    df_national_holiday = national_holiday(df, path_holiday= holiday_path)
    df = df.merge(df_national_holiday, left_on=['date_tz'], right_on=['date_tz'], how='left')

    start_date = df.index[0].strftime(format='%Y-%m-%d' ) 
    end_date= (df.index[-1] + timedelta(days=1)).strftime(format='%Y-%m-%d' ) 
    df_weather = temperature_hourly(df, 'temperature', start_date=start_date, end_date=end_date, time_zone = time_zone, weather_token_id= token_weather)
    # Reset index to merge dataframes based on index column and trafo column                                
    df.reset_index(drop=False, inplace=True)
    df = df.merge(df_weather, left_on=['date_tz', 'trafo'], right_on=['time', 'trafo'], how='left')

    # Drop these columns because we will not use them in ML model
    df.drop('time', axis =1, inplace=True)

    # Sine and cosine transformation
    cyclical_features = [('hourofday', 23), ('dayofweek', 6), ('monthyear',12)]
    for col in cyclical_features:
        sin_cos_transformation(df, col[0], col[1])
    
    # df = df.set_index('date_tz')
    # print("data for checking first dates", df.head())
    # print("data for checking last dates", df.tail())
    # df.reset_index(inplace=True)
    
    # Drop the 3 cyclical features and other columns because we will not use them in ML model
    df.drop(['aggregated_per_mp','trafo','date_tz','hourofday', 'dayofweek', 'monthyear', 'hourofweek', 'houroftheweek', 'station_name', 'kommunenavn','kommunenummer', 'fylkesnavn', 'fylkesnummer', 'Driftsmerking', 'long', 'lat'], axis =1, inplace=True)
