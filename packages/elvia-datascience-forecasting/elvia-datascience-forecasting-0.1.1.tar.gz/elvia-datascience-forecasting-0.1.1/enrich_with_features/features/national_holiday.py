...
def national_holiday(df: pd.DataFrame, path_holiday: str='') -> pd.DataFrame:
    ...
    for country_ferie in h_n['data']['national_holiday']:
        ...
        vec = pd.date_range(start=datetime.strptime(country_ferie['date_start'],'%Y-%m-%d'),
                                end=datetime.strptime(country_ferie['date_stop'],'%Y-%m-%d'),
                                freq='D',tz='Europe/Oslo',closed='left')
    ...
    frame['national_holiday'] = frame['date'].isin(country_level_holiday.index.date).values
