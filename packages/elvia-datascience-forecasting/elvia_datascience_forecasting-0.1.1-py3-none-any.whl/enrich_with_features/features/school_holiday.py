...
def school_holiday(df: pd.DataFrame, region: str= 'Hedmark', path_holiday: str='',
    ...
    elevdag_list = ['Fri inneklemt dag 2018', 'Fri inneklemt dag 2019', 'Fri inneklemt dag 2020', 'Fri inneklemt dag 2021']
    ...
    for station_id in df_holiday['trafo'].unique():
    # for station_id in df_holiday['station_region'].unique():
        station_region = df_holiday.loc[df_holiday['trafo'] == station_id]['fylkesnavn'].values[0]
            ...
            if i['region'] == station_region:
                for region_ferie in i['holiday']:
                    if region_ferie['description'] not in elevdag_list:
                        vec = pd.date_range(start=datetime.strptime(region_ferie['date_start'],'%Y-%m-%d'),
                                            end=datetime.strptime(region_ferie['date_stop'],'%Y-%m-%d'), 
                                            freq='D',
                                            tz='UTC', # Europe/Oslo
                                            closed='left')
        ...
        frame = pd.DataFrame()
        frame['date'] = df_holiday.loc[df_holiday['trafo'] == station_id].index.date

        # Checks if datetime is holiday and add True/False.
        df_holiday.loc[df_holiday['trafo'] == station_id,'school_holiday'] = frame['date'].isin(school_holiday.index.date).values
   
    return df_holiday[['school_holiday', 'trafo']]
