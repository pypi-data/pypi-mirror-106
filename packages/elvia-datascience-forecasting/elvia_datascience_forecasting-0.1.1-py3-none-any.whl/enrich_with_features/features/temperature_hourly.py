...
def temperature_hourly(df, weather_variable: str, start_date:str, end_date:str, 
                    time_zone:bool = False, weather_token_id:str = '') -> pd.DataFrame:
    ...
    for station in df['trafo'].unique():
        ...
        input_data = {"startTime": start_date,
                    "endTime": end_date,
        ...
        headers = {'Content-Type': 'application/json-patch+json', 'Ocp-Apim-Subscription-Key': token}
        ...
        weather_data_temp = weather_data_json['coordinates'][0]['variables'][0]['data']
        weather_data_df = pd.DataFrame.from_records(weather_data_temp) # converting structured data to dataframe
        
        # CHECK  TIME_ZONE IS
        weather_data_df['time'] = pd.to_datetime(weather_data_df['time'])
        weather_data_df = weather_data_df.set_index('time')
        weather_data_df.rename(columns = {'value':weather_variable}, inplace = True)
        weather_data_df['trafo'] = station
        dataframe_list.append(weather_data_df)

    all_weather_data_df = pd.concat(dataframe_list) # concatenating all dataframes

    if time_zone == True:
        # Convert to 'Europe/Oslo' time zone
        all_weather_data_df.index= all_weather_data_df.index.tz_convert(tz='Europe/Oslo')
