...
def add_metadata(row, df_metadata):
    ...
    list_t = []
    cnt = 0
    if (row != None) and (not row.startswith('T')) and (not row.startswith('Q')): # all placed in Innlandet and 'NS22LHUN' doesnot have any electricity measurement
        if row.startswith('ENS12160S'):
            new_name = row[:8]
            cnt += 1
            list_t.append(new_name)
            return new_name
        elif row.split('T')[0] not in df_trafo_metadata['Driftsmerking'].values:
    ...
    if row.startswith('Q'): # we have just one starting with Q, placed in Innlandet
        new_name = row[:4]
        cnt += 1
        list_t.append(new_name)
        return new_name
    ...
    if (row != None) and (row.startswith('T')):
            ...
            if (len(name) != 5) and (name.endswith('gml')) and (name[:len(name)-3] in df_trafo_metadata['Driftsmerking'].values):
                    new_name = name[:len(name)-3]   # removing 'gml'
                    cnt += 1
                    list_t.append(new_name)
                    return new_name
                    
            elif (len(name) != 5) and (name.endswith('gml')) and (not name[:len(name)-3] in df_trafo_metadata['Driftsmerking'].values):
                #print('its length is not 5 and it Ends with gml but not in Metadata', name)
                list_invalid_t.append(name)
                    
            elif (len(name) != 5) and (not name.endswith('gml')) and (name in df_trafo_metadata['Driftsmerking'].values): # add to invalid list
                new_name = name
                cnt += 1
                list_t.append(new_name)
                return new_name
                    
            elif (len(name) != 5) and (not name.endswith('gml')) and (name not in df_trafo_metadata['Driftsmerking'].values): 
            ...
            elif (len(name) == 5) and (not name.endswith('gml')) and (name in df_trafo_metadata['Driftsmerking'].values):
                new_name = name    # 56 of them are not placed in substation_metadata. Their names are in "not_in_trafo_metadat.json" file
                cnt += 1
                list_t.append(new_name)
                return new_name

            elif (len(name) == 5) and (name.endswith('gml')) and (name in df_trafo_metadata['Driftsmerking'].values):
                new_name = name    # 56 of them are not placed in substation_metadata. Their names are in "not_in_trafo_metadat.json" file
                cnt += 1
                list_t.append(new_name)
                return new_name
            ...
            elif (len(name) == 5) and (name.endswith('gml')) and (name not in df_trafo_metadata['Driftsmerking'].values):
