...
from web_frame.frame.BaseHandler import BaseHandler
from web_frame.utils.CommonUtil import get_host_ip, sys_date_format, generate_random_key, get_default_args, get_command, \
    parse_val
from web_frame.utils.DataClient import DataClient
from web_frame.utils.FileUtil import read_json, write_json
from web_frame.utils.JwtUtil import generate_session
from web_frame.utils.MysqlUtil import *
        ...
        for rule in rules:
            ...
            for method in method_list:
                ...
                for i in range(1, method.__code__.co_argcount):
                    ...
                    if func_params[i] not in request_params.keys():
                        request_params[func_params[i]] = {
                            "param_name": func_params[i],
                            "sample_data": default_val,
                            "default_val": default_val,
                            "data_type": type(default_val).__name__,
                            "required": "是" if required else "否",
                            "comment": default_val,
                        }
                    else:
                        request_params[func_params[i]]["default_val"] = default_val
                        request_params[func_params[i]]["required"] = "是" if required else "否"
