...
class RsCmwGprfMeas:
	"""395 total commands, 13 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.30.0031'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '3.7.30.0031'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.30.0031'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwGprfMeas version failed. Current version: '3.7.30.0031', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 9 Sub-classes, 1 commands."""
	...
	def power(self):
		...
		if not hasattr(self, '_power'):
			from .Implementations.Power import Power
	...
	def iqVsSlot(self):
		...
		if not hasattr(self, '_iqVsSlot'):
			from .Implementations.IqVsSlot import IqVsSlot
	...
	def extPwrSensor(self):
		...
		if not hasattr(self, '_extPwrSensor'):
			from .Implementations.ExtPwrSensor import ExtPwrSensor
	...
	def nrpm(self):
		...
		if not hasattr(self, '_nrpm'):
			from .Implementations.Nrpm import Nrpm
	...
	def iqRecorder(self):
		...
		if not hasattr(self, '_iqRecorder'):
			from .Implementations.IqRecorder import IqRecorder
	...
	def spectrum(self):
		...
		if not hasattr(self, '_spectrum'):
			from .Implementations.Spectrum import Spectrum
	...
	def fftSpecAn(self):
		...
		if not hasattr(self, '_fftSpecAn'):
			from .Implementations.FftSpecAn import FftSpecAn
	...
	def ploss(self):
		...
		if not hasattr(self, '_ploss'):
			from .Implementations.Ploss import Ploss
