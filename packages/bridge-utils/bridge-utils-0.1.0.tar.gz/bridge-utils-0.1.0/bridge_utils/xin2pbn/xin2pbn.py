...
def check_file(file, generate_all=True):
    ...
    if generate_all==False:
        ...
        for index, url in enumerate(urls):
            pbn_file=os.path.join(dir,"%s-%02d" % (filename, index + 1))
            print("remove temple file %s" % outfilename)
            if os.path.isfile(pbn_file):
                os.remove(pbn_file)

def generate_pbn_from_html(html_files, generate_all=True):
    print(">> checking %s" % html_files)
    for file in html_files:
        if file.endswith(".html") or file.endswith(".htm"):
            check_file(file, generate_all)
