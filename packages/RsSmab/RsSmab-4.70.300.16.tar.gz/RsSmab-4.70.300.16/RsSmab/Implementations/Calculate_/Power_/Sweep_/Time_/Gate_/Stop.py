...
class Stop:
	...
	def set(self, stop: float, gate=repcap.Gate.Default) -> None:
		...
		Snippet: driver.calculate.power.sweep.time.gate.stop.set(stop = 1.0, gate = repcap.Gate.Default) \n
		...
		gate_cmd_val = self._base.get_repcap_cmd_value(gate, repcap.Gate)
		self._core.io.write(f'CALCulate:POWer:SWEep:TIME:GATE{gate_cmd_val}:STOP {param}')

	def get(self, gate=repcap.Gate.Default) -> float:
		"""SCPI: CALCulate:[POWer]:SWEep:TIME:GATE<CH>:STOP \n
		Snippet: value: float = driver.calculate.power.sweep.time.gate.stop.get(gate = repcap.Gate.Default) \n
		Sets the start time of the selected gate. Insert value and unit. \n
			:param gate: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Gate')
			:return: stop: float"""
		gate_cmd_val = self._base.get_repcap_cmd_value(gate, repcap.Gate)
		response = self._core.io.query_str(f'CALCulate:POWer:SWEep:TIME:GATE{gate_cmd_val}:STOP?')
