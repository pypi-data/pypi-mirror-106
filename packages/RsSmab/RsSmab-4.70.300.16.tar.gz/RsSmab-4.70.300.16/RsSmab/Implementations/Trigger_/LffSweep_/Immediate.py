...
class Immediate:
	...
	def set(self, inputIx=repcap.InputIx.Default) -> None:
		...
		Snippet: driver.trigger.lffSweep.immediate.set(inputIx = repcap.InputIx.Default) \n
			...
			- SOUR:SWE:FREQ|POW:MODEAUTO or method RsSmab.Source.LfOutput.Sweep.Frequency.Mode.valueAUTO
	...
	def set_with_opc(self, inputIx=repcap.InputIx.Default) -> None:
		...
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmab.utilities.opc_timeout_set() to set the timeout value. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Trigger')"""
		self._core.io.write_with_opc(f'TRIGger{inputIx_cmd_val}:LFFSweep:IMMediate')
