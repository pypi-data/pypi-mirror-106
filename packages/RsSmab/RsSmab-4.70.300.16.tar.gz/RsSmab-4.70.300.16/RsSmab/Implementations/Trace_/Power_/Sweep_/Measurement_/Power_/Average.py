...
class Average:
	...
	def display(self):
		...
		if not hasattr(self, '_display'):
			from .Average_.Display import Display
	...
	def get(self, trace=repcap.Trace.Default) -> float:
		...
		Snippet: value: float = driver.trace.power.sweep.measurement.power.average.get(trace = repcap.Trace.Default) \n
		...
		measurement mode and with R&S NRP-Z81 power sensors. \n
			...
			:return: average: float Range: 0 to 100"""
		trace_cmd_val = self._base.get_repcap_cmd_value(trace, repcap.Trace)
		response = self._core.io.query_str(f'TRACe{trace_cmd_val}:POWer:SWEep:MEASurement:POWer:AVERage?')
