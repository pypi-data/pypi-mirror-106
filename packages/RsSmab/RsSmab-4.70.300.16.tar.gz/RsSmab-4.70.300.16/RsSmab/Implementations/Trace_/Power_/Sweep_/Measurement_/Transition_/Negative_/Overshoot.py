...
class Overshoot:
	...
	def get(self, trace=repcap.Trace.Default) -> float:
		...
		Snippet: value: float = driver.trace.power.sweep.measurement.transition.negative.overshoot.get(trace = repcap.Trace.Default) \n
		...
		response = self._core.io.query_str(f'TRACe{trace_cmd_val}:POWer:SWEep:MEASurement:TRANsition:NEGative:OVERshoot?')
