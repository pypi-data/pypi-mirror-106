...
class Duration:
	"""Duration commands group definition. 2 total commands, 1 Sub-groups, 1 group commands"""
	...
	def display(self):
		...
		if not hasattr(self, '_display'):
			from .Duration_.Display import Display
	...
	def get(self, trace=repcap.Trace.Default) -> float:
		...
		Snippet: value: float = driver.trace.power.sweep.measurement.transition.negative.duration.get(trace = repcap.Trace.Default) \n
		...
		response = self._core.io.query_str(f'TRACe{trace_cmd_val}:POWer:SWEep:MEASurement:TRANsition:NEGative:DURation?')
