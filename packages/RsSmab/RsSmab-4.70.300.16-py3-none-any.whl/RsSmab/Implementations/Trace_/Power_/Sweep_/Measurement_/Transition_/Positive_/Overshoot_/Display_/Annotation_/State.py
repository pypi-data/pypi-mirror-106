...
class State:
	...
	def set(self, state: bool, trace=repcap.Trace.Default) -> None:
		...
		Snippet: driver.trace.power.sweep.measurement.transition.positive.overshoot.display.annotation.state.set(state = False, trace = repcap.Trace.Default) \n
		...
		self._core.io.write(f'TRACe{trace_cmd_val}:POWer:SWEep:MEASurement:TRANsition:POSitive:OVERshoot:DISPlay:ANNotation:STATe {param}')

	def get(self, trace=repcap.Trace.Default) -> bool:
		"""SCPI: TRACe<CH>:[POWer]:SWEep:MEASurement:TRANsition:POSitive:OVERshoot:DISPlay:ANNotation:[STATe] \n
		Snippet: value: bool = driver.trace.power.sweep.measurement.transition.positive.overshoot.display.annotation.state.get(trace = repcap.Trace.Default) \n
		The above listed commands select the pulse parameters which are indicated in the display and hardcopy file.
		Only six parameters can be indicated at a time. Note: These commands are only available in time measurement mode and with
		R&S NRP-Z81 power sensors. \n
			:param trace: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Trace')
		...
		trace_cmd_val = self._base.get_repcap_cmd_value(trace, repcap.Trace)
		response = self._core.io.query_str(f'TRACe{trace_cmd_val}:POWer:SWEep:MEASurement:TRANsition:POSitive:OVERshoot:DISPlay:ANNotation:STATe?')
