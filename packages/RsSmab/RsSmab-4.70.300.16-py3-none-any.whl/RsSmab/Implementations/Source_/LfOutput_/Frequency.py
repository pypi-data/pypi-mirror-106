...
class Frequency:
	...
	def set(self, frequency: float, lfOutput=repcap.LfOutput.Default) -> None:
		...
		Sets the frequency of the LF signal in method RsSmab.Source.LfOutput.Frequency.mode CW|FIXed mode.
			INTRO_CMD_HELP: Note \n
			- If signal source 'Internal' is set, the instrument performs the analog modulations (AM/FM/PhiM/PM) with this frequency.
			- In sweep mode (method RsSmab.Source.LfOutput.Frequency.mode SWE) , the frequency is coupled with the sweep frequency. \n
		...
		lfOutput_cmd_val = self._base.get_repcap_cmd_value(lfOutput, repcap.LfOutput)
		self._core.io.write(f'SOURce:LFOutput{lfOutput_cmd_val}:FREQuency {param}')
	...
	def get(self, lfOutput=repcap.LfOutput.Default) -> float:
		...
		lfOutput_cmd_val = self._base.get_repcap_cmd_value(lfOutput, repcap.LfOutput)
		response = self._core.io.query_str(f'SOURce:LFOutput{lfOutput_cmd_val}:FREQuency?')
	...
	def get_manual(self) -> float:
		...
		Sets the frequency of the subsequent sweep step if LFO:SWE:MODE MAN. Use a separate command for each sweep step. \n
			:return: manual: float You can select any value within the setting range, where: STARt is set with method RsSmab.Source.LfOutput.Frequency.start STOP is set with method RsSmab.Source.LfOutput.Frequency.stop Range: STARt to STOP
		"""
	...
	def set_manual(self, manual: float) -> None:
		...
		Sets the frequency of the subsequent sweep step if LFO:SWE:MODE MAN. Use a separate command for each sweep step. \n
			:param manual: float You can select any value within the setting range, where: STARt is set with method RsSmab.Source.LfOutput.Frequency.start STOP is set with method RsSmab.Source.LfOutput.Frequency.stop Range: STARt to STOP
		"""
	...
	def get_mode(self) -> enums.LfFreqMode:
		...
		Sets the mode for the output of the LF generator frequency, and determines the commands to be used for frequency settings. \n
			:return: mode: CW| FIXed| SWEep CW|FIXed Sets the fixed-frequency mode. CW and FIXed are synonyms. To set the output frequency, use command method RsSmab.Source.LfOutput.Frequency.set SWEep Sets sweep mode. To set the frequency, use the commands: method RsSmab.Source.LfOutput.Frequency.start and method RsSmab.Source.LfOutput.Frequency.stop Or method RsSmab.Source.LfOutput.Frequency.manual
		"""
	...
	def get_start(self) -> float:
		...
		Sets the start/stop frequency for method RsSmab.Source.LfOutput.Frequency.mode SWEep. \n
