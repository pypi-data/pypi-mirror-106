...
class RsSmab:
	"""1084 total commands, 24 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMA/SMAB/SMCV, SupportedIdnPatterns = SMA/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMA100B,100001,4.70.300.0016'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.70.300.0016'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.70.300.0016'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmab version failed. Current version: '4.70.300.0016', minimum required version: '{min_version}'")
	...
	def calculate(self):
		...
		if not hasattr(self, '_calculate'):
			...
			self._calculate = Calculate(self._core, self._base)
	...
	def csynthesis(self):
		...
		if not hasattr(self, '_csynthesis'):
			from .Implementations.Csynthesis import Csynthesis
			self._csynthesis = Csynthesis(self._core, self._base)
	...
	@property
	def display(self):
		"""display commands group. 6 Sub-classes, 4 commands."""
