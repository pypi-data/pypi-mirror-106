...
def test_get_table_list():
    ...
    with requests_mock.Mocker() as m:
        m.register_uri('GET', dummy_endpoint + '/schemas/dummySchema/tables', text=mock_response_text, status_code=mock_response_code)
        runner = CliRunner()
...
def test_create_table():
    ...
    expected_result = '{\n  "response": "Created"\n}\n'
    with requests_mock.Mocker() as m:
        m.register_uri('PUT', dummy_endpoint + '/schemas/dummySchema/tables/dummyTable', text="", status_code=mock_response_code)
        runner = CliRunner()
        result = runner.invoke(geospock_cli,
                               ["--endpoint", dummy_endpoint, "--user", "dummy", "--password", "dummy", "table",
                                "create", "--schema", "dummySchema", "--table", "dummyTable",
                                "--data-source-description-file", "test/banks_dsd.json"])
        assert result.exit_code == 0
...
def test_insert_into_table():
    ...
    expected_result = '{\n  "response": "Accepted"\n}\n'
    with requests_mock.Mocker() as m:
        m.register_uri('POST', dummy_endpoint + '/schemas/dummySchema/tables/dummyTable', text="", status_code=mock_response_code)
        runner = CliRunner()
        result = runner.invoke(geospock_cli,
                               ["--endpoint", dummy_endpoint, "--user", "dummy", "--password", "dummy", "table",
...
def test_drop_table():
    ...
    expected_result = '{\n  "response": "No Content"\n}\n'
    with requests_mock.Mocker() as m:
        m.register_uri('DELETE', dummy_endpoint + '/schemas/dummySchema/tables/dummyTable', text="", status_code=mock_response_code)
        runner = CliRunner()
        result = runner.invoke(geospock_cli,
                               ["--endpoint", dummy_endpoint, "--user", "dummy", "--password", "dummy", "table",
...
def test_create_table_with_invalid_name():
    mock_response_code = 400
    mock_response_text = 'Invalid table name'
    with requests_mock.Mocker() as m:
        m.register_uri('PUT', dummy_endpoint + '/schemas/dummySchema/tables/dummyTable', text=mock_response_text,
