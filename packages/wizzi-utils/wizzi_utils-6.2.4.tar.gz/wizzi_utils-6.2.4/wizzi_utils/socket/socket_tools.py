...
def download_file(url: str, dst_path: str = './file', tabs: int = 1) -> bool:
    ...
    def download_progress_hook(count, blockSize, totalSize):
        percent = min(float(count * blockSize) / float(totalSize) * 100.0, 100)
        print("\r{}Completed: {:.1f}% from {:,.2f}MB - {}".format((tabs + 1) * '\t', percent, totalSize / 1000000,
                                                                  filename), end="")
        return
    ...
    if not os.path.exists(dst_path):
        try:
            ...
            mt.exception_error(msg, tabs=tabs)
