...
def to_str(var: any,
    ...
    if isinstance(var, (float, np.float, np.float32, np.float64)):
        if wm:
        ...
        if fp is not None and fp >= 0:
            ...
            if all(isinstance(item, (float, np.float, np.float32, np.float64, int)) for item in var):
