...
def get_tflite_version(ack: bool = False, tabs: int = 1) -> str:
    ...
    try:
        ...
        string = '* TFLite Version {}'.format(tflite_runtime.__version__)
        if ack:
            print('{}{}'.format(tabs * '\t', string))
    except ModuleNotFoundError as e:
        string = '* {}'.format(e)
...
def hard_disc(one_liner: bool = False, tabs: int = 1):
    ...
    if one_liner:
        string = '{}Total {}, Used {}({:.2f}%), Free {}'.format(
            tabs * '\t',
            convert_size(total),
            convert_size(used),
            used_p,
        ...
        string += '\n{}\tTotal {}'.format(tabs * '\t', convert_size(total))
        string += '\n{}\tUsed {}({:.2f}%)'.format(tabs * '\t', convert_size(used), used_p)
        string += '\n{}\tFree {}'.format(tabs * '\t', convert_size(free))
    return string
