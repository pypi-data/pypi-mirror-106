...
class PrimeRobot:
    ...
    def _log_file(self, path: str):
        self.s3_client.upload_file(
            path, os.environ['BUCKET_RESULTADOS'], f'{os.environ["CLIENTE_ID"]}/{os.environ["TASK_ID"]}/log.html')

    def output_file(self, path: str):
        self.s3_client.upload_file(
            path, os.environ['BUCKET_RESULTADOS'], f'{os.environ["CLIENTE_ID"]}/{os.environ["TASK_ID"]}/output.xml')
    ...
    def salvar_output(self, dataframe):
        ...
        if self.is_aws():
            if output:
                self.s3_client.upload_file(output['data'], os.environ['BUCKET_RESULTADOS'],
