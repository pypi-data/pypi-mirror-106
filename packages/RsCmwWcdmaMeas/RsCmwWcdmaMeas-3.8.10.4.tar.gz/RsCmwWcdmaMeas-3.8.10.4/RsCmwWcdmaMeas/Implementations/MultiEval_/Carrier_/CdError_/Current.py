...
class Current:
	...
	def read(self, carrier=repcap.Carrier.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.carrier.cdError.current.read(carrier = repcap.Carrier.Default) \n
		...
		return self._core.io.query_struct(f'READ:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:CDERror:CURRent?', self.__class__.ResultData())
	...
	def fetch(self, carrier=repcap.Carrier.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.carrier.cdError.current.fetch(carrier = repcap.Carrier.Default) \n
		Returns the RMS CDE vs. slot values measured in a selected slot. In addition to the current values, average, maximum and
		standard deviation values can be retrieved. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		return self._core.io.query_struct(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:CDERror:CURRent?', self.__class__.ResultData())
