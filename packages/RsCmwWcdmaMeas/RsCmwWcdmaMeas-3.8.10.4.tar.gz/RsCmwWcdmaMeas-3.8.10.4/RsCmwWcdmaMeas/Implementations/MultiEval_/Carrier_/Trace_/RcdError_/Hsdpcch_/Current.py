...
class Current:
	...
	def read(self, carrier=repcap.Carrier.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.carrier.trace.rcdError.hsdpcch.current.read(carrier = repcap.Carrier.Default) \n
	...
	def fetch(self, carrier=repcap.Carrier.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.carrier.trace.rcdError.hsdpcch.current.fetch(carrier = repcap.Carrier.Default) \n
		Returns the values of the relative CDE vs. slot traces for the HS-DPCCH and the E-DPCCH. Each current value is averaged
		over a half-slot or a full-slot, depending on the measurement period (see method RsCmwWcdmaMeas.Configure.MultiEval.
		Mperiod.modulation) . The number of results depends on the measurement length (see method RsCmwWcdmaMeas.Configure.
		MultiEval.msCount) . The results of the current, average, maximum and standard deviation traces can be retrieved.
