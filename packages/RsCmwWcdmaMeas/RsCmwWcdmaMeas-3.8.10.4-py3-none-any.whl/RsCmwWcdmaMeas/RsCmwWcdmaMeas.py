...
class RsCmwWcdmaMeas:
	"""842 total commands, 8 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.10.0004'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwWcdmaMeas._driver_options, options, direct_session)
		self._core.driver_version = '3.8.10.0004'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwWcdmaMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwWcdmaMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwWcdmaMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.10.0004'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwWcdmaMeas version failed. Current version: '3.8.10.0004', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 10 Sub-classes, 0 commands."""
	...
	@property
	def tpc(self):
		"""tpc commands group. 4 Sub-classes, 3 commands."""
		if not hasattr(self, '_tpc'):
			from .Implementations.Tpc import Tpc
	...
	def prach(self):
		...
		if not hasattr(self, '_prach'):
			from .Implementations.Prach import Prach
	...
	def ooSync(self):
		...
		if not hasattr(self, '_ooSync'):
			from .Implementations.OoSync import OoSync
			self._ooSync = OoSync(self._core, self._base)
		return self._ooSync

	@property
	def olpControl(self):
		"""olpControl commands group. 2 Sub-classes, 6 commands."""
		if not hasattr(self, '_olpControl'):
			from .Implementations.OlpControl import OlpControl
			self._olpControl = OlpControl(self._core, self._base)
		return self._olpControl

	def clone(self) -> 'RsCmwWcdmaMeas':
		"""Creates a deep copy of the RsCmwWcdmaMeas object. Also copies:
		...
		cloned = RsCmwWcdmaMeas.from_existing_session(self.get_session_handle(), self._options)
