...
class Phd:
	...
	def set(self, enable_phd: bool, segment=repcap.Segment.Default) -> None:
		...
		Snippet: driver.configure.multiEval.listPy.segment.phd.set(enable_phd = False, segment = repcap.Segment.Default) \n
		Enables the calculation of the phase discontinuity vs. slot results in segment no. <no>; see 'Multi-Evaluation List Mode'. \n
			:param enable_phd: OFF | ON OFF: Disable measurement ON: Enable measurement of phase discontinuity
	...
	def get(self, segment=repcap.Segment.Default) -> bool:
		...
		Enables the calculation of the phase discontinuity vs. slot results in segment no. <no>; see 'Multi-Evaluation List Mode'. \n
			:param segment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Segment')
			:return: enable_phd: OFF | ON OFF: Disable measurement ON: Enable measurement of phase discontinuity"""
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
