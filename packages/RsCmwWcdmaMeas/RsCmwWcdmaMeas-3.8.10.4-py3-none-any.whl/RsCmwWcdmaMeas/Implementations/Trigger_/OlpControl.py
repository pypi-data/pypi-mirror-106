...
class OlpControl:
	...
	def catalog(self):
		...
		if not hasattr(self, '_catalog'):
			from .OlpControl_.Catalog import Catalog
			self._catalog = Catalog(self._core, self._base)
		return self._catalog

	def get_delay(self) -> float:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:DELay \n
		Snippet: value: float = driver.trigger.olpControl.get_delay() \n
		...
		"""
		response = self._core.io.query_str('TRIGger:WCDMa:MEASurement<Instance>:OLPControl:DELay?')
		return Conversions.str_to_float(response)

	def set_delay(self, delay: float) -> None:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:DELay \n
		Snippet: driver.trigger.olpControl.set_delay(delay = 1.0) \n
		Defines a time delaying the start of the measurement relative to the trigger event. The delay is useful if the trigger
		event and the uplink DPCH slot border are not synchronous. A measurement starts always at an uplink DPCH slot border.
		Triggering a measurement at another time can yield a synchronization error. For internal trigger sources aligned to the
		downlink DPCH, an additional delay of 1024 chips is automatically applied. It corresponds to the assumed delay between
		...
		self._core.io.write(f'TRIGger:WCDMa:MEASurement<Instance>:OLPControl:DELay {param}')

	def get_mgap(self) -> float:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:MGAP \n
		Snippet: value: float = driver.trigger.olpControl.get_mgap() \n
		...
		"""
		response = self._core.io.query_str('TRIGger:WCDMa:MEASurement<Instance>:OLPControl:MGAP?')
		return Conversions.str_to_float(response)

	def set_mgap(self, minimum_gap: float) -> None:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:MGAP \n
		Snippet: driver.trigger.olpControl.set_mgap(minimum_gap = 1.0) \n
		Sets a minimum time during which the IF signal must be below the trigger threshold before the trigger is armed so that an
		IF power trigger event can be generated. \n
			:param minimum_gap: numeric Range: 0 s to 0.01 s, Unit: s
		"""
		param = Conversions.decimal_value_to_str(minimum_gap)
		self._core.io.write(f'TRIGger:WCDMa:MEASurement<Instance>:OLPControl:MGAP {param}')

	def get_source(self) -> str:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:SOURce \n
		Snippet: value: str = driver.trigger.olpControl.get_source() \n
	...
	def set_source(self, source: str) -> None:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:SOURce \n
		Snippet: driver.trigger.olpControl.set_source(source = '1') \n
		Selects the source of the trigger events. Some values are always available. They are listed below. Depending on the
		installed options, additional values are available. You can query a list of all supported values via TRIGger:...
		:CATalog:SOURce?. \n
	...
	def get_threshold(self) -> float:
		...
		Snippet: value: float = driver.trigger.olpControl.get_threshold() \n
		...
		"""
		response = self._core.io.query_str('TRIGger:WCDMa:MEASurement<Instance>:OLPControl:THReshold?')
		return Conversions.str_to_float(response)

	def set_threshold(self, level: float) -> None:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:THReshold \n
		Snippet: driver.trigger.olpControl.set_threshold(level = 1.0) \n
		Defines the trigger threshold for power trigger sources. \n
		...
		self._core.io.write(f'TRIGger:WCDMa:MEASurement<Instance>:OLPControl:THReshold {param}')

	# noinspection PyTypeChecker
	def get_slope(self) -> enums.SignalSlope:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:SLOPe \n
		Snippet: value: enums.SignalSlope = driver.trigger.olpControl.get_slope() \n
	...
	def get_timeout(self) -> float or bool:
		...
		"""
		response = self._core.io.query_str('TRIGger:WCDMa:MEASurement<Instance>:OLPControl:TOUT?')
		return Conversions.str_to_float_or_bool(response)

	def set_timeout(self, timeout: float or bool) -> None:
		"""SCPI: TRIGger:WCDMa:MEASurement<instance>:OLPControl:TOUT \n
		Snippet: driver.trigger.olpControl.set_timeout(timeout = 1.0) \n
		Selects the maximum time that the measurement waits for a trigger event before it stops in remote control mode or
		indicates a trigger timeout in manual operation mode. This setting has no influence on 'Free Run' measurements. \n
		...
		self._core.io.write(f'TRIGger:WCDMa:MEASurement<Instance>:OLPControl:TOUT {param}')
