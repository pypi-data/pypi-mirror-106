...
class Current:
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.cdError.current.fetch(segment = repcap.Segment.Default) \n
		Returns the RMS CDE vs. slot results for segment <no> in list mode. \n
			:param segment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Segment')
			:return: structure: for return value, see the help for FetchStruct structure arguments."""
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:CDERror:CURRent?', self.__class__.FetchStruct())
