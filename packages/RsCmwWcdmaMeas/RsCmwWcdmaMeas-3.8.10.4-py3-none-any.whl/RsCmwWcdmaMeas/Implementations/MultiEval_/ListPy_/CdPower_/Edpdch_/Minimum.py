...
class Minimum:
	...
	def fetch(self, edpdChannel=repcap.EdpdChannel.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.cdPower.edpdch.minimum.fetch(edpdChannel = repcap.EdpdChannel.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:LIST:CDPower:EDPDch{edpdChannel_cmd_val}:MINimum?', suppressed)
		return response
