...
class Current:
	...
	def read(self, carrier=repcap.Carrier.Default, edpdChannel=repcap.EdpdChannel.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.carrier.trace.cdError.edpdch.current.read(carrier = repcap.Carrier.Default, edpdChannel = repcap.EdpdChannel.Default) \n
	...
	def fetch(self, carrier=repcap.Carrier.Default, edpdChannel=repcap.EdpdChannel.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.carrier.trace.cdError.edpdch.current.fetch(carrier = repcap.Carrier.Default, edpdChannel = repcap.EdpdChannel.Default) \n
		Returns the values of the RMS CDE vs. slot traces for the E-DPDCH 1 to 4. Each current value is averaged over a half-slot
		or a full-slot, depending on the measurement period (see method RsCmwWcdmaMeas.Configure.MultiEval.Mperiod.modulation) .
		The number of results depends on the measurement length (see method RsCmwWcdmaMeas.Configure.MultiEval.msCount) .
		The results of the current, average, maximum and standard deviation traces can be retrieved. The standard deviation trace
