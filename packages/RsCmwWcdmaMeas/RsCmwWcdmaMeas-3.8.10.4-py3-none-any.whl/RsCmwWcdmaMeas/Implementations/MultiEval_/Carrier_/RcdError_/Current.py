...
class Current:
	...
	def calculate(self, carrier=repcap.Carrier.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.carrier.rcdError.current.calculate(carrier = repcap.Carrier.Default) \n
		...
		return self._core.io.query_struct(f'CALCulate:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:RCDerror:CURRent?', self.__class__.CalculateStruct())
	...
	def fetch(self, carrier=repcap.Carrier.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.carrier.rcdError.current.fetch(carrier = repcap.Carrier.Default) \n
		Returns the RCDE vs. slot values measured in a selected slot. In addition to the current values, average, maximum and
		standard deviation values can be retrieved. The values described below are returned by FETCh and READ commands. CALCulate
		commands return limit check results instead, one value for each result listed below. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
		...
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		return self._core.io.query_struct(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:RCDerror:CURRent?', self.__class__.ResultData())
	...
	def read(self, carrier=repcap.Carrier.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.carrier.rcdError.current.read(carrier = repcap.Carrier.Default) \n
		Returns the RCDE vs. slot values measured in a selected slot. In addition to the current values, average, maximum and
		standard deviation values can be retrieved. The values described below are returned by FETCh and READ commands. CALCulate
		commands return limit check results instead, one value for each result listed below. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		return self._core.io.query_struct(f'READ:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:RCDerror:CURRent?', self.__class__.ResultData())
