...
from timeseer_client.base import AugmentationException
...
def filter_series(
    ...
    if augmentation_strategy == AugmentationStrategy.KNN_IMPUTATION:
        ...
        if not HAS_SKLEARN:
            raise AugmentationException('sklearn is required to perform KNN imputation.')
...
def _hold_last_value(series: pd.DataFrame, out_of_event_frames: List[bool]) -> pd.DataFrame:
...
def _linear_interpolation(series: pd.DataFrame, out_of_event_frames: List[bool]) -> pd.DataFrame:
        ...
        if out_of_frame:
            if series_to_filter_count != 0 and last_acceptable is not None:
                for element in range(series_to_filter_count):
                    numerator = series['ts'][index - (series_to_filter_count - element)] - \
                                    series['ts'][index - (series_to_filter_count + 1)]
                    denominator = series['ts'][index] - \
                        series['ts'][index - (series_to_filter_count + 1)]
                    time_interpolation = numerator / denominator
                    new_value = last_acceptable * (1 - time_interpolation) + \
                        series['value'][index] * time_interpolation
                    series.loc[index - (series_to_filter_count - element), 'value'] = new_value
            if last_acceptable is None:
    ...
    series = series.loc[remove_first_elements:(len(out_of_event_frames) - series_to_filter_count - 1)]
    return series.reset_index(drop=True)


def _knn_imputation(series: pd.DataFrame, out_of_event_frames: List[bool], context: List[pd.DataFrame]):
    ...
    concatenated_df = pd.concat([series, *context], axis=1, sort=False, ignore_index=True)
    imputer = KNNImputer(n_neighbors=5)
    imputed_df = pd.DataFrame(data=imputer.fit_transform(concatenated_df), index=concatenated_df.index)
