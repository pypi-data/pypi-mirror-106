...
from red_warden.config import Config, Backends, Logger
...
class RWRedisRpcEngine(RWEngine):
    ...
    def __init__(self, redis_backend_name):
        ...
        self._rpc_key = "%s:rpc" % Config["RW_APP_NAME"].lower()
        self._pubsub_key = "%s:pubsub" % Config["RW_APP_NAME"].lower()

    async def _rpc_listener(self):
        while not self._shutdown_event.is_set():
            try:
                self._redis_rpc = Backends.get(self._redis_backend_name)
    ...
    async def _pubsub_listener(self):
        while not self._shutdown_event.is_set():
            try:
                self._redis_pubsub = Backends.get(self._redis_backend_name)
    ...
    async def execute(self, rpc_name, *args, **kwargs):
        rc = Backends.get(self._redis_backend_name)
        await rc.connect()
