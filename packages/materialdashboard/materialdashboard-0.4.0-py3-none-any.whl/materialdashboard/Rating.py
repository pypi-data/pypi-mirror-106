...
Keyword arguments:
- defaultValue (number; optional): The default value. Use when the component is not controlled.
- disabled (boolean; optional): If `true`, the component is disabled.
- emptyIcon (boolean | number | string | dict | list; optional): The icon to display when empty.
- emptyLabelText (boolean | number | string | dict | list; optional): The label read when the rating input is empty.
- highlightSelectedOnly (boolean; optional): If `true`, only the selected icon will be highlighted.
- icon (boolean | number | string | dict | list; optional): The icon to display.
- max (number; optional): Maximum rating.
- name (string; optional): The name attribute of the radio `input` elements.
This input `name` should be unique within the page.
Being unique within a form is insufficient since the `name` is used to generated IDs.
- precision (number; optional): The minimum increment value change allowed.
- readOnly (boolean; optional): Removes all hover effects and pointer events.
- size (a value equal to: "small", "medium", "large"; optional): The size of the component.
- value (number; optional): The rating value.
