...
- icon (boolean | number | string | dict | list; optional): The icon to display.
- label (boolean | number | string | dict | list; optional): The label element.
- showLabel (boolean; optional): If `true`, the `BottomNavigationAction` will show its label.
By default, only the selected `BottomNavigationAction`
inside `BottomNavigation` will show its label.

The prop defaults to the value (`false`) inherited from the parent BottomNavigation component.
- value (boolean | number | string | dict | list; optional): You can provide your own value. Otherwise, we fallback to the child position index.
