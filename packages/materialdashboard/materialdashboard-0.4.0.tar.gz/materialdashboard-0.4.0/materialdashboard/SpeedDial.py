...
Keyword arguments:
- children (a list of or a singular dash component, string or number; optional): SpeedDialActions to display when the SpeedDial is `open`.
- ariaLabel (string; optional): The aria-label of the button element.
Also used to provide the `id` for the `SpeedDial` element and its children.
- direction (a value equal to: "left", "right", "up", "down"; optional): The direction the actions open relative to the floating action button.
- hidden (boolean; optional): If `true`, the SpeedDial is hidden.
- icon (boolean | number | string | dict | list; optional): The icon to display in the SpeedDial Fab. The `SpeedDialIcon` component
provides a default Icon with animation.
- open (boolean; optional): If `true`, the component is shown.
- openIcon (boolean | number | string | dict | list; optional): The icon to display in the SpeedDial Fab when the SpeedDial is open.
