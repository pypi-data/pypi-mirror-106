...
def append(append_service, verbose):  # Ajoute un service a la liste et l'ajoute également au fichier (file_ini)
    ...
    if str(exist_service).find(".service"):
        ...
        print(f"Appending the service {service_to_append}")
...
def send_mail(subject, verbose):
    ...
    cpu_temp = "paste <(cat /sys/class/thermal/thermal_zone*/temp) | column -s $ -t | sed 's/\(.\)..$/°C/'"
    ...
    Information système : 
    CPU Temp = {os.popen(cpu_temp).read()}
