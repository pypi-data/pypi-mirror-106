...
class RsCmwEvdoMeas:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.10.0005'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwEvdoMeas._driver_options, options, direct_session)
		self._core.driver_version = '3.8.10.0005'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwEvdoMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwEvdoMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwEvdoMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.10.0005'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwEvdoMeas version failed. Current version: '3.8.10.0005', minimum required version: '{min_version}'")
	...
	@property
	def multiEval(self):
		"""multiEval commands group. 7 Sub-classes, 3 commands."""
	...
	def clone(self) -> 'RsCmwEvdoMeas':
		"""Creates a deep copy of the RsCmwEvdoMeas object. Also copies:
		...
		cloned = RsCmwEvdoMeas.from_existing_session(self.get_session_handle(), self._options)
