...
class Maximum:
	...
	class ReadStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_float('At_Power_1_M_23'),
		...
		def __init__(self):
			...
			self.At_Power_1_M_23: float = None
	...
	def read(self) -> ReadStruct:
		...
		Snippet: value: ReadStruct = driver.multiEval.modulation.maximum.read() \n
		...
		return self._core.io.query_struct(f'READ:EVDO:MEASurement<Instance>:MEValuation:MODulation:MAXimum?', self.__class__.ReadStruct())
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.modulation.maximum.fetch() \n
		Return the current, average, minimum, maximum and standard deviation modulation single value results.
		The values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. The ranges indicated below apply to all results except standard deviation results.
		The minimum for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two.
		...
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:MODulation:MAXimum?', self.__class__.FetchStruct())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.modulation.maximum.calculate() \n
		Return the current, average, minimum, maximum and standard deviation modulation single value results.
		The values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. The ranges indicated below apply to all results except standard deviation results.
		The minimum for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two.
		...
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:MODulation:MAXimum?', self.__class__.CalculateStruct())
