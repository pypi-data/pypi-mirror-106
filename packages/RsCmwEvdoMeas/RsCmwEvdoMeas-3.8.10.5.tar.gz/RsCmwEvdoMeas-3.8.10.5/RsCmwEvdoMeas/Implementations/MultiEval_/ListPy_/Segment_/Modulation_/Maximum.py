...
class Maximum:
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:MODulation:MAXimum?', self.__class__.FetchStruct())
	...
	class CalculateStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_enum('Ms_Power_1_M_23', enums.ResultStatus2),
		...
		def __init__(self):
			...
			self.Ms_Power_1_M_23: enums.ResultStatus2 = None
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.segment.modulation.maximum.calculate(segment = repcap.Segment.Default) \n
		Returns modulation single value results for segment <no> in list mode. To define the statistical length for AVERage,
		MAXimum, MINimum calculation and enable the calculation of the results use the command method RsCmwEvdoMeas.Configure.
		MultiEval.ListPy.Segment.Modulation.set. The ranges indicated below apply to all results except standard deviation
		results. The minimum for standard deviation results equals 0. The maximum equals the width of the indicated range divided
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:MODulation:MAXimum?', self.__class__.CalculateStruct())
