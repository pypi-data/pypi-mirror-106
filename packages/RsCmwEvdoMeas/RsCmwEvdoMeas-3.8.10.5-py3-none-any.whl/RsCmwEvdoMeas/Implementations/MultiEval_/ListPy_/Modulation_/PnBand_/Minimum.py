...
class Minimum:
	...
	def fetch(self) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:MODulation:PNBand:MINimum?', suppressed)
		return response
	...
	def calculate(self) -> List[float]:
		...
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: ms_power_1_m_23: float Comma-separated list of values, one per active segment. Range: -100 dBm to 50 dBm , Unit: dBm"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:MODulation:PNBand:MINimum?', suppressed)
		return response
