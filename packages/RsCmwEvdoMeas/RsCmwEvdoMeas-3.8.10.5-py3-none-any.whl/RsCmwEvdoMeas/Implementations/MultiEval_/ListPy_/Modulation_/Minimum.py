...
class Minimum:
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct('Ms_Power_1_M_23', DataType.FloatList, None, False, True, 1),
		...
		def __init__(self):
			...
			self.Ms_Power_1_M_23: List[float] = None
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.modulation.minimum.fetch() \n
		Returns modulation single value results in list mode. To define the statistical length for AVERage, MAXimum, MINimum
		calculation and enable the calculation of the results use the command method RsCmwEvdoMeas.Configure.MultiEval.ListPy.
		Segment.Modulation.set. The ranges indicated below apply to all results except standard deviation results. The minimum
		for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two. Exceptions
		are explicitly stated. The values listed below in curly brackets {} are returned for each active segment: {...}seg 1, {...
		}seg 2, ..., {...}seg n. The number of active segments n is determined by method RsCmwEvdoMeas.Configure.MultiEval.ListPy.
		count. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check results
		instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for FetchStruct structure arguments."""
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:MODulation:MINimum?', self.__class__.FetchStruct())
	...
	class CalculateStruct(StructBase):
		...
		__meta_args_list = [
			ArgStruct.scalar_int('Reliability', 'Reliability'),
			ArgStruct('Seg_Reliability', DataType.IntegerList, None, False, True, 1),
			ArgStruct('Evm_Rms', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Evm_Peak', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Merr_Rms', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Merr_Peak', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Perr_Rms', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Perr_Peak', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Iq_Offset', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Iq_Imbalance', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Cfreq_Error', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Trans_Time_Err', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Ms_Power_1_M_23', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Ms_Power_Wideband', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Wav_Quality', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Wav_Qual_Max_Pow', DataType.EnumList, enums.ResultStatus2, False, True, 1),
			ArgStruct('Wav_Qual_Min_Power', DataType.EnumList, enums.ResultStatus2, False, True, 1),
		...
		def __init__(self):
			...
			self.Reliability: int = None
			self.Seg_Reliability: List[int] = None
			self.Evm_Rms: List[enums.ResultStatus2] = None
			self.Evm_Peak: List[enums.ResultStatus2] = None
			self.Merr_Rms: List[enums.ResultStatus2] = None
			self.Merr_Peak: List[enums.ResultStatus2] = None
			self.Perr_Rms: List[enums.ResultStatus2] = None
			self.Perr_Peak: List[enums.ResultStatus2] = None
			self.Iq_Offset: List[enums.ResultStatus2] = None
			self.Iq_Imbalance: List[enums.ResultStatus2] = None
			self.Cfreq_Error: List[enums.ResultStatus2] = None
			self.Trans_Time_Err: List[enums.ResultStatus2] = None
			self.Ms_Power_1_M_23: List[enums.ResultStatus2] = None
			self.Ms_Power_Wideband: List[enums.ResultStatus2] = None
			self.Wav_Quality: List[enums.ResultStatus2] = None
			self.Wav_Qual_Max_Pow: List[enums.ResultStatus2] = None
			self.Wav_Qual_Min_Power: List[enums.ResultStatus2] = None
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.modulation.minimum.calculate() \n
		Returns modulation single value results in list mode. To define the statistical length for AVERage, MAXimum, MINimum
		calculation and enable the calculation of the results use the command method RsCmwEvdoMeas.Configure.MultiEval.ListPy.
		Segment.Modulation.set. The ranges indicated below apply to all results except standard deviation results. The minimum
		for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two. Exceptions
		...
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:MODulation:MINimum?', self.__class__.CalculateStruct())
