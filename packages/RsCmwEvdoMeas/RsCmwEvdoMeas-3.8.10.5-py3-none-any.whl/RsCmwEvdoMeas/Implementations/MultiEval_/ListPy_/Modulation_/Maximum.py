...
class Maximum:
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.modulation.maximum.fetch() \n
		...
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:MODulation:MAXimum?', self.__class__.FetchStruct())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.modulation.maximum.calculate() \n
		Returns modulation single value results in list mode. To define the statistical length for AVERage, MAXimum, MINimum
		calculation and enable the calculation of the results use the command method RsCmwEvdoMeas.Configure.MultiEval.ListPy.
		Segment.Modulation.set. The ranges indicated below apply to all results except standard deviation results. The minimum
		for standard deviation results equals 0. The maximum equals the width of the indicated range divided by two. Exceptions
		...
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:MODulation:MAXimum?', self.__class__.CalculateStruct())
