...
class RangeSerde(ExternalTypeSerializer):
    ...
    @classmethod
    def from_json(cls, as_list: list) -> range:
...
class TimeSerde(ExternalTypeSerializer):
    ...
    @classmethod
    def from_json(cls, isoformat: str) -> time:
...
class PyarrowDataType(ExternalTypeSerializer):
    ...
    _mapping = {data_type.id: data_type for data_type in
                [pyarrow.null(), pyarrow.string(), pyarrow.date32(), pyarrow.date64()]}
...
scalar_types = [
    DateTimeSerde, TimeStampSerde, TimeSerde,
    NumpyBoolSerde,
...
_other_types = [
    TypeSerde, PyarrowDataType,
    TupleSerde,
    SliceSerde, RangeSerde,
    PathSerde,
]
