...
def _serialize_ndarray(aws_credentials: AwsCredentialsFetcher, array: np.ndarray) -> str:
    ...
    import_id = DataTransfer.upload_bytes(Connection.session.upload_config, aws_credentials.get_credentials(), data_bytes)
    return import_id
...
def _serialize_index(aws_credentials: AwsCredentialsFetcher, index: pd.Index) -> PandasIndexMetadata:
...
def _serialize_series(aws_credentials: AwsCredentialsFetcher, series: pd.Series) -> PandasSeriesMetadata:
...
def encode(credentials_fetcher: AwsCredentialsFetcher, o):
