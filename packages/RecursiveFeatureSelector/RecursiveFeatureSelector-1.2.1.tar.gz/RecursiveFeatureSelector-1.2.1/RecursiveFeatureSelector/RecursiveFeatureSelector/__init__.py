...
class RecursiveFeatureSelector:
    ...
    def trial(self, estimator, X, y, cv, scoring, max_round=None, fail_tolerance=None, least_gain=None, max_feats=None, start_from=None, n_digit=4):
        trial_start_time = time.time()
        n_trial = 1
        
        if fail_tolerance != None:    
            if fail_tolerance < 0:
                return 'fail_tolerance(number of chances given Recursive Feature Selector to fail) must be positive number.'
        if fail_tolerance == None:
            fail_tolerance = 1
        ...
        if least_gain != None:
            if least_gain < 0 or isinstance(least_gain, int) != True:
        ...
        while True:
            ...
            if n_trial == 1 and start_from == None:
                for feature in features:
                    cross_val_score_res = cross_val_score(estimator, X[feature], y, cv=cv, scoring=scoring)
