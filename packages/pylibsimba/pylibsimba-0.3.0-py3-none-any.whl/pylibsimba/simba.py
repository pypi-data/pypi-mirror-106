...
class Simbachain(SimbaBase):
    ...
    def __init__(self, endpoint: str, wallet):
        ...
        resp = self.session.get("{}?format=openapi".format(self.endpoint))
        swagger = resp.json()
    ...
    def check_transaction_status(self, txn_id) -> dict:
        ...
        Returns:
            An object with status details
        """
        return self.check_transaction_status_from_object(
            self.get_transaction_status(txn_id)
        )
    ...
    def call_method_with_file(self, method: str, parameters: dict, files: dict) -> TwoPartTransactionResponse:
        """
        ...
        Args:
            method : the method to call
            parameters : the parameters for the method
            files : a dict of file like objects to be submitted with the API call
        Returns:
            A transaction id
        """
        if not self.wallet:
        ...
        #     elif isinstance(files[i], io.BufferedReader):
        #         file_handles["file[{}]".format(i)] = files[i]

        return self._send_method_request(method, form_data, files)
                    ...
                    if len(data2['errors']):
                        if 'code' in data2['errors'][0]:
                            if data2['errors'][0]['code'] == '15001':
                                if 'extra_detail' in data2['errors'][0]:
                                    if 'suggested_nonce' in data2['errors'][0]['extra_detail']:
                                        new_nonce = data2['errors'][0]['extra_detail']['suggested_nonce']
    ...
    def send_transaction_request(self, url: str) -> PagedResponse:
        ...
        Raises:
            GetTransactionsException: If there is a problem getting the transaction
        """
        response = self.session.get(url, headers=self.api_auth_headers())
    ...
    def get_organisations(self, base_url='https://api.simbachain.com/v1/') -> PagedResponse:
        ...
        response = self.session.get("{}organisations/".format(base_url), headers=self.api_auth_headers())
        json_resp = response.json()
    ...
    def get_bundle_for_transaction(self, transaction_id_or_hash: str, stream: bool = True) -> requests.models.Response:
        ...
        response = self.session.get(url, headers=self.api_auth_headers(), stream=stream)
