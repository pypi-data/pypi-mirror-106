...
def post_slack(state, stack, _data, plan_output=None, color="#27d200", 
               webhook=None, pipeline_url=None, job_url=None, limit=4000,
               author=None):
    if state == "plan":
        data = {
            'text': f"Terraform {state} for stack *{stack}*",
            'username': 'Terraform',
...
def tf_plan(tf, workspace_name, stack="", webhook=None, tfvar=False, 
    ...
    if not tfvar:
        return_code, stdout, stderr = tf.plan(
            out=f"{workspace_name}_{stack}.tfplan")
    else:
        return_code, stdout, stderr = tf.plan(
    ...
    if return_code == 2:
        ...
        if webhook is not None:
            post_slack("plan", stack, stdout, plan_out,
...
def tf_apply(tf, workspace_name, stack="", skip_plan=True, webhook=None, 
    ...
    if not tfplan:
        ...
        return_code_apply, stdout_apply, stderr_apply = tf.apply(
            f"{workspace_name}_{stack}.tfplan",
            var=None,
            skip_plan=skip_plan)
    ...
    if return_code_apply == 0:
        ...
        color = parse_plan(plan_out)
        if webhook is not None:
            post_slack("apply", stack, stdout_apply, plan_out,
                       color, webhook, pipeline_url, job_url, limit, author)
    else:
        print("ERROR:\n=======\n")
        print(stderr_apply)
