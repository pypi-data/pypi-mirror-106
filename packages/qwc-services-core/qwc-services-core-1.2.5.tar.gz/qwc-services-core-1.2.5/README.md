...
☐ todo ☑ completed ☒ postponed ❌ skip

TODO:
☑ qwc-admin-gui ENV: mailer + registration
☑ qwc-registration-gui
    -> config + workflow
☐ Development -> clone-qwc-services.sh
☐ helper scripts
...
cd qwc-services/
./qwc-services-core/scripts/gitall status
./qwc-services-core/scripts/gitall pull
...
X## Docker
    -> qwc-docker
    qwc2-demo-app -> no login/edit
## Local services
    -> Development

X# Configuration
X## Configuration database
    CONFIGDB_URL || service=qwc_configdb
    custom user fields -> USER_INFO_FIELDS
X## ENV / Service configurations
    -> ENV / docker-compose.yml
    table ENV<->service + desc
    mailer?
X### QWC2 / Map Viewer
    JS + assets
    config.json
    themesConfig.json
#### QWC2 Update -> build + re-copy
...
# DB Migrations
    update existing configdb
    qwc-config-db
        sqlalchemy.url = postgresql:///?service=qwc_configdb

# Development
    use clone-qwc-services.sh



resources/permissions -> link to core README from admin/config services
...
# TODO: optional services: api-gateway, print, legend, permalink, ...
# TODO: scripts clone

# TODO: permalink, legend services, LDAP Auth service
