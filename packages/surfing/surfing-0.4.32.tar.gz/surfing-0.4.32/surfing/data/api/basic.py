...
class BasicDataApi(metaclass=Singleton):
        ...
        with BasicDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    ...
                    EMailParserExec.manager_id == manager_id,
                )
                return pd.read_sql(query.statement, query.session.bind)
            except Exception as e:
    ...
    def get_product_details(self):
        try:
            ...
            fund_info_1 = fund_info[~fund_info.agency_name.isin(special_list)]
            fund_info_2 = fund_info[fund_info.agency_name.isin(special_list)]
            asset_list = fund_info.stg_type.unique().tolist()
            result = {}
            for asset_id in asset_list:
                result[asset_id] = []
                _df = fund_info_1[fund_info_1.stg_type == asset_id]
                for r in _df.itertuples():
            ...
            for agency_i in special_list:
                ...
                result[_agency_i] = []
                _df = fund_info_2[fund_info_2.agency_name == agency_i]
                for r in _df.itertuples():
                    result[_agency_i].append([r.fund_id, r.desc_name])
            return result
        except Exception as e:
            print(f'failed to get_fof_transit_money <err_msg> {e} from {AssetInfo.__tablename__}')
    ...
    def get_product_price(self, begin_date, end_date, time_para, product_list: Tuple[str] = (), price_type:str='价格线', benchmark_id:str='hs300'):#'价格比线'
        try:
            ...
            if price_type == '价格线':
                ...
                index_price = self.get_index_price_dt(index_list=[benchmark_id],start_date=begin_date,end_date=end_date)
    ...
    def get_all_asset_price(self, start_date:str='', end_date:str='', id_list:list=[]):
        try:
            result = []
            index_price = self.get_index_price_dt(start_date=begin_date, end_date=end_date, index_list=id_list)
            if not index_price.empty:
                index_price = index_price.pivot_table(index='datetime', values='close', columns='index_id')
                result.append(index_price)
            fund_nav = self.get_fund_nav_with_date(start_date=begin_date, end_date=end_date, fund_list=id_list)
            ...
            hf_fund_nav = raw.get_hf_fund_nav(fund_ids=id_list, start_date=begin_date, end_date=end_date)
            ...
            print(f'Failed to get data <err_msg> {e} from basic.get_all_asset_price')
            return []
