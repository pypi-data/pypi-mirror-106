...
class FOFDataManagerLite:
    ...
    def _calc_virtual_net_value(manager_id, fof_id, fund_list, asset_allocation=None):
        ...
        for row in nav.itertuples():
            ...
            if df.empty:
                continue
            # TODO: 处理不是简单字符串的业绩报酬比例
            v_nav = FOFDataManagerLite._do_calc_v_net_value(row.nav, df.unit_total, row.acc_net_value, df.water_line, float(incentive_fee_ratio[row.fund_id]), v_nav_decimals[row.fund_id])
    ...
    def dump_fof_nav_and_pos_to_db(self, manager_id: str, fof_id: str, debug_mode=False) -> bool:
        ...
        #                 'mv': mv,
        #                 'nav': round(res_status.last_unit_nav, 4),
        ...
        for fund_id in fund_list:
            try:
                ...
                if not fund_auv.empty:
                    ...
                    real_mv = mv + redempt_aa.amount.sum()
    ...
    def pull_hedge_fund_nav(self):
        ...
        for one in res.json()['data']:
            ...
            try:
                ...
                nav_df: Optional[pd.DataFrame] = fof_nav_r.read_navs_and_dump_to_db()
    ...
    def get_fof_nav(manager_id: str, fof_id_list: Tuple[str] = (), *, ref_index_id: str = '', ref_fund_id: str = '') -> Optional[pd.DataFrame]:
        ...
        if fof_nav is None:
            ...
            existed_fof_id_list = fof_nav.fof_id.unique()
            fof_nav = fof_nav.drop(columns=['update_time', 'create_time', 'is_deleted'])
            if fof_nav_public is not None:
                fof_nav = fof_nav.combine_first(fof_nav_public[~fof_nav_public.fof_id.isin(existed_fof_id_list)])
        if fof_nav is not None:
            if ref_index_id:
    ...
    def virtual_backtest(manager_id: str, hedge_fund_ids: Dict[str, float], ref_fund_ids: Dict[str, str], start_date: datetime.date, end_date: Optional[datetime.date] = None, benchmark_ids: Tuple[str] = ()) -> Optional[Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]]:
        ...
        try:
            ...
            # TODO: incentive_fee_str需要根据incentive_fee_type来使用
            fund_nav = FOFDataManagerLite._do_calc_v_net_value_by_nav(adj_nav, adj_nav, adj_nav.loc[adj_nav.index >= start_date, :].bfill().iloc[0, :], fund_info.incentive_fee_str.fillna(DEFAULT_INCENTIVE_RATIO), fund_info.v_nav_decimals.fillna(DEFAULT_DECIMAL))
    ...
    def virtual_backtest_sub(manager_id: str, hedge_fund_ids: Dict[str, str], start_date: datetime.date, end_date: Optional[datetime.date] = None) -> Optional[Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]]:
        ...
        try:
            ...
            # TODO: incentive_fee_str需要根据incentive_fee_type来处理
            fund_nav = FOFDataManagerLite._do_calc_v_net_value_by_nav(adj_nav, adj_nav, fund_info.water_line.fillna(INIT_NAV), fund_info.incentive_fee_str.fillna(DEFAULT_INCENTIVE_RATIO), fund_info.v_nav_decimals.fillna(DEFAULT_DECIMAL))
