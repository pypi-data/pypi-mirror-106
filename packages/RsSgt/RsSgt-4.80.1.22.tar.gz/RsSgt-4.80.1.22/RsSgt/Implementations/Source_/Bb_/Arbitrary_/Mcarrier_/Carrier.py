...
class Carrier:
	...
	def get_mode(self) -> enums.ArbMultCarrSpacMode:
		...
		The command sets the carrier frequency mode. \n
			:return: mode: EQUidistant| ARBitrary EQUidistant Sets an equidistant carrrier spacing. The carrier frequency in the carrier table is not configurable. ARBitrary Enables you to specify the carrier frequency in the carrier table. Carrier spacing is irrelevant.
		"""
	...
	def set_mode(self, mode: enums.ArbMultCarrSpacMode) -> None:
		...
		The command sets the carrier frequency mode. \n
			:param mode: EQUidistant| ARBitrary EQUidistant Sets an equidistant carrrier spacing. The carrier frequency in the carrier table is not configurable. ARBitrary Enables you to specify the carrier frequency in the carrier table. Carrier spacing is irrelevant.
		"""
	...
	def get_spacing(self) -> float:
		...
		The command sets the frequency spacing between adjacent carriers of the multi carrier waveform.
		The carriers are generated symmetrically around the RF carrier. The maximum carrier spacing is limited to Carrier spacing
		= Total baseband bandwidth /(Number of carriers - 1) . The total baseband bandwidth is 60, 120 or 160 MHz depending on
		the installed options. .
			INTRO_CMD_HELP: Note:In order to avoid wrap-around problems, the effective 'Carrier Spacing' might be slightly modified. The 'Carrier Spacing' is rounded in that way that the carrier closest to the center RF frequency shows no phase jump assuming that the carrier is unmodulated. \n
			- For odd number of carriers: RoundedCarrierSpacing=1/OutputSignalDuration* round(CarrierSpacing * OutputSignalDuration) ;
			- For even number of carriers: RoundedCarrierSpacing=2/OutputSignalDuration*round(0.5 *CarrierSpacing * OutputSignalDuration) ; \n
			:return: spacing: float Range: 0.0 to 120E6, Unit: Hz
		"""
	...
	def set_spacing(self, spacing: float) -> None:
		...
		the installed options. .
			...
			- For odd number of carriers: RoundedCarrierSpacing=1/OutputSignalDuration* round(CarrierSpacing * OutputSignalDuration) ;
			- For even number of carriers: RoundedCarrierSpacing=2/OutputSignalDuration*round(0.5 *CarrierSpacing * OutputSignalDuration) ; \n
			:param spacing: float Range: 0.0 to 120E6, Unit: Hz
		"""
