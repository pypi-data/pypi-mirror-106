...
class Mcarrier:
	"""Mcarrier commands group definition. 38 total commands, 9 Sub-groups, 4 group commands
	...
	def get_clock(self) -> float:
		...
		The command queries the resulting sample rate at which the multi carrier waveform is output by the arbitrary waveform
		generator. The output clock rate depends on the number of carriers, carrier spacing and input sample rate of the leftmost
		or rightmost carriers. \n
	...
	def get_ofile(self) -> str:
		...
		This command defines the output file name for the multi carrier waveform. This file name is used when a waveform is
		calculated (command method RsSgt.Source.Bb.Arbitrary.Mcarrier.Cload.set or method RsSgt.Source.Bb.Arbitrary.Mcarrier.
		Create.set) . The file extension is *.wv. \n
			:return: ofile: string
		"""
	...
	def get_samples(self) -> int:
		...
		The command queries the resulting file size. The file size is returned in samples. \n
			:return: samples: integer Range: 0 to INT_MAX
		"""
