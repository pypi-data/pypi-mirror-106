...
class TDA():
    ...
    def SetDataset(self, datasetName):
        ...
        if self.debug:
            print(f"\033[0;37;44m[SET_DATASET] dataset: {self.datasetName} , service: {self.host}\033[0m")

        self.dataset = Dataset(self.T_key, datasetName, self.host)
    ...
    def GetData(self, offset=0, limit=100, metaKey="", metaValue="", label="", sensor=""):
        ...
        :param metaKey: which metadata key you would like to use to search
        :param metaValue: set metadata value to your search  metadata key, if metaKey is not set, metaValue will be ignore
        :param label: search by label
        :param sensor: search by sensor
        :param all above criteria will be juxtaposition
        """
        if limit > 1000:
            raise Exception("limit must less than 1000")

        if (metaKey == "" and metaValue != "") or (metaKey != "" and metaValue == ""):
            raise Exception("both metaKey and metaValue are must set at the same time")

        return self.dataset.GetData(offset, limit, metaKey=metaKey, metaValue=metaValue, label=label, sensor=sensor, debug=self.debug)
