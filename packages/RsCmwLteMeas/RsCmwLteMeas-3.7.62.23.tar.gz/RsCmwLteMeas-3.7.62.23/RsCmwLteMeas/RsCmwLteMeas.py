...
class RsCmwLteMeas:
	"""839 total commands, 7 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.62.0023'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '3.7.62.0023'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.62.0023'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwLteMeas version failed. Current version: '3.7.62.0023', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 8 Sub-classes, 4 commands."""
	...
	@property
	def multiEval(self):
		"""multiEval commands group. 16 Sub-classes, 3 commands."""
	...
	def srs(self):
		...
		if not hasattr(self, '_srs'):
			from .Implementations.Srs import Srs
