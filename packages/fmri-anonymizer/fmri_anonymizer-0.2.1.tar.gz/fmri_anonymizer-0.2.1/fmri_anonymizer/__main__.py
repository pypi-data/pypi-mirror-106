...
def main(argv=None):
    ...
    if deid_flag:
        ...
        if dcm_files.__len__() > 0:
            ...
            with alive_bar(len(dcm_files)) as bar:
                for dcm_file in range(dcm_files):
    ...
    if deface_flag:
        ...
        for k in nifti_defaced:
            ...
            #image_multidimensional = sitk.GetArrayViewFromImage(nifti_image)
            ...
            with alive_bar(number_slices) as bar:
                for slice in range(number_slices):
                    ...
                    filtered_image = sitk.GetImageFromArray(np.swapaxes(nifti_data[:,:,:,slice], 0, 2).astype(np.uint16))
