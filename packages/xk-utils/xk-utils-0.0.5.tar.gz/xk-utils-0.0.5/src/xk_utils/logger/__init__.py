...
def get_file_handler(filename, level, logging_format):
    file_handler = TimeFileHandler(filename)
...
def get_logger(key, name='root', write_file=False, file_path=None, send_mail=False, host=None, port=None,
    ...
    if send_mail:
        if host is None or port is None or db is None:
            raise Exception('send_mail 为True 时，需要指定redis的来接受邮件缓存数据'
                            'host: redis host'
                            'port: redis port'
                            'db: redis db'
