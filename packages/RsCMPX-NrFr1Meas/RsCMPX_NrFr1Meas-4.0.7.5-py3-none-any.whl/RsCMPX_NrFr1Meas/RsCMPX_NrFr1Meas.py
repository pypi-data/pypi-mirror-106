...
class RsCMPX_NrFr1Meas:
	"""806 total commands, 4 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX500/CMP/CMW, SupportedIdnPatterns = CMX/CMP/CMW, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.7.0005'"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_NrFr1Meas version failed. Current version: '4.0.7.0005', minimum required version: '{min_version}'")
	...
	@property
	def nrSubMeas(self):
		"""nrSubMeas commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_nrSubMeas'):
			from .Implementations.NrSubMeas import NrSubMeas
