...
class Draw:
    ...
    def add_radar(self, x_labels, line_labels, line_data, sub_title='', ylim=(None, None), linewidth=1, markersize=4,
                  ...
                  frame='polygon', radii=None, title_pad=None, angle=0):
        """
        增加一个雷达图绘制
        :param x_labels: [标签名,..]
    ...
    def add_scatter(self, xyt_scatter, scatter_labels=None, scatter_c=None, scatter_ms=4, scatter_marker=None,
                    annotate_fs=3, sub_title='', sub_title_fs=None, xlabel=None, ylabel=None, n=1):
        """
        增加一个可以标记点的二维散点图
    ...
    def add_Ftest(self, x, yticks, alpha=0.05, asc=True, x_sort=True, color=None, sub_title='', sub_title_fs=None,
                  xlabel=None, ylabel=None, lw=3, n=1, p_vaule_f=lambda t, p: t.replace('{p}', str(round(p, 4)))):
        """
        增加一个 Friedman 检验图
        :param x: [[结果1,..],..]=[一个方法结果,..]; 每个方法的所有结果, 每列结果对应
        :param yticks: [方法名称,..]; 每个方法的名称
        :param alpha: float; 显著性水平
        :param asc: bool or [bool,..]; 结果是顺序还是倒叙, list时bool的数量等于每个方法的结果数量
        ...
        for n in range(len(x[0])):  # 对于每个数据集
            xi_res_asc_L = [[i, xi[n], 1 if a else -1] for i, (xi, a) in enumerate(zip(x, asc))]
        ...
        k = len(x)  # 算法个数
        N = len(x[0])  # 数据集个数
        qα = qsturng(p=1 - alpha, r=k, v=float('inf')) / 2 ** 0.5  # Tukey 分布的临界值
        CD = qα * (k * (k + 1) / (6 * N)) ** 0.5  # 临界值域
        # Friedman test
        ...
        for i, (avgno_tick, c) in enumerate(zip(x_avgno_tick_L, nc_L)):
            ...
            x1 = avgno - CD / 2
            x2 = avgno + CD / 2
            ax.plot([x1, x2], [yticks[i], yticks[i]], lw=lw, c=c, zorder=1, dash_capstyle='round')
            ax.scatter([avgno], [yticks[i]], s=lw * 6, c='black', zorder=2)
            ax.vlines(x2, yticks[-1], yticks[i], colors=c, ls='--', lw=lw / 6, zorder=3)
        # 绘制坐标轴标记
...
if __name__ == '__main__':
    ...
    draw.add_radar(labels, [f'{i}{i}' for i in range(4)], data, 'radar', n=1, radii=(0, 0.2, 0.4, 0.6, 0.8, 1),
                   fill_alpha=0.1, title_pad=20, frame='polygon', angle=30)
    # 三维透视图
    x_num = 8
    y_num = 6
    ...
    # 散点图
    draw.add_scatter((np.random.rand(4, 30, 3) * 2).tolist(), scatter_labels=['11', '22', '33', '44'], n=6,
                     sub_title='scatter')
    # Friedman 检验图
    alpha = 0.05
    print(draw.add_Ftest(x=[  # 数据参考 《机器学习》周志华 p42
        ...
        [3, 2.5, 3, 3],
        [1, 1, 1, 1],
    ], yticks=['B', 'C', 'A'], xlabel='Rank', n=7, alpha=alpha,
