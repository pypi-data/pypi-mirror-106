...
class TestStrategiesMergeList(unittest.TestCase):
    @patch(f"{class_path}.update_node_version", side_effect=lambda _v, n: n)
    def test_merge_new_node(self, *args):
        old_node = {
            'term': {'@id': 'old-term'},
            'value': 1
        }
