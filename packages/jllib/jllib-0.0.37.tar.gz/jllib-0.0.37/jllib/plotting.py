...
def do_cm_reduce_window_slices(y_true: np.ndarray, y_pred: np.ndarray, model_output_directory):
    from sklearn.metrics import classification_report, confusion_matrix

    assert isinstance(y_true, np.ndarray), 'Param `y_true` must be np.ndarray but is ' + str(type(y_true))
    assert isinstance(y_pred, np.ndarray), 'Param `y_pred` must be np.ndarray but is ' + str(type(y_pred))
