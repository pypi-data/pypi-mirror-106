...
def validate(
    ...
    def decorate_validate_request(func):
        ...
        def validate_request(*args, **kwargs):
            ...
            try:
                ...
                current_app.logger.error(
                    "url：{} validation error: {}".format(request.url, json.dumps(err))
                )
            # validate data
            ...
            try:
                ...
                err["data"] = ve.errors()
                current_app.logger.error(
                    "url：{} body validation error: {}".format(request.url, json.dumps(err))
                )
        ...
        for schema, name in zip((query, data, resp), ("query", "data", "resp")):
            if schema:
