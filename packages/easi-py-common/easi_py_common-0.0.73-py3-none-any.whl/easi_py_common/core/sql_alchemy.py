...
class RoutingSession(orm.Session):
    ...
    def get_bind(self, mapper=None, clause=None):
        ...
        if mapper is not None:
            full_class_name = "%s.%s" % (mapper.class_.__module__, mapper.class_.__name__)

        try:
            state = get_state(self.app)
        except (AssertionError, AttributeError, TypeError) as err:
            self.app.logger.info.warning("cant get configuration. default bind. Error:" + err)
        ...
        # 操作被改动过的对象，也使用 master
            ...
            if self.app.debug:
                ...
                self.app.logger.info(
                    "Connecting -> %s MASTER( %s - %s )" % (action, mapper.class_, bind_key)
                )
        ...
        # 其他请求使用 slave
            ...
            if self.app.debug:
                self.app.logger.info(
                    "Connecting -> READ SLAVE( %s - %s )" % (mapper.class_, bind_key)
                )
            return state.db.get_engine(self.app, bind=bind_key)
...
class RouteSQLAlchemy(SQLAlchemy):
    ...
    def get_engine(self, app=None, bind=None):
        ...
        if (
            bind == FIXED_SLAVE_BINDS
            and "SQLALCHEMY_BINDS_FIXED_SLAVE_COUNT" in current_app.config
            and current_app.config["SQLALCHEMY_BINDS_FIXED_SLAVE_COUNT"] > 0
        ):
            # 随机选则
            fixed_bind = "{}{}".format(
                FIXED_SLAVE_BINDS,
