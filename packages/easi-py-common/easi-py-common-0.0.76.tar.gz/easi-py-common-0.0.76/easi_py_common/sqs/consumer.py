...
from easi_py_common.sqs import constants, utils
...
class ConsumerStart:
    ...
    def wrapper_call(self, message, call, client, qurl, address, msg_lifetime, get_message_time):
        with self.app.app_context():
            try:
                ...
                sub_log_ret["receive_success_time"] = utils.format_timestamp(
    ...
    def __init_sub_log(self, msg_id, address, qurl, body, get_message_time):
        ...
        return {
            ...
            "message_wait_time": -1,
            "receive_time": utils.format_timestamp(
                utils.get_utc_timestamp(), formats="%Y.%m.%d %H:%M:%S.%f"
            )[:-3],
    ...
    def __save_sub_log(self, sub_log_ret):
        try:
            ...
            if (
                not can_bulk_save
                and sub_log_len < ConsumerLogConstants.CONSUMER_LOG_ES_BULK_BUFFER_SIZE
            ):
                return
            # 如果可以保存 或者 是200的倍数 则尝试进行保存
            if (
                can_bulk_save
                or sub_log_len % 200 == 0
                or sub_log_len >= ConsumerLogConstants.CONSUMER_LOG_MAX_SLEEP_ES_BULK_BUFFER_SIZE
            ):
...
class ConsumerRun:
    ...
    def run(
        self, cpu_count=cpu_count() * 2, db_pool_size=None, db_pool_size_every=2, db_pool_timeout=15
    ):
        ...
        for handler in phandlers:
            p = Process(
                target=self.start, args=(handler, db_pool_size, db_pool_size_every, db_pool_timeout)
            )
            p.start()
            self.process_list.append(p)

        print(
            "CPU核数:{0}, 队列数:{1}, 进程数:{2}, Main Pid:{3}".format(
