...
from easi_py_common.core.redis import redis_ro_store, redis_store
from easi_py_common.core.sql_alchemy import db
from easi_py_common.id import IdGenerator
...
def wrap_route(self, rule=None, menu=None, **options):
    if rule is not None:
        ...
        if self.url_prefix:
            url_path = "{0}{1}".format(
                self.url_prefix, rule if rule.startswith("/") else "/{0}".format(rule)
            )
