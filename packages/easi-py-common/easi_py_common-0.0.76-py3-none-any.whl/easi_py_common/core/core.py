...
class OkPage(GenericModel, Generic[DataT]):
    code: int = 0
    data: List[DataT]
    total: int

    def __init__(self, data: Any = None, total: int = 0):
        if data is None:
            data = []
        super(OkPage, self).__init__(data=data)
        self.data = data
        self.total = total
...
class OkNext(GenericModel, Generic[DataT]):
    ...
    @staticmethod
    def build_not_data():
        return OkNext(data=[], next_key=None)

    @staticmethod
    def build_encode_next_key(data: List[DataT], next_key: str):
        return OkNext(data=data, next_key=encode_list_next_key(next_key))

    @staticmethod
    def build_encode_next_key_int(data: List[DataT], next_key: int):
        return OkNext(data=data, next_key=encode_list_next_key(str(next_key)))
        ...
        def wrapper(*args, **kwargs):
            ...
            if err:
                ...
                return make_response(
                    jsonify({"code": 100, "message": "validation error", "validation_error": err}),
                    status_code,
                )
