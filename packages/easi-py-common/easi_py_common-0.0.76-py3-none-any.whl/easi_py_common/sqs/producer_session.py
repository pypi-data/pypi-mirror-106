...
class ProducerSession:
    ...
    def add(self, address, msg, key="", group_id="", delay_seconds=0, sign=None):
        ...
        self.msgs.append(
            dict(address=address, body=msg, key=key, group_id=group_id, delay_seconds=delay_seconds)
        )
    ...
    def commit(self):
        ...
        for address, msg_list in msg_group_map.items():
            ...
            if msg_buffer_list:
                pool.spawn(
                    self.sqs_client.send_message_batch, msg_buffer_list, cb=self.cb, app=self.app
                )
