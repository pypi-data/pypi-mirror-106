...
class PyMongoDAO(DAO):
    ...
    def find_one_by_id(self, _id):
        ...
        try:
            ...
            if result is not None:
                ...
                raise ObjectNotFoundError(self.collection.name, {'_id': _id})
    ...
    def find_one_by_key(self, key):
        ...
        :raise: ObjectNotFoundError
        """
        return self.find_one_by_id(key)
    ...
    def find(self, query=None, projection=None, skip=0, limit=0, sort=None):
        ...
        :param projection: of field names that should be returned in the result set or a dict specifying the fields to
        include or exclude.
    ...
    def delete(self, _id):
        ...
        try:
            ...
            self.logger.error('Failed to delete document {\'_id\': {}}'.format(_id))
            raise PersistenceError('Unexpected error', e)

    def update_from_dict(self, _id, change_set):
        ...
        try:
            result: UpdateResult = self.collection.update_one({'_id': _id}, {'$set': self._encode_dict(change_set)})
    ...
    def update(self, _id, **change_set):
        ...
        # TODO : Verify if the change_set is merge with persistent data or replace it.
        try:
            result = self.collection.update_one({'_id': _id}, {'$set': change_set})
            return result.modified_count
        except PyMongoError as e:
            self.logger.error('Failed to update document {\'_id\': {}} applying change set {}'.format(_id, change_set))
            raise PersistenceError('Unexpected error', e)

    def insert(self, data_set: dict):
        """Inserts the data set as a new document
        :param data_set: a dictionary defining the document
    ...
    def lookup(self, query, skip=0, limit=None, sort=None):
