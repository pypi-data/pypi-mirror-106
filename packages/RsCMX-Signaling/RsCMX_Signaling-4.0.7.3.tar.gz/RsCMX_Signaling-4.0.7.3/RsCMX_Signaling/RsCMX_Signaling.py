...
class RsCMX_Signaling:
	"""305 total commands, 12 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX/CMW, SupportedIdnPatterns = CMX/CMW, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.7.0003'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.7.0003'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.7.0003'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMX_Signaling version failed. Current version: '4.0.7.0003', minimum required version: '{min_version}'")
	...
	def fetch(self):
		...
		if not hasattr(self, '_fetch'):
			...
			self._fetch = Fetch(self._core, self._base)
		return self._fetch
	...
	def signaling(self):
		...
		if not hasattr(self, '_signaling'):
			from .Implementations.Signaling import Signaling
