...
class En:
	"""En commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("en", core, parent)
	...
	def get_flexible(self) -> FlexibleStruct:
		...
		Snippet: value: FlexibleStruct = driver.route.scenario.en.get_flexible() \n
		Activates the scenario '5CC - nx4 nx4 nx2 nx2 nx2' and selects the signal paths. For possible parameter values, see
		'Values for Signal Path Selection'. \n
			:return: structure: for return value, see the help for FlexibleStruct structure arguments.
		"""
		return self._core.io.query_struct('ROUTe:LTE:SIGNaling<Instance>:SCENario:EN:FLEXible?', self.__class__.FlexibleStruct())

	def set_flexible(self, value: FlexibleStruct) -> None:
		"""SCPI: ROUTe:LTE:SIGNaling<instance>:SCENario:EN[:FLEXible] \n
		Snippet: driver.route.scenario.en.set_flexible(value = FlexibleStruct()) \n
		Activates the scenario '5CC - nx4 nx4 nx2 nx2 nx2' and selects the signal paths. For possible parameter values, see
		'Values for Signal Path Selection'. \n
			:param value: see the help for FlexibleStruct structure arguments.
		"""
		self._core.io.write_struct('ROUTe:LTE:SIGNaling<Instance>:SCENario:EN:FLEXible', value)
