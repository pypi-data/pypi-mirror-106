...
class Path:
	...
	def get(self, path=repcap.Path.Default) -> GetStruct:
		...
		Snippet: value: GetStruct = driver.sense.iqOut.pcc.path.get(path = repcap.Path.Default) \n
		...
		return self._core.io.query_struct(f'SENSe:LTE:SIGNaling<Instance>:IQOut:PCC:PATH{path_cmd_val}?', self.__class__.GetStruct())
