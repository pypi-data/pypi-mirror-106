...
class Setb:
	"""Setb commands group definition. 17 total commands, 3 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("setb", core, parent)

	@property
	def pusch(self):
		"""pusch commands group. 1 Sub-classes, 1 commands."""
		if not hasattr(self, '_pusch'):
			from .Setb_.Pusch import Pusch
	...
	def apPower(self):
		...
		if not hasattr(self, '_apPower'):
			from .Setb_.ApPower import ApPower
	...
	def clone(self) -> 'Setb':
		...
		new_group = Setb(self._core, self._base.parent)
