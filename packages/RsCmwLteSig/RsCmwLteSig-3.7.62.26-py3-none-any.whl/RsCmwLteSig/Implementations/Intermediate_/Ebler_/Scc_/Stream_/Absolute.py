...
class Absolute:
	...
	def fetch(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> FetchStruct:
		"""SCPI: FETCh:INTermediate:LTE:SIGNaling<instance>:EBLer:SCC<Carrier>:STReam<Stream>:ABSolute \n
		Snippet: value: FetchStruct = driver.intermediate.ebler.scc.stream.absolute.fetch(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:INTermediate:LTE:SIGNaling<Instance>:EBLer:SCC{secondaryCompCarrier_cmd_val}:STReam{stream_cmd_val}:ABSolute?', self.__class__.FetchStruct())
