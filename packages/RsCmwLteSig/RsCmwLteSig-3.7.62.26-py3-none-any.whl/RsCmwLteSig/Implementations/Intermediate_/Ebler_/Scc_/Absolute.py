...
class Absolute:
	...
	def fetch(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.intermediate.ebler.scc.absolute.fetch(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default) \n
		Returns the absolute overall results of the BLER measurement for the sum of all DL streams of one carrier. The number to
		...
		return self._core.io.query_struct(f'FETCh:INTermediate:LTE:SIGNaling<Instance>:EBLer:SCC{secondaryCompCarrier_cmd_val}:ABSolute?', self.__class__.FetchStruct())
