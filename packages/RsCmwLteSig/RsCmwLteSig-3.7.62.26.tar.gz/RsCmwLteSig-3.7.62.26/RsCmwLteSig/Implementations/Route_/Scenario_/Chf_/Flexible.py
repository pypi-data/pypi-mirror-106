...
class Flexible:
	...
	def get_internal(self) -> InternalStruct:
		...
		Snippet: value: InternalStruct = driver.route.scenario.chf.flexible.get_internal() \n
		Activates the scenario '3CC - Fading - nx4 nx2 nx2' with internal fading and selects the signal paths. For possible
		...
		"""
		return self._core.io.query_struct('ROUTe:LTE:SIGNaling<Instance>:SCENario:CHF:FLEXible:INTernal?', self.__class__.InternalStruct())

	def set_internal(self, value: InternalStruct) -> None:
		"""SCPI: ROUTe:LTE:SIGNaling<instance>:SCENario:CHF[:FLEXible]:INTernal \n
		Snippet: driver.route.scenario.chf.flexible.set_internal(value = InternalStruct()) \n
		Activates the scenario '3CC - Fading - nx4 nx2 nx2' with internal fading and selects the signal paths. For possible
		parameter values, see 'Values for Signal Path Selection'. \n
			:param value: see the help for InternalStruct structure arguments.
		"""
		self._core.io.write_struct('ROUTe:LTE:SIGNaling<Instance>:SCENario:CHF:FLEXible:INTernal', value)
