...
class Downlink:
	...
	def set(self, structure: DownlinkStruct, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDCHannels:LAA:FBURst:FSUBframes:MCLuster:DL<Stream> \n
		Snippet: driver.configure.connection.scc.udChannels.laa.fburst.fullSubFrames.mcluster.downlink.set(value = [PROPERTY_STRUCT_NAME](), secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:FBURst:FSUBframes:MCLuster:DL{stream_cmd_val}', structure)

	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDCHannels:LAA:FBURst:FSUBframes:MCLuster:DL<Stream> \n
		Snippet: value: DownlinkStruct = driver.configure.connection.scc.udChannels.laa.fburst.fullSubFrames.mcluster.downlink.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with multi-cluster allocation, for LAA, fixed bursts. The <Cluster> setting
		applies to all subframes of the burst and to all DL streams. The other settings apply to subframes with full allocation
		and DL stream <s>. The allowed input ranges have dependencies and are described in the background information, see
		'User-Defined Channels for LAA' and especially Table 'RBG parameters'. \n
		...
		secondaryCompCarrier_cmd_val = self._base.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:FBURst:FSUBframes:MCLuster:DL{stream_cmd_val}?', self.__class__.DownlinkStruct())
