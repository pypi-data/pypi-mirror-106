...
class Relative:
	...
	def fetch(self, stream=repcap.Stream.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.ebler.pcc.harq.stream.subframe.relative.fetch(stream = repcap.Stream.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:LTE:SIGNaling<Instance>:EBLer:PCC:HARQ:STReam{stream_cmd_val}:SUBFrame:RELative?', self.__class__.FetchStruct())
