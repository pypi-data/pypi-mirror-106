...
class RsCmwLteSig:
	"""1279 total commands, 15 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.62.0026'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwLteSig._driver_options, options, direct_session)
		self._core.driver_version = '3.7.62.0026'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwLteSig':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwLteSig session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwLteSig with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.62.0026'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwLteSig version failed. Current version: '3.7.62.0026', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 23 Sub-classes, 1 commands."""
	...
	@property
	def sense(self):
		"""sense commands group. 13 Sub-classes, 1 commands."""
	...
	@property
	def prepare(self):
		"""prepare commands group. 2 Sub-classes, 0 commands."""
	...
	def a(self):
		...
		if not hasattr(self, '_a'):
			from .Implementations.A import A
	...
	def b(self):
		...
		if not hasattr(self, '_b'):
			from .Implementations.B import B
	...
	def scc(self):
		...
		if not hasattr(self, '_scc'):
			from .Implementations.Scc import Scc
	...
	@property
	def ebler(self):
		"""ebler commands group. 5 Sub-classes, 3 commands."""
		if not hasattr(self, '_ebler'):
			from .Implementations.Ebler import Ebler
			self._ebler = Ebler(self._core, self._base)
		return self._ebler

	@property
	def intermediate(self):
		"""intermediate commands group. 1 Sub-classes, 0 commands."""
	...
	def clone(self) -> 'RsCmwLteSig':
		"""Creates a deep copy of the RsCmwLteSig object. Also copies:
		...
		cloned = RsCmwLteSig.from_existing_session(self.get_session_handle(), self._options)
