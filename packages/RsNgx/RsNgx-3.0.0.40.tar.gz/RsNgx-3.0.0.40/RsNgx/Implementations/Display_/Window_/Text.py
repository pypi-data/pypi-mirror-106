...
class Text:
	...
	def set_data(self, current: str) -> None:
		"""SCPI: DISPlay[:WINDow]:TEXT[:DATA] \n
		Snippet: driver.display.window.text.set_data(current = '1') \n
		Displays a text message box on the front display. \n
			:param current: Text message for display.
		"""
		param = Conversions.value_to_quoted_str(current)
