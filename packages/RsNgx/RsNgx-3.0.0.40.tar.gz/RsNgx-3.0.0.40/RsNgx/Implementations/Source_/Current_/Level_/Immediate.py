...
class Immediate:
	...
	def get_amplitude(self) -> float:
		...
		Sets or queries the current value of the selected channel. \n
			:return: current: numeric value | MIN | MINimum | MAX | MAXimum | UP | DOWN | list numeric value Numeric value in the range of 0.000 to 6.010. MIN | MINimum Minimum current at 0.010 A. MAX | MAXimum Depending on the set voltage level, the maximum set current is 6.010 A. For voltage range from 0 V to 6 V, maximum set current is 6.01 A. For voltage 6 V, maximum set current is 3.01 A. UP Increases current by a defined step size. See [SOURce:]CURRent[:LEVel][:IMMediate]:STEP[:INCRement]. DOWN Decreases current by a defined step size. See [SOURce:]CURRent[:LEVel][:IMMediate]:STEP[:INCRement].
		"""
	...
	def set_amplitude(self, current: float) -> None:
		...
		Sets or queries the current value of the selected channel. \n
			:param current: numeric value | MIN | MINimum | MAX | MAXimum | UP | DOWN | list numeric value Numeric value in the range of 0.000 to 6.010. MIN | MINimum Minimum current at 0.010 A. MAX | MAXimum Depending on the set voltage level, the maximum set current is 6.010 A. For voltage range from 0 V to 6 V, maximum set current is 6.01 A. For voltage 6 V, maximum set current is 3.01 A. UP Increases current by a defined step size. See [SOURce:]CURRent[:LEVel][:IMMediate]:STEP[:INCRement]. DOWN Decreases current by a defined step size. See [SOURce:]CURRent[:LEVel][:IMMediate]:STEP[:INCRement].
		"""
