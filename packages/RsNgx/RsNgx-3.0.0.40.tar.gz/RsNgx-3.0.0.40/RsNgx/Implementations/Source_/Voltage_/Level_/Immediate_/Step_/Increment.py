...
class Increment:
	...
	def set(self, desired_stepsize: float, optional_default_step_query: enums.OptDfltStep = None) -> None:
		...
		Snippet: driver.source.voltage.level.immediate.step.increment.set(desired_stepsize = 1.0, optional_default_step_query = enums.OptDfltStep.DEFault) \n
		Sets or queries the incremental step size for the [SOURce:]VOLTage[:LEVel][:IMMediate][:AMPLitude] command. \n
			:param desired_stepsize: No help available
			:param optional_default_step_query: No help available
		"""
		...
		self._core.io.write(f'SOURce:VOLTage:LEVel:IMMediate:STEP:INCRement {param}'.rstrip())

	def get(self, optional_default_step_query: enums.OptDfltStep = None) -> float:
		"""SCPI: [SOURce]:VOLTage[:LEVel][:IMMediate]:STEP[:INCRement] \n
		Snippet: value: float = driver.source.voltage.level.immediate.step.increment.get(optional_default_step_query = enums.OptDfltStep.DEFault) \n
		Sets or queries the incremental step size for the [SOURce:]VOLTage[:LEVel][:IMMediate][:AMPLitude] command. \n
			:param optional_default_step_query: No help available
		...
		response = self._core.io.query_str(f'SOURce:VOLTage:LEVel:IMMediate:STEP:INCRement? {param}'.rstrip())
