...
class Arbitrary:
	"""Arbitrary commands group definition. 12 total commands, 4 Sub-groups, 7 group commands"""
	...
	@property
	def triggered(self):
		"""triggered commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_triggered'):
			from .Arbitrary_.Triggered import Triggered
			self._triggered = Triggered(self._core, self._base)
	...
	def fname(self):
		...
		if not hasattr(self, '_fname'):
			from .Arbitrary_.Fname import Fname
	...
	def behavior(self):
		...
		if not hasattr(self, '_behavior'):
			from .Arbitrary_.Behavior import Behavior
			self._behavior = Behavior(self._core, self._base)
	...
	def priority(self):
		...
		if not hasattr(self, '_priority'):
			from .Arbitrary_.Priority import Priority
	...
	def get_data(self) -> List[float]:
		...
		Sets or queries the arbitrary points for the previous selected channel. Max. 4096 arbitrary points can be defined.
		The dwell time between 2 arbitrary points is specified from 1 ms to 20 days. \n
			:return: arg_0: No help available
		"""
	...
	def set_data(self, arg_0: List[float]) -> None:
		...
		Sets or queries the arbitrary points for the previous selected channel. Max. 4096 arbitrary points can be defined.
		The dwell time between 2 arbitrary points is specified from 1 ms to 20 days. \n
			:param arg_0: voltage1, current1, time1, interpolation mode1, voltage2, current2, time2, interpolation mode2, ... Voltage and current settings depending on the instrument type. If the interpolation mode is sets to 1, it indicates that the mode is activated. If the interpolation mode is sets to 0, it indicates that the mode is not activated.
		"""
	...
	def clear_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'ARBitrary:CLEar')
	...
	def save_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'ARBitrary:SAVE')
	...
	def load_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'ARBitrary:LOAD')
