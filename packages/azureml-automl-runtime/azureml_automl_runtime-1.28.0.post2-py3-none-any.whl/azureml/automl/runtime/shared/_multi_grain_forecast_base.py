...
class _MultiGrainForecastBase:
    ...
    def _construct_tsdf(
        ...
        if constants.TimeSeriesInternal.ORIGIN_TIME_COLNAME_DEFAULT in tsdf_no_origin.index.names:
            tsdf_no_origin = X.droplevel(constants.TimeSeriesInternal.ORIGIN_TIME_COLNAME_DEFAULT)
    ...
    def _remove_origin_dependant_info_for_classical_forecasting_algo(
        ...
        if isinstance(self.grain_column_names, str):
            exogenous_colnames = list(
                arimax_raw_columns - {self.time_column_name} - {self.grain_column_names})  # type: List[Any]
        else:
            exogenous_colnames = list(arimax_raw_columns - {self.time_column_name} - set(self.grain_column_names))
        # TODO: Modify this logic so it drop the columns created in lags/rolling_windows transformer.
        for column in X.columns:
            if isinstance(column, str):
                if ((column.startswith('_automl_target_') and ("lag" in column or "window" in column)) or
                        column == 'horizon_origin'):
                    columns_to_drop.append(column)
                else:
                    for feature in exogenous_colnames:
                        if column.startswith(str(feature)) and ("_lag" in column):
                            columns_to_drop.append(column)
        X.drop(list(set(columns_to_drop)), axis=1, inplace=True)
