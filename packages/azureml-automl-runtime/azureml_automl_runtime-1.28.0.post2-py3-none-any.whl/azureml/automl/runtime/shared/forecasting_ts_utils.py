...
def get_stl_decomposition(ts_values: np.ndarray, seasonality: int = 1) \
    ...
    period_max = seasonality if len(endog) > seasonality else len(endog)
    period_means = np.fromiter((np.mean(endog_detrend[p::seasonality]) for p in range(period_max)), float)
