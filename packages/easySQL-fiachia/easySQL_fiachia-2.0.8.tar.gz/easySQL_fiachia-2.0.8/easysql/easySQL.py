...
class dataSQL:
    ...
    def setAttribute(self, results):
        ...
        if results is None:
            return
        for result in results:
            self.attribute.append(result[0])
    ...
    def get(self, *args):
        ...
        if len(args) == 0:
            ...
            get_dataList = []
            for dataSQL_data in self.dataList:
                get_dataList.append(dataSQL_data.get(args[0], None))
            return get_dataList
        else:
            get_dataList = []
            for dataSQL_data in self.dataList:
                ...
                get_dataList.append(tuple(get_data))
            return get_dataList
            ...
            if model == "dict-list":
                ...
                for dataSQL_data in self.dataList:
                    ...
                    for key in dataDic:
                        ...
                        index += 1
                    dataSQL_dataDicList.append(dataSQL_dataDic)
...
class easySQL:
    ...
    def add(self, tableName, is_commit: bool = True, data_from_dataSQL: dataSQL = None, **kwargs):
        """
        添加数据
        :param tableName:
        :param data_from_dataSQL:
            ...
            for values in data_from_dataSQL.values:
                ...
                for value in values:
                    if type(value) == str:
                        ...
                        sql1 += """%s,""" % value
                sql1 = sql1[:-1] + """)"""
                self.sqlList.append(sql + sql0 + sql1)
        else:
            sql = """INSERT INTO %s""" % tableName
            sql0 = """("""
            sql1 = """("""
    ...
    def addList(self, tableName, **kwargs):
        ...
        for i in range(0, temp):
            ...
            self.add(tableName, is_commit=False, **tKwargs)
        self.commit()
    ...
    def addSQL(self, *sqlList: str):
        for sql in sqlList:
            self.sqlList.append(sql)

    def showSQL(self, index: int = None):
        if index is None:
            for i in range(len(self.sqlList)):
                print("%s: %s" % (i, self.sqlList[i]))
        else:
            print("%s: %s" % (index, self.sqlList[index]))

    def deleteSQL(self, index: int = None):
        if index is None:
            self.sqlList = []
        else:
            self.sqlList.pop(index)
...
if __name__ == "__main__":
    ...
    dataImgDic = db.find("herospecificdata_skins", isBase=1).getDic("heroid", "heroname", "iconImg", "mainImg",
                                                                    dataDic=dataDic)
    print(dataList)
    print(dataImgDic)
