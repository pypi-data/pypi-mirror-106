...
# Zmatrix utilities for Shave/Sprout adapted from https://github.com/tmpchem/computational_chemistry/blob/b44ee6d2e1d2ec203811140b732cdb38f5164d76/scripts/geometry_analysis/zmat2xyz.py
...
def _get_local_axes(coords1, coords2, coords3):
    ...
    if (abs(_get_udp(u21, u23)) >= 1.0):
        raise ValueError('Error: Co-linear atoms in an internal coordinate definition')
...
def _make_tmat(x, d):
    ...
    where i, j, k, l are the Z-matrix indices (i is the terminal atom, j, k
    and l are non-terminal) and r, a, t are the bond length, angle and torsion.
    Indices i-l are cast to floats.
    """
    l = len(x)
    G = utils.make_graph(d)
    terminii = [i for i in range(l) if len(G[i]) == 1]
    d = squareform(d)
...
def shave(x, tmat=None):
    ...
    Args:
        ...
        tmat: optional, [nterminii, 7] array, the T-matrix. If not supplied,
              it is generated from analysis of x.
    ...
    core = [i for i in range(l) if not i in terminii]
    xcore = x[:, core]
    if one_frame:
        xcore = xcore[0]
    return xcore, tmat
...
def sprout(xcore, tmat):
    ...
    """
    xcore = utils.check_dimensions(xcore)
    one_frame = len(xcore.shape) == 2
    if one_frame:
        xcore = xcore.reshape((1, -1, 3))
    n_core = xcore.shape[1]
    n_frames = len(xcore)
    terminii = tmat[:, 0].astype(np.int32)
    n_terminii = len(terminii)
    n_all = n_core + n_terminii
    xnew = np.zeros((n_frames, n_all, 3))
    core = [i for i in range(n_all) if not i in terminii]
    xnew[:, core] = xcore
    Hind = tmat[:, :4].astype(np.int32)
    Harr = tmat[:, 4:]
    for i in range(n_frames):
        for ijkl, rat in zip(Hind, Harr):
            local_axes = _get_local_axes(xnew[i, ijkl[1]], xnew[i, ijkl[2]], xnew[i, ijkl[3]])
...
def triangulate(xc, zmat):
    ...
    for k in range(n_frames):
        for i in range(n_atoms):
            ...
            bond_vector = _get_bond_vector(zarrm[i, 0], zarrm[i, 1], zarrm[i, 2])
            disp_vector = np.array(np.dot(bond_vector, local_axes))
...
class Shave(object):
    ...
    def fit(self, X):
        ...
        Args:
            ...
            X: [n_xparticles, 3] or [nframes, n_xparticles, 3] array of coordinates
        """
        ...
        self.core = [i for i in range(self.n_tot) if not i in self.terminii]
        self.tmat = tmat
    ...
    def transform(self, X):
        ...
        if not X.shape[1] == self.n_tot:
            raise ValueError('Error: X must be shape [Any, {}, 3]'.format(self.n_tot))
    ...
    def inverse_transform(self, Y):
        ...
        if not Y.shape[1] == self.n_core:
            raise ValueError('Error: Y must be shape [Any, {}, 3]'.format(self.n_core))
    ...
    def transform(self, X):
        ...
        if self.use_pca:
            ...
            if X.shape[1] != self.x_fitter.mean.shape[0]:
                raise ValueError('Error: X contains {} atoms, was expecting {}'.format(X.shape[1], self.x_fitter.mean.shape[0]))
    ...
    def inverse_transform(self, Y):
        ...
        if self.use_pca:
            ...
            if Y.shape[1] != self.y_fitter.mean.shape[0]:
                raise ValueError('Error: Y contains {} atoms, was expecting {}'.format(Y.shape[1], self.y_fitter.mean.shape[0]))
