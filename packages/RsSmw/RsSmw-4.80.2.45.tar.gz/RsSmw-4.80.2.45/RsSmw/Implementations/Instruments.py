...
class Instruments:
	"""Instruments commands group definition. 16 total commands, 6 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("instruments", core, parent)

	@property
	def active(self):
		"""active commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_active'):
			from .Instruments_.Active import Active
	...
	def eaccess(self):
		...
		if not hasattr(self, '_eaccess'):
			from .Instruments_.Eaccess import Eaccess
			self._eaccess = Eaccess(self._core, self._base)
	...
	def gpib(self):
		...
		if not hasattr(self, '_gpib'):
			from .Instruments_.Gpib import Gpib
	...
	def mapping(self):
		...
		if not hasattr(self, '_mapping'):
			from .Instruments_.Mapping import Mapping
	...
	def remote(self):
		...
		if not hasattr(self, '_remote'):
			from .Instruments_.Remote import Remote
	...
	def scan(self):
		...
		if not hasattr(self, '_scan'):
			from .Instruments_.Scan import Scan
	...
	def clone(self) -> 'Instruments':
		...
		new_group = Instruments(self._core, self._base.parent)
