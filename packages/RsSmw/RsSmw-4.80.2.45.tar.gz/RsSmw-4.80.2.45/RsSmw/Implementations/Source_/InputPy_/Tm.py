...
class Tm:
	"""Tm commands group definition. 8 total commands, 4 Sub-groups, 0 group commands
	Repeated Capability: TmConnector, default value after init: TmConnector.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tm", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_tmConnector_get', 'repcap_tmConnector_set', repcap.TmConnector.Nr1)

	def repcap_tmConnector_set(self, enum_value: repcap.TmConnector) -> None:
	...
	def direction(self):
		...
		if not hasattr(self, '_direction'):
			from .Tm_.Direction import Direction
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Tm_.Trigger import Trigger
