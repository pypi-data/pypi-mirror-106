...
class Radar:
	"""Radar commands group definition. 13 total commands, 4 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("radar", core, parent)

	@property
	def analyzer(self):
		"""analyzer commands group. 1 Sub-classes, 1 commands."""
		if not hasattr(self, '_analyzer'):
			from .Radar_.Analyzer import Analyzer
	...
	def antenna(self):
		...
		if not hasattr(self, '_antenna'):
			from .Radar_.Antenna import Antenna
	...
	def ota(self):
		...
		if not hasattr(self, '_ota'):
			from .Radar_.Ota import Ota
			self._ota = Ota(self._core, self._base)
		return self._ota
	...
	def power(self):
		...
		if not hasattr(self, '_power'):
			from .Radar_.Power import Power
	...
	def clone(self) -> 'Radar':
		...
		new_group = Radar(self._core, self._base.parent)
