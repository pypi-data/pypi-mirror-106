...
class Rcs:
	"""Rcs commands group definition. 6 total commands, 6 Sub-groups, 0 group commands"""
	...
	def mean(self):
		...
		if not hasattr(self, '_mean'):
			from .Rcs_.Mean import Mean
			self._mean = Mean(self._core, self._base)
		return self._mean
	...
	def sper(self):
		...
		if not hasattr(self, '_sper'):
			from .Rcs_.Sper import Sper
			self._sper = Sper(self._core, self._base)
		return self._sper
	...
	def tcoverage(self):
		...
		if not hasattr(self, '_tcoverage'):
			from .Rcs_.Tcoverage import Tcoverage
			self._tcoverage = Tcoverage(self._core, self._base)
	...
	def upInterval(self):
		...
		if not hasattr(self, '_upInterval'):
			from .Rcs_.UpInterval import UpInterval
			self._upInterval = UpInterval(self._core, self._base)
