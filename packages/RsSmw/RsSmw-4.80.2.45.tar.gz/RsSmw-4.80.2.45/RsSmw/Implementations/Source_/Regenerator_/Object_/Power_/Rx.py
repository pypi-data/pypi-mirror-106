...
class Rx:
	"""Rx commands group definition. 4 total commands, 3 Sub-groups, 1 group commands"""
	...
	def dedication(self):
		...
		if not hasattr(self, '_dedication'):
			from .Rx_.Dedication import Dedication
			self._dedication = Dedication(self._core, self._base)
	...
	def set(self, radar_power_rx: float, objectIx=repcap.ObjectIx.Default) -> None:
		...
		In method RsSmw.Source.Regenerator.Radar.Power.modeMANual mode, sets the Rx power of each object. \n
