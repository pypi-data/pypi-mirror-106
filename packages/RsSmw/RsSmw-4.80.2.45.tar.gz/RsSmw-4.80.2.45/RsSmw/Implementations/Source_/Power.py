...
class Power:
	"""Power commands group definition. 39 total commands, 8 Sub-groups, 10 group commands"""
	...
	def get_manual(self) -> float:
		...
		Sets the level for the subsequent sweep step if method RsSmw.Source.Sweep.Power.mode. Use a separate command for each
		sweep step. \n
			:return: manual: float You can select any level within the setting range, where: STARt is set with method RsSmw.Source.Power.start STOP is set with method RsSmw.Source.Power.stop OFFSet is set with OFFSet Range: (STARt + OFFSet) to (STOP + OFFSet) , Unit: dBm
		"""
	...
	def set_manual(self, manual: float) -> None:
		...
		Sets the level for the subsequent sweep step if method RsSmw.Source.Sweep.Power.mode. Use a separate command for each
		sweep step. \n
			:param manual: float You can select any level within the setting range, where: STARt is set with method RsSmw.Source.Power.start STOP is set with method RsSmw.Source.Power.stop OFFSet is set with OFFSet Range: (STARt + OFFSet) to (STOP + OFFSet) , Unit: dBm
		"""
	...
	def get_power(self) -> float:
		...
		the level containing offset. See 'Displayed RF Frequency and Level Values with Downstream Instruments'. \n
	...
	def set_start(self, start: float) -> None:
		...
		Sets the RF start/stop level in sweep mode. \n
			:param start: float Sets the setting range calculated as follows: (Level_min + OFFSet) to (Level_max + OFFSet) Where the values are set with the commands: OFFSet method RsSmw.Source.Power.start method RsSmw.Source.Power.stop Range: Minimum level to maximum level , Unit: dBm
		"""
	...
	def get_stop(self) -> float:
		...
		Sets the RF start/stop level in sweep mode. \n
			:return: stop: float Sets the setting range calculated as follows: (Level_min + OFFSet) to (Level_max + OFFSet) Where the values are set with the commands: OFFSet method RsSmw.Source.Power.start method RsSmw.Source.Power.stop Range: Minimum level to maximum level , Unit: dBm
		"""
	...
	def set_stop(self, stop: float) -> None:
		...
		Sets the RF start/stop level in sweep mode. \n
			:param stop: float Sets the setting range calculated as follows: (Level_min + OFFSet) to (Level_max + OFFSet) Where the values are set with the commands: OFFSet method RsSmw.Source.Power.start method RsSmw.Source.Power.stop Range: Minimum level to maximum level , Unit: dBm
		"""
