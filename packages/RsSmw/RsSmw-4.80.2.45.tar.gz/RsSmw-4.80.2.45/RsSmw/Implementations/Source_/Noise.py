...
class Noise:
	"""Noise commands group definition. 18 total commands, 4 Sub-groups, 1 group commands"""
	...
	def impulsive(self):
		...
		if not hasattr(self, '_impulsive'):
			from .Noise_.Impulsive import Impulsive
			self._impulsive = Impulsive(self._core, self._base)
	...
	def phasenoise(self):
		...
		if not hasattr(self, '_phasenoise'):
			from .Noise_.Phasenoise import Phasenoise
			self._phasenoise = Phasenoise(self._core, self._base)
