...
class Frequency:
	"""Frequency commands group definition. 37 total commands, 6 Sub-groups, 9 group commands"""
	...
	def converter(self):
		...
		if not hasattr(self, '_converter'):
			from .Frequency_.Converter import Converter
			self._converter = Converter(self._core, self._base)
	...
	def get_mode(self) -> enums.FreqMode:
		...
		further frequency settings. \n
			:return: mode: CW| FIXed | SWEep| LIST | COMBined CW|FIXed Sets the fixed frequency mode. CW and FIXed are synonyms. The instrument operates at a defined frequency, set with command [:​SOURcehw]:​FREQuency[:​CW|FIXed]. SWEep Sets sweep mode. The instrument processes frequency (and level) settings in defined sweep steps. Set the range and current frequency with the commands: method RsSmw.Source.Frequency.start and method RsSmw.Source.Frequency.stop, method RsSmw.Source.Frequency.center, method RsSmw.Source.Frequency.span, method RsSmw.Source.Frequency.manual LIST Sets list mode. The instrument processes frequency and level settings by means of values loaded from a list. To configure list mode settings, use the commands of the 'SOURce:LIST Subsystem'. COMBined Sets the combined RF frequency / level sweep mode. The instrument processes frequency and level settings in defined sweep steps. Set the range and current frequency with the commands: method RsSmw.Source.Combined.Frequency.start and method RsSmw.Source.Combined.Frequency.stop, method RsSmw.Source.Combined.Power.start and method RsSmw.Source.Combined.Power.stop
		"""
	...
	def get_offset(self) -> float:
		...
		NFREQ:MULT + fFREQ:OFFSer See 'Displayed RF Frequency and Level Values with Downstream Instruments'. Note: The offset
		also affects RF frequency sweep. \n
			:return: offset: float
		"""
