...
class V5G:
	"""V5G commands group definition. 663 total commands, 15 Sub-groups, 6 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("v5G", core, parent)

	@property
	def clipping(self):
		"""clipping commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_clipping'):
			from .V5G_.Clipping import Clipping
	...
	def downlink(self):
		...
		if not hasattr(self, '_downlink'):
			from .V5G_.Downlink import Downlink
	...
	def filterPy(self):
		...
		if not hasattr(self, '_filterPy'):
			from .V5G_.FilterPy import FilterPy
	...
	def logGen(self):
		...
		if not hasattr(self, '_logGen'):
			from .V5G_.LogGen import LogGen
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .V5G_.Setting import Setting
	...
	def symbolRate(self):
		...
		if not hasattr(self, '_symbolRate'):
			from .V5G_.SymbolRate import SymbolRate
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .V5G_.Trigger import Trigger
	...
	def uplink(self):
		...
		if not hasattr(self, '_uplink'):
			from .V5G_.Uplink import Uplink
	...
	def waveform(self):
		...
		if not hasattr(self, '_waveform'):
			from .V5G_.Waveform import Waveform
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmw.Source.Bb.V5G.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:V5G:PRESet')
	...
	def clone(self) -> 'V5G':
		...
		new_group = V5G(self._core, self._base.parent)
