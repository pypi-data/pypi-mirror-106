...
class Set:
	"""Set commands group definition. 2 total commands, 2 Sub-groups, 0 group commands
	Repeated Capability: SetItem, default value after init: SetItem.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("set", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_setItem_get', 'repcap_setItem_set', repcap.SetItem.Nr1)

	def repcap_setItem_set(self, enum_value: repcap.SetItem) -> None:
	...
	def rbCount(self):
		...
		if not hasattr(self, '_rbCount'):
			from .Set_.RbCount import RbCount
	...
	def vrbOffset(self):
		...
		if not hasattr(self, '_vrbOffset'):
			from .Set_.VrbOffset import VrbOffset
