...
class Packet:
	"""Packet commands group definition. 3 total commands, 2 Sub-groups, 0 group commands"""
	...
	def multiplex(self):
		...
		if not hasattr(self, '_multiplex'):
			from .Packet_.Multiplex import Multiplex
			self._multiplex = Multiplex(self._core, self._base)
	...
	def unfiltered(self):
		...
		if not hasattr(self, '_unfiltered'):
			from .Packet_.Unfiltered import Unfiltered
			self._unfiltered = Unfiltered(self._core, self._base)
