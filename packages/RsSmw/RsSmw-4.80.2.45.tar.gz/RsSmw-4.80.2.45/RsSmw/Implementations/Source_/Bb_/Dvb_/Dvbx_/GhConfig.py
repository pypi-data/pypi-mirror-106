...
class GhConfig:
	"""GhConfig commands group definition. 11 total commands, 7 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("ghConfig", core, parent)

	@property
	def fid(self):
		"""fid commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_fid'):
			from .GhConfig_.Fid import Fid
			self._fid = Fid(self._core, self._base)
		return self._fid
	...
	def fiUse(self):
		...
		if not hasattr(self, '_fiUse'):
			from .GhConfig_.FiUse import FiUse
			self._fiUse = FiUse(self._core, self._base)
		return self._fiUse
	...
	def label(self):
		...
		if not hasattr(self, '_label'):
			from .GhConfig_.Label import Label
			self._label = Label(self._core, self._base)
		return self._label
	...
	def luse(self):
		...
		if not hasattr(self, '_luse'):
			from .GhConfig_.Luse import Luse
			self._luse = Luse(self._core, self._base)
		return self._luse
	...
	def ptUse(self):
		...
		if not hasattr(self, '_ptUse'):
			from .GhConfig_.PtUse import PtUse
			self._ptUse = PtUse(self._core, self._base)
		return self._ptUse
	...
	def ptype(self):
		...
		if not hasattr(self, '_ptype'):
			from .GhConfig_.Ptype import Ptype
	...
	def tluse(self):
		...
		if not hasattr(self, '_tluse'):
			from .GhConfig_.Tluse import Tluse
			self._tluse = Tluse(self._core, self._base)
		return self._tluse
	...
	def clone(self) -> 'GhConfig':
		...
		new_group = GhConfig(self._core, self._base.parent)
