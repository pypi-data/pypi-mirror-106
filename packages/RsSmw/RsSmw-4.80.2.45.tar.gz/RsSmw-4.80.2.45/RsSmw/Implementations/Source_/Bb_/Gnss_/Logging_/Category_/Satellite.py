...
class Satellite:
	"""Satellite commands group definition. 22 total commands, 4 Sub-groups, 17 group commands"""
	...
	def get_cphase(self) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:LOGGing:CATegory:SATellite:CPHase \n
		Snippet: value: bool = driver.source.bb.gnss.logging.category.satellite.get_cphase() \n
		Enables the parameter for logging. \n
			:return: state: 0| 1| OFF| ON
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:LOGGing:CATegory:SATellite:CPHase?')
		return Conversions.str_to_bool(response)

	def set_cphase(self, state: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:LOGGing:CATegory:SATellite:CPHase \n
		Snippet: driver.source.bb.gnss.logging.category.satellite.set_cphase(state = False) \n
		Enables the parameter for logging. \n
			:param state: 0| 1| OFF| ON
		"""
		param = Conversions.bool_to_str(state)
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:LOGGing:CATegory:SATellite:CPHase {param}')
