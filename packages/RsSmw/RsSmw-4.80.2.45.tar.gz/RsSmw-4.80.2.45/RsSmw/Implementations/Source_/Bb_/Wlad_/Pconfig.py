...
class Pconfig:
	"""Pconfig commands group definition. 55 total commands, 11 Sub-groups, 9 group commands"""
	...
	def btRequest(self):
		...
		if not hasattr(self, '_btRequest'):
			from .Pconfig_.BtRequest import BtRequest
			self._btRequest = BtRequest(self._core, self._base)
	...
	def coding(self):
		...
		if not hasattr(self, '_coding'):
			from .Pconfig_.Coding import Coding
	...
	def data(self):
		...
		if not hasattr(self, '_data'):
			from .Pconfig_.Data import Data
	...
	def dindicator(self):
		...
		if not hasattr(self, '_dindicator'):
			from .Pconfig_.Dindicator import Dindicator
			self._dindicator = Dindicator(self._core, self._base)
	...
	def gpi(self):
		...
		if not hasattr(self, '_gpi'):
			from .Pconfig_.Gpi import Gpi
			self._gpi = Gpi(self._core, self._base)
		return self._gpi
	...
	def mpdu(self):
		...
		if not hasattr(self, '_mpdu'):
			from .Pconfig_.Mpdu import Mpdu
	...
	def preamble(self):
		...
		if not hasattr(self, '_preamble'):
			from .Pconfig_.Preamble import Preamble
	...
	def scrambler(self):
		...
		if not hasattr(self, '_scrambler'):
			from .Pconfig_.Scrambler import Scrambler
	...
	def taRound(self):
		...
		if not hasattr(self, '_taRound'):
			from .Pconfig_.TaRound import TaRound
			self._taRound = TaRound(self._core, self._base)
	...
	def tdWindowing(self):
		...
		if not hasattr(self, '_tdWindowing'):
			from .Pconfig_.TdWindowing import TdWindowing
