...
class Wlad:
	"""Wlad commands group definition. 109 total commands, 8 Sub-groups, 6 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("wlad", core, parent)

	@property
	def clipping(self):
		"""clipping commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_clipping'):
			from .Wlad_.Clipping import Clipping
	...
	def filterPy(self):
		...
		if not hasattr(self, '_filterPy'):
			from .Wlad_.FilterPy import FilterPy
	...
	def pconfig(self):
		...
		if not hasattr(self, '_pconfig'):
			from .Wlad_.Pconfig import Pconfig
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Wlad_.Setting import Setting
	...
	def symbolRate(self):
		...
		if not hasattr(self, '_symbolRate'):
			from .Wlad_.SymbolRate import SymbolRate
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Wlad_.Trigger import Trigger
	...
	def waveform(self):
		...
		if not hasattr(self, '_waveform'):
			from .Wlad_.Waveform import Waveform
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmw.Source.Bb.Wlad.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:WLAD:PRESet')
	...
	def clone(self) -> 'Wlad':
		...
		new_group = Wlad(self._core, self._base.parent)
