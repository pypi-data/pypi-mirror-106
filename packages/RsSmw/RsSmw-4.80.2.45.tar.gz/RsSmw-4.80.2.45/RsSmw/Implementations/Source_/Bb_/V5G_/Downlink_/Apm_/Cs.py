from .......Internal.Core import Core
from .......Internal.CommandsGroup import CommandsGroup


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class Cs:
	"""Cs commands group definition. 3 total commands, 3 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("cs", core, parent)

	@property
	def ap(self):
		"""ap commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_ap'):
			from .Cs_.Ap import Ap
			self._ap = Ap(self._core, self._base)
		return self._ap

	@property
	def csiAp(self):
		"""csiAp commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_csiAp'):
			from .Cs_.CsiAp import CsiAp
			self._csiAp = CsiAp(self._core, self._base)
		return self._csiAp

	@property
	def xssap(self):
		"""xssap commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_xssap'):
			from .Cs_.Xssap import Xssap
			self._xssap = Xssap(self._core, self._base)
		return self._xssap

	def clone(self) -> 'Cs':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Cs(self._core, self._base.parent)
		self._base.synchronize_repcaps(new_group)
		return new_group
