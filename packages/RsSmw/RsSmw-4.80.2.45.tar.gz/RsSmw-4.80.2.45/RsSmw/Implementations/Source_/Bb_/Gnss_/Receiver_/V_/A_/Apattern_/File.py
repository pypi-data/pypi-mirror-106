...
class File:
	...
	def set(self, filename: str, vehicle=repcap.Vehicle.Default, antenna=repcap.Antenna.Default) -> None:
		...
		the default and in a specific directory. \n
			:param filename: 'filename' Filename or complete file path; file extension can be omitted. Query the existing files with the following commands: method RsSmw.Source.Bb.Gnss.Apattern.Catalog.predefined method RsSmw.Source.Bb.Gnss.Apattern.Catalog.user
	...
	def get(self, vehicle=repcap.Vehicle.Default, antenna=repcap.Antenna.Default) -> str:
		...
		the default and in a specific directory. \n
			:param vehicle: optional repeated capability selector. Default value: Nr1 (settable in the interface 'V')
			:param antenna: optional repeated capability selector. Default value: Nr1 (settable in the interface 'A')
			:return: filename: 'filename' Filename or complete file path; file extension can be omitted. Query the existing files with the following commands: method RsSmw.Source.Bb.Gnss.Apattern.Catalog.predefined method RsSmw.Source.Bb.Gnss.Apattern.Catalog.user"""
		vehicle_cmd_val = self._base.get_repcap_cmd_value(vehicle, repcap.Vehicle)
		antenna_cmd_val = self._base.get_repcap_cmd_value(antenna, repcap.Antenna)
