...
class Slot:
	"""Slot commands group definition. 18 total commands, 1 Sub-groups, 0 group commands
	...
	def ldirection(self):
		...
		if not hasattr(self, '_ldirection'):
			from .Slot_.Ldirection import Ldirection
			self._ldirection = Ldirection(self._core, self._base)
