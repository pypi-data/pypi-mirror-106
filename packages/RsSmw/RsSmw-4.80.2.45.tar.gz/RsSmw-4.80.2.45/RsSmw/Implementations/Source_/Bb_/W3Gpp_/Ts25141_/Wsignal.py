...
class Wsignal:
	"""Wsignal commands group definition. 21 total commands, 5 Sub-groups, 6 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("wsignal", core, parent)

	@property
	def dpcch(self):
		"""dpcch commands group. 1 Sub-classes, 1 commands."""
		if not hasattr(self, '_dpcch'):
			from .Wsignal_.Dpcch import Dpcch
	...
	def dpdch(self):
		...
		if not hasattr(self, '_dpdch'):
			from .Wsignal_.Dpdch import Dpdch
	...
	def pcpch(self):
		...
		if not hasattr(self, '_pcpch'):
			from .Wsignal_.Pcpch import Pcpch
	...
	def prach(self):
		...
		if not hasattr(self, '_prach'):
			from .Wsignal_.Prach import Prach
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Wsignal_.Trigger import Trigger
	...
	def clone(self) -> 'Wsignal':
		...
		new_group = Wsignal(self._core, self._base.parent)
