...
class Dfinding:
	"""Dfinding commands group definition. 7 total commands, 3 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dfinding", core, parent)

	@property
	def file(self):
		"""file commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_file'):
			from .Dfinding_.File import File
	...
	def sequencer(self):
		...
		if not hasattr(self, '_sequencer'):
			from .Dfinding_.Sequencer import Sequencer
	...
	def clone(self) -> 'Dfinding':
		...
		new_group = Dfinding(self._core, self._base.parent)
