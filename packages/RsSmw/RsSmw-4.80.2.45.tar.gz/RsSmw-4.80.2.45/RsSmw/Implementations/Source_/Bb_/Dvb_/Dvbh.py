...
class Dvbh:
	"""Dvbh commands group definition. 32 total commands, 6 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dvbh", core, parent)

	@property
	def iinterleaver(self):
		"""iinterleaver commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_iinterleaver'):
			from .Dvbh_.Iinterleaver import Iinterleaver
	...
	def sample(self):
		...
		if not hasattr(self, '_sample'):
			from .Dvbh_.Sample import Sample
	...
	def clone(self) -> 'Dvbh':
		...
		new_group = Dvbh(self._core, self._base.parent)
