...
class Stream:
	"""Stream commands group definition. 4 total commands, 4 Sub-groups, 0 group commands
	Repeated Capability: TwoStreams, default value after init: TwoStreams.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("stream", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_twoStreams_get', 'repcap_twoStreams_set', repcap.TwoStreams.Nr1)
	...
	def eratio(self):
		...
		if not hasattr(self, '_eratio'):
			from .Stream_.Eratio import Eratio
			self._eratio = Eratio(self._core, self._base)
		return self._eratio
	...
	def seqCount(self):
		...
		if not hasattr(self, '_seqCount'):
			from .Stream_.SeqCount import SeqCount
			self._seqCount = SeqCount(self._core, self._base)
	...
	def sequencer(self):
		...
		if not hasattr(self, '_sequencer'):
			from .Stream_.Sequencer import Sequencer
