...
class Tetra:
	"""Tetra commands group definition. 105 total commands, 11 Sub-groups, 9 group commands"""
	...
	def bbncht(self):
		...
		if not hasattr(self, '_bbncht'):
			from .Tetra_.Bbncht import Bbncht
			self._bbncht = Bbncht(self._core, self._base)
		return self._bbncht
	...
	def clipping(self):
		...
		if not hasattr(self, '_clipping'):
			from .Tetra_.Clipping import Clipping
	...
	def clock(self):
		...
		if not hasattr(self, '_clock'):
			from .Tetra_.Clock import Clock
	...
	def pramping(self):
		...
		if not hasattr(self, '_pramping'):
			from .Tetra_.Pramping import Pramping
	...
	def sattenuation(self):
		...
		if not hasattr(self, '_sattenuation'):
			from .Tetra_.Sattenuation import Sattenuation
	...
	def sconfiguration(self):
		...
		if not hasattr(self, '_sconfiguration'):
			from .Tetra_.Sconfiguration import Sconfiguration
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Tetra_.Setting import Setting
	...
	def symbolRate(self):
		...
		if not hasattr(self, '_symbolRate'):
			from .Tetra_.SymbolRate import SymbolRate
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Tetra_.Trigger import Trigger
	...
	def waveform(self):
		...
		if not hasattr(self, '_waveform'):
			from .Tetra_.Waveform import Waveform
	...
	def preset(self) -> None:
		...
		set with the command method RsSmw.Source.Bb.Tetra.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:TETRa:PRESet')
