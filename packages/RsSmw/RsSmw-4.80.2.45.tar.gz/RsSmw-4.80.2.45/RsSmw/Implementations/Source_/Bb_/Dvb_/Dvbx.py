...
class Dvbx:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dvbx", core, parent)

	@property
	def adLength(self):
		"""adLength commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_adLength'):
			from .Dvbx_.AdLength import AdLength
			self._adLength = AdLength(self._core, self._base)
	...
	def bhConfig(self):
		...
		if not hasattr(self, '_bhConfig'):
			from .Dvbx_.BhConfig import BhConfig
			self._bhConfig = BhConfig(self._core, self._base)
	...
	def binterleaver(self):
		...
		if not hasattr(self, '_binterleaver'):
			from .Dvbx_.Binterleaver import Binterleaver
	...
	def bscrambler(self):
		...
		if not hasattr(self, '_bscrambler'):
			from .Dvbx_.Bscrambler import Bscrambler
			self._bscrambler = Bscrambler(self._core, self._base)
	...
	def crc(self):
		...
		if not hasattr(self, '_crc'):
			from .Dvbx_.Crc import Crc
			self._crc = Crc(self._core, self._base)
		return self._crc

	@property
	def data(self):
		"""data commands group. 0 Sub-classes, 6 commands."""
		if not hasattr(self, '_data'):
			from .Dvbx_.Data import Data
	...
	def ghConfig(self):
		...
		if not hasattr(self, '_ghConfig'):
			from .Dvbx_.GhConfig import GhConfig
			self._ghConfig = GhConfig(self._core, self._base)
	...
	def icoder(self):
		...
		if not hasattr(self, '_icoder'):
			from .Dvbx_.Icoder import Icoder
			self._icoder = Icoder(self._core, self._base)
		return self._icoder
	...
	def mtab(self):
		...
		if not hasattr(self, '_mtab'):
			from .Dvbx_.Mtab import Mtab
			self._mtab = Mtab(self._core, self._base)
		return self._mtab
	...
	def ocoder(self):
		...
		if not hasattr(self, '_ocoder'):
			from .Dvbx_.Ocoder import Ocoder
			self._ocoder = Ocoder(self._core, self._base)
		return self._ocoder
	...
	def pscrambler(self):
		...
		if not hasattr(self, '_pscrambler'):
			from .Dvbx_.Pscrambler import Pscrambler
			self._pscrambler = Pscrambler(self._core, self._base)
	...
	def pstate(self):
		...
		if not hasattr(self, '_pstate'):
			from .Dvbx_.Pstate import Pstate
			self._pstate = Pstate(self._core, self._base)
		return self._pstate
	...
	def thConfig(self):
		...
		if not hasattr(self, '_thConfig'):
			from .Dvbx_.ThConfig import ThConfig
			self._thConfig = ThConfig(self._core, self._base)
	...
	def ucMode(self):
		...
		if not hasattr(self, '_ucMode'):
			from .Dvbx_.UcMode import UcMode
			self._ucMode = UcMode(self._core, self._base)
		return self._ucMode
	...
	def clone(self) -> 'Dvbx':
		...
		new_group = Dvbx(self._core, self._base.parent)
