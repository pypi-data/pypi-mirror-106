...
class Data:
	...
	def set(self, data: enums.V5GdlDataSourceUser, subframeNull=repcap.SubframeNull.Default, allocationNull=repcap.AllocationNull.Default) -> None:
		...
		Sets the data source for the selected allocation. \n
			:param data: MIB| XPDCch USERx Assign a user to the xPDSCH allocation. Specify the data source of the user via: method RsSmw.Source.Bb.V5G.Downlink.User.Data.set MIB (Result parameter) Indicates that the xPBCH transmits master information blocks. XPDCch (Result parameter) Indicates the connection type xPDCCH.
	...
	def get(self, subframeNull=repcap.SubframeNull.Default, allocationNull=repcap.AllocationNull.Default) -> enums.V5GdlDataSourceUser:
		...
		Sets the data source for the selected allocation. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')
			:return: data: MIB| XPDCch USERx Assign a user to the xPDSCH allocation. Specify the data source of the user via: method RsSmw.Source.Bb.V5G.Downlink.User.Data.set MIB (Result parameter) Indicates that the xPBCH transmits master information blocks. XPDCch (Result parameter) Indicates the connection type xPDCCH."""
		subframeNull_cmd_val = self._base.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		allocationNull_cmd_val = self._base.get_repcap_cmd_value(allocationNull, repcap.AllocationNull)
