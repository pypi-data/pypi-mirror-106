...
class DciConf:
	"""DciConf commands group definition. 43 total commands, 40 Sub-groups, 0 group commands"""
	...
	def f3Ri(self):
		...
		if not hasattr(self, '_f3Ri'):
			from .DciConf_.F3Ri import F3Ri
			self._f3Ri = F3Ri(self._core, self._base)
		return self._f3Ri
	...
	def firstTrans(self):
		...
		if not hasattr(self, '_firstTrans'):
			from .DciConf_.FirstTrans import FirstTrans
			self._firstTrans = FirstTrans(self._core, self._base)
	...
	def retrans(self):
		...
		if not hasattr(self, '_retrans'):
			from .DciConf_.Retrans import Retrans
			self._retrans = Retrans(self._core, self._base)
	...
	def tb3(self):
		...
		if not hasattr(self, '_tb3'):
			from .DciConf_.Tb3 import Tb3
			self._tb3 = Tb3(self._core, self._base)
		return self._tb3
