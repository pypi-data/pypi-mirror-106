...
class Subf:
	"""Subf commands group definition. 8 total commands, 8 Sub-groups, 0 group commands
	...
	def frIndex(self):
		...
		if not hasattr(self, '_frIndex'):
			from .Subf_.FrIndex import FrIndex
	...
	def ncsConf(self):
		...
		if not hasattr(self, '_ncsConf'):
			from .Subf_.NcsConf import NcsConf
	...
	def rbOffset(self):
		...
		if not hasattr(self, '_rbOffset'):
			from .Subf_.RbOffset import RbOffset
	...
	def rsequence(self):
		...
		if not hasattr(self, '_rsequence'):
			from .Subf_.Rsequence import Rsequence
	...
	def sindex(self):
		...
		if not hasattr(self, '_sindex'):
			from .Subf_.Sindex import Sindex
