...
class Xpusch:
	"""Xpusch commands group definition. 13 total commands, 7 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("xpusch", core, parent)

	@property
	def ccoding(self):
		"""ccoding commands group. 3 Sub-classes, 0 commands."""
		if not hasattr(self, '_ccoding'):
			from .Xpusch_.Ccoding import Ccoding
	...
	def conflict(self):
		...
		if not hasattr(self, '_conflict'):
			from .Xpusch_.Conflict import Conflict
	...
	def nscid(self):
		...
		if not hasattr(self, '_nscid'):
			from .Xpusch_.Nscid import Nscid
	...
	def pcrs(self):
		...
		if not hasattr(self, '_pcrs'):
			from .Xpusch_.Pcrs import Pcrs
			self._pcrs = Pcrs(self._core, self._base)
		return self._pcrs
	...
	def precoding(self):
		...
		if not hasattr(self, '_precoding'):
			from .Xpusch_.Precoding import Precoding
	...
	def rmIndex(self):
		...
		if not hasattr(self, '_rmIndex'):
			from .Xpusch_.RmIndex import RmIndex
			self._rmIndex = RmIndex(self._core, self._base)
	...
	def clone(self) -> 'Xpusch':
		...
		new_group = Xpusch(self._core, self._base.parent)
