...
class Sdata:
	"""Sdata commands group definition. 3 total commands, 2 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("sdata", core, parent)

	@property
	def sdPattern(self):
		"""sdPattern commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_sdPattern'):
			from .Sdata_.SdPattern import SdPattern
			self._sdPattern = SdPattern(self._core, self._base)
	...
	def sdSelection(self):
		...
		if not hasattr(self, '_sdSelection'):
			from .Sdata_.SdSelection import SdSelection
			self._sdSelection = SdSelection(self._core, self._base)
	...
	def clone(self) -> 'Sdata':
		...
		new_group = Sdata(self._core, self._base.parent)
