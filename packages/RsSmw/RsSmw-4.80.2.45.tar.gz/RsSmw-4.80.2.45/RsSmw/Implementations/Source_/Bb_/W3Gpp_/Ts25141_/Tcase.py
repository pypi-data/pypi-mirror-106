...
class Tcase:
	"""Tcase commands group definition. 2 total commands, 1 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tcase", core, parent)

	@property
	def execute(self):
		"""execute commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_execute'):
			from .Tcase_.Execute import Execute
	...
	def get_value(self) -> enums.Ts25141Tc:
		...
		activated with command method RsSmw.Source.Bb.W3Gpp.Ts25141.Tcase.Execute.set \n
	...
	def clone(self) -> 'Tcase':
		...
		new_group = Tcase(self._core, self._base.parent)
