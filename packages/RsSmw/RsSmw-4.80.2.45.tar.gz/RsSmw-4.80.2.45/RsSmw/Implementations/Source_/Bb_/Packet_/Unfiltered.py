...
class Unfiltered:
	"""Unfiltered commands group definition. 2 total commands, 1 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("unfiltered", core, parent)

	@property
	def nibble(self):
		"""nibble commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_nibble'):
			from .Unfiltered_.Nibble import Nibble
			self._nibble = Nibble(self._core, self._base)
		return self._nibble
	...
	def clone(self) -> 'Unfiltered':
		...
		new_group = Unfiltered(self._core, self._base.parent)
