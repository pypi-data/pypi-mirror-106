...
class Lp:
	"""Lp commands group definition. 7 total commands, 5 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("lp", core, parent)

	@property
	def data(self):
		"""data commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_data'):
			from .Lp_.Data import Data
	...
	def ointerleaver(self):
		...
		if not hasattr(self, '_ointerleaver'):
			from .Lp_.Ointerleaver import Ointerleaver
			self._ointerleaver = Ointerleaver(self._core, self._base)
	...
	def pnScrambler(self):
		...
		if not hasattr(self, '_pnScrambler'):
			from .Lp_.PnScrambler import PnScrambler
	...
	def clone(self) -> 'Lp':
		...
		new_group = Lp(self._core, self._base.parent)
