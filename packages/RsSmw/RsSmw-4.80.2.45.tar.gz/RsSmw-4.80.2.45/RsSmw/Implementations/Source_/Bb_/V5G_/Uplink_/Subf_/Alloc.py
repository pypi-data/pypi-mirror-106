...
class Alloc:
	"""Alloc commands group definition. 29 total commands, 10 Sub-groups, 0 group commands
	...
	def xpucch(self):
		...
		if not hasattr(self, '_xpucch'):
			from .Alloc_.Xpucch import Xpucch
			self._xpucch = Xpucch(self._core, self._base)
		return self._xpucch
	...
	def xpusch(self):
		...
		if not hasattr(self, '_xpusch'):
			from .Alloc_.Xpusch import Xpusch
			self._xpusch = Xpusch(self._core, self._base)
		return self._xpusch
	...
	def vrbOffset(self):
		...
		if not hasattr(self, '_vrbOffset'):
			from .Alloc_.VrbOffset import VrbOffset
