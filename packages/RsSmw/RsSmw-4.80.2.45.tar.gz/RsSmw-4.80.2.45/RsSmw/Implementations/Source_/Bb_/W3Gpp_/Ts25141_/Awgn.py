...
class Awgn:
	"""Awgn commands group definition. 6 total commands, 3 Sub-groups, 3 group commands"""
	...
	def rblock(self):
		...
		if not hasattr(self, '_rblock'):
			from .Awgn_.Rblock import Rblock
			self._rblock = Rblock(self._core, self._base)
		return self._rblock
	...
	def rpDetection(self):
		...
		if not hasattr(self, '_rpDetection'):
			from .Awgn_.RpDetection import RpDetection
			self._rpDetection = RpDetection(self._core, self._base)
