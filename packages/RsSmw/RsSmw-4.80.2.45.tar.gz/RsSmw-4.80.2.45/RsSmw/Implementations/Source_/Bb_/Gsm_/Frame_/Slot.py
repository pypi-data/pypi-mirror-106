...
class Slot:
	"""Slot commands group definition. 29 total commands, 3 Sub-groups, 0 group commands
	...
	def typePy(self):
		...
		if not hasattr(self, '_typePy'):
			from .Slot_.TypePy import TypePy
	...
	def voJitter(self):
		...
		if not hasattr(self, '_voJitter'):
			from .Slot_.VoJitter import VoJitter
			self._voJitter = VoJitter(self._core, self._base)
	...
	def subChannel(self):
		...
		if not hasattr(self, '_subChannel'):
			from .Slot_.SubChannel import SubChannel
