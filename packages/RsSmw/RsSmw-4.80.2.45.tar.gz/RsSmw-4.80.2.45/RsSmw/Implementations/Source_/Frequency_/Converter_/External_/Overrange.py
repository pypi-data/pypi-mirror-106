...
class Overrange:
	"""Overrange commands group definition. 4 total commands, 2 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("overrange", core, parent)

	@property
	def allowed(self):
		"""allowed commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_allowed'):
			from .Overrange_.Allowed import Allowed
			self._allowed = Allowed(self._core, self._base)
	...
	def frequency(self):
		...
		if not hasattr(self, '_frequency'):
			from .Overrange_.Frequency import Frequency
	...
	def get_state(self) -> bool:
		...
		Enables the extended frequency range of a connected R&S SZU. \n
			:return: overrang_state: 0| 1| OFF| ON
		"""
	...
	def set_state(self, overrang_state: bool) -> None:
		...
		Enables the extended frequency range of a connected R&S SZU. \n
			:param overrang_state: 0| 1| OFF| ON
		"""
	...
	def clone(self) -> 'Overrange':
		...
		new_group = Overrange(self._core, self._base.parent)
