...
class RfAlignment:
	"""RfAlignment commands group definition. 12 total commands, 4 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("rfAlignment", core, parent)

	@property
	def calibrated(self):
		"""calibrated commands group. 1 Sub-classes, 1 commands."""
		if not hasattr(self, '_calibrated'):
			from .RfAlignment_.Calibrated import Calibrated
			self._calibrated = Calibrated(self._core, self._base)
	...
	def correction(self):
		...
		if not hasattr(self, '_correction'):
			from .RfAlignment_.Correction import Correction
	...
	def fresponse(self):
		...
		if not hasattr(self, '_fresponse'):
			from .RfAlignment_.Fresponse import Fresponse
	...
	def rfInfo(self):
		...
		if not hasattr(self, '_rfInfo'):
			from .RfAlignment_.RfInfo import RfInfo
			self._rfInfo = RfInfo(self._core, self._base)
		return self._rfInfo
	...
	def get_foffset(self) -> float:
		...
		No command help available \n
			:return: bb_freq_offset: No help available
		"""
	...
	def clone(self) -> 'RfAlignment':
		...
		new_group = RfAlignment(self._core, self._base.parent)
