...
class Fsimulator:
	"""Fsimulator commands group definition. 722 total commands, 26 Sub-groups, 11 group commands"""
	...
	def birthDeath(self):
		...
		if not hasattr(self, '_birthDeath'):
			from .Fsimulator_.BirthDeath import BirthDeath
			self._birthDeath = BirthDeath(self._core, self._base)
	...
	def bypass(self):
		...
		if not hasattr(self, '_bypass'):
			from .Fsimulator_.Bypass import Bypass
	...
	def cdynamic(self):
		...
		if not hasattr(self, '_cdynamic'):
			from .Fsimulator_.Cdynamic import Cdynamic
			self._cdynamic = Cdynamic(self._core, self._base)
	...
	def clock(self):
		...
		if not hasattr(self, '_clock'):
			from .Fsimulator_.Clock import Clock
	...
	def copy(self):
		...
		if not hasattr(self, '_copy'):
			from .Fsimulator_.Copy import Copy
	...
	def couple(self):
		...
		if not hasattr(self, '_couple'):
			from .Fsimulator_.Couple import Couple
			self._couple = Couple(self._core, self._base)
		return self._couple
	...
	def delPy(self):
		...
		if not hasattr(self, '_delPy'):
			from .Fsimulator_.DelPy import DelPy
			self._delPy = DelPy(self._core, self._base)
		return self._delPy
	...
	def delay(self):
		...
		if not hasattr(self, '_delay'):
			from .Fsimulator_.Delay import Delay
	...
	def dsSimulation(self):
		...
		if not hasattr(self, '_dsSimulation'):
			from .Fsimulator_.DsSimulation import DsSimulation
			self._dsSimulation = DsSimulation(self._core, self._base)
	...
	def frequency(self):
		...
		if not hasattr(self, '_frequency'):
			from .Fsimulator_.Frequency import Frequency
	...
	def hopping(self):
		...
		if not hasattr(self, '_hopping'):
			from .Fsimulator_.Hopping import Hopping
			self._hopping = Hopping(self._core, self._base)
		return self._hopping

	@property
	def hsTrain(self):
		"""hsTrain commands group. 3 Sub-classes, 5 commands."""
		if not hasattr(self, '_hsTrain'):
			from .Fsimulator_.HsTrain import HsTrain
			self._hsTrain = HsTrain(self._core, self._base)
	...
	def ignore(self):
		...
		if not hasattr(self, '_ignore'):
			from .Fsimulator_.Ignore import Ignore
			self._ignore = Ignore(self._core, self._base)
		return self._ignore
	...
	def mdelay(self):
		...
		if not hasattr(self, '_mdelay'):
			from .Fsimulator_.Mdelay import Mdelay
			self._mdelay = Mdelay(self._core, self._base)
		return self._mdelay
	...
	def mimo(self):
		...
		if not hasattr(self, '_mimo'):
			from .Fsimulator_.Mimo import Mimo
	...
	def scm(self):
		...
		if not hasattr(self, '_scm'):
			from .Fsimulator_.Scm import Scm
			self._scm = Scm(self._core, self._base)
		return self._scm
	...
	def siso(self):
		...
		if not hasattr(self, '_siso'):
			from .Fsimulator_.Siso import Siso
	...
	def speed(self):
		...
		if not hasattr(self, '_speed'):
			from .Fsimulator_.Speed import Speed
	...
	def standard(self):
		...
		if not hasattr(self, '_standard'):
			from .Fsimulator_.Standard import Standard
	...
	def sum(self):
		...
		if not hasattr(self, '_sum'):
			from .Fsimulator_.Sum import Sum
	...
	def synchronize(self):
		...
		if not hasattr(self, '_synchronize'):
			from .Fsimulator_.Synchronize import Synchronize
	...
	def tcInterferer(self):
		...
		if not hasattr(self, '_tcInterferer'):
			from .Fsimulator_.TcInterferer import TcInterferer
			self._tcInterferer = TcInterferer(self._core, self._base)
	...
	def test(self):
		...
		if not hasattr(self, '_test'):
			from .Fsimulator_.Test import Test
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:FSIMulator:PRESet')
	...
	def get_route(self) -> enums.FadConfPathOut:
		...
		Selects on which baseband path the faded signal is output. The input signal of the fader is selected with command method
		RsSmw.Source.Bb.route. For one-path instruments this command is query only. It returns value FAA (Fader A always outputs
		the signal on baseband A) . Note: All MIMO configurations are enabled only in SCONfiguration:MODE ADVanced.
	...
	def get_sdestination(self) -> enums.FadSignDest:
		...
		Defines the frequency to that the signal of the whole Fader block is dedicated. \n
			:return: sdestination: RF| BB RF The Doppler shift is calculated based on the actual RF frequency, that is dynamically estimated. To query the estimated dedicated frequency, use the command method RsSmw.Source.Fsimulator.Frequency.value. To query the output connector, use the command method RsSmw.Source.Fsimulator.Frequency.detect. BB Set the fader frequency manually by means of the command method RsSmw.Source.Fsimulator.Frequency.value.
		"""
