...
class Power:
	"""Power commands group definition. 10 total commands, 3 Sub-groups, 7 group commands"""
	...
	def get_mode(self) -> enums.AutoManStep:
		...
		Sets the cycle mode for the level sweep. \n
			:return: mode: AUTO| MANual| STEP AUTO Each trigger triggers exactly one complete sweep. MANual The trigger system is not active. You can trigger every step individually with the command method RsSmw.Source.Power.manual. The level value increases at each step by the value that you define with method RsSmw.Source.Power.Level.Immediate.amplitude. Values directly entered with the command method RsSmw.Source.Power.manual are not taken into account. STEP Each trigger triggers one sweep step only. The level increases by the value entered with method RsSmw.Source.Power.Level.Immediate.amplitude.
		"""
