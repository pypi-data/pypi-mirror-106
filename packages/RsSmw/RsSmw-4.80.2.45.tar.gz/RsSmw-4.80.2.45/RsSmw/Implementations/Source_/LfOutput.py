...
class LfOutput:
	"""LfOutput commands group definition. 31 total commands, 9 Sub-groups, 0 group commands
	Repeated Capability: LfOutput, default value after init: LfOutput.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("lfOutput", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_lfOutput_get', 'repcap_lfOutput_set', repcap.LfOutput.Nr1)

	def repcap_lfOutput_set(self, enum_value: repcap.LfOutput) -> None:
	...
	def offset(self):
		...
		if not hasattr(self, '_offset'):
			from .LfOutput_.Offset import Offset
	...
	def voltage(self):
		...
		if not hasattr(self, '_voltage'):
			from .LfOutput_.Voltage import Voltage
