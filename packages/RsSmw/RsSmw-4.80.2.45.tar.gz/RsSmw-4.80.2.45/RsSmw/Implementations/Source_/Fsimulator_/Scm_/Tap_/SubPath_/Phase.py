...
class Phase:
	"""Phase commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""
	...
	def vh(self):
		...
		if not hasattr(self, '_vh'):
			from .Phase_.Vh import Vh
			self._vh = Vh(self._core, self._base)
		return self._vh
	...
	def vv(self):
		...
		if not hasattr(self, '_vv'):
			from .Phase_.Vv import Vv
			self._vv = Vv(self._core, self._base)
		return self._vv
