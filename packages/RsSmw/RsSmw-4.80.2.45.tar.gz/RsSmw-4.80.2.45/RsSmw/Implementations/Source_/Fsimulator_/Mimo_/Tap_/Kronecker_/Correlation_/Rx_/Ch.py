...
class Ch:
	"""Ch commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""
	...
	def imaginary(self):
		...
		if not hasattr(self, '_imaginary'):
			from .Ch_.Imaginary import Imaginary
	...
	def magnitude(self):
		...
		if not hasattr(self, '_magnitude'):
			from .Ch_.Magnitude import Magnitude
	...
	def clone(self) -> 'Ch':
		...
		new_group = Ch(self._core, self._base.parent)
