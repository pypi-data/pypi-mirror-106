...
class Spread:
	"""Spread commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("spread", core, parent)
	...
	def set(self, arr_zen_spr: float, mimoTap=repcap.MimoTap.Default) -> None:
		...
		Sets the AoA (angle of arrival) / AoD (angle of departure) spread (AS) of the cluster. \n
			:param arr_zen_spr: float Range: 1 to 75
	...
	def get(self, mimoTap=repcap.MimoTap.Default) -> float:
		...
		Sets the AoA (angle of arrival) / AoD (angle of departure) spread (AS) of the cluster. \n
			:param mimoTap: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Tap')
			:return: arr_zen_spr: No help available"""
		mimoTap_cmd_val = self._base.get_repcap_cmd_value(mimoTap, repcap.MimoTap)
