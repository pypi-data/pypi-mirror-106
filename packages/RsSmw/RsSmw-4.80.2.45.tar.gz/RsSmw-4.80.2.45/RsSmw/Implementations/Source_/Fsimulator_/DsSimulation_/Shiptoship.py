...
class Shiptoship:
	"""Shiptoship commands group definition. 18 total commands, 3 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("shiptoship", core, parent)

	@property
	def rx(self):
		"""rx commands group. 2 Sub-classes, 3 commands."""
		if not hasattr(self, '_rx'):
			from .Shiptoship_.Rx import Rx
	...
	def water(self):
		...
		if not hasattr(self, '_water'):
			from .Shiptoship_.Water import Water
			self._water = Water(self._core, self._base)
		return self._water
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:FSIMulator:DSSimulation:SHIPtoship:PRESet')
	...
	def clone(self) -> 'Shiptoship':
		...
		new_group = Shiptoship(self._core, self._base.parent)
