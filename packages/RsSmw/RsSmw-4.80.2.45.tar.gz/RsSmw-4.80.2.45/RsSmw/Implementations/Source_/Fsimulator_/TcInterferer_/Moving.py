...
class Moving:
	"""Moving commands group definition. 9 total commands, 2 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("moving", core, parent)

	@property
	def delay(self):
		"""delay commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_delay'):
			from .Moving_.Delay import Delay
	...
	def fdoppler(self):
		...
		if not hasattr(self, '_fdoppler'):
			from .Moving_.Fdoppler import Fdoppler
	...
	def get_state(self) -> bool:
		...
		configuration and the fading simulator must be switched on separately, see method RsSmw.Source.Fsimulator.state and .
	...
	def clone(self) -> 'Moving':
		...
		new_group = Moving(self._core, self._base.parent)
