...
class Frequency:
	...
	def get_detect(self) -> enums.TmastConn:
		...
		Queries the output interface the steam used to estimate the dedicated frequency is mapped to. \n
			:return: detect_master: RFA| BBMM1| RFB| BBMM2| IQOUT1| IQOUT2| FAD1| FAD2| FAD4| FAD3| DEF
		"""
	...
	def get_value(self) -> float:
		...
		Snippet: value: float = driver.source.fsimulator.frequency.get_value() \n
			...
			- If method RsSmw.Source.Fsimulator.sdestinationRF is selected, queries the estimated RF frequency.
			- If method RsSmw.Source.Fsimulator.sdestinationBB is selected, sets the frequency used for the calculation of the Doppler shift. \n
