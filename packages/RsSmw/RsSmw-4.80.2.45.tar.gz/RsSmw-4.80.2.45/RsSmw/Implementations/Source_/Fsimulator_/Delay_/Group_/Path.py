from ......Internal.Core import Core
from ......Internal.CommandsGroup import CommandsGroup
from ......Internal.RepeatedCapability import RepeatedCapability
from ...... import repcap


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class Path:
	"""Path commands group definition. 22 total commands, 16 Sub-groups, 0 group commands
	Repeated Capability: Path, default value after init: Path.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("path", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_path_get', 'repcap_path_set', repcap.Path.Nr1)

	def repcap_path_set(self, enum_value: repcap.Path) -> None:
		"""Repeated Capability default value numeric suffix.
		This value is used, if you do not explicitely set it in the child set/get methods, or if you leave it to Path.Default
		Default value after init: Path.Nr1"""
		self._base.set_repcap_enum_value(enum_value)

	def repcap_path_get(self) -> repcap.Path:
		"""Returns the current default repeated capability for the child set/get methods"""
		# noinspection PyTypeChecker
		return self._base.get_repcap_enum_value()

	@property
	def adelay(self):
		"""adelay commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_adelay'):
			from .Path_.Adelay import Adelay
			self._adelay = Adelay(self._core, self._base)
		return self._adelay

	@property
	def bdelay(self):
		"""bdelay commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_bdelay'):
			from .Path_.Bdelay import Bdelay
			self._bdelay = Bdelay(self._core, self._base)
		return self._bdelay

	@property
	def correlation(self):
		"""correlation commands group. 3 Sub-classes, 0 commands."""
		if not hasattr(self, '_correlation'):
			from .Path_.Correlation import Correlation
			self._correlation = Correlation(self._core, self._base)
		return self._correlation

	@property
	def cphase(self):
		"""cphase commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_cphase'):
			from .Path_.Cphase import Cphase
			self._cphase = Cphase(self._core, self._base)
		return self._cphase

	@property
	def custom(self):
		"""custom commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_custom'):
			from .Path_.Custom import Custom
			self._custom = Custom(self._core, self._base)
		return self._custom

	@property
	def fdoppler(self):
		"""fdoppler commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_fdoppler'):
			from .Path_.Fdoppler import Fdoppler
			self._fdoppler = Fdoppler(self._core, self._base)
		return self._fdoppler

	@property
	def fratio(self):
		"""fratio commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_fratio'):
			from .Path_.Fratio import Fratio
			self._fratio = Fratio(self._core, self._base)
		return self._fratio

	@property
	def fshift(self):
		"""fshift commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_fshift'):
			from .Path_.Fshift import Fshift
			self._fshift = Fshift(self._core, self._base)
		return self._fshift

	@property
	def fspread(self):
		"""fspread commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_fspread'):
			from .Path_.Fspread import Fspread
			self._fspread = Fspread(self._core, self._base)
		return self._fspread

	@property
	def logNormal(self):
		"""logNormal commands group. 3 Sub-classes, 0 commands."""
		if not hasattr(self, '_logNormal'):
			from .Path_.LogNormal import LogNormal
			self._logNormal = LogNormal(self._core, self._base)
		return self._logNormal

	@property
	def loss(self):
		"""loss commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_loss'):
			from .Path_.Loss import Loss
			self._loss = Loss(self._core, self._base)
		return self._loss

	@property
	def pratio(self):
		"""pratio commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_pratio'):
			from .Path_.Pratio import Pratio
			self._pratio = Pratio(self._core, self._base)
		return self._pratio

	@property
	def profile(self):
		"""profile commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_profile'):
			from .Path_.Profile import Profile
			self._profile = Profile(self._core, self._base)
		return self._profile

	@property
	def rdelay(self):
		"""rdelay commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_rdelay'):
			from .Path_.Rdelay import Rdelay
			self._rdelay = Rdelay(self._core, self._base)
		return self._rdelay

	@property
	def speed(self):
		"""speed commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_speed'):
			from .Path_.Speed import Speed
			self._speed = Speed(self._core, self._base)
		return self._speed

	@property
	def state(self):
		"""state commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_state'):
			from .Path_.State import State
			self._state = State(self._core, self._base)
		return self._state

	def clone(self) -> 'Path':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Path(self._core, self._base.parent)
		self._base.synchronize_repcaps(new_group)
		return new_group
