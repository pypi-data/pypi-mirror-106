...
class Tap:
	"""Tap commands group definition. 377 total commands, 5 Sub-groups, 1 group commands
	...
	def antenna(self):
		...
		if not hasattr(self, '_antenna'):
			from .Tap_.Antenna import Antenna
	...
	def gvector(self):
		...
		if not hasattr(self, '_gvector'):
			from .Tap_.Gvector import Gvector
			self._gvector = Gvector(self._core, self._base)
	...
	def kronecker(self):
		...
		if not hasattr(self, '_kronecker'):
			from .Tap_.Kronecker import Kronecker
			self._kronecker = Kronecker(self._core, self._base)
	...
	def matrix(self):
		...
		if not hasattr(self, '_matrix'):
			from .Tap_.Matrix import Matrix
