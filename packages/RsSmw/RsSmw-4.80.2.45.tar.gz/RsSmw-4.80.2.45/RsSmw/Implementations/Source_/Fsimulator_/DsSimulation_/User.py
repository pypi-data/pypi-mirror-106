...
class User:
	"""User commands group definition. 39 total commands, 4 Sub-groups, 0 group commands"""
	...
	def profile(self):
		...
		if not hasattr(self, '_profile'):
			from .User_.Profile import Profile
	...
	def tpa(self):
		...
		if not hasattr(self, '_tpa'):
			from .User_.Tpa import Tpa
			self._tpa = Tpa(self._core, self._base)
		return self._tpa
