...
class Polarization:
	"""Polarization commands group definition. 3 total commands, 1 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("polarization", core, parent)

	@property
	def pratio(self):
		"""pratio commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_pratio'):
			from .Polarization_.Pratio import Pratio
	...
	def clone(self) -> 'Polarization':
		...
		new_group = Polarization(self._core, self._base.parent)
