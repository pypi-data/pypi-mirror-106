...
class Bh:
	"""Bh commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("bh", core, parent)

	@property
	def imaginary(self):
		"""imaginary commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_imaginary'):
			from .Bh_.Imaginary import Imaginary
	...
	def magnitude(self):
		...
		if not hasattr(self, '_magnitude'):
			from .Bh_.Magnitude import Magnitude
	...
	def clone(self) -> 'Bh':
		...
		new_group = Bh(self._core, self._base.parent)
