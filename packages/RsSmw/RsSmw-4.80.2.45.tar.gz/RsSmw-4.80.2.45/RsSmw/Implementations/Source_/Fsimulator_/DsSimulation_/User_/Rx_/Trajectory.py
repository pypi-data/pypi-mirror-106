...
class Trajectory:
	"""Trajectory commands group definition. 12 total commands, 3 Sub-groups, 1 group commands"""
	...
	def ephemeris(self):
		...
		if not hasattr(self, '_ephemeris'):
			from .Trajectory_.Ephemeris import Ephemeris
	...
	def fapoint(self):
		...
		if not hasattr(self, '_fapoint'):
			from .Trajectory_.Fapoint import Fapoint
			self._fapoint = Fapoint(self._core, self._base)
	...
	def tdf(self):
		...
		if not hasattr(self, '_tdf'):
			from .Trajectory_.Tdf import Tdf
			self._tdf = Tdf(self._core, self._base)
		return self._tdf
