...
class Cdynamic:
	"""Cdynamic commands group definition. 7 total commands, 2 Sub-groups, 2 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("cdynamic", core, parent)

	@property
	def catalog(self):
		"""catalog commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_catalog'):
			from .Cdynamic_.Catalog import Catalog
	...
	def path(self):
		...
		if not hasattr(self, '_path'):
			from .Cdynamic_.Path import Path
	...
	def clone(self) -> 'Cdynamic':
		...
		new_group = Cdynamic(self._core, self._base.parent)
