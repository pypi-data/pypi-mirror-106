...
class Level:
	"""Level commands group definition. 4 total commands, 2 Sub-groups, 0 group commands"""
	...
	def via(self):
		...
		if not hasattr(self, '_via'):
			from .Level_.Via import Via
			self._via = Via(self._core, self._base)
		return self._via
	...
	def immediate(self):
		...
		if not hasattr(self, '_immediate'):
			from .Level_.Immediate import Immediate
