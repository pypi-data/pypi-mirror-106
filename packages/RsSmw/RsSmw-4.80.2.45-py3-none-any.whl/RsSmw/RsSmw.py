...
class RsSmw:
	"""11472 total commands, 27 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMW/SMBV/SMM/SMA/SMCV, SupportedIdnPatterns = SMW/SMBV/SMM/SMA/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMW200A,100001,4.80.2.0045'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.80.2.0045'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.80.2.0045'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmw version failed. Current version: '4.80.2.0045', minimum required version: '{min_version}'")
	...
	@property
	def display(self):
		"""display commands group. 5 Sub-classes, 3 commands."""
	...
	def instruments(self):
		"""instruments commands group. 6 Sub-classes, 4 commands."""
		if not hasattr(self, '_instruments'):
			from .Implementations.Instruments import Instruments
			self._instruments = Instruments(self._core, self._base)
	...
	@property
	def source(self):
		"""source commands group. 27 Sub-classes, 1 commands."""
