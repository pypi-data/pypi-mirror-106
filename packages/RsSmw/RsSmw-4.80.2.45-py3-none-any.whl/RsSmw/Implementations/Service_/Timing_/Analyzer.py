...
class Analyzer:
	"""Analyzer commands group definition. 3 total commands, 2 Sub-groups, 1 group commands"""
	...
	def enable(self):
		...
		if not hasattr(self, '_enable'):
			from .Analyzer_.Enable import Enable
	...
	def execute(self):
		...
		if not hasattr(self, '_execute'):
			from .Analyzer_.Execute import Execute
	...
	def clone(self) -> 'Analyzer':
		...
		new_group = Analyzer(self._core, self._base.parent)
