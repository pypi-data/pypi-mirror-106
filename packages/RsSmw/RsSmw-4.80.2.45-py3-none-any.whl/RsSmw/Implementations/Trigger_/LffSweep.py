...
class LffSweep:
	"""LffSweep commands group definition. 3 total commands, 2 Sub-groups, 1 group commands"""
	...
	def set(self, inputIx=repcap.InputIx.Default) -> None:
		...
		Snippet: driver.trigger.lffSweep.set(inputIx = repcap.InputIx.Default) \n
			...
			- method RsSmw.Source.LfOutput.Sweep.Frequency.mode AUTO \n
	...
	def set_with_opc(self, inputIx=repcap.InputIx.Default) -> None:
		inputIx_cmd_val = self._base.get_repcap_cmd_value(inputIx, repcap.InputIx)
		"""SCPI: TRIGger<HW>:LFFSweep \n
		...
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmw.utilities.opc_timeout_set() to set the timeout value. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Trigger')"""
		self._core.io.write_with_opc(f'TRIGger{inputIx_cmd_val}:LFFSweep')
