...
class All:
	"""All commands group definition. 6 total commands, 1 Sub-groups, 5 group commands"""
	...
	def get_warn(self) -> bool:
		"""SCPI: CALibration:ALL:WARN \n
		Snippet: value: bool = driver.calibration.all.get_warn() \n
		Generates a warning message, if the instrument is in uncalibrated state. \n
			:return: uncal_warning: 0| 1| OFF| ON
		"""
		response = self._core.io.query_str('CALibration:ALL:WARN?')
		return Conversions.str_to_bool(response)

	def set_warn(self, uncal_warning: bool) -> None:
		"""SCPI: CALibration:ALL:WARN \n
		Snippet: driver.calibration.all.set_warn(uncal_warning = False) \n
		Generates a warning message, if the instrument is in uncalibrated state. \n
			:param uncal_warning: 0| 1| OFF| ON
		"""
		param = Conversions.bool_to_str(uncal_warning)
		self._core.io.write(f'CALibration:ALL:WARN {param}')
