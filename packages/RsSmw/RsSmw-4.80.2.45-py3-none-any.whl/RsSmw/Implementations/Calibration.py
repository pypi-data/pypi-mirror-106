...
class Calibration:
	"""Calibration commands group definition. 37 total commands, 10 Sub-groups, 2 group commands"""
	...
	@property
	def all(self):
		"""all commands group. 1 Sub-classes, 5 commands."""
	...
	def loscillator(self):
		...
		if not hasattr(self, '_loscillator'):
			from .Calibration_.Loscillator import Loscillator
	...
	def selected(self):
		...
		if not hasattr(self, '_selected'):
			from .Calibration_.Selected import Selected
			self._selected = Selected(self._core, self._base)
