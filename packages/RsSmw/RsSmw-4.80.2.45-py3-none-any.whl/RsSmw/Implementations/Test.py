...
class Test:
	"""Test commands group definition. 46 total commands, 13 Sub-groups, 5 group commands"""
	...
	def fader(self):
		...
		if not hasattr(self, '_fader'):
			from .Test_.Fader import Fader
			self._fader = Fader(self._core, self._base)
		return self._fader
	...
	def generator(self):
		...
		if not hasattr(self, '_generator'):
			from .Test_.Generator import Generator
	...
	def plimit(self):
		...
		if not hasattr(self, '_plimit'):
			from .Test_.Plimit import Plimit
			self._plimit = Plimit(self._core, self._base)
		return self._plimit
