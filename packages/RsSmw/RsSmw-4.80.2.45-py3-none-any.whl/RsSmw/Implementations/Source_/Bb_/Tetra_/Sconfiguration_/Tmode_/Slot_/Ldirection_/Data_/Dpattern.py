...
class Dpattern:
	"""Dpattern commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dpattern", core, parent)
	...
	def set(self, structure: DpatternStruct, testMode=repcap.TestMode.Default, slot=repcap.Slot.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:TETRa:SCONfiguration:TMODe<DI>:SLOT<ST>:LDIRection<CH>:DATA:DPATtern \n
		Snippet: driver.source.bb.tetra.sconfiguration.tmode.slot.ldirection.data.dpattern.set(value = [PROPERTY_STRUCT_NAME](), testMode = repcap.TestMode.Default, slot = repcap.Slot.Default, channel = repcap.Channel.Default) \n
		Selects the data pattern for data source pattern (method RsSmw.Source.Bb.Tetra.Sconfiguration.Tmode.Slot.Ldirection.Data.
		set) . \n
			:param structure: for set value, see the help for DpatternStruct structure arguments.
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:TETRa:SCONfiguration:TMODe{testMode_cmd_val}:SLOT{slot_cmd_val}:LDIRection{channel_cmd_val}:DATA:DPATtern', structure)
