...
class Arbitrary:
	"""Arbitrary commands group definition. 144 total commands, 11 Sub-groups, 2 group commands"""
	...
	def coupled(self):
		...
		if not hasattr(self, '_coupled'):
			from .Arbitrary_.Coupled import Coupled
			self._coupled = Coupled(self._core, self._base)
	...
	def notch(self):
		...
		if not hasattr(self, '_notch'):
			from .Arbitrary_.Notch import Notch
	...
	@property
	def trigger(self):
		"""trigger commands group. 6 Sub-classes, 6 commands."""
