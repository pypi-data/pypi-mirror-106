...
class Bbmm:
	"""Bbmm commands group definition. 8 total commands, 7 Sub-groups, 0 group commands
	...
	def iqRatio(self):
		...
		if not hasattr(self, '_iqRatio'):
			from .Bbmm_.IqRatio import IqRatio
	...
	def leakage(self):
		...
		if not hasattr(self, '_leakage'):
			from .Bbmm_.Leakage import Leakage
	...
	def poffset(self):
		...
		if not hasattr(self, '_poffset'):
			from .Bbmm_.Poffset import Poffset
	...
	def quadrature(self):
		...
		if not hasattr(self, '_quadrature'):
			from .Bbmm_.Quadrature import Quadrature
