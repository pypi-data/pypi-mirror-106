...
class Xpdcch:
	"""Xpdcch commands group definition. 67 total commands, 14 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("xpdcch", core, parent)

	@property
	def alRegs(self):
		"""alRegs commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_alRegs'):
			from .Xpdcch_.AlRegs import AlRegs
	...
	def avcces(self):
		...
		if not hasattr(self, '_avcces'):
			from .Xpdcch_.Avcces import Avcces
	...
	def avRegs(self):
		...
		if not hasattr(self, '_avRegs'):
			from .Xpdcch_.AvRegs import AvRegs
	...
	def dcRegs(self):
		...
		if not hasattr(self, '_dcRegs'):
			from .Xpdcch_.DcRegs import DcRegs
	...
	def dregs(self):
		...
		if not hasattr(self, '_dregs'):
			from .Xpdcch_.Dregs import Dregs
	...
	def dselect(self):
		...
		if not hasattr(self, '_dselect'):
			from .Xpdcch_.Dselect import Dselect
	...
	def formatPy(self):
		...
		if not hasattr(self, '_formatPy'):
			from .Xpdcch_.FormatPy import FormatPy
	...
	def noPdcchs(self):
		...
		if not hasattr(self, '_noPdcchs'):
			from .Xpdcch_.NoPdcchs import NoPdcchs
	...
	def pattern(self):
		...
		if not hasattr(self, '_pattern'):
			from .Xpdcch_.Pattern import Pattern
	...
	def power(self):
		...
		if not hasattr(self, '_power'):
			from .Xpdcch_.Power import Power
	...
	def scrambling(self):
		...
		if not hasattr(self, '_scrambling'):
			from .Xpdcch_.Scrambling import Scrambling
	...
	def clone(self) -> 'Xpdcch':
		...
		new_group = Xpdcch(self._core, self._base.parent)
