...
class Xpucch:
	"""Xpucch commands group definition. 9 total commands, 9 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("xpucch", core, parent)

	@property
	def state(self):
		"""state commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_state'):
			from .Xpucch_.State import State
	...
	def conflict(self):
		...
		if not hasattr(self, '_conflict'):
			from .Xpucch_.Conflict import Conflict
	...
	def napUsed(self):
		...
		if not hasattr(self, '_napUsed'):
			from .Xpucch_.NapUsed import NapUsed
	...
	def nxPucch(self):
		...
		if not hasattr(self, '_nxPucch'):
			from .Xpucch_.NxPucch import NxPucch
			self._nxPucch = NxPucch(self._core, self._base)
	...
	def physBits(self):
		...
		if not hasattr(self, '_physBits'):
			from .Xpucch_.PhysBits import PhysBits
	...
	def power(self):
		...
		if not hasattr(self, '_power'):
			from .Xpucch_.Power import Power
	...
	def rbCount(self):
		...
		if not hasattr(self, '_rbCount'):
			from .Xpucch_.RbCount import RbCount
	...
	def rbOffset(self):
		...
		if not hasattr(self, '_rbOffset'):
			from .Xpucch_.RbOffset import RbOffset
	...
	def formatPy(self):
		...
		if not hasattr(self, '_formatPy'):
			from .Xpucch_.FormatPy import FormatPy
	...
	def clone(self) -> 'Xpucch':
		...
		new_group = Xpucch(self._core, self._base.parent)
