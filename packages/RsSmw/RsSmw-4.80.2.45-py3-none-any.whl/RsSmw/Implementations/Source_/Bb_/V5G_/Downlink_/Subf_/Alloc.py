...
class Alloc:
	"""Alloc commands group definition. 40 total commands, 22 Sub-groups, 0 group commands
	...
	def rbCount(self):
		...
		if not hasattr(self, '_rbCount'):
			from .Alloc_.RbCount import RbCount
	...
	def symCount(self):
		...
		if not hasattr(self, '_symCount'):
			from .Alloc_.SymCount import SymCount
	...
	def xpdsch(self):
		...
		if not hasattr(self, '_xpdsch'):
			from .Alloc_.Xpdsch import Xpdsch
			self._xpdsch = Xpdsch(self._core, self._base)
		return self._xpdsch
