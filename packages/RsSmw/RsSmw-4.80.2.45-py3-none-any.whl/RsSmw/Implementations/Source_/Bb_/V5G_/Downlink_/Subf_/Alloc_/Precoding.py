...
class Precoding:
	"""Precoding commands group definition. 9 total commands, 9 Sub-groups, 0 group commands"""
	...
	def cdd(self):
		...
		if not hasattr(self, '_cdd'):
			from .Precoding_.Cdd import Cdd
			self._cdd = Cdd(self._core, self._base)
		return self._cdd
	...
	def lcount(self):
		...
		if not hasattr(self, '_lcount'):
			from .Precoding_.Lcount import Lcount
			self._lcount = Lcount(self._core, self._base)
		return self._lcount
