...
class Item:
	"""Item commands group definition. 41 total commands, 11 Sub-groups, 0 group commands
	Repeated Capability: ItemNull, default value after init: ItemNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("item", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_itemNull_get', 'repcap_itemNull_set', repcap.ItemNull.Nr0)

	def repcap_itemNull_set(self, enum_value: repcap.ItemNull) -> None:
	...
	def symbol(self):
		...
		if not hasattr(self, '_symbol'):
			from .Item_.Symbol import Symbol
