...
class Cw:
	"""Cw commands group definition. 13 total commands, 2 Sub-groups, 0 group commands
	Repeated Capability: Codeword, default value after init: Codeword.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("cw", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_codeword_get', 'repcap_codeword_set', repcap.Codeword.Nr1)
	...
	def puach(self):
		...
		if not hasattr(self, '_puach'):
			from .Cw_.Puach import Puach
			self._puach = Puach(self._core, self._base)
		return self._puach
