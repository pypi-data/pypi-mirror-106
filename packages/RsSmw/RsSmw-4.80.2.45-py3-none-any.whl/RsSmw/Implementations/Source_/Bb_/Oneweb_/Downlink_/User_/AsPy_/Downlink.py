...
class Downlink:
	"""Downlink commands group definition. 31 total commands, 6 Sub-groups, 0 group commands"""
	...
	def afSeq(self):
		...
		if not hasattr(self, '_afSeq'):
			from .Downlink_.AfSeq import AfSeq
	...
	def indi(self):
		...
		if not hasattr(self, '_indi'):
			from .Downlink_.Indi import Indi
	...
	def nhids(self):
		...
		if not hasattr(self, '_nhids'):
			from .Downlink_.Nhids import Nhids
	...
	def nhtrans(self):
		...
		if not hasattr(self, '_nhtrans'):
			from .Downlink_.Nhtrans import Nhtrans
	...
	def skProcess(self):
		...
		if not hasattr(self, '_skProcess'):
			from .Downlink_.SkProcess import SkProcess
