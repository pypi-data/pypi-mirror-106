...
class Puach:
	"""Puach commands group definition. 16 total commands, 9 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("puach", core, parent)

	@property
	def codewords(self):
		"""codewords commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_codewords'):
			from .Puach_.Codewords import Codewords
	...
	def conflict(self):
		...
		if not hasattr(self, '_conflict'):
			from .Puach_.Conflict import Conflict
	...
	def ndmrs(self):
		...
		if not hasattr(self, '_ndmrs'):
			from .Puach_.Ndmrs import Ndmrs
	...
	def power(self):
		...
		if not hasattr(self, '_power'):
			from .Puach_.Power import Power
	...
	def state(self):
		...
		if not hasattr(self, '_state'):
			from .Puach_.State import State
	...
	def clone(self) -> 'Puach':
		...
		new_group = Puach(self._core, self._base.parent)
