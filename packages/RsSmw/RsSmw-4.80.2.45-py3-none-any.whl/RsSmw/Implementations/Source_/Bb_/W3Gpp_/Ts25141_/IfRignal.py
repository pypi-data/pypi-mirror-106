...
class IfRignal:
	"""IfRignal commands group definition. 14 total commands, 3 Sub-groups, 6 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("ifRignal", core, parent)

	@property
	def cw(self):
		"""cw commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_cw'):
			from .IfRignal_.Cw import Cw
	...
	def modulated(self):
		...
		if not hasattr(self, '_modulated'):
			from .IfRignal_.Modulated import Modulated
			self._modulated = Modulated(self._core, self._base)
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .IfRignal_.Setting import Setting
	...
	def clone(self) -> 'IfRignal':
		...
		new_group = IfRignal(self._core, self._base.parent)
