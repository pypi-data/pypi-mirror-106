...
class Ts25141:
	"""Ts25141 commands group definition. 55 total commands, 8 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("ts25141", core, parent)

	@property
	def awgn(self):
		"""awgn commands group. 3 Sub-classes, 3 commands."""
		if not hasattr(self, '_awgn'):
			from .Ts25141_.Awgn import Awgn
	...
	def bsSignal(self):
		...
		if not hasattr(self, '_bsSignal'):
			from .Ts25141_.BsSignal import BsSignal
			self._bsSignal = BsSignal(self._core, self._base)
	...
	def fsimulator(self):
		...
		if not hasattr(self, '_fsimulator'):
			from .Ts25141_.Fsimulator import Fsimulator
	...
	def ifRignal(self):
		...
		if not hasattr(self, '_ifRignal'):
			from .Ts25141_.IfRignal import IfRignal
			self._ifRignal = IfRignal(self._core, self._base)
	...
	def scode(self):
		...
		if not hasattr(self, '_scode'):
			from .Ts25141_.Scode import Scode
	...
	def tcase(self):
		...
		if not hasattr(self, '_tcase'):
			from .Ts25141_.Tcase import Tcase
			self._tcase = Tcase(self._core, self._base)
		return self._tcase
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Ts25141_.Trigger import Trigger
	...
	def wsignal(self):
		...
		if not hasattr(self, '_wsignal'):
			from .Ts25141_.Wsignal import Wsignal
			self._wsignal = Wsignal(self._core, self._base)
	...
	def get_rx_diversity(self) -> bool:
		...
		for two-path-instruments and only for test cases that do not use both paths anyway. \n
			:return: rx_diversity: 0| 1| OFF| ON
		"""
	...
	def clone(self) -> 'Ts25141':
		...
		new_group = Ts25141(self._core, self._base.parent)
