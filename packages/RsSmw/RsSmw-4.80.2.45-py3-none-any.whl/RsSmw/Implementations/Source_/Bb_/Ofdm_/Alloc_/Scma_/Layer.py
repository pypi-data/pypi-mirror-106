...
class Layer:
	"""Layer commands group definition. 3 total commands, 3 Sub-groups, 0 group commands
	Repeated Capability: LayerNull, default value after init: LayerNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("layer", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_layerNull_get', 'repcap_layerNull_set', repcap.LayerNull.Nr0)

	def repcap_layerNull_set(self, enum_value: repcap.LayerNull) -> None:
	...
	def state(self):
		...
		if not hasattr(self, '_state'):
			from .Layer_.State import State
