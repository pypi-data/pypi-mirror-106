...
class Beidou:
	"""Beidou commands group definition. 207 total commands, 13 Sub-groups, 0 group commands"""
	...
	def healthy(self):
		...
		if not hasattr(self, '_healthy'):
			from .Beidou_.Healthy import Healthy
	...
	def listPy(self):
		...
		if not hasattr(self, '_listPy'):
			from .Beidou_.ListPy import ListPy
	...
	def mcontrol(self):
		...
		if not hasattr(self, '_mcontrol'):
			from .Beidou_.Mcontrol import Mcontrol
	...
	def mpath(self):
		...
		if not hasattr(self, '_mpath'):
			from .Beidou_.Mpath import Mpath
	...
	def power(self):
		...
		if not hasattr(self, '_power'):
			from .Beidou_.Power import Power
	...
	def prErrors(self):
		...
		if not hasattr(self, '_prErrors'):
			from .Beidou_.PrErrors import PrErrors
	...
	def present(self):
		...
		if not hasattr(self, '_present'):
			from .Beidou_.Present import Present
	...
	def sdynamics(self):
		...
		if not hasattr(self, '_sdynamics'):
			from .Beidou_.Sdynamics import Sdynamics
	...
	def signal(self):
		...
		if not hasattr(self, '_signal'):
			from .Beidou_.Signal import Signal
	...
	def simulated(self):
		...
		if not hasattr(self, '_simulated'):
			from .Beidou_.Simulated import Simulated
	...
	def state(self):
		...
		if not hasattr(self, '_state'):
			from .Beidou_.State import State
	...
	def visibility(self):
		...
		if not hasattr(self, '_visibility'):
			from .Beidou_.Visibility import Visibility
