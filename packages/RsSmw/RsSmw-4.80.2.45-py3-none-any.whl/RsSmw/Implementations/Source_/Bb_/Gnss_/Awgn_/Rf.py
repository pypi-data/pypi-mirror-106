...
class Rf:
	"""Rf commands group definition. 10 total commands, 7 Sub-groups, 0 group commands
	...
	def bandwidth(self):
		...
		if not hasattr(self, '_bandwidth'):
			from .Rf_.Bandwidth import Bandwidth
	...
	def cndRatio(self):
		...
		if not hasattr(self, '_cndRatio'):
			from .Rf_.CndRatio import CndRatio
			self._cndRatio = CndRatio(self._core, self._base)
	...
	def frequency(self):
		...
		if not hasattr(self, '_frequency'):
			from .Rf_.Frequency import Frequency
	...
	def jsRatio(self):
		...
		if not hasattr(self, '_jsRatio'):
			from .Rf_.JsRatio import JsRatio
			self._jsRatio = JsRatio(self._core, self._base)
