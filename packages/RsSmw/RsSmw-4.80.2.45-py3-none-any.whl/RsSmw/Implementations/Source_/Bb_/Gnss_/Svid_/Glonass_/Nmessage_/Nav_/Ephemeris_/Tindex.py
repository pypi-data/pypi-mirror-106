...
class Tindex:
	"""Tindex commands group definition. 2 total commands, 1 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tindex", core, parent)

	@property
	def unscaled(self):
		"""unscaled commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_unscaled'):
			from .Tindex_.Unscaled import Unscaled
	...
	def clone(self) -> 'Tindex':
		...
		new_group = Tindex(self._core, self._base.parent)
