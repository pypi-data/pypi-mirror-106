...
class Esequencer:
	"""Esequencer commands group definition. 94 total commands, 11 Sub-groups, 6 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("esequencer", core, parent)

	@property
	def archive(self):
		"""archive commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_archive'):
			from .Esequencer_.Archive import Archive
			self._archive = Archive(self._core, self._base)
	...
	def clock(self):
		...
		if not hasattr(self, '_clock'):
			from .Esequencer_.Clock import Clock
	...
	def dfinding(self):
		...
		if not hasattr(self, '_dfinding'):
			from .Esequencer_.Dfinding import Dfinding
			self._dfinding = Dfinding(self._core, self._base)
	...
	def pramp(self):
		...
		if not hasattr(self, '_pramp'):
			from .Esequencer_.Pramp import Pramp
	...
	def psequencer(self):
		...
		if not hasattr(self, '_psequencer'):
			from .Esequencer_.Psequencer import Psequencer
			self._psequencer = Psequencer(self._core, self._base)
	...
	def rtci(self):
		...
		if not hasattr(self, '_rtci'):
			from .Esequencer_.Rtci import Rtci
			self._rtci = Rtci(self._core, self._base)
		return self._rtci
	...
	def sequencer(self):
		...
		if not hasattr(self, '_sequencer'):
			from .Esequencer_.Sequencer import Sequencer
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Esequencer_.Setting import Setting
	...
	def stream(self):
		...
		if not hasattr(self, '_stream'):
			from .Esequencer_.Stream import Stream
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Esequencer_.Trigger import Trigger
	...
	def user(self):
		...
		if not hasattr(self, '_user'):
			from .Esequencer_.User import User
	...
	def set_mode(self, mode: enums.ExtSeqMode) -> None:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:MODE \n
		Snippet: driver.source.bb.esequencer.set_mode(mode = enums.ExtSeqMode.DFINding) \n
		Selects the processing mode for the extended sequencer. \n
	...
	def preset(self) -> None:
		...
		affected is the state set with the command method RsSmw.Source.Bb.Esequencer.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:ESEQuencer:PRESet')
	...
	def get_seq_count(self) -> int:
		...
		No command help available \n
			:return: numb_of_sequencer: No help available
		"""
	...
	def clone(self) -> 'Esequencer':
		...
		new_group = Esequencer(self._core, self._base.parent)
