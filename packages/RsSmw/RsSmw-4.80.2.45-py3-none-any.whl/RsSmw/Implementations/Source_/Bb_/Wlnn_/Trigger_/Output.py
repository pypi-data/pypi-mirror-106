...
class Output:
	"""Output commands group definition. 14 total commands, 10 Sub-groups, 0 group commands
	Repeated Capability: Output, default value after init: Output.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("output", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_output_get', 'repcap_output_set', repcap.Output.Nr1)
	...
	def fbIndex(self):
		...
		if not hasattr(self, '_fbIndex'):
			from .Output_.FbIndex import FbIndex
			self._fbIndex = FbIndex(self._core, self._base)
	...
	def feShift(self):
		...
		if not hasattr(self, '_feShift'):
			from .Output_.FeShift import FeShift
			self._feShift = FeShift(self._core, self._base)
	...
	def findex(self):
		...
		if not hasattr(self, '_findex'):
			from .Output_.Findex import Findex
			self._findex = Findex(self._core, self._base)
		return self._findex
	...
	def pattern(self):
		...
		if not hasattr(self, '_pattern'):
			from .Output_.Pattern import Pattern
	...
	def pulse(self):
		...
		if not hasattr(self, '_pulse'):
			from .Output_.Pulse import Pulse
	...
	def reShift(self):
		...
		if not hasattr(self, '_reShift'):
			from .Output_.ReShift import ReShift
			self._reShift = ReShift(self._core, self._base)
