...
class Setting:
	...
	def set_store(self, filename: str) -> None:
		...
		Stores the current IEEE 802.11a/b/g/n/ac settings into the selected file. The directory is set using command method RsSmw.
		MassMemory.currentDirectory. A path can also be specified, in which case the files in the specified directory are read.
		Only the file name has to be entered. IEEE 802.11a/b/g/n/ac settings are stored as files with the specific file
		extensions *.wlann. \n
			:param filename: string
		"""
		param = Conversions.value_to_quoted_str(filename)
