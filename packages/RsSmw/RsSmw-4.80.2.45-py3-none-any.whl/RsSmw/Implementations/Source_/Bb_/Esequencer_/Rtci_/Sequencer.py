...
class Sequencer:
	"""Sequencer commands group definition. 20 total commands, 4 Sub-groups, 0 group commands
	Repeated Capability: Sequencer, default value after init: Sequencer.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("sequencer", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_sequencer_get', 'repcap_sequencer_set', repcap.Sequencer.Nr1)

	def repcap_sequencer_set(self, enum_value: repcap.Sequencer) -> None:
	...
	def network(self):
		...
		if not hasattr(self, '_network'):
			from .Sequencer_.Network import Network
	...
	def stream(self):
		...
		if not hasattr(self, '_stream'):
			from .Sequencer_.Stream import Stream
	...
	def wave(self):
		...
		if not hasattr(self, '_wave'):
			from .Sequencer_.Wave import Wave
			self._wave = Wave(self._core, self._base)
		return self._wave

	@property
	def wlist(self):
		"""wlist commands group. 3 Sub-classes, 1 commands."""
		if not hasattr(self, '_wlist'):
			from .Sequencer_.Wlist import Wlist
			self._wlist = Wlist(self._core, self._base)
		return self._wlist
