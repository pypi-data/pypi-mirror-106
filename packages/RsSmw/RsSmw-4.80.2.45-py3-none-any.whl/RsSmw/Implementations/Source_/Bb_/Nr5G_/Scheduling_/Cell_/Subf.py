...
class Subf:
	"""Subf commands group definition. 358 total commands, 3 Sub-groups, 0 group commands
	...
	def common(self):
		...
		if not hasattr(self, '_common'):
			from .Subf_.Common import Common
	...
	def nuser(self):
		...
		if not hasattr(self, '_nuser'):
			from .Subf_.Nuser import Nuser
			self._nuser = Nuser(self._core, self._base)
		return self._nuser
