from ........Internal.Core import Core
from ........Internal.CommandsGroup import CommandsGroup
from ........Internal.RepeatedCapability import RepeatedCapability
from ........ import repcap


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class Att:
	"""Att commands group definition. 13 total commands, 2 Sub-groups, 0 group commands
	Repeated Capability: AttenuationNull, default value after init: AttenuationNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("att", core, parent)
		self._base.rep_cap = RepeatedCapability(self._base.group_name, 'repcap_attenuationNull_get', 'repcap_attenuationNull_set', repcap.AttenuationNull.Nr0)

	def repcap_attenuationNull_set(self, enum_value: repcap.AttenuationNull) -> None:
		"""Repeated Capability default value numeric suffix.
		This value is used, if you do not explicitely set it in the child set/get methods, or if you leave it to AttenuationNull.Default
		Default value after init: AttenuationNull.Nr0"""
		self._base.set_repcap_enum_value(enum_value)

	def repcap_attenuationNull_get(self) -> repcap.AttenuationNull:
		"""Returns the current default repeated capability for the child set/get methods"""
		# noinspection PyTypeChecker
		return self._base.get_repcap_enum_value()

	@property
	def emtc(self):
		"""emtc commands group. 8 Sub-classes, 0 commands."""
		if not hasattr(self, '_emtc'):
			from .Att_.Emtc import Emtc
			self._emtc = Emtc(self._core, self._base)
		return self._emtc

	@property
	def niot(self):
		"""niot commands group. 5 Sub-classes, 0 commands."""
		if not hasattr(self, '_niot'):
			from .Att_.Niot import Niot
			self._niot = Niot(self._core, self._base)
		return self._niot

	def clone(self) -> 'Att':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Att(self._core, self._base.parent)
		self._base.synchronize_repcaps(new_group)
		return new_group
