...
class Oneweb:
	"""Oneweb commands group definition. 654 total commands, 16 Sub-groups, 7 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("oneweb", core, parent)

	@property
	def clipping(self):
		"""clipping commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_clipping'):
			from .Oneweb_.Clipping import Clipping
	...
	def clock(self):
		...
		if not hasattr(self, '_clock'):
			from .Oneweb_.Clock import Clock
	...
	def downlink(self):
		...
		if not hasattr(self, '_downlink'):
			from .Oneweb_.Downlink import Downlink
	...
	def filterPy(self):
		...
		if not hasattr(self, '_filterPy'):
			from .Oneweb_.FilterPy import FilterPy
	...
	def notch(self):
		...
		if not hasattr(self, '_notch'):
			from .Oneweb_.Notch import Notch
	...
	def refSignal(self):
		...
		if not hasattr(self, '_refSignal'):
			from .Oneweb_.RefSignal import RefSignal
			self._refSignal = RefSignal(self._core, self._base)
	...
	def setting(self):
		...
		if not hasattr(self, '_setting'):
			from .Oneweb_.Setting import Setting
	...
	def symbolRate(self):
		...
		if not hasattr(self, '_symbolRate'):
			from .Oneweb_.SymbolRate import SymbolRate
	...
	def trigger(self):
		...
		if not hasattr(self, '_trigger'):
			from .Oneweb_.Trigger import Trigger
	...
	def uplink(self):
		...
		if not hasattr(self, '_uplink'):
			from .Oneweb_.Uplink import Uplink
	...
	def waveform(self):
		...
		if not hasattr(self, '_waveform'):
			from .Oneweb_.Waveform import Waveform
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmw.Source.Bb.Oneweb.state. \n
		"""
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:ONEWeb:PRESet')
	...
	def clone(self) -> 'Oneweb':
		...
		new_group = Oneweb(self._core, self._base.parent)
