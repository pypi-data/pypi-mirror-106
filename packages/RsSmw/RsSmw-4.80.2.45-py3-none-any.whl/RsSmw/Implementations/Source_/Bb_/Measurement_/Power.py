...
class Power:
	"""Power commands group definition. 16 total commands, 3 Sub-groups, 13 group commands"""
	...
	def change(self):
		...
		if not hasattr(self, '_change'):
			from .Power_.Change import Change
			self._change = Change(self._core, self._base)
		return self._change
	...
	def abort_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce:BB:MEASurement:POWer:ABORt')
	...
	def delete_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce:BB:MEASurement:POWer:DELete')
	...
	def get_gsource(self) -> enums.BbMeasPowGateSour:
		...
		acquisition (method RsSmw.Source.Bb.Measurement.Power.acquisition) . \n
	...
	def get_peak(self) -> List[float]:
		...
		Queries the peak power of the baseband signal at the measurement point determined with the command method RsSmw.Source.Bb.
	...
	def get_rms(self) -> List[float]:
		...
		Queries the RMS power of the baseband signal at the measurement point determined with the command method RsSmw.Source.Bb.
		Measurement.Power.output. \n
