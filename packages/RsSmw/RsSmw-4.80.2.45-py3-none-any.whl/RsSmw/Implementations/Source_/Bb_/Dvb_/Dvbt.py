...
class Dvbt:
	"""Dvbt commands group definition. 32 total commands, 6 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dvbt", core, parent)

	@property
	def iinterleaver(self):
		"""iinterleaver commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_iinterleaver'):
			from .Dvbt_.Iinterleaver import Iinterleaver
			self._iinterleaver = Iinterleaver(self._core, self._base)
	...
	def sample(self):
		...
		if not hasattr(self, '_sample'):
			from .Dvbt_.Sample import Sample
	...
	def tps(self):
		...
		if not hasattr(self, '_tps'):
			from .Dvbt_.Tps import Tps
			self._tps = Tps(self._core, self._base)
		return self._tps
	...
	def lp(self):
		...
		if not hasattr(self, '_lp'):
			from .Dvbt_.Lp import Lp
			self._lp = Lp(self._core, self._base)
		return self._lp
	...
	def clone(self) -> 'Dvbt':
		...
		new_group = Dvbt(self._core, self._base.parent)
