...
class Gsm:
	"""Gsm commands group definition. 108 total commands, 19 Sub-groups, 8 group commands"""
	...
	def foffset(self):
		...
		if not hasattr(self, '_foffset'):
			from .Gsm_.Foffset import Foffset
	...
	def waveform(self):
		...
		if not hasattr(self, '_waveform'):
			from .Gsm_.Waveform import Waveform
	...
	def get_flength(self) -> int:
		...
		ARB file is determined by the parameter Frame Repetition (method RsSmw.Source.Bb.Gsm.Frame.Repetitions.set) for both
		frames: Sequence Length = Frame Repetition of Frame 1 + Frame Repetition of Frame 2. For GSM mode (method RsSmw.Source.Bb.
		Gsm.mode) set to Unframed, the length of the generated ARB file is set in symbols with the command BB:GSM:SLENgth. \n
	...
	def preset(self) -> None:
		...
		Not affected is the state set with the command method RsSmw.Source.Bb.Gsm.state. \n
		"""
