...
class Mimo:
	"""Mimo commands group definition. 428 total commands, 6 Sub-groups, 6 group commands"""
	...
	def antenna(self):
		...
		if not hasattr(self, '_antenna'):
			from .Mimo_.Antenna import Antenna
	...
	def scwi(self):
		...
		if not hasattr(self, '_scwi'):
			from .Mimo_.Scwi import Scwi
			self._scwi = Scwi(self._core, self._base)
		return self._scwi
	...
	def tgn(self):
		...
		if not hasattr(self, '_tgn'):
			from .Mimo_.Tgn import Tgn
			self._tgn = Tgn(self._core, self._base)
		return self._tgn
	...
	def copy_all_with_opc(self) -> None:
		"""SCPI: [SOURce<HW>]:FSIMulator:MIMO:COPY:ALL \n
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:FSIMulator:MIMO:COPY:ALL')
