...
class HsTrain:
	"""HsTrain commands group definition. 10 total commands, 3 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("hsTrain", core, parent)

	@property
	def distance(self):
		"""distance commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_distance'):
			from .HsTrain_.Distance import Distance
	...
	def downlink(self):
		...
		if not hasattr(self, '_downlink'):
			from .HsTrain_.Downlink import Downlink
	...
	def path(self):
		...
		if not hasattr(self, '_path'):
			from .HsTrain_.Path import Path
	...
	def clone(self) -> 'HsTrain':
		...
		new_group = HsTrain(self._core, self._base.parent)
