...
class BirthDeath:
	"""BirthDeath commands group definition. 13 total commands, 3 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("birthDeath", core, parent)

	@property
	def delay(self):
		"""delay commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_delay'):
			from .BirthDeath_.Delay import Delay
	...
	def hopping(self):
		...
		if not hasattr(self, '_hopping'):
			from .BirthDeath_.Hopping import Hopping
	...
	def path(self):
		...
		if not hasattr(self, '_path'):
			from .BirthDeath_.Path import Path
	...
	def clone(self) -> 'BirthDeath':
		...
		new_group = BirthDeath(self._core, self._base.parent)
