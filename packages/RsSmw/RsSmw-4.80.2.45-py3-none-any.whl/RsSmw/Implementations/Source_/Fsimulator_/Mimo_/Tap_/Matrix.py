...
class Matrix:
	"""Matrix commands group definition. 5 total commands, 4 Sub-groups, 0 group commands"""
	...
	def accept(self):
		...
		if not hasattr(self, '_accept'):
			from .Matrix_.Accept import Accept
			self._accept = Accept(self._core, self._base)
		return self._accept
	...
	def conflict(self):
		...
		if not hasattr(self, '_conflict'):
			from .Matrix_.Conflict import Conflict
