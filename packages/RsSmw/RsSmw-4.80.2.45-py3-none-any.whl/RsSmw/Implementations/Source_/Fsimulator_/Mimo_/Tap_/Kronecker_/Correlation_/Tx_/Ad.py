...
class Ad:
	"""Ad commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""
	...
	def imaginary(self):
		...
		if not hasattr(self, '_imaginary'):
			from .Ad_.Imaginary import Imaginary
	...
	def magnitude(self):
		...
		if not hasattr(self, '_magnitude'):
			from .Ad_.Magnitude import Magnitude
	...
	def clone(self) -> 'Ad':
		...
		new_group = Ad(self._core, self._base.parent)
