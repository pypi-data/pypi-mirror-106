...
class Eg:
	"""Eg commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""
	...
	def imaginary(self):
		...
		if not hasattr(self, '_imaginary'):
			from .Eg_.Imaginary import Imaginary
	...
	def magnitude(self):
		...
		if not hasattr(self, '_magnitude'):
			from .Eg_.Magnitude import Magnitude
	...
	def clone(self) -> 'Eg':
		...
		new_group = Eg(self._core, self._base.parent)
