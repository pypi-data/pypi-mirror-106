...
class Bf:
	"""Bf commands group definition. 4 total commands, 4 Sub-groups, 0 group commands"""
	...
	def imaginary(self):
		...
		if not hasattr(self, '_imaginary'):
			from .Bf_.Imaginary import Imaginary
	...
	def magnitude(self):
		...
		if not hasattr(self, '_magnitude'):
			from .Bf_.Magnitude import Magnitude
	...
	def clone(self) -> 'Bf':
		...
		new_group = Bf(self._core, self._base.parent)
