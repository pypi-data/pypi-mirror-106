...
class Tx:
	"""Tx commands group definition. 9 total commands, 6 Sub-groups, 1 group commands
	...
	def antenna(self):
		...
		if not hasattr(self, '_antenna'):
			from .Tx_.Antenna import Antenna
	...
	def calc(self):
		...
		if not hasattr(self, '_calc'):
			from .Tx_.Calc import Calc
			self._calc = Calc(self._core, self._base)
		return self._calc
	...
	def espacing(self):
		...
		if not hasattr(self, '_espacing'):
			from .Tx_.Espacing import Espacing
			self._espacing = Espacing(self._core, self._base)
	...
	def polarization(self):
		...
		if not hasattr(self, '_polarization'):
			from .Tx_.Polarization import Polarization
			self._polarization = Polarization(self._core, self._base)
	...
	def get_pattern(self) -> enums.AntMod3DaNtPattern:
		...
		Sets the antenna pattern mode. \n
			:return: ant_tx_patt_descr: ISOtropic| USER| SEC3| SEC6| DIPole
		"""
	...
	def set_pattern(self, ant_tx_patt_descr: enums.AntMod3DaNtPattern) -> None:
		...
		Sets the antenna pattern mode. \n
			:param ant_tx_patt_descr: ISOtropic| USER| SEC3| SEC6| DIPole
		"""
