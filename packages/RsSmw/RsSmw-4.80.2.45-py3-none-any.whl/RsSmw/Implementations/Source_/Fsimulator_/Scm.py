...
class Scm:
	"""Scm commands group definition. 53 total commands, 5 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("scm", core, parent)

	@property
	def antenna(self):
		"""antenna commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_antenna'):
			from .Scm_.Antenna import Antenna
	...
	def d3Mode(self):
		...
		if not hasattr(self, '_d3Mode'):
			from .Scm_.D3Mode import D3Mode
			self._d3Mode = D3Mode(self._core, self._base)
		return self._d3Mode
	...
	def los(self):
		...
		if not hasattr(self, '_los'):
			from .Scm_.Los import Los
			self._los = Los(self._core, self._base)
		return self._los
	...
	def polarization(self):
		...
		if not hasattr(self, '_polarization'):
			from .Scm_.Polarization import Polarization
	...
	def clone(self) -> 'Scm':
		...
		new_group = Scm(self._core, self._base.parent)
