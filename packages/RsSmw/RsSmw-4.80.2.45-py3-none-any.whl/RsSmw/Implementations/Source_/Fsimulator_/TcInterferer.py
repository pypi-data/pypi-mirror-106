...
class TcInterferer:
	"""TcInterferer commands group definition. 21 total commands, 2 Sub-groups, 3 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tcInterferer", core, parent)

	@property
	def moving(self):
		"""moving commands group. 2 Sub-classes, 5 commands."""
		if not hasattr(self, '_moving'):
			from .TcInterferer_.Moving import Moving
	...
	def reference(self):
		...
		if not hasattr(self, '_reference'):
			from .TcInterferer_.Reference import Reference
	...
	def get_state(self) -> bool:
		...
		separately, see FSIMulator:TCINterferer:STATe and [:​SOURce<hw>]:​FSIMulator[:​STATe]. \n
	...
	def clone(self) -> 'TcInterferer':
		...
		new_group = TcInterferer(self._core, self._base.parent)
