...
class Tdf:
	"""Tdf commands group definition. 6 total commands, 2 Sub-groups, 3 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tdf", core, parent)

	@property
	def enu(self):
		"""enu commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_enu'):
			from .Tdf_.Enu import Enu
	...
	def macceleration(self):
		...
		if not hasattr(self, '_macceleration'):
			from .Tdf_.Macceleration import Macceleration
			self._macceleration = Macceleration(self._core, self._base)
	...
	def clone(self) -> 'Tdf':
		...
		new_group = Tdf(self._core, self._base.parent)
