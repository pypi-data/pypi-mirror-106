...
class TowerToAir:
	"""TowerToAir commands group definition. 21 total commands, 7 Sub-groups, 4 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("towerToAir", core, parent)

	@property
	def altitude(self):
		"""altitude commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_altitude'):
			from .TowerToAir_.Altitude import Altitude
			self._altitude = Altitude(self._core, self._base)
	...
	def angle(self):
		...
		if not hasattr(self, '_angle'):
			from .TowerToAir_.Angle import Angle
	...
	def flength(self):
		...
		if not hasattr(self, '_flength'):
			from .TowerToAir_.Flength import Flength
	...
	def radius(self):
		...
		if not hasattr(self, '_radius'):
			from .TowerToAir_.Radius import Radius
			self._radius = Radius(self._core, self._base)
		return self._radius
	...
	def rate(self):
		...
		if not hasattr(self, '_rate'):
			from .TowerToAir_.Rate import Rate
	...
	def speed(self):
		...
		if not hasattr(self, '_speed'):
			from .TowerToAir_.Speed import Speed
	...
	def trip(self):
		...
		if not hasattr(self, '_trip'):
			from .TowerToAir_.Trip import Trip
			self._trip = Trip(self._core, self._base)
		return self._trip
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:FSIMulator:DSSimulation:TOWertoair:PRESet')
	...
	def clone(self) -> 'TowerToAir':
		...
		new_group = TowerToAir(self._core, self._base.parent)
