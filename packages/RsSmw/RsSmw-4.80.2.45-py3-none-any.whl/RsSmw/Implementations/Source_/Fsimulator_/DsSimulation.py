...
class DsSimulation:
	"""DsSimulation commands group definition. 84 total commands, 4 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dsSimulation", core, parent)

	@property
	def create(self):
		"""create commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_create'):
			from .DsSimulation_.Create import Create
	...
	def shiptoship(self):
		...
		if not hasattr(self, '_shiptoship'):
			from .DsSimulation_.Shiptoship import Shiptoship
			self._shiptoship = Shiptoship(self._core, self._base)
	...
	def towerToAir(self):
		...
		if not hasattr(self, '_towerToAir'):
			from .DsSimulation_.TowerToAir import TowerToAir
			self._towerToAir = TowerToAir(self._core, self._base)
	...
	def user(self):
		...
		if not hasattr(self, '_user'):
			from .DsSimulation_.User import User
	...
	def clone(self) -> 'DsSimulation':
		...
		new_group = DsSimulation(self._core, self._base.parent)
