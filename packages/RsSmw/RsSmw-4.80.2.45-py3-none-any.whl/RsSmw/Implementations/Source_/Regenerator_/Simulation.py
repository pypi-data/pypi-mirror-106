...
class Simulation:
	"""Simulation commands group definition. 18 total commands, 5 Sub-groups, 6 group commands"""
	...
	def analyzer(self):
		...
		if not hasattr(self, '_analyzer'):
			from .Simulation_.Analyzer import Analyzer
	...
	def calibration(self):
		...
		if not hasattr(self, '_calibration'):
			from .Simulation_.Calibration import Calibration
	...
	def latency(self):
		...
		if not hasattr(self, '_latency'):
			from .Simulation_.Latency import Latency
			self._latency = Latency(self._core, self._base)
	...
	def level(self):
		...
		if not hasattr(self, '_level'):
			from .Simulation_.Level import Level
	...
	def minRange(self):
		...
		if not hasattr(self, '_minRange'):
			from .Simulation_.MinRange import MinRange
			self._minRange = MinRange(self._core, self._base)
	...
	def get_pri(self) -> float:
		...
		No command help available \n
			:return: sim_pri: No help available
		"""
	...
	def set_pri(self, sim_pri: float) -> None:
		...
		No command help available \n
			:param sim_pri: No help available
		"""
	...
	def get_speriod(self) -> float:
		...
		No command help available \n
			:return: sim_scan_period: No help available
		"""
	...
	def set_speriod(self, sim_scan_period: float) -> None:
		...
		No command help available \n
			:param sim_scan_period: No help available
		"""
