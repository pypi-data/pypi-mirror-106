...
class Restart:
	"""Restart commands group definition. 7 total commands, 3 Sub-groups, 4 group commands"""
	...
	def execute(self):
		...
		if not hasattr(self, '_execute'):
			from .Restart_.Execute import Execute
	...
	def synchronize(self):
		...
		if not hasattr(self, '_synchronize'):
			from .Restart_.Synchronize import Synchronize
	...
	def get_st_attenuation(self) -> float:
		...
		If method RsSmw.Source.Regenerator.Restart.modeAAUT, sets the attenuation applied on the output signal during the time
