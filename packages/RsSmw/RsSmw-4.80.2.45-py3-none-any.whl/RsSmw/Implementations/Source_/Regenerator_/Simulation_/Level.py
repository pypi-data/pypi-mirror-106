...
class Level:
	"""Level commands group definition. 4 total commands, 3 Sub-groups, 1 group commands"""
	...
	def apply(self):
		...
		if not hasattr(self, '_apply'):
			from .Level_.Apply import Apply
	...
	def updated(self):
		...
		if not hasattr(self, '_updated'):
			from .Level_.Updated import Updated
			self._updated = Updated(self._core, self._base)
	...
	def valid(self):
		...
		if not hasattr(self, '_valid'):
			from .Level_.Valid import Valid
