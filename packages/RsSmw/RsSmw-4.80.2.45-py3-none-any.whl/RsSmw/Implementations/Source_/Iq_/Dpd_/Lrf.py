...
class Lrf:
	"""Lrf commands group definition. 4 total commands, 1 Sub-groups, 2 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("lrf", core, parent)

	@property
	def file(self):
		"""file commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_file'):
			from .Lrf_.File import File
	...
	def get_state(self) -> bool:
		...
		Activates linearization of the RF. \n
			:return: linearize_rf: 0| 1| OFF| ON
		"""
	...
	def clone(self) -> 'Lrf':
		...
		new_group = Lrf(self._core, self._base.parent)
