...
class Regenerator:
	"""Regenerator commands group definition. 78 total commands, 7 Sub-groups, 5 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("regenerator", core, parent)

	@property
	def diagram(self):
		"""diagram commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_diagram'):
			from .Regenerator_.Diagram import Diagram
			self._diagram = Diagram(self._core, self._base)
	...
	def object(self):
		...
		if not hasattr(self, '_object'):
			from .Regenerator_.Object import Object
			self._object = Object(self._core, self._base)
		return self._object
	...
	def radar(self):
		...
		if not hasattr(self, '_radar'):
			from .Regenerator_.Radar import Radar
			self._radar = Radar(self._core, self._base)
		return self._radar
	...
	def restart(self):
		...
		if not hasattr(self, '_restart'):
			from .Regenerator_.Restart import Restart
	...
	def simulation(self):
		...
		if not hasattr(self, '_simulation'):
			from .Regenerator_.Simulation import Simulation
	...
	def test(self):
		...
		if not hasattr(self, '_test'):
			from .Regenerator_.Test import Test
	...
	def unit(self):
		...
		if not hasattr(self, '_unit'):
			from .Regenerator_.Unit import Unit
	...
	def preset_with_opc(self) -> None:
		...
		"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:REGenerator:PRESet')
	...
	def clone(self) -> 'Regenerator':
		...
		new_group = Regenerator(self._core, self._base.parent)
