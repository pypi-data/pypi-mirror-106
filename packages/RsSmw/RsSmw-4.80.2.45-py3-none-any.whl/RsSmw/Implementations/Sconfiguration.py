...
class Sconfiguration:
	"""Sconfiguration commands group definition. 29 total commands, 7 Sub-groups, 5 group commands"""
	...
	def duplicate(self):
		...
		if not hasattr(self, '_duplicate'):
			from .Sconfiguration_.Duplicate import Duplicate
			self._duplicate = Duplicate(self._core, self._base)
	...
	def gnss(self):
		...
		if not hasattr(self, '_gnss'):
			from .Sconfiguration_.Gnss import Gnss
	...
	def get_bb_bw(self) -> enums.SystConfBbBandwidth:
		...
		selected MIMO configuration. For example, with MIMO configurations with fewer than 8 channels, the max.
		baseband bandwidth is 400 MHz. See 'Welcome to the Fading Simulator'. \n
			:return: bandwidth: BB040| BB050| BB080| BB100| BB160| BB200| BB800| BB400| BB500| BB1G| BB2G| BB120| BBOUTDEF
		"""
	...
	def set_bb_bw(self, bandwidth: enums.SystConfBbBandwidth) -> None:
		...
		selected MIMO configuration. For example, with MIMO configurations with fewer than 8 channels, the max.
		baseband bandwidth is 400 MHz. See 'Welcome to the Fading Simulator'. \n
			:param bandwidth: BB040| BB050| BB080| BB100| BB160| BB200| BB800| BB400| BB500| BB1G| BB2G| BB120| BBOUTDEF
		"""
	...
	def get_cabw(self) -> enums.SystConfBbBandwidth:
		...
		Queries the resulting carrier aggregation bandwidth, i.e. the signal bandwidth at the outputs of the stream mapper. The
		value is calculated automatically and depends on the selected MIMO configuration, the installed options and the selected
	...
	def get_mode(self) -> enums.SystConfMode:
		...
		Switches between the operating modes. \n
			:return: mode: ADVanced| STANdard| REGenerator | GNSS | ESEQuencer ADVanced|STANdard Switches between the and . REGenerator Enables the R&S SMW to work as a radar echo generator. The fading simulator is disabled. See 'Welcome to the Option'. GNSS Enables the R&S SMW to work as in advanced GNSS mode. The fading simulator is disabled. See 'Welcome to the GNSS Satellite Navigation Options'. ESEQuencer Enables the R&S SMW to work in an advanced extended sequencer mode. The fading simulator, the AWGN, the BB input and all baseband digital standards are disabled. See 'Welcome to the Extended Sequencer'.
		"""
	...
	def set_mode(self, mode: enums.SystConfMode) -> None:
		...
		Switches between the operating modes. \n
			:param mode: ADVanced| STANdard| REGenerator | GNSS | ESEQuencer ADVanced|STANdard Switches between the and . REGenerator Enables the R&S SMW to work as a radar echo generator. The fading simulator is disabled. See 'Welcome to the Option'. GNSS Enables the R&S SMW to work as in advanced GNSS mode. The fading simulator is disabled. See 'Welcome to the GNSS Satellite Navigation Options'. ESEQuencer Enables the R&S SMW to work in an advanced extended sequencer mode. The fading simulator, the AWGN, the BB input and all baseband digital standards are disabled. See 'Welcome to the Extended Sequencer'.
		"""
