...
from src.driver.vizio.vizio_remote import VizioRemote
from src.driver.screen import Screen


class VizioDriver(BaseDriver):
    pass

    http_client = None

    """
	Starts a new Vizio driver session.
    ...
    """
	Stops the Vizio driver session and releases all assets. Should be called as the last command of every session.
    ...
    """
	Initiates the Vizio Remote for sending remote control commands.
	
	:returns: VizioRemote
	"""

    def remote(self):
        return VizioRemote(self.http_client, super().get_session())
