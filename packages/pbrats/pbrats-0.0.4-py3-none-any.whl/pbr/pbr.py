...
def split_contract(contract):
    ...
    # TODO
    if "-" not in contract and "+" not in contract and "=" not in contract:
        upper=upper+"="
    declarer = upper[0]
...
def html_id(board,vuls, dealside):
...
def onerow(idx, board, team):
    ...
    # TODO
    host = [d for d in board if d['id'] == team[0]][0]
    guest = [d for d in board if d['id'] == team[1]][0]
    # need IMP, VP
    vul = vul_table[board_vul[idx]]
    dealside = get_dealside(idx)
    host_html, host_score = html_id(host,board_vul[idx], dealside)
    guest_html, guest_score = html_id(guest,board_vul[idx], dealside)
...
def get_match(xls_file, teamno, team):
    """
    calculate one match
    """
    rows = ""
    total_hostimp = 0
    total_guestimp = 0
    boards = read_boards(xls_file, team)
    ...
    template = open("match-temp.html", "r",encoding="utf-8").read()
...
def pbr(record_xls, teams_xls):
    ...
    output = base_file + "html"
     ...
     # TODO it will read teams.xlsx for several teams
    teams = read_teams(teams_xls)

    teams_html =""
    summary=[]
    for idx, team in enumerate(teams):
        html, imp, vp = get_match(record_xls, idx+1, team)
        teams_html += html
        summary.append([team,imp, vp])
    summary_html=get_team_summary(summary)
    result = { "teams" : teams_html, "summary" : summary_html}
    template = open("index-temp.html", "r",encoding="utf-8").read()
    src = Template(template)
    contents = src.safe_substitute(result)
...
def download_sample():
    if __name__ == '__main__':
        ...
        return
    sample_file='student-group1.xlsx'
    if os.path.exists(sample_file):
        print("%s already exists, ingore (or remove it first)" % sample_file)
    else:
        print("download sample excel file -> [%s]" % sample_file)
        shutil.copy(os.path.join(os.path.realpath(__file__),sample_file),".")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) > 2:
            ...
            teams_xls = "teams.xlsx"
        if record_xls.endswith(".xlsx") and teams_xls.endswith(".xlsx"):
            pbr(record_xls, teams_xls)
        else:
            print("pbr.py <file.xlsx> [<team.xlsx>]")
    else:
        print("pbr.py <file.xlsx> [<team.xlsx>]")
