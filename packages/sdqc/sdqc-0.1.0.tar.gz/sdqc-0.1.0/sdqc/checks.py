...
def missing_values_data(data, series, completness):
    """
    ...
    series: ndarray (1D)
        The series to check.
    completness_type: str (optional)
        If completness_type is 'any' (Default) the check will not pass
        is there is any missing value for a given series value. If
        completness_type is 'all' the check will not pass all the data
        values are missing for a given series value (column).
        It only has effects when data is a table (2D).
    ...
    pass, outs: bool, list
        ...
        values and the total values depens on the completness_type.
    ...
    if len(data.shape) == 2:
        if completness == 'all':
            data_nan = np.all(data_nan, axis=1)
            n_nan = int(np.sum(data_nan))
            n_values = int(np.prod(data_nan.shape))
        elif completness == 'any':
            data_nan = np.any(data_nan, axis=1)
        else:
            raise ValueError("Invalid value for completness_type argument.")
