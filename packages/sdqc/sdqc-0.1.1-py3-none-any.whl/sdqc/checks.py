...
def missing_values_data(data, series, completness):
    ...
    series: ndarray (1D)
        The series to check.
    completness: str (optional)
        If completness is 'any' (Default) the check will not pass
        is there is any missing value for a given series value. If
        completness is 'all' the check will not pass all the data
    ...
    pass, outs: bool, list
        ...
        values and the total values depens on the completness.
    ...
    if len(data.shape) >= 2:
        if completness == 'all':
            ...
            raise ValueError("Invalid value for completness argument.")
