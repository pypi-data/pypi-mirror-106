...
class RsCmwGsmMeas:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW100/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.30.0005'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwGsmMeas._driver_options, options, direct_session)
		self._core.driver_version = '3.7.30.0005'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwGsmMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwGsmMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwGsmMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.30.0005'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwGsmMeas version failed. Current version: '3.7.30.0005', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 2 Sub-classes, 2 commands."""
	...
	def clone(self) -> 'RsCmwGsmMeas':
		"""Creates a deep copy of the RsCmwGsmMeas object. Also copies:
		...
		cloned = RsCmwGsmMeas.from_existing_session(self.get_session_handle(), self._options)
