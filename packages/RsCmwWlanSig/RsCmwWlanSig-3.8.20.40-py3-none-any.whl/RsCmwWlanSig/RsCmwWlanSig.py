...
class RsCmwWlanSig:
	"""244 total commands, 13 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.20.0040'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '3.8.20.0040'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.8.20.0040'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwWlanSig version failed. Current version: '3.8.20.0040', minimum required version: '{min_version}'")
	...
	def second(self):
		...
		if not hasattr(self, '_second'):
			from .Implementations.Second import Second
	...
	def third(self):
		...
		if not hasattr(self, '_third'):
			from .Implementations.Third import Third
	...
	def hetBased(self):
		...
		if not hasattr(self, '_hetBased'):
			from .Implementations.HetBased import HetBased
	...
	def packRate(self):
		...
		if not hasattr(self, '_packRate'):
			from .Implementations.PackRate import PackRate
			self._packRate = PackRate(self._core, self._base)
