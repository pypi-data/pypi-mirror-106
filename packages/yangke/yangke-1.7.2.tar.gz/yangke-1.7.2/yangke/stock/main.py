...
def config_program():
    ...
    # 处理命令行参数
    # 调用示例：python main.py -c debug --symbol 600006 --debug
    args_define = {"kv": [
    ...
    if args is None:
        ...
        try:
            ...
            gv.command, gv.symbol, gv.debugMode = 'debug', '600006', True
    if gv.debugMode:
        gv.logger = initLogger(logging.DEBUG, levelColor="default")
        holdLoggingLevel(logging.DEBUG, outer=False)  # paddle会覆盖configLogging(logging.DEBUG)的设置，用该方法顶掉paddle的日志设置
    else:
        gv.logger = initLogger(logging.WARNING, levelColor="default")
    ...
    if gv.storage == "file":
        ...
        # 初始化符号数据表
        if not gv.mysql_engine.has_table("symbols"):
