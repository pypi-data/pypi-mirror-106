...
class ModelRepository:
    ...
    def __download_model(self, model_name, phase):
        ...
        try:
            ...
            cur = conn.cursor()
            if conn:
                cur.execute("SELECT model from %s where model_name = '%s'", (ModelRepository.MODEL_REPO_TABLE, model_name))
                blob = cur.fetchone()
    ...
    def upload_model(self, model_name):
        ...
        try:
            conn = self.conpool.getconn()
            cur = conn.cursor()
            cur.execute("INSERT INTO io_fintix.custom_model (model_uid, model_name, model) VALUES(%s, %s, %s)",
                        (model_name, model_name, psycopg2.Binary(python_script_content)))
            conn.commit()
        except Exception as e:
            return (
                NoResult,
