    ...
    def upload_model(self, model_name):
        ...
        try:
            ...
            if should_update:
                ...
                cur.execute(
                    f"UPDATE io_fintix.custom_model SET model=%s WHERE model_uid='{model_name}'",(script_content))
            else:
                logging.info(f"Model '{model_name}' does not exist! Insert new model into database.")
                cur.execute(f"INSERT INTO io_fintix.custom_model (model_uid, model_name, model) VALUES('{model_name}', '{model_name}', %s)", (script_content))
            conn.commit()
