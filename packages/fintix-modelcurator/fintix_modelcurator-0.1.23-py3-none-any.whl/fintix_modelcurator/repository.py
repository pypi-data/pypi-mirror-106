...
class ModelRepository:
    ...
    def __download_model(self, model_name, phase):
        ...
        try:
            ...
            cur = conn.cursor()
            if conn:
                cur.execute(f"SELECT model from io_fintix.custom_model where model_name = '{model_name}'")
                blob = cur.fetchone()
                output_dir = os.path.join(LOCAL_DEPLOY_MODEL_REPO, model_name)
                ...
                conn.commit()
            logging.info(f"model {model_name} have bean downloaded into {output_dir}")
    ...
    def __convert_model(self, model_name):
        try:
            ...
            logging.info(f"Successfully convert model '{model_name}' notebook into python executable script")
            return res, NoError
        except Exception as e:
            logging.error(f"Failed to convert notebook of model {model_name}", e)
            return (
                NoResult,
    ...
    def upload_model(self, model_name):
        ...
        try:
            ...
            logging.error(f"Failed to read converted python script of model '{model_name}' notebook", e)
            return (
                NoResult,
        ...
        try:
            conn = self.conpool.getconn()
            cur = conn.cursor()
            script_content = psycopg2.Binary(python_script_content)
            cur.execute(f"SELECT model_name from io_fintix.custom_model where model_uid = '{model_name}'")
            records = cur.fetchall()
            should_update = len(records)
            cur.close()
            cur = conn.cursor()
            if should_update:
                logging.info(f"Model '{model_name}' already exist! Update model to newer version.")
                cur.execute(
                    f"UPDATE io_fintix.custom_model SET model=%s WHERE model_uid=%s", (script_content, model_name))
            else:
                logging.info(f"Model '{model_name}' does not exist! Insert new model into database.")
                cur.execute(f"INSERT INTO io_fintix.custom_model (model_uid, model_name, model) VALUES(%s, %s, %s)", (model_name, model_name, script_content))
            conn.commit()
            logging.info(f"Successfully upload converted python script of model '{model_name}' notebook into database")
        except Exception as e:
            logging.error(f"Failed to upload converted python script of model '{model_name}' notebook into database", e)
            return (
                NoResult,
    ...
    def import_model(self, model_name, phase):
        ...
        if err:
            return NoResult, err
        model_dir = os.path.join(LOCAL_DEPLOY_MODEL_REPO, model_name)
        try:
            loader = importlib.machinery.SourceFileLoader(model_name, os.path.join(model_dir, model_name + ".py"))
            model = loader.load_module(model_name)
            logging.info(f"Successfully load model '{model_name}' module from {model_dir}")
        except Exception as e:
            logging.error(f"Failed to load model '{model_name}' module form {model_dir}", e)
            return (
                NoResult,
                FintixError(
