...
def init_logging():
    ...
    logdir = os.path.join(FINTIX_IO_LOG_DIR, model_name, phase)
    if not os.path.exists(logdir):
        os.makedirs(logdir)
    logfile = os.path.join(logdir, f"{model_name}_{phase}.log")
    needRoll = os.path.isfile(logfile)
    fileHandler = logging.handlers.RotatingFileHandler(filename=logfile, backupCount=100)
...
def start():
    ...
    model_name = setting.get('model')
    phase = setting.get('phase')
    data_source = setting.get('data_source')
    ...
    logging.info(f"Process is started with settings {setting}")
