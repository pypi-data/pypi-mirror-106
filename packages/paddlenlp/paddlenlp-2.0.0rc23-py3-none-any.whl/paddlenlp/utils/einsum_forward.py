...
def torch_einsum(eqn, *operands):
    ...
    torch_operands = [torch.tensor(x, device=device) for x in operands]
...
def paddlenlp_einsum(eqn, *operands):
    paddle_operands = [paddle.to_tensor(x) for x in operands]
...
if __name__ == "__main__":
    ...
    large_shape = (512, 768, 8, 4, 256, 11, 1024, 512, 128, 64, 256)
    # small shape
    if args.shape == "base":
