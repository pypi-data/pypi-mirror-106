...
class BlackBoxODEvaluator:
    ...
    def __init__(self, model, obj_class, loader, batch_iterator, df=None, device='cuda',
                 iou_thresh=0.5, nms_thresh=0.05, step_size=0.1, verbose=True, return_prc=True, **attkwargs):
        self.model = model
                ...
                if gt == None:
                    ...
                    if self.return_prc:
                        data_dict = {'flip':flip, 'TmAP':troj_map, 'Adv_TmAP':adv_troj_map, 'Linf':linf_pert.item(), 
                            'precision':perc, "recall": rec, "thresh": thresh,
                            'adv_precision':adv_perc, "adv_recall": adv_rec, "adv_thresh": adv_thresh}
                    else:
                        data_dict = {'flip':flip, 'TmAP':troj_map, 'Adv_TmAP':adv_troj_map, 'Linf':linf_pert.item(),}
