...
CAPTION_COMMAND_REGEX = (
    r".*\\caption{("+CAPTION_BLOCK_ID_REGEX+r")}.*\\*"
)
LABEL_COMMAND_REGEX = (
    r".*\\label{([A-Za-z0-9_-]*)}.*\\*"
)
...
# FIGURE_BLOCK_REGEX = (
#     r".*\\begin{figure}\[h\].*\\includegraphics.*{(block:[A-Za-z0-9]{20}/[A-Za-z0-9]{20}/[0-9]+)}.*\\caption.*{(block:[A-Za-z0-9]{20}/[A-Za-z0-9]{20}/[0-9]+.caption)}.*\\label{(.*)}.*\\end{figure}.*\\*"
# )
...
def localize_images_from_content_block(
    ...
    for block_path, local_path in zip(patch.block_paths, patch.local_paths):
        ...
        content_with_extensions = content_with_extensions.replace(f"{local_path}.caption", image_block.caption)
...
def localize_figures_from_content_block(
    session: Session, assets_folder: str, content: str
):
    patch = patch_local_images_from_tex_block(INLINE_IMAGE_BLOCK_REGEX, content)
    content_with_extensions = patch.content
