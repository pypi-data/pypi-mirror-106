...
class DiffConvert:
    ...
    'old/new' and 'rem/upddiff' parameters to the action methods; this
    ...
    def _pathmatches(self, d, path):
        ...
        if path_head is None:
            ...
            for d_key in d:
                ...
                if path_tail:
                    ...
                    # this key
                    for matches in self._pathmatches(d[d_key], path_tail):
        ...
        if path_head in d:
            return [ ([path_head] + matches)
                            for matches
                            in self._pathmatches(d[path_head], path_tail) ]
    ...
    def full_matches(self, d):
        ...
        return self._pathmatches(d, self._path_full)


    def context_removed(self, d, match):
        """This method take a remove dictionary, as returned by
    ...
    def convert(self, from_cfg, to_cfg):
        ...
        if self._dump_diff:
            print("=> differences - remove:",
                  ...
                  yaml.dump(remove_cfg_full, default_flow_style=False),
        ...
        for cvt in self._cvts:
            ...
            for match in all_matches:
                ...
                if match in remove_matches:
                    ...
                    try:
                        diff = cvt.remove(cvt_old, cvt_rem, *args)
