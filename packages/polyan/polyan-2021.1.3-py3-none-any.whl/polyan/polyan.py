...
def fp2poly(df, has_RNA= True, has_length = False, parset = 'Scer', include_idle = True, poly_limit=30, remove_spurious_RNAs=True):
    ...
    if df.shape[1] > 2 + has_RNA + has_length:
        print('Warning: too many colums. Columns ' + str(2 + has_RNA + has_length + 1) + ' to ' + str(df.shape[1]) + ' will be ignored.')
    ...
    if (not has_RNA) or (not has_length):
        ...
        if parset == 'Scer':
            ...
            #the 95% is to catch typos (if non-sysematic gene names are specified)
            ...
            if df.ORF[0][:4] == 'ENSG':
                ...
                add_columns = 'gene'
        #combine the dataframe with the required reference data (RNA abundance and ORF lengths)
        if not has_RNA:
...
def rmsd_profile(peakvols1,peakvols2,parset='Scer'):
    ...
    if type(peakvols2) == str and peakvols2 == 'ref':
        if parset == 'Scer':
            ...
            ref_peaks = peak_scales = [0.0048, 0.008, 0.0033, 0.0019, 0.0032, 0.004, 0.004, 0.0032, 0.003, 0.003, 0.00255, 0.00265, 0.002, 0.00195, 0.0018, 0.0017, 0.0013, 0.00097, 0.00079, 0.00043, 0.00035, 0.00036, 0.00036]
            peakvols2 = ref_peaks/ np.sum(ref_peaks)
    ...
    rmsd = np.sqrt(sum(np.square(np.subtract(peakvols1,peakvols2)))/len(peakvols1))
