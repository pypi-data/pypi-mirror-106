...
from toku.exceptions import NoEncoderAvailable, ConnectionTerminated, TokuDecoderError, StreamDefunct, \
    NotClientException, NotServerException, TokuErrorReceived
...
from stream_handler cimport TokuStreamHandler
...
class CloseReasons(object):
    NORMAL = bytes(0)
    PING_TIMEOUT = bytes(1)
    UNKNOWN_ENCODER = bytes(2)
    NO_MUTUAL_ENCODERS = bytes(3)
    DIDNT_STOP_IN_TIME = bytes(4)
    DECODER_ERROR = bytes(5)

cdef class TokuSocketSession:
    ...
    cdef _encode_data(self, object data):
        ...
        return 0, bytes(self._encoder_dumps(data), encoding="utf-8")
    ...
    cdef _decode_data(self, uint8_t flags, object data):
        ...
        return bytes(self._encoder_loads(data), encoding="utf-8")
