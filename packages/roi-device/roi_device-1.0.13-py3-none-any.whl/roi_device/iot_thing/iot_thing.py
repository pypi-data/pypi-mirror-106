...
from ..iot_topic.iot_thing_property_desired import IoThingPropertyDesired
from ..iot_topic.iot_thing_property_desired_clear import IoThingPropertyDesiredClear
...
class IoThing(metaclass=abc.ABCMeta):
    ...
    def init_thing(self, lk: LinkKit, device_info: IoTDeviceInfo):
        ...
        # 属性期望值操作: 获取
        self.thing_property_desired = IoThingPropertyDesired()
        # 属性期望值操作：清空
        self.thing_property_desired_clear = IoThingPropertyDesiredClear()
        # 属性云端下发回调
    ...
    def __enable_subscribe_topic(self):
        ...
        : 订阅与取消订阅，成对出现
        """
        self.thing_topic_custom(self.iot_topic)
    ...
    def __on_topic_message(self, topic, payload, qos, userdata):
        ...
        params = json.loads(str(payload, encoding='utf-8'))
        # 属性期望值回调
        if topic == self.thing_property_desired.get_subscribe_topic():
            self.thing_get_props_desired_callback(
                params["code"], params["data"])
        if topic == self.thing_property_desired_clear.get_subscribe_topic():
            self.thing_get_props_desired_clear_callback(
                params["code"], params["data"])
    ...
    def post_thing_props_get_desired(self, *prop):
        """
        获取属性期望值
        """
        rc, mid = self.lk.publish_topic(
            self.thing_property_desired.get_post_topic(),
            self.thing_property_desired.get_post_payload_data(prop)
        )
        self.logger('>> topic mid: %s' % (str(mid)))
        return rc == 0

    def post_thing_props_clear_desired(self, prop, version=-1):
        """
        清空属性期望值
        """
        rc, mid = self.lk.publish_topic(
            self.thing_property_desired_clear.get_post_topic(),
            self.thing_property_desired_clear.get_post_payload_data(
                prop, version)
        )
        self.logger('>> topic mid: %s' % (str(mid)))
        return rc == 0

    def thing_topic_custom(self, iotopic: IoTopic):
        """
        自定义 Topic 或 Link 协议开发
        """
        # 物模型-获取属性期望值-订阅
        iotopic.add_thing_topic(self.thing_property_desired)
        # 物模型-清空属性期望值-订阅
        iotopic.add_thing_topic(self.thing_property_desired_clear)
    ...
    def thing_get_props_desired_callback(self, code, params):
        """
        重写实现：回调 属性期望值获取
        code: 200 ，成功，其它 ，失败
        """
    ...
    def thing_get_props_desired_clear_callback(self, code, params):
        """
        重写实现：回调 属性期望值清空
        code: 200 ，成功，其它，失败
        """
