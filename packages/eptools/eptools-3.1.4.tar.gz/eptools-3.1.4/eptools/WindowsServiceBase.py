    ...
    def write_to_db(self,status = 'start', options=['FINISH','SUCCESS'],retry=0):
        ...
        if db_conn:
            try:
                ...
                if status == 'run':
                    cursor.execute(self.run_query.format(description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily,table=self._svc_name_,status=options[0],status_type='RUNNING',status_description=options[1]))
                if status == 'finish':
