...
from examples.basic.my_callbacks import my_callbacks
from mmcc_framework import Framework, NoNluAdapter, validate_process, validate_callbacks, DictCallback
...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.framework = Framework.from_file("my_process.json",
                                             "my_kb.json",
                                             {},
                                             DictCallback(my_callbacks),
                                             NoNluAdapter(["name", "nickname", "age", "name_nickname"]))
        self.master = master
        self.pack()

        # Check that the process and the callbacks are valid.
        errors = validate_process(self.framework._process) + \
                 validate_callbacks(self.framework._process, DictCallback(my_callbacks))
        if errors:
            print(errors)
            exit()
