...
class Dataset:
    ...
    @staticmethod
    def import_biological_datasets() -> List:
        res = []
        folder_bio = str(Path(__file__).parent.absolute().parent)+"/data/"
        for fichier in os.listdir(folder_bio):
            res.append(Dataset(folder_bio + fichier))
        return res
