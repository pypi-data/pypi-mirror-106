...
class MPNN(ReaxFF):
  ...
  def supervise(self):
      ...
      l_atol = 0.0
      self.diffn,self.diffa,self.diffb,self.diffe,self.diffesi = {},{},{},{},{}
      for bd in self.bonds: 
          ...
          if self.nbd[bd]>0:
             ...
             self.diffn[bd]  = tf.reduce_sum(input_tensor=tf.nn.relu(self.bo0[bd]*fao-self.botol)) # self.atol
             l_atol = tf.add(self.diffn[bd],l_atol)
             ...
             if self.spv_be:
                fe  = tf.where(tf.less_equal(self.rbd[bd],self.re[bd]),1.0,0.0) ##### r< 0.9*re that bo > bore_
             ...
             if self.EnergyFunction == 1:
                fe  = tf.where(tf.less_equal(self.bo0[bd],self.botol),1.0,0.0) ##### bo <= 0.0*re that e = 0.0
                self.diffesi[bd]  = tf.reduce_sum(input_tensor=tf.nn.relu((self.EBD[bd])*fe))
                l_atol  = tf.add(self.diffesi[bd],l_atol)
