    ...
    def match(self, job: Job) -> bool:
        ...
        if self._image:
            if not job._image:
                ...
                return False
            elif isinstance(self._image, str) and not re.match(self._image, job._image._name):
                return False
        ...
        if self._needs:
            ...
            for need in self._needs:
                ...
                if (isinstance(need, Job) or isinstance(need, Sequence)) and need not in job._needs:
                    return False

                match_in_this_iteration = False
                if isinstance(need, Need):
                    for job_need in job._needs:
