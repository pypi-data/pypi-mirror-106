...
class Py3status:
    ...
    def jenkins_status(self):
        ...
        for job in self.jobs:
            ...
            if not success:
                job_response["full_text"] =  self.py3.safe_format(self.format_job, {"status": self.text_common_error})
                job_response["color"] = self.py3.COLOR_BAD
            else:
                job_response["full_text"] = self.py3.safe_format(self.format_job, {"status": text_status})
