...
class ModePlane(object):
    ...
    def _set_yee_sim(self, sim):
        ...
        for im, mesh in enumerate(meshes):
            ...
            sim_mesh = [mesh[self.new_ax[a]] for a in self.new_ax]
    ...
    def compute_modes(self, Nmodes, pml_layers=[0, 0]):
        ...
        for (ifreq, freq) in enumerate(self.freqs):
            modes = self._compute_modes_ifreq(ifreq, Nmodes, pml_layers)
            self.modes[ifreq] = modes 

    def _compute_modes_ifreq(self, ifreq, Nmodes, pml_layers=[0, 0]):
        """ Compute the ``Nmodes`` eigenmodes in decreasing order of 
