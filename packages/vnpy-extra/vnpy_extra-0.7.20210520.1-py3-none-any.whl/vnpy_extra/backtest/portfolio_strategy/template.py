...
class TargetPosAndPrice2SymbolTemplate(StrategyTemplate):
    # 该参数仅用于回测时使用（0，两腿同时下单，1：两腿分不同的bar下单）
    ...
    def order_waiting_since(self):
        return None if self.active_order_waiting_since_dt is None else datetime_2_str(
            self.active_order_waiting_since_dt)
    ...
    @property
    def difficult_ratio(self):
        return {key: _[0] for key, _ in self.deal_difficult_ratio.items()}

    @difficult_ratio.setter
    def difficult_ratio(self, val):
        pass

    @thread_save
    def _handle_active_order(self, vt_symbol):
        """执行主动单"""
        if self.active_order_waiting_since_dt is None:
            self.active_order_waiting_since_dt = datetime.now()
            mismatched_seconds = 0
        else:
            mismatched_seconds = (datetime.now() - self.active_order_waiting_since_dt).seconds
        ...
        if no_relative_order:
            ...
            if intolerable and order_wait_timeout:
                self.write_log("单腿等待超时，撤单重下")
                # self.cancel_all()
                self.cancel_order_by_vt_symbol(vt_symbol)

    @thread_save
    def _handle_passive_order(self, vt_symbol):
        """执行被动单（同时用于回测及实盘）"""
        if len(self.active_orderids) == 0:
            # 首次开仓
            ...
            if self.target_refreshed:
                ...
                for active_orderid in list(self.active_orderids):
                    ...
                    if order.price != target_price:
                        ...
                        self.write_log(f"订单已经被刷星，取消旧订单", 'debug')
                        self.cancel_all()
    ...
    def _algo_trading_2_target_pos(self):
        # 检查持仓
        # 计算是否存在仓位不匹配情况
        # 首先分别计算 symbol1 symbol2 的仓位差
        vt_symbol = self.vt_symbol1
        target_pos1, target_price1 = self.targets[vt_symbol]
        current_pos1 = self.get_pos(vt_symbol)
        pos_diff1 = target_pos1 - current_pos1
        pos_diff1_abs = abs(pos_diff1)
        vt_symbol = self.vt_symbol2
        target_pos2, target_price2 = self.targets[vt_symbol]
        current_pos2 = self.get_pos(vt_symbol)
        pos_diff2 = target_pos2 - current_pos2
        pos_diff2_abs = abs(pos_diff2)
        if pos_diff1 == pos_diff2 == 0:
            # 没有需要操作的合约或订单
            self.cancel_all()
            self.active_order_waiting_since_dt = None
            self.target_refreshed = False
        ...
        deal_passive_order = True
        # 是否为初始开仓交易阶段
        is_open_pos = current_pos1 == 0 and current_pos2 == 0
        # 是否为初始关仓交易阶段
        is_close_pos = current_pos1 != 0 and current_pos2 != 0
        # 首先做是否单腿判断
        if pos_diff1_abs > pos_diff2_abs and not (is_open_pos or is_close_pos):
            # 如果 leg1 leg2 数量不匹配，即，存在单腿，以瘸腿的一个为主动腿，另一条为被动腿
            # leg_active 主动腿, leg_passive 被动腿
            leg_passive = self.vt_symbol2
            leg_active = self.vt_symbol1
            # 发主动成交申请
            deal_passive_order = False
            # 重置主动单等待时间
            # self.active_order_waiting_since_dt = None
        elif pos_diff1_abs < pos_diff2_abs and not (is_open_pos or is_close_pos):
            leg_passive = self.vt_symbol1
            leg_active = self.vt_symbol2
            # 发主动成交申请
            deal_passive_order = False
            # 重置主动单等待时间
            # self.active_order_waiting_since_dt = None
            ...
            # 初始建仓，或关仓阶段
            # 不存在单腿情况，进行成交评估。
            leg_passive, leg_active = None, None
            if len(self.active_orderids) > 0:
                for orderid in self.get_all_active_orderids():
                    order = self.get_order(orderid)
                    if order is None:
                    ...
                    leg_passive = order.vt_symbol
                    leg_active = self.vt_symbol1 if self.vt_symbol1 != order.vt_symbol else self.vt_symbol2
                    break

            if leg_passive is None:
                # 选择评估时间长的作为被动腿（优先下单），主动腿等被动腿成交后在发交易申请
                leg1_ratio, leg2_ratio = self.estimate_deal_seconds_all()
                if leg1_ratio == leg2_ratio:
                    # 如果难度相同，leg2 先下单
                    leg_passive = self.vt_symbol2
                    leg_active = self.vt_symbol1
                elif leg1_ratio == -1:
                    ...
                    leg_passive = self.vt_symbol2
                    leg_active = self.vt_symbol1
                elif leg1_ratio < leg2_ratio:
                    # leg1 成交耗时比 leg2 短，leg2 先下被动单
                    leg_passive = self.vt_symbol2
                    leg_active = self.vt_symbol1
                else:
                    # leg2 成交耗时比 leg1 短，leg1 先下被动单
                    leg_passive = self.vt_symbol1
                    leg_active = self.vt_symbol2

        if deal_passive_order:
            # 执行被动单
            self._handle_passive_order(leg_passive)
        else:
            # 执行主动单
            self._handle_active_order(leg_active)
    ...
    def cancel_order_by_vt_symbol(self, vt_symbol) -> bool:
        ...
        for vt_orderid in self.active_orderids.copy():
            ...
            if order.vt_symbol == vt_symbol:
                self.write_log(f'Unbalanced，撤单{vt_orderid}[{vt_symbol}]')
    ...
    def estimate_deal_seconds(self, vt_symbol):
    ...
    def estimate_deal_seconds_all(self):
        """计算leg1 leg2成交时间"""
        leg1_ratio, dt = self.deal_difficult_ratio[self.vt_symbol1]
        if (datetime.now() - dt).seconds > 1:
            leg1_ratio = self.estimate_deal_seconds(self.vt_symbol1)
            self.deal_difficult_ratio[self.vt_symbol1] = (leg1_ratio, datetime.now())

        leg2_ratio, dt = self.deal_difficult_ratio[self.vt_symbol2]
        if (datetime.now() - dt).seconds > 1:
            leg2_ratio = self.estimate_deal_seconds(self.vt_symbol2)
            self.deal_difficult_ratio[self.vt_symbol2] = (leg2_ratio, datetime.now())

        return leg1_ratio, leg2_ratio

    def _estimate_deal_difficult_ratio_by_bar_backtest_only(self) -> Tuple[float, float]:
        """通过最近一个bar判断成交难度（仅用于回测）"""
        ratios = [-1, -1]
        for num, vt_symbol in enumerate([self.vt_symbol1, self.vt_symbol2]):
            target_pos, target_price = self.targets[vt_symbol]
            target_price = np.round(target_price, 2)
            current_pos = self.get_pos(vt_symbol)
            ...
            if pos_diff > 0:
                # 做多或平空
                # 当前bar 涨得越多，意味着越难成交
                ratio = -(bar.close_price - bar.open_price)
            elif pos_diff < 0:
                # 做空或平多
                # 当前bar 涨得越多，意味着越易成交
                ratio = bar.close_price - bar.open_price
    ...
    def _handle_order_to_target_pos_one_by_one_backtest_only(self):
        ...
        # 确定哪一条是主动腿，哪一条是被动腿。判断逻辑如下：
        deal_passive_order = True
        if pos_diff1_abs > pos_diff2_abs:
            # 如果 leg1 leg2 数量不匹配，即，存在单腿，以瘸腿的一个为主动腿，另一条为被动腿
            # leg_active 主动腿, leg_passive 被动腿
            # TODO: 目前仅考虑 1:1 配对交易，其他情况稍后覆盖
            leg_passive = self.vt_symbol2
            leg_active = self.vt_symbol1
            # 发主动成交申请
            deal_passive_order = False
            # 重置主动单等待时间
            self.active_order_waiting_since_dt = None
        elif pos_diff1_abs < pos_diff2_abs:
            leg_passive = self.vt_symbol1
            leg_active = self.vt_symbol2
            # 发主动成交申请
            deal_passive_order = False
            ...
            leg_passive, leg_active = None, None
            if len(self.active_orderids) > 0:
                # self.get_all_active_orderids() 实际返回值是 list(self.active_orderids)
                # 因此 orderid 是 str
                for orderid in self.get_all_active_orderids():
                    order = self.get_order(orderid)
                    if order is None:
            ...
            if leg_passive is None:
                # 选择评估时间长的作为被动腿（优先下单），主动腿等被动腿成交后在发交易申请
                leg1_ratio, leg2_ratio = self._estimate_deal_difficult_ratio_by_bar_backtest_only()
                if leg1_ratio == leg2_ratio:
                    # 如果难度相同，leg2 先下单
                    leg_passive = self.vt_symbol2
                    leg_active = self.vt_symbol1
        ...
        if deal_passive_order:
            ...
            self._handle_active_order_by_bar_backtest_only(leg_active)
...
class TargetPosAndPriceSpreadTemplate(TargetPosAndPrice2SymbolTemplate):
    ...
    def __init__(
        ...
        super().__init__(strategy_engine, strategy_name, vt_symbols, setting)
        # 价差数组
        self.price_spreads = np.zeros(0)

    def on_init(self) -> None:
        super().on_init()
        self.price_spreads = np.zeros(self.spread_array_size)

    def update_spread(self, spread):
        self.price_spreads[:-1] = self.price_spreads[1:]
        self.price_spreads[-1] = self.curr_spread = spread
