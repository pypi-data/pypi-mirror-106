...
class CtaTemplate(CtaTemplateBase):
    ...
    def update_trailing_stop_price(self):
        """根据仓位方向，更新跟踪止损价格"""
        if self.pos == 0:
            self.trailing_stop_hl_price = 0
            self.trailing_stop_price = 0
            ...
            # 设置跟踪止损价格
            hl_price = self.get_price_by_price_type(self.trailing_stop_price_type_enum)
            if self.trailing_stop_hl_price == 0 or hl_price > self.trailing_stop_hl_price:
                self.trailing_stop_hl_price = hl_price
                self.trailing_stop_price = hl_price * (1 - self.trailing_stop_rate)
    ...
    def set_target_pos(self, target_pos, price=None):
        ...
        if self._target_pos < 0 <= target_pos or self._target_pos > 0 >= target_pos:
            if self.stop_vt_orderids is not None and len(self.stop_vt_orderids) > 0:
                ...
                self.write_log(f"目标仓位方向改变，取消停止单 {stop_vt_orderids}", 'debug')
