...
class StrategyPositionMonitor(Thread):
    ...
    def output_report_between(date_since, date_until, file_name=None):
        ...
        since_df = pd.DataFrame([
            {'strategy_name': _.strategy_name, 'symbol': _.symbol, date_since: _.offset_acc_gl} for _ in
            PositionStatusModel.query_position_status_list_until(date_since)
        ])
        until_df = pd.DataFrame([
            {'strategy_name': _.strategy_name, 'symbol': _.symbol, date_until: _.offset_acc_gl} for _ in
            PositionStatusModel.query_position_status_list_until(date_until)
        ])
        count_df = pd.DataFrame([
            {'strategy_name': key[0], 'symbol': key[1], "交易次数": value[0], "盈利次数": value[1], "亏损次数": value[2]}
            for key, value in PositionStatusModel.query_strategy_symbol_count_dic(date_since, date_until).items()
        ]).set_index(['strategy_name', 'symbol'])
        if since_df.shape[0] > 0:
            df = pd.merge(
                since_df.set_index(['strategy_name', 'symbol']),
                until_df.set_index(['strategy_name', 'symbol']),
                left_index=True, right_index=True, how='outer')
        elif until_df.shape[0] > 0:
            until_df[date_since] = 0
            df = until_df.set_index(['strategy_name', 'symbol'])
        else:
        ...
        if file_name is None:
            file_name = f"{date_2_str(date_since)} {date_2_str(date_until)} 交易汇总.xlsx"
...
def test_report_between():
    ...
    date_until = str_2_date('2021-03-31')
