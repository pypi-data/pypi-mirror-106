...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def calculate_by_trade(self, df: DataFrame = None):
        ...
        for trade_data in trade_data_list:
            ...
            if trade_data.offset.value == Offset.OPEN.value:
                ...
                print('avg_price_new is :', avg_price_new)
                print('volume_new is    :', volume_new)
                print('holding_pnl is   :', holding_pnl)
                ...
                if volume == 0:
                    ...
                    # print('offset_pnl is ', offset_pnl)
                    # print('offset_daily_pnl is ', offset_daily_pnl)
                    # print('offset_acc_pnl is ', offset_acc_pnl)
                    ...
                    # print('pos_status_dict is ', pos_status_dict)
            ...
            if holding_start_dt is None and pos_status_dict['volume'] > 0:
                print('=====持仓开始时间=====')
                print(pos_status_dict['trade_dt'])
            ...
            if holding_start_dt is not None and pos_status_dict['volume'] == 0:
                ...
                print('持仓持续时间:', holding_days_seconds)
                ...
                print(offset_acc_pnl_last)
                print(offset_acc_pnl_list)
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True):
        ...
        indicator_dic = {"start_date": str(start_date),
        ...
        indicator_dic.update(indicator_by_trade_dic)
        for k, v in indicator_dic.items():
            if isinstance(v, np.int64) or isinstance(v, np.int32):
                indicator_dic[k] = int(v)
            elif isinstance(v, np.float64) or isinstance(v, np.float32) or isinstance(v, np.float16):
                indicator_dic[k] = float(v)
