...
def _get_original_app_or_fail(app):
    _app = find_original_application(app, falcon.API)
    if isinstance(_app, falcon.API):
        return _app
    msg = (
        "Unable to find the original Falcon API Application object. "
        "Please provide the original Falcon API Application object as the second argument to ContrastMiddleware"
    )
    logger.error(msg)
