...
from contrast.test.contract.events import (
    assert_propagation_event,
    assert_source_event,
    assert_trigger_event,
    check_event,
)
...
def assert_flask_sqli_finding_events(finding, source_class_name):
    ...
    check_event(
        ...
        method_name="CAST",
        source_types=[],
        # TODO: PYT-1448 missing parent ids with CAST propagation
        first_parent=None,
    )
    event_idx += 1
    check_event(
...
class FlaskAssessTestMixin(object):
    ...
    @python3_only
    def test_pathlib_trigger(self):
        """
        Sanity check that ensures pathlib and os patches are applied in the right order
        """
        user_input = "/foo/bar.txt"
        self.client.get("/pathlib-path-traversal", {"user_input": user_input})

        validate_finding_in_context(
            self.request_context, "path-traversal", user_input, trigger_arg=0
        )

    @pytest.mark.xfail(reason="TODO: PYT-1464 clean up source events")
    def test_simple_events_num_events(self):
        param_val = "doesnt matter"
        self.client.get("/simple-events-single-source/", {"user_input": param_val})

        assert len(self.request_context.activity.findings) == 1
        assert len(self.request_context.activity.findings[0].events) == 2
    ...
    def test_simple_events(self):
        ...
        source_event = self.request_context.activity.findings[0].events[-2]
        trigger_event = self.request_context.activity.findings[0].events[-1]

        assert_source_event(
            source_event, "werkzeug.datastructures.TypeConversionDict", "get"
        )
        assert_trigger_event(trigger_event, "os", "utime")

    @pytest.mark.xfail(reason="TODO: PYT-1464 clean up source events")
    def test_simple_events_two_sources_num_events(self):
        self.client.get("/simple-events-two-sources/", dict(foo="foo", bar="bar"))

        assert len(self.request_context.activity.findings) == 1
        assert len(self.request_context.activity.findings[0].events) == 4

        # This should be the first event
        foo_source = self.request_context.activity.findings[0].events[-4]
        assert_source_event(
            foo_source, "werkzeug.datastructures.TypeConversionDict", "get"
        )

    def test_simple_events_two_sources(self):
        self.client.get("/simple-events-two-sources/", dict(foo="foo", bar="bar"))

        assert len(self.request_context.activity.findings) == 1

        # First event assertion is in previous test
        bar_source = self.request_context.activity.findings[0].events[-3]
        prop_event = self.request_context.activity.findings[0].events[-2]
        trigger_event = self.request_context.activity.findings[0].events[-1]

        assert_source_event(
            bar_source, "werkzeug.datastructures.TypeConversionDict", "get"
        )
        assert_propagation_event(prop_event, "str", "concat")
        assert_trigger_event(trigger_event, "os", "utime")

    @pytest.mark.xfail(reason="TODO: PYT-1464 clean up source events")
    def test_simple_events_same_source_unused_num_events(self):
        user_input = "doesnt matter"
        self.client.get(
            "/simple-events-same-source-unused/", dict(user_input=user_input)
        )

        assert len(self.request_context.activity.findings) == 1
        assert len(self.request_context.activity.findings[0].events) == 2

    def test_simple_events_same_source_unused(self):
        param_val = "doesnt matter"
        self.client.get("/simple-events-same-source-unused/", {"user_input": param_val})
    ...
    @pytest.mark.xfail(reason="TODO: PYT-1464 clean up source events")
    def test_simple_events_same_source_used_twice_num_events(self):
        user_input = "doesnt matter"
        self.client.get(
            "/simple-events-same-source-used-twice/", dict(user_input=user_input)
        )
        ...
        # This should be the first event
        first_source = self.request_context.activity.findings[0].events[-4]
        assert_source_event(
            first_source, "werkzeug.datastructures.TypeConversionDict", "get"
        )

    def test_simple_events_same_source_used_twice(self):
        user_input = "doesnt matter"
        self.client.get(
            "/simple-events-same-source-used-twice/", dict(user_input=user_input)
        )
        ...
        # First event assertion is in previous test
        # TODO: PYT-1464 If it's not possible to come up with a solution that *doesn't*
        # involve omitting this particular event, then delete this assertion. Since the
        # sources are actually identical, I think it would be reasonable if only one
        # source event shows up for this trace, although ideally we can keep both.
        second_source = self.request_context.activity.findings[0].events[-3]
        prop_event = self.request_context.activity.findings[0].events[-2]
        trigger_event = self.request_context.activity.findings[0].events[-1]

        assert_source_event(
            second_source, "werkzeug.datastructures.TypeConversionDict", "get"
        )
        assert_propagation_event(prop_event, "str", "concat")
        assert_trigger_event(trigger_event, "os", "utime")

    @pytest.mark.xfail(reason="TODO: PYT-1464 clean up source events")
    def test_simple_events_two_sources_multiple_propagation_num_events(self):
        self.client.get(
        ...
        assert len(self.request_context.activity.findings) == 1
        assert len(self.request_context.activity.findings[0].events) == 7

    def test_simple_events_two_sources_multiple_propagation(self):
        self.client.get(
            "/simple-events-two-sources-multiple-propagators/",
            dict(foo="foo", bar="bar"),
        )
        ...
        events = self.request_context.activity.findings[0].events

        # TODO: PYT-1464 enable commented out event assertions
        # foo = request.args.get("foo")
        # assert_source_event(
        #     events[-7], "werkzeug.datastructures.TypeConversionDict", "get"
        # )
        # # foo = foo + "something"
        # assert_propagation_event(events[-6], "str", "concat")
        # # foo = foo.capitalize()
        # assert_propagation_event(events[-5], "str", "capitalize", a2r=False)

        # bar = request.args.get("bar")
        assert_source_event(
            events[-4], "werkzeug.datastructures.TypeConversionDict", "get"
        )
        # bar = "whatever" + bar
        assert_propagation_event(events[-3], "str", "concat")
        # result = foo + bar
        assert_propagation_event(events[-2], "str", "concat")
        # os.utime(result)
        assert_trigger_event(events[-1], "os", "utime")

    @pytest.mark.xfail(reason="TODO: PYT-1464 clean up source events")
    def test_simple_events_single_source_circular_usage_num_events(self):
        user_input = "doesnt matter"
        self.client.get(
            "/simple-events-single-source-circular-usage", dict(user_input=user_input)
        )

        assert len(self.request_context.activity.findings) == 1
        # TODO: PYT-1464 there's an argument for only 4 events here (i.e. the source
        # event only appears once, but I think that 5 is more accurate if possible
        assert len(self.request_context.activity.findings[0].events) == 5

        events = self.request_context.activity.findings[0].events

        assert_source_event(
            events[-5], "werkzeug.datastructures.TypeConversionDict", "get"
        )
        assert_propagation_event(events[-4], "str", "capitalize", a2r=False)

    def test_simple_events_single_source_circular_usage(self):
        user_input = "doesnt matter"
        self.client.get(
            "/simple-events-single-source-circular-usage", dict(user_input=user_input)
        )
        ...
        assert_source_event(
            events[-3], "werkzeug.datastructures.TypeConversionDict", "get"
        )
        assert_propagation_event(events[-2], "str", "concat")
        assert_trigger_event(events[-1], "os", "utime")
