...
class _MockedInputAnalysisMixin(object):
    ...
    def test_unsafe_file_upload(self, mocker):
        ...
        response = self.client.post(
            "/vulnpy",
            upload_files=[(field_name, attack, b"file content doesn't matter")],
            status=403,
            expect_errors=True,
        )

        multipart_headers = self.request_context.activity.http_request.multipart_headers
        assert len(multipart_headers) == 1
        assert multipart_headers[0].key == field_name
        assert multipart_headers[0].value == attack
...
class _NoInputAnalysisMixin(object):
    ...
    @pytest.mark.parametrize(
        "attack",
        ["../../../../etc/passwd", "super/duper/../../etc/hosts", "::$DATA"],
    )
    @pytest.mark.parametrize("trigger", ["io-open", "open"])
