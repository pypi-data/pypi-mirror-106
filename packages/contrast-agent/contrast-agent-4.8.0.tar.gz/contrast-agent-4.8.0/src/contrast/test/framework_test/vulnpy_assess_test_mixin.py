...
class VulnpyAssessTestMixin(object):
    @pytest.mark.parametrize(
        "trigger_func",
        ["tarfile-open", "tarfile-class", "tarfile-bz2", "bz2-class", "bz2-open"],
    )
    def test_compressed_files(self, trigger_func):
        if "bz2" in trigger_func and sys.version_info[0] < 3:
            pytest.skip("Python 3 only")

        user_input = "does not matter"
        self.client.get(
            "/vulnpy/pt/{}".format(trigger_func), {"user_input": user_input}
        )
