...
class CheckResponse(object):
    ...
    def __init__(self, ok=None, is_authenticated=None, roles=None, permissions=None):  # noqa: E501
    ...
    def ok(self, ok):
        ...
        if ok not in allowed_values:
            raise ValueError(
                "Invalid value for `ok` ({0}), must be one of {1}"  # noqa: E501
                .format(ok, allowed_values)
            )
