...
def to_json(xml_etree, xsd_annotated_schema):
    ...
    def json_change_occurance(json_dict, parent, xsd_element):
        for k, v in json_dict.items():
            
            if isinstance(v, dict):            
                json_change_occurance(v, k, xsd_element)
                #if k in list_elements_names:
                #    json_dict[k] = []
                #    json_dict[k].append(v)
                
                if parent!=None:
                    elements = xsd_element.xpath(
                        '//xs:element[@name="'+k+'"]', namespaces=ns
                    )
                    
                    element_found = None
                    for xsd_elem in elements:
                        #print(xsd_elem.attrib['name'])
                        try:
                            if xsd_elem.getparent() != None:
                                a = xsd_elem.getparent().getparent().getparent()
                                if 'name' in a.attrib:
                                    if a.attrib['name'] == parent:
                                        element_found = xsd_elem
                        except Exception as e:
                            pass
                    ...
                    if element_found!= None:
                        if 'maxOccurs' in element_found.attrib:
                            if element_found.attrib['maxOccurs'] == 'unbounded':
                                json_dict[k] = []
                                json_dict[k].append(v)                                 
                            elif int(element_found.attrib['maxOccurs']) > 1:
                                json_dict[k] = []
                                json_dict[k].append(v)                                

            elif isinstance(v, list):
                for json_obj in v:
                    json_change_occurance(json_obj, k, xsd_element)
    ...
    exploded_schema = _explode_schema.main(xsd_annotated_schema)
    json_change_occurance(json_object, None, exploded_schema)
